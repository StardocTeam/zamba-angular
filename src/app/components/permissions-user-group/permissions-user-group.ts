...
export class PermissionsUserGroupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      next: (res: any) => {
        ...
        groups.sort((a: { _name: string }, b: { _name: any }) => a._name.localeCompare(b._name, 'en', { sensitivity: 'base' }));
  ...
  filterGroups(): void {
    this.filteredData = this.groups().filter(
      item =>
  ...
  filterOtherGroups(): void {
    this.filteredUnassignedRoles = this.unassignedRoles.filter(
      g => !this.inheritedGroups.some((ig: any) => ig._id === g._id) && g._id !== this.selectedItem._id,
    );
  ...
  filterOtherGroupsForUserMainTab(): void {
    this.filteredOtherGroupsForUserMainTab = this.otherGroups.filter(
      g => !this.groupsBelongUser.some((ig: any) => ig._id === g._id) && !g._name.toLowerCase().startsWith('rol_'),
    );
  ...
  selectSidebarGroupItem(item: any): void {
    ...
    forkJoin({
      ...
      next: (res: any) => {
        ...
        this.usersForAGroup.sort((a: any, b: any) => (a._apellidos || '').localeCompare(b._apellidos || '', 'en', { sensitivity: 'base' }));
  ...
  selectSidebarUserItem(item: any): void {
    ...
    this.adminService.GetGroupsForAUser(this.selectedUserSidebarItem._id).subscribe({
      next: (res: any) => {
        ...
        assignedGroupsFiltered.sort((a: any, b: any) => (a._name || '').localeCompare(b._name || '', 'en', { sensitivity: 'base' }));
  ...
  addUserIntoGroup2(item: any): void {
    ...
    this.adminService.AddUserIntoGroup(this.selectedUserSidebarItem._id, item._id).subscribe({
      next: (res: any) => {
        ...
        this.groupsBelongUser.sort((a: any, b: any) => (a._name || '').localeCompare(b._name || '', 'en', { sensitivity: 'base' }));
        this.filterGroupsBelongUser();
        this.filterOtherGroupsForUserMainTab();
        this.isLoadingRightPanel = false;
        this.cdr.detectChanges();
  ...
  addInheritedGroup(item: any): void {
    ...
    this.adminService.AddInheritedGroup(this.selectedItem._id, item._id).subscribe({
      next: (res: any) => {
        ...
        this.inheritedGroups.sort((a: any, b: any) => (a._name || '').localeCompare(b._name || '', 'en', { sensitivity: 'base' }));
