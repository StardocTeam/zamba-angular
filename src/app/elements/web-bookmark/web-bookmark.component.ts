...
export class WebBookmarkComponent implements OnInit, OnDestroy {
  @Input() singleLinkMode: boolean = false;
  @Input() assetsUrl: string = 'assets/web-bookmark';
  ...
  private ensureExternalStylesheet(): void {
    const href = this.resolveAssetUrl('web-bookmark.css');
    const existing = this.document.querySelector(`link[data-zamba-web-bookmark-styles="${href}"]`);
    ...
    link.rel = 'stylesheet';
    link.href = href;
    link.setAttribute('data-zamba-web-bookmark-styles', href);
    this.document.head.appendChild(link);
  }

  private resolveAssetUrl(pathSuffix?: string): string {
    const detectedAssetsUrl = this.detectAssetsUrlFromBundle();
    const assetsUrl = this.assetsUrl && this.assetsUrl !== 'assets/web-bookmark' ? this.assetsUrl : detectedAssetsUrl;
  ...
  private detectAssetsUrlFromBundle(): string {
    ...
    if (!bundleScript?.src) {
      return 'assets/web-bookmark';
    }

    try {
      return new URL('./assets/web-bookmark/', bundleScript.src).toString();
    } catch {
      return 'assets/web-bookmark';
    }
  }
