...
class LoginTicket
{
    ...
    /// <remarks></remarks>
    public LoginTicket ObtenerLoginTicketResponse(string argServicio, string argUrlWsaa, string argRutaCertX509Firmante, SecureString argPassword, string argProxy, string argProxyUser, string argProxyPassword, bool argVerbose)
    {
        ...
        try
        {
            ...
            Zamba.AFIPAPI.Wsaa.LoginCMSService servicioWsaa = new Zamba.AFIPAPI.Wsaa.LoginCMSService();
    ...
    internal static string GetPUCLevel3(string CUIT, string Token, string Sign, SecureString argPassword, string url, string action)
    {
        try
        {
            XmlDocument XmlcontribuyentePKRequest = null;
            string XmlcontribuyentePKTemplate = "<contribuyentePK><id></id></contribuyentePK>";

            XmlcontribuyentePKRequest = new XmlDocument();
            XmlcontribuyentePKRequest.LoadXml(XmlcontribuyentePKTemplate);
            XmlNode xmlNodoId = default(XmlNode);
            xmlNodoId = XmlcontribuyentePKRequest.SelectSingleNode("//id");
            xmlNodoId.InnerText = Convert.ToString(CUIT);
            Console.WriteLine(XmlcontribuyentePKRequest.OuterXml);


            DynamicWebservice dws = new DynamicWebservice();
            Object[] Parameters = new object[3];
            Parameters.SetValue(XmlcontribuyentePKRequest.OuterXml, 0);
            Parameters.SetValue(Token, 1);
            Parameters.SetValue(Sign, 2);
            var response = dws.Consume(url, action, ref Parameters, false);
...
public class GetPUCLevel3
{
    ...
    const string DEFAULT_SERVICIO = "padron-puc-ws-consulta-nivel3";
    const string DEFAULT_CERTSIGNER = "c:\\OpenSSL\\Zamba.pfx";
    ...
    {
        var url = @"https://awshomo.afip.gov.ar/padron-puc-ws/services/select.ContribuyenteNivel3SelectServiceImpl?WSDL";
        var action = "get";
        ...
        foreach (char c in "zamba123456") strPasswordSecureString.AppendChar(c);
        ...
        LoginTicket strTicketRespuesta = null;
        ...
        try
        {
            ...
            return LoginTicket.GetPUCLevel3(CUIT, strTicketRespuesta.Token, strTicketRespuesta.Sign, strPasswordSecureString, url, action);


        }
        catch (Exception excepcionAlObtenerTicket)
        {
            Console.WriteLine(ID_FNC + "***EXCEPCION AL OBTENER TICKET: " + excepcionAlObtenerTicket.ToString());
            Console.ReadLine();
            return excepcionAlObtenerTicket.ToString();
        }
    }
