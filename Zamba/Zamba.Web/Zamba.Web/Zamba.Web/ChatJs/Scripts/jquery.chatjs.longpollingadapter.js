...
function LongPollingAdapter(options) {
    ...
    this.defaults = {
        ...
        sendMsgToUsersUrl: '/chat/sendmsgtousers',
        sendTypingSignalUrl: '/chat/sendtypingsignal',
        getMessageHistoryUrl: '/chat/getmessagehistory',
        getMsgHistoryGroupUrl: '/chat/getmsghistorygroup',
        userInfoUrl: '/chat/getuserinfo',
        usersListUrl: '/chat/getuserslist',
        changeStatus: '/chat/changestatus',
        changeName: '/chat/changename',
        changeGroupName: '/chat/changegroupname',
        changeAvatar: '/chat/changeavatar'
    };
...
LongPollingAdapter.prototype = {
    init: function (chat, done) {
        ...
        _this.server = {
            changeStatus: function (userId, status, done) {             
                $.ajax({
                    type: "POST",
                    url: _this.opts.changeStatus,
                    data: {
                        userId: userId,
                       status:status
                    },
            ...
            changeName: function (userId, name, done) {
                $.ajax({
                    type: "POST",
                    url: _this.opts.changeName,
                    data: {
                        userId: userId,
                        name: name
                    },
            ...
            changeGroupName: function (userId, chatId, name, done) {
                $.ajax({
                    type: "POST",
                    url: _this.opts.changeGroupName,
                    data: {
                        userId:userId,
                        chatId: chatId,
                        name: name
            ...
            changeAvatar: function (userId, avatar, done) {
                $.ajax({
                    type: "POST",
                    url: _this.opts.changeAvatar,
                    data: {
                        userId: userId,
                        avatar: avatar
                    },
            ...
            sendMsgToUsers: function (usersIds,chatId, messageText,isGroup, clientGuid, done) {
                /// <summary>Sends a message to server</summary>
                /// <param FullName="otherUserId" type="Number">The id of the user to which the message is being sent</param>
                /// <param FullName="messageText" type="String">Message text</param>
                /// <param FullName="clientGuid" type="String">Message client guid. Each message must have a client id in order for it to be recognized when it comes back from the server</param>
                ...
                $.ajax({
                    type: "POST",
                    url: _this.opts.sendMsgToUsersUrl,
                    data: {                        
                        usersIds: usersIds,
                        chatId:chatId,
                        message: messageText,
     ...
     /// <summary>Gets message history from the server</summary>
            ...
            getMessageHistory: function (otherUserId,chatType, done) {           
                $.ajax({
                    type: "GET",
                    async: false,
      ...
      /// <summary>Gets message history from the server</summary>
                /// <param FullName="otherUserId" type="Number">The id of the user from which you want the history</param>
                /// <param FullName="done" type="Number">Function to be called when this method completes</param>
            getMsgHistoryGroup: function (usersIds, done) {          
                //var usersSplit = usersIds.split("/");
                $.ajax({
                    type: "GET",
                    async: false,
                    url: _this.opts.getMsgHistoryGroupUrl,
                    data: {
                        users: 0
                    },
                    cache: false,
