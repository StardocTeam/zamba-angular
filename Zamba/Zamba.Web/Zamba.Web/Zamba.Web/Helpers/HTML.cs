    ...
    public static class HTML
{
    ...
    private static string ReplaceValuesWithDirectory(string Text, string Attribute, string Directory)
    {
        ...
        int attributeToFindlength = strAttributeToFind.Length;
        ...
        while (attributeStartIndex != -1)
        {
            ...
            attributeEndIndex = strToReturn.IndexOf('"', attributeStartIndex + attributeToFindlength + 1);
            ...
            if (attributeStartIndex < attributeEndIndex)
            {
                ...
                strToReplaceValue = strToReturn.Substring(attributeStartIndex + attributeToFindlength, attributeEndIndex - (attributeStartIndex + attributeToFindlength));
                ...
                if (!(strToReplaceValue.StartsWith("..") || strToReplaceValue.StartsWith(MembershipHelper.Protocol)))
                {
                    ...
                    strToReturn = strToReturn.Remove(attributeStartIndex + attributeToFindlength,
                        attributeEndIndex - (attributeStartIndex + attributeToFindlength));

                    //Insertamos el nuevo valor
                    strToReturn = strToReturn.Insert(attributeStartIndex + attributeToFindlength, strToReplaceValue);
                }
            }

            //Buscamos el siguiente atributo
            attributeStartIndex = strToReturn.IndexOf(strAttributeToFind, attributeStartIndex + attributeToFindlength);
        }
    ...
    public static string GetAttributeValue(string tag, string attributeName)
    {
        ...
        public static string[] GetMinValueAttributes(IIndex index, ITaskResult taskResult)
        {
            ...
            var containsequal = minValue.Contains("=");
            minValue = String.Format("{0:MM/dd/yyyy}", minValue.Replace("=", ""));
            if (containsequal) minValue = "=" + minValue;
    ...
    public static string[] GetOriginalValueAttribute(IIndex index, string HtmlTag)
    {
            if (HtmlTag.Contains("ng-model") && index.Data != string.Empty)
            {
                var model = HtmlTag.Replace("ng-model=\"", "|");
                string modelpart = model.Split(char.Parse("|"))[1];
                modelpart = modelpart.Substring(0, modelpart.IndexOf("\""));
                return new string[] { string.Empty, "ZOriginalValue=\"" + index.Data + "\"" + "  ng-init=\"" + modelpart + "=" + index.Data + "\"" };
            }
            else {
