...
public partial class Views_Aysa_GeneralReportStandAlone2 : System.Web.UI.Page
{
                ...
                foreach (String Id in Ids)
                {
                    if (Id != string.Empty)
                    {
                        ...
                        if (FromQuery.Contains("from"))
                        {
                            ...
                            FromQuery += " on " + docTable + Id + FirstDocTable + Id + whereSLTS.ToString();
                        }
                        else
                        {
    ...
    private String GetQuery(bool useTop)
    {
        ...
        try
        {
            ...
            string whereQuery = GetWhereForRegion(Ids);
            ...
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Se genero la siguiente consulta para el reporte general : ");
            ZTrace.WriteLineIf(ZTrace.IsVerbose, Query);
    ...
    protected void ApplyExpressions(object sender, Telerik.Web.UI.RadFilterApplyExpressionsEventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(s))
        {
            ...
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Se aplico el siguiente filtro : ");
            ZTrace.WriteLineIf(ZTrace.IsVerbose, s.Replace("(", "").Replace(")", ""));
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "");
        ...
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "La consulta finalmente es : ");
        ZTrace.WriteLineIf(ZTrace.IsVerbose, FilteredQuery);
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "");
    ...
    public int GetTaskIdByIdIndustria(int IdIndustria)
    {
        ...
        try
        {
            ...
            ds = Zamba.Servers.Server.get_Con(false, true, false).ExecuteDataset(CommandType.Text, "SELECT doc_id FROM " + id );
    ...
    protected void OpenTask()
    {
        ...
        try
        {
            ...
            if (hdnIdIndustria != null && hdnIdIndustria.Value != null && hdnIdIndustria.Value != string.Empty)
            {
                ...
                if (docId != 0)
                {
                    script = "OpenTaskInOpener('" + "../WF/TaskSelector.ashx?DocId=" + docId + "&DocTypeId=&userId=" + Zamba.Membership.MembershipHelper.CurrentUser.ID.ToString() + "'," + docId + ",'" +
                             id + "');";
                }
    ...
    private string GetWhereForRegion(List<string> Ids)
    {
                   ...
                   foreach (String Id in Ids)
                      {
                        switch (groupName)
                            {
                                case "DRCF":
                                   regions.Add("Doc_I" + Id + ".I1181 = 1");
                                   break;
                                case "DRN":
                                    regions.Add("Doc_I" + Id + ".I1181 = 2");
                                  break;
                                case "DRO":
                                    regions.Add("Doc_I" + Id + ".I1181 = 5");
                                    break;
                                case "DRSE":
                                    regions.Add("Doc_I" + Id + ".I1181 = 3");
                                    break;
                                case "DRSO":
                                    regions.Add("Doc_I" + Id + ".I1181 = 4");
                                 break;
                           }
                       }
                    ...
                    foreach (String Id in Ids)
                      {
                        ...
                        sbQuery.Append("Doc_I" + Id + ".i1181 is null and )");
