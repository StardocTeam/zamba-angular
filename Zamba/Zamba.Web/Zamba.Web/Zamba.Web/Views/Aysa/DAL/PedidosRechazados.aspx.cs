...
public partial class Views_Aysa_PedidosRechazados : System.Web.UI.Page
{
    long lastDocIDSelected = 0;
    long lastdoctypeid = 0;
    private const string PAGELOCATION = "Aysa/DAL/PedidosRechazados.aspx{0}";
...
" SELECT * FROM ( (select CAST( wfdoc.Checkin AS smalldatetime) AS [Fecha], Aysa.i12362  as [Numero de Pedido],  " +
" Aysa.i12475  as [Numero de Requerimiento], usuario1.nombres + ' ' + usuario1.apellido as [Administrativo],  " +
" usuario2.nombres + ' ' + usuario2.apellido as [Autorizador], Aysa.i12368  as [Usuario Solicitante], Area.name as [Direccion],  " +
" Aysa.i12387  as [Centro de Costo], Sitio.i12404 as [Sitio], Deposito.i12352 as [Deposito], etapa.name  as [Etapa],  " +
" estado.name  as [Estado], Proceso.i12344  as [Proceso], wfdoc.Doc_type_ID as [Id Entidad],  wfdoc.Doc_ID as [ID de Zamba]   " +
" from doc_i12084 Aysa  " +
" inner join wfdocument wfdoc on wfdoc.doc_id = Aysa.doc_id  inner join wfworkflow wfwk  on wfwk.work_id = wfdoc.work_id  " +
" inner join wfstep etapa on etapa.step_id = wfdoc.step_id     and etapa.step_id = 12115  " +
" inner join wfstepstates estado on estado.doc_state_id = wfdoc.do_state_id  " +
" inner join usrtable usuario1 on usuario1.id = Aysa.i12461 inner join usrtable usuario2 on usuario2.id = Aysa.i12433  " +
" inner join usrgroup area on area.id = Aysa.i12451 inner join doc_i12116 sitio on sitio.i12403 = Aysa.i12427  " +
" inner join doc_i12098 deposito on deposito.i12351 = Aysa.i12428  " +
" inner join doc_i12125 proceso on proceso.i12436 =  Aysa.i12437)  " +
" union ( select CAST( wfdoc.Checkin AS smalldatetime) AS [Fecha], DAL.i12362  as [Numero de Pedido],  " +
" DAL.i12475  as [Numero de Requerimiento], usuario1.nombres + ' ' + usuario1.apellido as [Administrativo],  " +
" usuario2.nombres + ' ' + usuario2.apellido as [Autorizador], DAL.i12368  as [Usuario Solicitante], Area.name as [Direccion],  " +
" ''    as [Centro de Costo], Sitio.i12404 as [Sitio], Deposito.i12352 as [Deposito] , etapa.name  as [Etapa],  " +
" estado.name  as [Estado], Proceso.i12344  as [Proceso], wfdoc.Doc_type_ID as [Id Entidad],  wfdoc.Doc_ID as [ID de Zamba]    " +
" from doc_i12123 DAL  " +
" inner join wfdocument wfdoc on wfdoc.doc_id = DAL.doc_id  inner join wfworkflow wfwk  on wfwk.work_id = wfdoc.work_id  " +
" inner join wfstep etapa on etapa.step_id = wfdoc.step_id  and etapa.step_id = 12134  " +
" inner join wfstepstates estado on estado.doc_state_id = wfdoc.do_state_id  " +
...
" inner join doc_i12125 proceso on proceso.i12436 =  DAL.i12437 ) ) A ORDER BY a. proceso, A.FECHA " ;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Zamba.Membership.MembershipHelper.CurrentUser != null)
            {
                ...
                if (!Page.IsPostBack)
                {
                    this.Title = "Reporte de Pedidos Rechazados" + " - Zamba Software";
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        try
        {
            if (CheckBox2.Checked == true)
            {
                ...
                String url = "http://" + Request.ServerVariables["HTTP_HOST"] + Request.ApplicationPath + "/log/temp/exportPedidosRechazados" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".xls";

                String path = Zamba.Membership.MembershipHelper.AppTempPath + "\\temp\\exportPedidosRechazados" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".xls";
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        try
        {
            if (CheckBox2.Checked == true)
            {
                ...
                String url = "http://" + Request.ServerVariables["HTTP_HOST"] + Request.ApplicationPath + "/log/temp/exportPedidosRechazados" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".csv";

                String path = Zamba.Membership.MembershipHelper.AppTempPath + "\\temp\\exportPedidosRechazados" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".csv";
