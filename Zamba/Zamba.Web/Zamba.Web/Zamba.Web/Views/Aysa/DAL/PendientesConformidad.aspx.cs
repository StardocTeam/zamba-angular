...
public partial class Views_Aysa_PendientesConformidad : System.Web.UI.Page
{
    long lastDocIDSelected = 0;
    long lastdoctypeid = 0;
    private const string PAGELOCATION = "Aysa/DAL/PendientesConformidad.aspx{0}";
...
" usuario2.nombres + ' ' + usuario2.apellido as [Autorizador], Aysa.i12368 as [Usuario Solicitante],  " +
" Area.name as [Direccion], Aysa.i12387 as [Centro de Costo], Sitio.i12404 as [Sitio], Deposito.i12352 as [Deposito],  " +
"  etapa.name as [Etapa], estado.name as [Estado], Proceso.i12344 as [Proceso],  " +
" wfdoc.Doc_type_ID as [Id Entidad],  wfdoc.Doc_ID as [ID de Zamba]   " +
" from doc_i12084 Aysa   " +
" inner join wfdocument wfdoc on wfdoc.doc_id = Aysa.doc_id   " +
" inner join wfworkflow wfwk on wfwk.work_id = wfdoc.work_id  " +
" inner join wfstep etapa on etapa.step_id = wfdoc.step_id and etapa.step_id = 12120  " +
...
" Area.name as [Direccion], '' as [Centro de Costo], Sitio.i12404 as [Sitio], Deposito.i12352 as [Deposito],  " +
" etapa.name as [Etapa], estado.name as [Estado], Proceso.i12344 as [Proceso],  " +
" wfdoc.Doc_type_ID as [Id Entidad],  wfdoc.Doc_ID as [ID de Zamba]   " +
" from doc_i12123 DAL  " +
" inner join wfdocument wfdoc on wfdoc.doc_id = DAL.doc_id   " +
" inner join wfworkflow wfwk on wfwk.work_id = wfdoc.work_id  " +
" inner join wfstep etapa on etapa.step_id = wfdoc.step_id and etapa.step_id = 12132   " +
" inner join wfstepstates estado on estado.doc_state_id = wfdoc.do_state_id   " +
...
" inner join doc_i12125 proceso on proceso.i12436 =  DAL.i12437 ) ) A ORDER BY A.FECHA  " ;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Zamba.Membership.MembershipHelper.CurrentUser != null)
            {
                ...
                if (!Page.IsPostBack)
                {
                    this.Title = "Reporte de Pedido Pendientes de Conformidad" + " - Zamba Software";
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        try
        {
            if (CheckBox2.Checked == true)
            {
                ...
                String url = "http://" + Request.ServerVariables["HTTP_HOST"] + Request.ApplicationPath + "/log/temp/exportPendientesConformidad" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".xls";

                String path = Zamba.Membership.MembershipHelper.AppTempPath + "\\temp\\exportPendientesConformidad" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".xls";
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        try
        {
            if (CheckBox2.Checked == true)
            {
                ...
                String url = "http://" + Request.ServerVariables["HTTP_HOST"] + Request.ApplicationPath + "/log/temp/exportPendientesConformidad" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".csv";

                String path = Zamba.Membership.MembershipHelper.AppTempPath + "\\temp\\exportPendientesConformidad" + Zamba.Membership.MembershipHelper.CurrentUser.ID + ".csv";
