...
app.controller('ZambaAssociatedController', function ($scope, $filter, $http, ZambaAssociatedService, Search, ruleExecutionService, $uibModal) {
    ...
    $scope.LoadKendoGrid = function (results) {
        ...
        var models = generateModel(localSearchResults);
...
//                    model: models
    ...
    $scope.onClick = function (e) {
        ...
        if (row.hasClass("k-state-selected")) {
            ...
            //thumb check
            //$("span." + $scope.thumbSelectionClass)[row[0].sectionRowIndex].className = "glyphicon gridSelection glyphicon-ok-circle " + $scope.thumbSelectionClass;
            ...
            //thumb check
            //$("span." + $scope.thumbSelectionClass)[row[0].sectionRowIndex].className = "glyphicon glyphicon-ok-sign " + $scope.thumbSelectionClass;
            if ($scope.mulipleSelectionGridActive == false) {
                $scope.Opentask(row[0].sectionRowIndex);
            }

            if (!IsSelected && $scope.mulipleSelectionGridActive == true) {
                $(e.currentTarget).addClass("k-state-selected");
    ...
    function generateColumns(response) {
        ...
        return columnNames.map(function (name) {
            ...
            if (name == "MODIFICADO" || name == "CREADO") {
                return {
                    field: name,
            ...
            if (name == "Icon") {
                return {
                    field: name.trim(), format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""), width: 80, template: "<div class='customer-photo'" +
