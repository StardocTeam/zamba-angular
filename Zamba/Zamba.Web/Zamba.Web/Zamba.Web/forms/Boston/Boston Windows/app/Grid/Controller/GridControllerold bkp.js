    ...
    function generateColumns(response) {
        ...
        return columnNames.map(function (name) {
		   ...
		   if (name == "SeleccionDespachante") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 60, sortable: true, filterable: false, columnMenu: false
               };
           }
    ...
    $scope.Opentask = function (arg) {
        ...
        if (thumbsCollection.length == 0) {
            ...
            if (stepid != null && stepid != undefined && stepid != 0) {
                var Url = (thisDomain + "/views/WF/TaskViewer.aspx?DocType=" + result.DOC_TYPE_ID + "&docid=" + result.DOC_ID + "&taskid=" + result.TASK_ID + "&mode=s"
