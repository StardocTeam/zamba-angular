...
app.controller('ZambaAssociatedController', function ($scope, $filter, $http, ZambaAssociatedService, Search, ruleExecutionService, $uibModal) {
    ...
    $scope.LoadResults = function (parentResultId, partentEntityId, associatedIds, parentTaskId) {
        ...
        var results = ZambaAssociatedService.getResults(parentResultId, partentEntityId, associatedIds, parentTaskId);
        if (results == null || results == undefined) {
            results = [];
        }
        $scope.associatedResults = JSON.parse(results);
    };
    ...
    $scope.LoadKendoGrid = function (results) {
        ...
        FixAssociatedKendoGrid(gridToClick, columns);
    ...
    function generateColumns(response) {
        ...
        return columnNames.map(function (name) {
            ...
            if (name == "MODIFICADO" || name == "CREADO" || name == "Icon") {
                return {
                    field: name,
					...
					hidden: true
			            ...
			            if (name == "ENTIDAD" || name == "Task_Id" || name == "Asignado" || name == "Etapa") {
                return {
                    field: name,
					hidden: true
                }
            ...
            if (name == "Icon") {
                return {
                    field: name.trim(), format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""), width: 40, template: "<div class='customer-photo'" +
                    "style='background-image: url(" + "../../Content/Images/icons/#:ICON_ID#.png); paddin: 5 5 5 5'></div>",
                    sortable: false, filterable: false, columnMenu: false
                };
            }


            // if (name == "Id_Pedidos_de_Fondos") {
                // return {
                    // field: name.trim(),
            ...
            // }
         	...
         	if (name == "Nro_Guia") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 60, template: "<p style=\""+
				   "font-family:'IDAutomationHC39M'; font-size:16pt; padding: 0 0 0 0; margin-bottom: 25px;\">(#:Nro_Guia#)</p>", sortable: false, filterable: false, columnMenu: false
               };
           }
		   if (name == "Despacho_Codigo") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 80, template: "<p style=\""+
				   "font-family:'IDAutomationHC39M'; font-size:16pt; padding: 0 0 0 0; margin-bottom: 25px;\">(#:Despacho_Codigo#)</p>", sortable: false, filterable: false, columnMenu: false
               };
           }
		   if (name == "SeleccionDespachante") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 50, sortable: true, filterable: false, columnMenu: false
               };
           }
		   if (name == "Cuit_Despachante" ) {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 30, sortable: true, filterable: false, columnMenu: false, hidden: true
               };
           }
		   if ( name == "Cuit_I_E") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 40, sortable: true, filterable: false, columnMenu: false,hidden: true
               };
           }

		   if (name == "Nro_Despacho" || name == "Nro_Guia") {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 40, sortable: true, filterable: false, columnMenu: false
               };
           }
		   if (name == "Estado_Despacho" ) {
               return {
                   field: name.trim(),format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""),width: 40, sortable: true, filterable: false, columnMenu: false
               };
           }
            ...
            if (name == "DOC_ID" || name == "DOC_TYPE_ID" || name == "STR_ENTIDAD" || name == "THUMB" || name == "FULLPATH" || name == "ICON_ID" || name == "USER_ASIGNED" || name == "EXECUTION" || name == "DO_STATE_ID" || name == "TASK_ID" || name == "RN" || name == "LEIDO" || name == "WORKFLOW" || name == "Proceso" || name == "Estado" || name == "DISK_GROUP_ID"
                || name == "DISK_GROUP_ID"
                || name == "DISK_GROUP_ID" || name == "DISK_GROUP_ID" || name == "VOL_ID"
                || name == "OFFSET" || name == "PLATTER_ID" || name == "SHARED" || name == "VER_PARENT_ID"
                || name == "ROOTID" || name == "DISK_VOL_PATH" || name == "STEP_ID"
                || name == "WORK_ID" || name == "DISK_VOL_ID" || name == "VERSION" || name == "NUMEROVERSION" || name == "DOC_FILE" || name == "NAME" || name == "ORIGINAL" || name == "AsignedTo" || name == "Step" || name == "ThumbImg" || name == "Tarea" || name == "ShowUnread"
                || name.toLowerCase().startsWith("i") && isNumeric(name.slice(1, name.length - 1))) {
                return {
                    field: name, format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""), hidden: true, filterable: false, width: 10
                }
            }
            else {
                ...
                return {
                    ...
                    field: name, format: (isDateField[name] ? "{0:dd-MM-yyyy}" : ""), width: 50, filterable: true
    ...
    function FixAssociatedKendoGrid(grid, columns) {
        ...
        if (UpdateDate != undefined)
            ...
            var gridToFix = $("#" + $scope.gridIndex).data("kendoGrid");
           // var grid = $('#Kgrid').data("kendoGrid");
              for (var i = 0; i < columns.length; i++) {
                gridToFix.autoFitColumn(i);
               }
    ...
    $scope.Opentask = function (arg) {
        ...
        if (thumbsCollection.length == 0) {
            ...
            switch (zambaApplication) {
                case "ZambaWeb":
                    OpenDocTask2(result.TASK_ID, result.DOC_ID, result.DOC_TYPE_ID, false, "Reemplazar", Url, userid);
