...
loadTax(); 
    var porcentaje = $F('zamba_Index_27574');	   
	var subTotal = $('zamba_Index_27551');	
	
	var bruto = $('zamba_Index_25557');    
		if (bruto.value.empty()) {
        $('neto').value = '';
        return;
    }
    else 
        if (isNaN(bruto.value)) {
            alert('El valor bruto debe ser un numero.');
            bruto.value = '';
            $('neto').value = '';
            return;
        }
		
	var value = (parseFloat(porcentaje) / 100) * parseFloat(bruto.value);
	...
	var neto =	parseFloat(bruto.value) - parseFloat(subTotal.value);
	if (isNaN(neto))		
		$('neto').value = '' ;
	else
		$('neto').value = neto;
...
/*function calculateTotal(){
    ...
    var total = $('total');
    total.value = parseFloat(neto.value) - parseFloat(descuento.value);    
}*/

function loadTax()
{
	var selectedTaxCode = $F('zamba_Index_25558');		
	if(!selectedTaxCode)
	$('zamba_Index_27574').value='';
	
	selectedTaxCode = selectedTaxCode.split(" - ")[0];

	$('zamba_Index_27574').value = getTaxValue(selectedTaxCode);		
}

function getTaxValue(taxName)
{
	if(!taxName)
		return '';
	
	var taxes = $F('hdnTaxes');
	taxes = taxes.replace(/\'/g,'\"');	
	var json = taxes.toString().evalJSON(true);	
	var hash = $H(json);
	return parseFloat(hash.get(taxName))/1000;	
}

Event.observe(window, 'load', function(){	
    Event.observe('zamba_Index_25557', 'blur', ZFUNCTION);
	Event.observe('zamba_Index_27574','change',ZFUNCTION);    
	Event.observe('zamba_Index_25558', 'change', loadTax);	
	

	/*	
	Cargar los valores del hash en el combo , son los mismo valores de options que cargaria zamba pero con texto diferente
	ej:
	IVSECR19   IVA CREDITO FISCAL GS. SECRET  19000.0
	
	se muestra 19000.0 en el texto y IVSECR19 de codigo  , zamba lo cargaria 
	IVA CREDITO FISCAL GS en texto y IVSECR19 de codigo. En el onchange , obtengo el IVSECR19 ,
	lo busco en el hash del hiddenfield y le cargo el valor del impuseto.
	
	var taxes = $F('hdnTaxes');
	taxes = taxes.replace(/\'/g,'\"');	
	var json = taxes.toString().evalJSON(true);	
	var hash = $H(json);
	var select = $('zamba_Index_25558');
	...
	$('zamba_Index_27574').value = getTaxValue(selectedTaxCode);
	ZFUNCTION();*/
	window.setTimeout(ZFUNCTION, 300);	
	window.setTimeout(ZFUNCTION, 2000);	
	window.setTimeout(ZFUNCTION, 5000);	
	window.setTimeout(ZFUNCTION, 10000);	
