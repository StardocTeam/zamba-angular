/*
 *	Funciones gen�ricas para los formularios de Boston.
 */
...
function CheckMaxLengthAfterLosingFocus(Object){
	...
	if (Object.value.length > len){
		alert("Se ha superado el l�mite m�ximo de " + len + " caracteres.\Se truncar� dicho texto hasta el l�mite.");
...
function IntegerCheck(e){
	//Verifica que el n�mero sea un entero
...
function Redondear(num) {
	//Redondea a 2 d�gitos despu�s de la coma
...
function valSep(oTxt){
	var sep;
	//Busca alg�n caracter separador y lo guarda (ac� podr�an agregarse nuevos formatos)
	...
	if(sep.length > 0){
		//Verifica que haya m�s de un separador
		if(oTxt.value.indexOf(sep) != oTxt.value.lastIndexOf(sep)){
			...
			if(oTxt.value.indexOf(sep) != 2){
				...
				//Verifica nuevamente los dos d�gitos
				if(oTxt.value.indexOf(sep) != 2){
					//Si pasa por ac� es porque ten�a m�s de 2 d�gitos de entrada
					return false;
				}
			}
		...
		alert("Los d�as, meses y a�os deben estar separados por el caracter / o el -");
		return false;
	}
}
...
//M�todo para sumar n dias h�biles.
Date.prototype.SumarLaborables = function(n) { 		
	for(var i=0; i < n; i++) { 
		this.setTime( this.getTime() + 24*60*60*1000 ); 
		//Si es s�bado o domingo se cuenta otro d�a. 
		if( (this.getDay()==6) || (this.getDay()==0) )    
			i--;  
	} 
	return this; 
} 
//Suma a una fecha determinada X cantidad de d�as h�biles y devuelve la fecha calculada.
...
function setIframeScrollAttribute(){
	var src = $( "#zamba_innerdoctype_original" ).attr("src");
	if(src == undefined)
	{
		$("#iframeStyle").css("overflow", "hidden");
	}
	else{
		$("#iframeStyle").css("overflow", "scroll");
	}	
}
function OnlyNumbers(event){
	
		var value = event.target.value; //texto actual del control
		var code = event.keyCode || event.which; //siempre va a ser un numero el KeyCode
		var RE = /^\d*\.?\d{0,1}$/; //Expreson Regular para numero con dos decimales
...
function SetInitialConfigurations(){
	var minimo_ancho = window.screen.width * 0.33;
	$("body").css("min-width", minimo_ancho);
	
	var maximo_alto = window.screen.height * 0.55;
	$(".iframeStyle").css("max-height", maximo_alto);

	var minimo_ancho_formulario = window.screen.width * 0.6;
	$("#container-panel").css("min-width",minimo_ancho_formulario);
