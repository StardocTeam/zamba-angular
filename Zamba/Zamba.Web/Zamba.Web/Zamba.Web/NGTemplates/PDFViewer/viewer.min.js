...
$(document).ready(function () {
    ...
    $('#viewerContainer').on('wheel', function (event) {
        ...
        var ScopeDocumentViewer = parent.angular.element(parent.document.querySelector("#GridDocumentViewer")).scope();
        var Grid = parent.$("#zamba_grid_index_all > .k-grid-content")[0].querySelectorAll("tr");
            ...
            case "next":
                if ($('#viewerContainer').children().height() > 0 && $('#viewerContainer').height() + $('#viewerContainer').scrollTop() >= $('#viewerContainer').children().height()) {
                    ...
                    for (var i = 0; i < Grid.length; i++) {
                        if ($(Grid[i]).hasClass("k-state-selected")) {
                            if (i + 1 != Grid.length) {
                                Data = nextRow(Grid, i);
                                ScopeDocumentViewer.ShowDocument_FromItem(parent.GetUID(), Data.DOC_TYPE_ID, Data.DOC_ID);
                                sessionStorage.setItem("zamba_grid_index_all_" + parent.GetUID(), Data.DOC_ID);
                                break;
                            }
                        }
                    }
...
function previousRow(Grid,index) {
    $(Grid[index]).removeClass("k-state-selected");
    $(Grid[index - 1]).addClass("k-state-selected");
    return parent.GetDataKendoGrid(index - 1);
}

function nextRow(Grid, index) {
    $(Grid[index]).removeClass("k-state-selected");
    $(Grid[index + 1]).addClass("k-state-selected");
    return parent.GetDataKendoGrid(index + 1);
}
    ...
    (function (root, factory) {
        ...
        var PDFViewer = (function pdfViewer() {
            function PDFPageViewBuffer(size) {
                ...
                this.resize = function (BCHistoryize) {
                    size = BCHistoryize;
            ...
            function isSameScale(oldScale, BCHistorycale) {
                if (BCHistorycale === oldScale) {
                    return true;
                }
                if (Math.abs(BCHistorycale - oldScale) < 1e-15) {
            ...
            PDFViewer.prototype = /** @lends PDFViewer.prototype */{
                ...
                _setScaleDispatchEvent: function pdfViewer_setScaleDispatchEvent(
                    BCHistorycale, newValue, preset) {
                    var event = document.createEvent('UIEvents');
                    event.initUIEvent('scalechange', true, true, window, 0);
                    event.scale = BCHistorycale;
                    if (preset) {
                ...
                _setScaleUpdatePages: function pdfViewer_setScaleUpdatePages(
                    BCHistorycale, newValue, noScroll, preset) {
                    this._currentScaleValue = newValue;

                    if (isSameScale(this._currentScale, BCHistorycale)) {
                        if (preset) {
                            this._setScaleDispatchEvent(BCHistorycale, newValue, true);
                        }
                        return;
                    }

                    for (var i = 0, ii = this._pages.length; i < ii; i++) {
                        this._pages[i].update(BCHistorycale);
                    }
                    this._currentScale = BCHistorycale;
                    ...
                    this._setScaleDispatchEvent(BCHistorycale, newValue, preset);
    ...
    (function (root, factory) {
        ...
        var PDFViewerApplication = {
            ...
            zoomIn: function pdfViewZoomIn(ticks) {
                var BCHistorycale = this.pdfViewer.currentScale;
                do {
                    BCHistorycale = (BCHistorycale * DEFAULT_SCALE_DELTA).toFixed(2);
                    BCHistorycale = Math.ceil(BCHistorycale * 10) / 10;
                    BCHistorycale = Math.min(MAX_SCALE, BCHistorycale);
                } while (--ticks > 0 && BCHistorycale < MAX_SCALE);
                this.pdfViewer.currentScaleValue = BCHistorycale;
            },
            ...
            zoomOut: function pdfViewZoomOut(ticks) {
                ...
                do {
                    BCHistorycale = (BCHistorycale / DEFAULT_SCALE_DELTA).toFixed(2);
                    BCHistorycale = Math.floor(BCHistorycale * 10) / 10;
                    BCHistorycale = Math.max(MIN_SCALE, BCHistorycale);
                } while (--ticks > 0 && BCHistorycale > MIN_SCALE);
                this.pdfViewer.currentScaleValue = BCHistorycale;
            },
            ...
            open: function pdfViewOpen(file, args) {
                ...
                var result = loadingTask.promise.then(
                    ...
                    function getDocumentError(exception) {
                        if (exception.message == "stream must have data") {
                            console.debug("No se encontro el archivo PDF")
                            return;
                        }
                        ...
                        if (exception.message != "Worker was destroyed") {
                            //$("#viewer").load("blank.pdf");
                            toastr.warning("No se encontró documento");

                            //alert("No se encontro documento");
        ...
        function webViewerInitialized() {
            ...
            if (!PDFViewerApplication.supportsFullscreen) {
                ...
                if (appConfig.secondaryToolbar.presentationModeButton != null)
        ...
        window.addEventListener('change', function webViewerChange(evt) {
            ...
            appConfig.secondaryToolbar.viewBookmark.setAttribute('hidden', 'false');
