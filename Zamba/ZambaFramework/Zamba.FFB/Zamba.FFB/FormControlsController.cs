...
public class FormControlsController
{
    ...
    public String TestCompleteFormAsociates(Int64 TaskId, Int64 UserId) { 
        ...
        IResult Task = new Zamba.Services.STasks().GetTask(TaskId);
    ...
    public string GetAsociatedResults(String TableId, string TBodyId, IResult Task, IUser user, Int64 AsociatedId)
    {
        ...
        //{
            AsociatedResults.AddRange(sda.getAsociatedResultsFromResultAsList(AsociatedId, Task));
        //}
            ...
            foreach (IResult CurrentResult in results)
            {
                ...
                if (onlyWF == false | TaskId != -1)
                {
                    ...
                    if (lastDocTypeId != CurrentResult.DocTypeId)
                    {
                        ...
                        IRI = UserBusiness.Rights.GetAssociatedIndexsRightsCombined(Task.DocTypeId, lastDocTypeId, user.ID, true);
                    }

                    foreach (IIndex CurrentIndex in CurrentResult.Indexs)
                    {
                        bool ShowIndex = false;
                        if (SRights.GetUserRights(ObjectTypes.DocTypes, Zamba.Core.RightsType.ViewAssociateRightsByIndex, Int32.Parse(Task.DocTypeId.ToString())))
                        {
