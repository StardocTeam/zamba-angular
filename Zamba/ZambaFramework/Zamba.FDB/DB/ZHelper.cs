...
public enum ColType
    ...
    public class ZSqlHelper
    {
        IDbCommand cmd = null;

        public ZSqlHelper(IDbCommand command)
        {
            cmd = command;
        }
        ...
        {
            return Select("SELECT * FROM ZSql_master;");
        }
        ...
        {
            return Select(sql, new List<IDbDataParameter>());
        }

        public DataTable Select(string sql, Dictionary<string, object> dicParameters = null)
        {
            List<IDbDataParameter> lst = GetParametersList(dicParameters);
            return Select(sql, lst);
        }

        public DataTable Select(string sql, IEnumerable<IDbDataParameter> parameters = null)
        {
            cmd.CommandText = sql;
            if (parameters != null)
            {
        ...
        IDbDataAdapter da = null;// = new IDbDataAdapter(cmd);
        ...
        {
            Execute(sql, new List<IDbDataParameter>());
        }
        ...
        public void Execute(string sql, IEnumerable<IDbDataParameter> parameters = null)
        {
            cmd.CommandText = sql;
            if (parameters != null)
            {
        ...
        public object ExecuteScalar(string sql, IEnumerable<IDbDataParameter> parameters = null)
        {
            cmd.CommandText = sql;
            if (parameters != null)
            {
        ...
        {
            List<IDbDataParameter> lst = null;
            if (dicParameters != null)
            {
                lst = new List<IDbDataParameter>();
                foreach (KeyValuePair<string, object> kv in dicParameters)
                {
//                    lst.Add(new ZSqlParameter(kv.Key, kv.Value));
                }
            }
            return ExecuteScalar<dataType>(sql, lst);
        }

        public dataType ExecuteScalar<dataType>(string sql, IEnumerable<IDbDataParameter> parameters = null)
        {
            cmd.CommandText = sql;
            if (parameters != null)
            {
        ...
        private List<IDbDataParameter> GetParametersList(Dictionary<string, object> dicParameters)
        {
...
//                    lst.Add(new ZSqlParameter(kv.Key, kv.Value));
        ...
        public void CreateTable(ZSqlTable table)
        {
            ...
            foreach (ZSqlColumn col in table.Columns)
            {
        ...
        public void UpdateTableStructure(string targetTable, ZSqlTable newStructure)
        {
