        ...
        Try
            ...
            If (Zamba.Core.ZTrace.Level = TraceLevel.Verbose) Then
                ...
                If Not hsSQLTraces.ContainsKey("SQL") Then
                    ...
                    End If
                    'Fecha formateada
                    fecha = DateTime.Now.ToShortDateString.ToString.Replace("/", "-").Trim & " " &
                                DateTime.Now.ToShortTimeString.ToString.Replace(":", "-").Replace("a.m.", String.Empty).Replace("p.m.", String.Empty).Trim

                    fileName = path & "\SQL " & Environment.MachineName & " " & Application.ExecutablePath.Substring(Application.ExecutablePath.LastIndexOf("\") + 1) & " " & Now.ToString("dd-MM-yyyy HH-mm-ss") & ".txt"
