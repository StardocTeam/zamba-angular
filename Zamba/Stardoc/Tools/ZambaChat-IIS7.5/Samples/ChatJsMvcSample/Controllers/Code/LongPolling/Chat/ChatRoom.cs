...
namespace ChatJsMvcSample.Code.LongPolling.Chat
{
    ...
    public class ChatRoom
    {
        ...
        private long lastTimeRoomChanged = DateTime.Now.Ticks;

        public ChatRoom(string id)
        {
            this.Id = id;
            this.UsersById = new Dictionary<decimal, ChatUser>();
        ...
        /// </summary>
        public Dictionary<decimal, ChatUser> UsersById { get; private set; }
        ...
        public void SetUserOnline(int userId)
        {
            lock (this.usersInRoomLock)
            {
                ...
                this.UsersById[userId].LastActiveOn = DateTime.Now;
        ...
        public List<ChatUser> UpdateStatusesAndGetUserList()
        {
            lock (this.usersInRoomLock)
            {
                var lastSeenLimit = DateTime.Now - InactivityTolerance;
                ...
                if (inactiveUsers.Any())
                    this.lastTimeRoomChanged = DateTime.Now.Ticks;
        ...
        public void SendMessage(decimal userFromId, int userToId, string message, string clientGuid)
        {
        ...
        public void SendMsgToUsers(List<int> usersIds, int userToId, string message, string clientGuid)
        {
            if (message == null) throw new ArgumentNullException("message");
            if (clientGuid == null) throw new ArgumentNullException("clientGuid");
            ...
            lock (this.messageLock)
            {
                ...
                var userFrom = this.UsersById[usersIds[99]];
                var userTo = this.UsersById[userToId];
        ...
        /// <param name="userFromId"></param>
        /// <param name="userToId"></param>
        public void SendTypingSignal(decimal userFromId, int userToId)
        {
            var userFrom = this.UsersById[userFromId];
            var userTo = this.UsersById[userToId];
        ...
        {          
            this.ChangeStatusEvent(userId, status);
        }

        public void ChangeName(int userId, string name)
        {
            this.ChangeNameEvent(userId, name);
        }
        public void ChangeGroupName(int userId, int chatId, string name)
        {
            this.ChangeGroupNameEvent(userId,chatId, name);
        }
        public void ChangeAvatar(int userId, string avatar)
        {
            this.ChangeAvatarEvent(userId, avatar);
        }
        ...
        public event Action<int, int> ChangeStatusEvent;
        public event Action<int, string> ChangeNameEvent;
        public event Action<int, int, string> ChangeGroupNameEvent;
        public event Action<int, string> ChangeAvatarEvent;
        /// <summary>
        /// This method is supposed to be used inside a 'using' clause.
        /// </summary>
        /// <returns></returns>
