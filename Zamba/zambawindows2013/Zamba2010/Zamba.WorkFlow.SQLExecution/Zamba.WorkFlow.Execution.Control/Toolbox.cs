namespace Zamba.WorkFlow.Execution.Control
{
    ...
    using Zamba.Core;
    using WFActivity.Xoml;

    #region Class Toolbox
    /// <summary>
    /// Class implementing the toolbox functionality.
    /// Toolbox displays workflow components
    /// </summary>
    [ToolboxItem(false)]
	public class Toolbox: Panel, IToolboxService
	{
		...
		public Toolbox(IServiceProvider provider)
		{
			...
			AddToolboxEntries(listBox, string.Empty);
		}

        private void Searchbox_TextChanged(object sender, EventArgs e)
        {
            var searchterm = searchbox.Text.Trim();
            AddToolboxEntries(listBox, searchterm);
        }
		...
		public object SerializeToolboxItem(ToolboxItem toolboxItem)
		{
            ...
            ((IResultActivity)activities[0]).RuleClass = toolboxItem.DisplayName;
        ...
        /// <summary>
        /// Obtengo todas las reglas y las cargo en el toolbox
        /// </summary>
        /// <param name="lb"></param>
        private void AddToolboxEntries(ListBox lb, string searchTerm)
        {
            SortedList st = new SortedList();
            System.Reflection.Assembly a = System.Reflection.Assembly.LoadFile(System.Windows.Forms.Application.StartupPath + "\\Zamba.WFActivity.Regular.dll");
            foreach (System.Type M in a.GetTypes())
            {
                if ((M.Name.ToLower().StartsWith("if") || M.Name.ToLower().StartsWith("do"))
                    && string.Compare(M.Name.ToUpper(), "IFBRANCH") != 0)
                {

                    //Instancio el tipo de regla base
                    string entry;
                    if (M.Name.ToUpper().StartsWith("DO"))
                        entry = "Zamba.WFActivity.Xoml.ZRule, Zamba.WFActivity.Xoml";
                    else
                        entry = "Zamba.WFActivity.Xoml.ZParallelRule, Zamba.WFActivity.Xoml";

                    //Creo el item del toolbox
                    SelfHostToolboxItem item = new SelfHostToolboxItem(entry);
                    foreach (Object o in M.GetCustomAttributes(true))
                    {
                        try
                        {
                            ...
                            {
                                if (String.Compare(o.GetType().Name.ToLower(), "regla padre") != 0)
                                {
                                    if (string.IsNullOrEmpty(((RuleDescription)o).Description) == false)
                                    {
                                        item.Name = M.Name;
                                        item.Description = ((RuleDescription)o).Description;
                                        //Los cargo en una sorted asi aparecen ordenados
                                        st.Add(item.Description, item);
                                        break;
                                    }
                                }
                            }
            ...
            lb.Items.Clear();
            foreach (SelfHostToolboxItem item in st.Values)
            {
                if (item.Name.ToLower().Contains(searchTerm.ToLower()) || item.Description.ToLower().Contains(searchTerm.ToLower()) || item.ClassName.ToLower().Contains(searchTerm.ToLower()) || searchTerm == string.Empty)
                    lb.Items.Add(item);
		...
		private void OnListBoxMouseMove(object sender, MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left && this.listBox.SelectedItem != null)
			{
                ...
                toolboxItem.DisplayName = selectedItem.Name;
	...
	internal class SelfHostToolboxItem
	{
		...
		public Type ComponentClass
		{
			get
			{
                ...
                {
                    if (componentClassName.Contains("ZRule"))
                    {
                        ZRule ZR = new ZRule();
                        componentClass = ZR.GetType();
                    }
        ...
        public string Description
        {
            get
            {
                ...
                {
                    if (ComponentClass != null)
                        description = ComponentClass.Name;
                    else
