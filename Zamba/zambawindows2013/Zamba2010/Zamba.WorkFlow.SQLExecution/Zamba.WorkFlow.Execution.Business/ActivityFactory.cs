    ...
    public class ActivityFactory
    {
        ...
        public static DataSet GetWFActivities(Int64 id)
        {
            try
            {
                ...
                DataSet ds = Zamba.Servers.Server.get_Con().ExecuteDataset(CommandType.Text, sql);
                return ds;
            }
            catch (Exception ex)
            {
        ...
        public static void InsertRule(Int64 id, string Name, String Class, Int64 step_id, Int64 ParentID,Int32 ParentType, Int32 Version, Boolean Enable,Int32 Type)
        {
            try
            {
                ...
                Zamba.Servers.Server.get_Con().ExecuteNonQuery(CommandType.Text, sql);
            }
            catch (Exception ex)
            {
                ZClass.raiseerror(ex);
        ...
        public static void DeleteStepActivities(Int32 ID)
        {
            try
            {
                ...
                Zamba.Servers.Server.get_Con().ExecuteDataset(CommandType.Text, sql);
            }
            catch (Exception ex)
            {
                ZClass.raiseerror(ex);
        ...
        public static void UpdateParamItem(Int32 ItemPos, Int64 RuleId, String Value)
        {
            try
            {
                if (Server.isSQLServer)
                {
                    ...
                    Server.get_Con().ExecuteNonQuery("Zsp_workflow_100_UpdateParamItem", parvalues);
                }
                else
                {
                    //TODO Oracle
                    String sql = "Update WFRuleParamItems set c_value='" + Value + "' where rule_id=" + RuleId + " And Item = " + ItemPos;
                    Server.get_Con().ExecuteNonQuery(CommandType.Text, sql);
                }
            }
            catch (Exception ex)
            {
