...
public partial class WfReports_UserControls_UCGenericReport_UcGenericReport : System.Web.UI.UserControl
{
        ...
        try
        {
            ...
            if (null != ds && ds.Tables[0].Rows.Count > 0)
            {
                if (string.IsNullOrEmpty(Image1.ImageUrl))
                {
                    ...
                    string filename;
                    filename = "~/" + lblTitle.Text + ".jpg";
                    string filename2;
                    filename2 = Server.MapPath("~") + "//" + lblTitle.Text + ".jpg";
                    try
                    {
                        excelService.setExcelGraphics(ds, "COLUMNAS", filename2, false, false);
                    }
                    catch (Exception ex)
                    {
                        excelService.setExcelGraphics2(ds, "COLUMNAS", filename2, false);
                    }
    ...
    {
        switch (RadioButtonList1.SelectedValue.ToString().ToUpper())
        {
            case "TABLA":
