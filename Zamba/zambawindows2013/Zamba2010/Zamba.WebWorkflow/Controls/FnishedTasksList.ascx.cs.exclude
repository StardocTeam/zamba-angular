...
public partial class TasksList : UserControl
{
   ...
   public List<ITaskResult> Tasks
   {
      set
      {
         ...
         Boolean EnableSelectionButton = EnableButtons;
         Boolean EnableSelectionCheckbox = EnableCheckboxes;
         ...
         for (int i = 0; i < gvTasks.PageSize; i++)
         {
            ...
            if (CurrentIndex <= value.Count && i < gvTasks.Rows.Count)
            {
               ...
               if (!EnableSelectionButton)
                  CurrentRow.Cells[VIEW_BUTTON_INDEX].Enabled = false;

               if (!EnableSelectionCheckbox)
                  CurrentRow.Cells[SELECT_CHECKBOX_INDEX].Enabled = false;
