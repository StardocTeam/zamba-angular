...
namespace Zamba.WFShapes
{
	public class Connector : ConnectorBase
    {
        ...
        //private Zamba.WFShapes.Win.DiagramControl diagram;
        ...
        #region Constructor
        //public Connector(Win.DiagramControl diagrama):base(diagram){}
		...
		public override void Move(Point p)
		{
            ...
            for (int k = 0; k < AttachedConnectors.Count; k++)
            {
                if (typeof(IConnection).IsInstanceOfType(AttachedConnectors[k].Parent))
                {
                    ...
                    ///cambia los puntos de inicio y de fin por los de los 2 conectores mas cercanos
                    Zamba.WFShapes.Core.DiagramShape LocationConector;
                    LocationConector = new Zamba.WFShapes.Core.DiagramShape();
                    ...
                    shape1 = LocationConector.GetClassShape(con.Id1.ToString(), Model);
                    shape2 = LocationConector.GetClassShape(con.Id2.ToString(), Model);
                    if (shape1 != null && shape2 != null)
                    {
                        LocationConector.ElegirConector(shape1, shape2, out C1, out C2);
                        p1 = shape1.Connectors[C1].Point;
                        p2 = shape2.Connectors[C2].Point;
                        con.label.Location = new Point((p1.X + p2.X) / 2, (p1.Y + p2.Y) / 2);
                ...
                // Esto es parte del codigo original, al comentarlo no modifica la ejecucion de la aplicacion
                //if (con != null)
                //{
