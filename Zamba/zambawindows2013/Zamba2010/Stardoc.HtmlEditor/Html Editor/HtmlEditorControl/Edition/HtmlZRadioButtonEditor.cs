...
namespace Stardoc.HtmlEditor.Edition
{
    internal partial class HtmlZRadioButtonEditor : Form
    {
        ...
        public HtmlZRadioButtonEditor(ZRadioButtonItem radio, HtmlFormState state)
            : this()
        {
            tbCategory.Text = radio.Category;
            tbName.Text = radio.Name;
            _indexId = radio.IndexId;

            chkChecked.Checked = radio.Checked;
            chkEnabled.Checked = radio.Enabled;
            chkRequired.Checked = radio.Required; 

            if (radio.SelectionValue)
                rdbYes.Checked = true;
            else
                rdbNo.Checked = true;

            _state = state;
            if (state == HtmlFormState.Insert)
        ...
        public ZRadioButtonItem Radio
        {
            get
            {
                ...
                RadioButton.Required = chkRequired.Checked;
