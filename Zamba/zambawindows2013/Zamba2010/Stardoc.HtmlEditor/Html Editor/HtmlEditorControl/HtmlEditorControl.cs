...
namespace Stardoc.HtmlEditor
{
    ...
    public sealed class HtmlEditorControl : UserControl
    {
        ...
        public void SaveFilePrompt()
        {
            ...
            string contents = CreateHtml(_body.innerHTML);
        ...
        private String CreateHtml(string bodyInnerHtml)
        {
            ...
            try
            {
                ...
                HtmlBuilder.AppendLine("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\" />");
                HtmlBuilder.AppendLine("<title></title>");
                HtmlBuilder.AppendLine("<link rel=\"stylesheet\" href=\"Calendar.css\" type=\"text/css\" />");
                HtmlBuilder.AppendLine("<script type=\"text/javascript\" src=\"Calendar.js\"  language=\"JavaScript\" />");
                HtmlBuilder.AppendLine("<script type=\"text/javascript\" src=\"DateComparer.js\" language=\"JavaScript\" />");
                HtmlBuilder.AppendLine("<script type=\"text/javascript\" src=\"RequiredFields.js\" language=\"JavaScript\" />");
                HtmlBuilder.AppendLine("<script type=\"text/javascript\" src=\"ValidateInteger.js\"language=\"JavaScript\"  />");
                HtmlBuilder.AppendLine("<script type=\"text/javascript\" src=\"ValidateRange.js\" language=\"JavaScript\"  />");
                HtmlBuilder.AppendLine("</head>");
                HtmlBuilder.AppendLine("<body>");
                HtmlBuilder.AppendLine("<form onsubmit=\"return ValidateForm()\">");
                HtmlBuilder.AppendLine(bodyInnerHtml);
        ...
        {
            using (HtmlMainEditor dialog = new HtmlMainEditor())
            {
            ...
            using (ListHtmlCheckbox dialog = new ListHtmlCheckbox())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (CheckBoxItem CurrentCheckBox in dialog.CheckBoxes)
                    {
                        ...
                        if (String.IsNullOrEmpty(CurrentCheckBox.Text))
                            ProcessCheckBox(range, InputElement);
                        else
                            ProcessCheckBox(range, InputElement, CurrentCheckBox.Text);
                    }
                }
            }
        }
            ...
            try
            {
                ...
                switch (command)
                {
                    ...
                    case INTERNAL_COMMAND_INSERTTEXTAREA:
                        // Display a dialog to enable user to insert the text area
                        InsertTextAreaPrompt(GetTextRange());
                        break;
