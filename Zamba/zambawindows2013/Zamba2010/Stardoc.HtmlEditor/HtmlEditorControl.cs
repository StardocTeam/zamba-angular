...
using mshtml;
using Stardoc.HtmlEditor.COM;
using Stardoc.HtmlEditor.COM.IOleCommandTarget;
...
namespace Stardoc.HtmlEditor
{
    ...
    public sealed class HtmlEditorControl
        : UserControl
    {
        private IHtmlWritable _itemToInsert = null;
        public IHtmlWritable ItemToInsert
        {
            set { _itemToInsert = value; }
        }
        private String _htmlCode = null;
        private RulesSelector _frmRules = null;
        private String _filePath = null;
        private MenuItem menuInsertRules;
        private Boolean _parseHead = true;

        public String FilePath
        {
            get { return _filePath; }
        }
        public RulesSelector FrmRules
        {
            get
            {
                if (null == _frmRules)
                {
                    _frmRules = new RulesSelector();
                    _frmRules.OnLoadWorkflows += new RulesSelector.LoadWorkflows(_frmRules_OnLoadWorkflows);
                    _frmRules.OnSelectedStep += new RulesSelector.SelectedStep(_frmRules_OnSelectedStep);
                    _frmRules.OnSelectedWorkflowChanged += new RulesSelector.SelectedWorkflow(_frmRules_OnSelectedWorkflowChanged);
                    _frmRules.OnSelectedHtmlControl += new RulesSelector.SelectedHtmlControl(_frmRules_OnSelectedHtmlControl);
                }
        ...
        void _frmRules_OnSelectedHtmlControl(String html)
        {
            _htmlCode = html;
        }
        ...
        {
            set { FrmRules.Workflows = value; }
        }
        public Dictionary<Int64, String> Steps
        {
            set { FrmRules.Steps = value; }
        }
        public Dictionary<Int64, String> Rules
        {
            set { FrmRules.Rules = value; }
        }

        /// <summary>
        /// Contenido original del editor. Se usa para compararlo con el body antes de cerrar
        /// </summary>
        private String _originalContent = null;

        private Boolean IsContentChanged()
        {
            //Si el body del html y el original son iguales , da 0.
            return BodyHtml.CompareTo(_originalContent) != 0;
        }
        ...
        public event OnInsertedHtmlControl InsertedHtmlControl;
        public delegate void OnInsertedHtmlControl();
        ...
        private void _frmRules_OnSelectedWorkflowChanged(long workflowId)
        {
            OnSelectedWorkflowChanged(workflowId);
        }

        private void _frmRules_OnSelectedStep(long stepId)
        {
            OnSelectedStep(stepId);
        }

        private void _frmRules_OnLoadWorkflows()
        {
            OnLoadWorkflows();
        }
        ...
        private const string HTML_COMMAND_INSERT_TEXT_AREA = "InsertTextArea ";
        ...
        private const string INTERNAL_COMMAND_INSERTTEXTAREA = "InsertTextArea";
        private const string INTERNAL_COMMAND_INSERTSELECT = "InsertSelect";
        private const string INTERNAL_COMMAND_INSERTCHECKBOXES = "InsertCheckBoxes";
        ...
        private const string PASTE_TITLE_TEXT_AREA = "Enter Text Area";
        ...
        private String HeadHtml
        {
            get
            {
                if (String.IsNullOrEmpty(_headHtml))
                    _headHtml = "<HEAD></HEAD>";
                return _headHtml;
            }
            set { _headHtml = value; }
        }
        private HtmlStyleSheet _stylesheet;
        private HtmlScriptElement _script;
        private volatile bool _loading = false;
        private volatile bool _codeNavigate = false;
        private volatile bool _rebaseUrlsNeeded = false;
        ...
        public HtmlEditorControl()
        {
            ...
            _scrollBars = DisplayScrollBarOption.Automatico;
        ...
        private void DefineBodyAttributes()
        {
            ...
            if (_body.bgColor == null)
            {
                _bodyBackColor = _defaultBackColor;
            }
            else
            {
                _bodyBackColor = ColorTranslator.FromHtml((string)_body.bgColor);
            }
            if (_body.text == null)
            {
                _bodyForeColor = _defaultForeColor;
            ...
            {
                _bodyForeColor = ColorTranslator.FromHtml((string)_body.text);
            }

            // define the font object based on current font of new document
            // deafult used unless _frmRules style on the body modifies the value
            HtmlStyle bodyStyle = _body.style;
            if (bodyStyle != null)
            {
        ...
        private void InitializeComponent()
        {
            ...
            this.menuInsertRules = new System.Windows.Forms.MenuItem();
            ...
            this.menuInsertRules});
            resources.ApplyResources(this.menuItem1, "menuItem1");
            // 
            // menuInsertTable
            // 
            this.menuInsertTable.Index = 0;
            ...
            // menuInsertRules
            // 
            this.menuInsertRules.Index = 1;
            resources.ApplyResources(this.menuInsertRules, "menuInsertRules");
            this.menuInsertRules.Click += new System.EventHandler(this.menuInsertRules_Click);
            // 
            // menuItem3
            // 
        ...
        // defined with _frmRules TAG value set to the appropriate format Tag
        ...
        {
            // obtain the event object and ensure _frmRules context menu has been applied to the document
            HtmlEventObject eventObject = _document.parentWindow.@event;
        ...
        // call made sync using _frmRules loading variable
        private void BrowserCodeNavigate(string url)
        {
            ...
            object oUrl = (object)url;
            this.editorWebBrowser.Navigate2(ref oUrl);
            ...
            while (_loading)
            {
                ...
                {
                    MessageBox.Show("No se ha podido cargar el formulario.");
                    return;
                }
        ...
        // this event can be used to canel the navigation and open _frmRules new window
        ...
        private void BrowserBeforeNavigate(object sender, AxSHDocVw.DWebBrowserEvents2_BeforeNavigate2Event e)
        {
            ...
            if (!_codeNavigate)
            {
                ...
                {
                    // cancel the current navigation and load url into _frmRules new window
                    e.cancel = true;
        ...
        {
            Trace.WriteLineIf(ZTrace.IsInfo,"Instanciando Documento");
            // get access to the HTMLDocument
            _document = (HtmlDocument)this.editorWebBrowser.Document;

            Trace.WriteLineIf(ZTrace.IsInfo,"Casteando Body");
            _body = (HtmlBody)_document.body;
            ...
            Trace.WriteLineIf(ZTrace.IsInfo,"Ok:" + HRESULT.S_OK);
            int hResult = HRESULT.S_OK;
            // try to obtain the command target for the web browser document
            try
            {
                Trace.WriteLineIf(ZTrace.IsInfo,"Documento Target");
                // cast the document to _frmRules command target
                target = (IOleCommandTarget)_document;
                // set the appropriate no url fixups on paste
                Trace.WriteLineIf(ZTrace.IsInfo,"HResult");
            ...
            ((mshtml.DispHTMLDocument)_document).oncontextmenu = this;
            ...
            HTMLDocument doc = (mshtml.HTMLDocument)editorWebBrowser.Document;
            DOMEventHandler OnMouseUp = new DOMEventHandler(doc);
            OnMouseUp.Handler += new DOMEvent(MouseUp);
            ((mshtml.DispHTMLDocument)doc).onmouseup = OnMouseUp;
        }

        void MouseUp(IHTMLEventObj pEvtObj)
        {
            IHTMLTxtRange Range = GetTextRange();
            if (null != _itemToInsert && null != Range)
            {
                Range.pasteHTML(_itemToInsert.ToHtml());
                _itemToInsert = null;
            }
            else if (!String.IsNullOrEmpty(_htmlCode))
            {
                Range.pasteHTML(_htmlCode);
                _htmlCode = null;
            }
        ...
        // create _frmRules new focus method that ensure the body gets the focus
        ...
        [DefaultValue(DisplayScrollBarOption.Automatico)]
        public DisplayScrollBarOption ScrollBars
        {
            ...
            set
            {
                try
                {
                    ...
                    _body.scroll = _scrollBars.ToString();
        ...
        // defines the default action when _frmRules user click on _frmRules link
        ...
        public bool ReadOnly
        {
            ...
            set
            {
                ...
                _body.contentEditable = (!_readOnly).ToString();
                // define the menu bar state
        ...
        public string BodyHtml
        {
            get
            {
                ...
                _body.contentEditable = CONTENT_EDITABLE_INHERIT;
                string html = _body.outerHTML.Trim();
                this.ReadOnly = _readOnly;
            ...
            set
            {
                ...
                try
                {
                    ...
                    //{
                    //    // define _frmRules regular expression for the html body parsing and obtain the match expression
                    ...
                    //    // see if _frmRules match was found
                    //    if (match.success)
                    //    {
                    ...
                    //// first navigate to _frmRules blank page to reset the html header
                    BrowserCodeNavigate(BLANK_HTML_PAGE);



                    if (_parseHead && _document.firstChild.firstChild is HTMLHeadElement)
                    {
                        String[] separators = { "<head>", "</head>" };
                        String[] header = innerHtml.Split(separators, StringSplitOptions.RemoveEmptyEntries);
                        if (header.Length > 2) // <html> || contenido del head || body ...
                            HeadHtml = separators[0] + header.GetValue(1).ToString() + separators[1];
                        else
                            HeadHtml = separators[0] + separators[1];
                    }


                    // replace the body tag with the one passed in
                    HtmlDomNode oldBodyNode = (HtmlDomNode)_document.body;
                    HtmlDomNode newBodyNode = (HtmlDomNode)_document.createElement(bodyElement);
                    oldBodyNode.replaceNode(newBodyNode);

                    // define the new inner html and body objects
                    _body = (HtmlBody)_document.body;                
                    _body.innerHTML = innerHtml;
        ...
        public string DocumentHtml
        {
            get
            {
                return _document.documentElement.outerHTML.Trim();
            }
            ...
            set
            {
                try
                {
                    ...
                    if (range != null)
                    {
                        ...
                        // once have _frmRules range execute the pasteHtml command
        ...
        public Color BodyBackColor
        {
            ...
            set
            {
                // set the new value using _frmRules default if Empty passed in
                ...
                if (_body != null)
                {
                    ...
                    {
                        _document.bgColor = string.Empty;
                    }
                    else
                    {
                        _document.bgColor = ColorTranslator.ToHtml(_bodyBackColor);
                    }
                }
            }
        ...
        public Color BodyForeColor
        {
            ...
            set
            {
                ...
                if (_body != null)
                {
                    ...
                    {
                        _document.fgColor = string.Empty;
                    }
                    else
                    {
                        _document.fgColor = ColorTranslator.ToHtml(_bodyForeColor);
                    }
                }
            }
        ...
        // allow the user to load _frmRules document by navigation
        ...
        // allow the user to load _frmRules document into the specified window
        public void NavigateToUrl(string url, bool newWindow)
        {
            if (newWindow)
            {
                // open the Url in _frmRules new window
        ...
        // allow the user to load _frmRules document from _frmRules Url
        ...
        public void LoadFromUrl(string url)
        {
            ...
            try
            {
                ...
                if (IsStatedTag(maintype, "text") && IsStatedTag(subtype, "html"))
                {
                    ...
                    {
                        // use _frmRules default UTF8 encoder
                ...
                {
                    // navigated to _frmRules none Html document so throw exception
        ...
        // allow _frmRules user to load _frmRules file given _frmRules file name
        ...
        // allow the user to select _frmRules file and read the contents into the Html stream
        ...
        public void OpenFile(String filePath)
        {
            ...
            {
                contents = reader.ReadToEnd();
                _htmlDirectory = Path.GetDirectoryName(filePath);
            }
        ...
        /// <summary>
        /// Salva el archivo en un path que elige el user
        /// </summary>
        ...
        /// <summary>
        /// Salva el archivo , si ya existe pisa el archivo y sino el user elige donde
        /// </summary>
        ...
        /// <summary>
        /// allow the user to persist the Html stream to _frmRules file
        /// </summary>
        private void SaveFilePrompt()
        {
            // obtain the html contents
            
            string contents = CreateHeader();
            ValidateScripts();
            contents=contents.Insert(contents.Length, this.BodyHtml + "</html>");
            
            // create _frmRules file save dialog
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if ((stream = dialog.OpenFile()) != null)
                    {
                        ...
                        using (StreamWriter writer = new StreamWriter(stream))
                        {
                            ...
                            _htmlDirectory = Path.GetDirectoryName(dialog.FileName);
                            _filePath = dialog.FileName;
                            //Se salvo el documento , el contenido original pasa _frmRules ser el salvado , no el anterior
                            _originalContent = BodyHtml;

                            if (null != OnSavedDocument)
                                OnSavedDocument(dialog.FileName);
                        }
                        MessageBox.Show("El documento se ha guardado satisfactoriamente.", "Editor de Documentos HTML");
                    }
                    // close the input stream
                    if (stream != null) 
                        stream.Close();
        ...
        /// <summary>
        ///  Salva el contenido del html en un path especifico
        /// </summary>
        /// <param name="filePath"></param>
        private void SaveFile(String filePath)
        {
            //[Tomás] - 15/05/2009 - Modified - Se comento esta línea ya que insertaba etiquetas erroneas
            //                                  dentro del código y generaba un html que no se veía visualmente
            //                                  como el original.
            //ValidateScripts(); 
        ...
        public void LinkStyleSheet(string stylesheetHref)
        {
            if (_stylesheet != null)
            {
                ...
                _stylesheet.href = stylesheetHref;
            }
            else
            {
                _stylesheet = (HtmlStyleSheet)_document.createStyleSheet(stylesheetHref, 0);
            }
        ...
        {
            if (_stylesheet != null) return _stylesheet.href;
            else return string.Empty;

        } //GetStyleSheetHref


        // define _frmRules script element that is to be used by all documents
        ...
        public void LinkScriptSource(string scriptSource)
        {
            ...
            else
            {
                ...
                _script = (HtmlScriptElement)_document.createElement(SCRIPT_TAG);
                _script.src = scriptSource;
                _script.defer = true;
                // insert into the document
                HtmlDomNode node = (HtmlDomNode)_document.documentElement;
                node.appendChild((HtmlDomNode)_script);
            }
        ...
        {
            if (_script != null) return _script.src;
            else return string.Empty;
        ...
        {
            using (HtmlEditor dialog = new HtmlEditor())
            {
        ...
        // using the document toggles the selection with _frmRules bold tag
        ...
        // using the document toggles the selection with _frmRules underline tag
        ...
        // using the document toggles the selection with _frmRules italic tag
        ...
        // using the document toggles the selection with _frmRules Subscript tag
        ...
        // using the document toggles the selection with _frmRules Superscript tag
        ...
        // using the document toggles the selection with _frmRules Strikeout tag
        ...
        // given _frmRules int value increase the font size by that amount
        ...
        // insert _frmRules ordered or unordered list
        ...
        // insert _frmRules horizontal line in the body
        // if have _frmRules control range rather than text range one could overwrite the controls with the line
        ...
        // public function to insert _frmRules image and prompt _frmRules user for the link
        ...
        public void InsertImagePrompt()
        {
            ...
            using (EnterImageForm dialog = new EnterImageForm())
            {
                ...
                // after one has _frmRules valid image href
                if (dialog.ShowDialog(this.ParentForm) == DialogResult.OK)
                {
        ...
        // create _frmRules web link from the users selected text
        ...
        // public function to insert _frmRules link and prompt _frmRules user for the href
        ...
        public void InsertLinkPrompt()
        {
            ...
            if (hrefText != null)
            {
                ...
                // if text is _frmRules valid href then set the link
                ...
                // prompt the user for the new href
                using (HrefEditor dialog = new HrefEditor())
                {
                    ...
                    // based on the user interaction perform the neccessary action
                    // after one has _frmRules valid href
        ...
        // remove _frmRules web link from the users selected text
        ...
        public void InsertTextAreaPrompt()
        {
            InsertTextAreaPrompt(GetTextRange());
        }
        public void InsertTextAreaPrompt(IHTMLTxtRange range)
        {
            // display the dialog to obtain the Html to enter
            using (HtmlTextAreaEditor dialog = new HtmlTextAreaEditor())
            {
                ...
                if (dialog.ShowDialog(this.ParentForm) == DialogResult.OK)
                {
                    HTMLTextAreaElement TextArea = (HTMLTextAreaElement)_document.createElement(TEXT_AREA_TAG);
                    TextAreaItem CurrentTextArea = dialog.TextArea;
                    TextArea.cols = (Int32)CurrentTextArea.ColumnCount;
                    TextArea.rows = (Int32)CurrentTextArea.RowCount;
                    TextArea.innerText = CurrentTextArea.InnerText;

                    ProcessTextArea(range, TextArea);
                }
            }
        }

        public void InsertSelectPrompt()
        {
            InsertSelectPrompt(GetTextRange());
        }
        public void InsertSelectPrompt(IHTMLTxtRange range)
        {
            using (HtmlSelectEditor dialog = new HtmlSelectEditor())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    HTMLSelectElement Select = (HTMLSelectElement)_document.createElement(SELECT_TAG);
                    HTMLOptionElement Option = null;

                    foreach (OptionItem CurrentOption in dialog.SelectItem.Options)
                    {
                        Option = (HTMLOptionElement)_document.createElement(OPTION_TAG);

                        Option.value = CurrentOption.Value;
                        Option.text = CurrentOption.Name;
                        Option.selected = CurrentOption.Default;

                        Select.add((IHTMLElement)Option, null);
                    }


                    ProcessSelect(range, Select);
                }
            }
        }

        public void InsertCheckBoxesPrompt()
        {
            InsertCheckBoxesPrompt(GetTextRange());
        }
        public void InsertCheckBoxesPrompt(IHTMLTxtRange range)
        {
            using (ListHtmlCheckbox dialog = new ListHtmlCheckbox())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    IHTMLDOMAttribute NameAttribute = null;

                    foreach (CheckBoxItem CurrentCheckBox in dialog.CheckBoxes)
                    {
                        InputElement = (HTMLInputElement)_document.createElement(INPUT_TAG);
                        InputElement.type = INPUT_CHECKBOX_TYPE;

                        NameAttribute = _document.createAttribute("name");
                        NameAttribute.nodeValue = CurrentCheckBox.Checked;

                        //Los checkbox pueden o no tener texto.
                        if (String.IsNullOrEmpty(CurrentCheckBox.Name))
                            ProcessCheckBox(range, InputElement);
                        else
                            ProcessCheckBox(range, InputElement, CurrentCheckBox.Name);
                    }
                }
            }
        }

        public void InsertRadioButtonsPrompt()
        {
            InsertRadioButtonsPrompt(GetTextRange());
        }
        public void InsertRadioButtonsPrompt(IHTMLTxtRange range)
        {
            using (ListHtmlRadioButton dialog = new ListHtmlRadioButton())
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (RadioButtonItem CurrentRadio in dialog.RadioButtons)
                    {
                        InputElement = (HTMLInputElement)_document.createElement(INPUT_TAG);
                        InputElement.type = INPUT_RADIO_TYPE;
                        InputElement.name = CurrentRadio.Category;
                        InputElement.setAttribute("name", CurrentRadio.Category, 1);

                        if (CurrentRadio.Checked)
                            InputElement.setAttribute("checked", "checked", 0);

                        //Los radioButtons pueden o no tener texto.
                        if (String.IsNullOrEmpty(CurrentRadio.Name))
                            ProcessRadioButton(range, InputElement);
                        else
                            ProcessRadioButton(range, InputElement, CurrentRadio.Name);
                    }
                }
            }
        }
        ...
        // formats the selected text wrapping items in _frmRules PRE tag for direct editing
        ...
        // inserts _frmRules heading tag values Heading 1,2,3,4,5
        ...
        public void FormatFontAttributes(HtmlFontProperty font)
        {
            ...
            if (range != null && HtmlFontProperty.IsNotNull(font))
            {
                ...
                // set the font size provide set to _frmRules value
            ...
            {
                // do not have text selected or _frmRules valid font class
        ...
        // determine if the current font selected is bold given _frmRules range
        ...
        // determine if the current font selected is Underline given _frmRules range
        ...
        // determine if the current font selected is Italic given _frmRules range
        ...
        // determine if the current font selected is Strikeout given _frmRules range
        ...
        // determine if the current font selected is Superscript given _frmRules range
        ...
        // determine if the current font selected is Subscript given _frmRules range
        ...
        // dialog to allow the user to perform _frmRules find and replace
        public void FindReplacePrompt()
        {

            // define _frmRules default value for the text to find
            HtmlTextRange range = GetTextRange();
        ...
        // reset the find and replace options to initialize _frmRules new search
        public void FindReplaceReset()
        {
            // reset the range being worked with
            _findRange = (HtmlTextRange)_body.createTextRange();
            ((HtmlSelection)_document.selection).empty();
        ...
        // finds the next occurrence of _frmRules given text string
        // assumes _frmRules previous search was made
        // uses false for the search options
        ...
        public bool FindReplaceOne(string findText, string replaceText, bool matchWhole, bool matchCase)
        {
            ...
            {
                // if text found perform _frmRules replace
        ...
        public int FindReplaceAll(string findText, string replaceText, bool matchWhole, bool matchCase)
        {
            ...
            do
            {
                ...
                // if found perform _frmRules replace
                if (replaceRange != null)
                {
                    replaceRange.text = replaceText;
                    found++;
        ...
        private HtmlTextRange FindText(string findText, bool matchWhole, bool matchCase)
        {
            ...
            if (_findRange.findText(findText, _findRange.text.Length, searchOptions))
            {
                ...
                HtmlTextRange foundRange = (HtmlTextRange)_document.selection.createRange();
                _findRange = (HtmlTextRange)_body.createTextRange();
        ...
        // public function to create _frmRules table class
        // insert method then works on this table
        public void TableInsert(HtmlTableProperty tableProperties)
        {
            // call the private insert table method with _frmRules null table entry
            ProcessTable(null, tableProperties);

        } //TableInsert

        // public function to modify _frmRules tables properties
        // ensure _frmRules table is currently selected or insertion point is within _frmRules table
        public bool TableModify(HtmlTableProperty tableProperties)
        {
            // define the Html Table element
            HtmlTable table = GetTableElement();

            // if _frmRules table has been selected then process
            if (table != null)
            {
        ...
        {
            // if user has selected _frmRules table create _frmRules reference
        ...
        // will insert _frmRules new row into the table
        // based on the current user row and insertion after
        public void TableInsertRow()
        {
            // see if _frmRules table selected or insertion point inside _frmRules table
        ...
        // function to insert _frmRules basic table
        // will honour the existing table if passed in
        private void ProcessTable(HtmlTable table, HtmlTableProperty tableProperties)
        {
            try
            {
                // obtain _frmRules reference to the body node and indicate table present
                HtmlDomNode bodyNode = (HtmlDomNode)_document.body;
                bool tableCreated = false;

                // ensure _frmRules table node has been defined to work with
                if (table == null)
                {
                    // create the table and indicate it was created
                    table = (HtmlTable)_document.createElement(TABLE_TAG);
                    tableCreated = true;
                }
                ...
                if (tableProperties.TableAlignment != HorizontalAlignOption.Predeterminado) table.align = tableProperties.TableAlignment.ToString().ToLower();
                ...
                if (caption != null && caption != string.Empty)
                {
                    ...
                    if (tableProperties.CaptionAlignment != HorizontalAlignOption.Predeterminado) tableCaption.align = tableProperties.CaptionAlignment.ToString().ToLower();
                    if (tableProperties.CaptionLocation != VerticalAlignOption.Predeterminado) tableCaption.vAlign = tableProperties.CaptionLocation.ToString().ToLower();
                }
                else
                {
                ...
                if (tableCreated)
                {
                    // this section is an optimization based on creating _frmRules new table
                ...
                if (tableCreated)
                {
                    ...
                    HtmlSelection selection = _document.selection;
                    HtmlTextRange textRange = GetTextRange();
                    // final insert dependant on what user has selected
                    if (textRange != null)
                    {
                        // text range selected so overwrite with _frmRules table
                        try
                        {
        ...
        private void ProcessTextArea(IHTMLTxtRange range, HTMLTextAreaElement textArea)
        {
            if (null != range)
                range.pasteHTML(textArea.outerHTML);
        }

        private void ProcessSelect(IHTMLTxtRange range, HTMLSelectElement select)
        {
            if (null != range)
                range.pasteHTML(select.outerHTML);
        }

        private void ProcessRadioButton(IHTMLTxtRange range, HTMLInputElement radio)
        {
            if (null != range)
                range.pasteHTML(radio.outerHTML);
        }
        private void ProcessRadioButton(IHTMLTxtRange range, HTMLInputElement radio, String name)
        {
            if (null != range)
            {
                range.pasteHTML(radio.outerHTML);
                range.pasteHTML(name);
                range.pasteHTML("</input>");
            }
        }

        private void ProcessCheckBox(IHTMLTxtRange range, HTMLInputElement checkBox)
        {
            if (null != range)
                range.pasteHTML(checkBox.outerHTML);
        }
        private void ProcessCheckBox(IHTMLTxtRange range, HTMLInputElement checkBox, String name)
        {
            if (null != range)
            {
                range.pasteHTML(checkBox.outerHTML);
        ...
        // determine if the current selection is _frmRules table
        ...
        {
            // define _frmRules set of base table properties
            HtmlTableProperty tableProperties = new HtmlTableProperty(true);

            // if user has selected _frmRules table extract those properties
            if (table != null)
            {
                try
                {
                    // have _frmRules table so extract the properties
                    HtmlTableCaption caption = table.caption;
                    // if have _frmRules caption persist the values
                    if (caption != null)
                    {
                        tableProperties.CaptionText = ((HtmlElement)table.caption).innerText;
                        if (caption.align != null) tableProperties.CaptionAlignment = (HorizontalAlignOption)TryParseEnum(typeof(HorizontalAlignOption), caption.align, HorizontalAlignOption.Predeterminado);
                        if (caption.vAlign != null) tableProperties.CaptionLocation = (VerticalAlignOption)TryParseEnum(typeof(VerticalAlignOption), caption.vAlign, VerticalAlignOption.Predeterminado);
                    }
                    ...
                    if (table.align != null) tableProperties.TableAlignment = (HorizontalAlignOption)TryParseEnum(typeof(HorizontalAlignOption), table.align, HorizontalAlignOption.Predeterminado);
                    ...
                    if (table.width != null)
                    {
                        ...
                        if (tableWidth.TrimEnd(null).EndsWith("%"))
                        {
                            ...
                            tableProperties.TableWidthMeasurement = MeasurementOption.Porcentaje;
                        }
                        else
                        {
        ...
        // based on the user selection return _frmRules table definition
        ...
        // Determine if the insertion point or selection is _frmRules table
        ...
        private void ExecuteCommandRange(HtmlTextRange range, string command, object data)
        {
            try
            {
                if (range != null)
                {
                    // ensure command is _frmRules valid command and then enabled for the selection
        ...
        // executes the execCommand on the document with _frmRules system prompt
        ...
        private void ExecuteCommandDocument(string command, bool prompt)
        {
            ...
            {
                // ensure command is _frmRules valid command and then enabled for the selection
                if (_document.queryCommandSupported(command))
                {
                    // if (document.queryCommandEnabled(command)) {}
                    // Test fails with _frmRules COM exception if command is Print

                    // execute the given command
                    _document.execCommand(command, prompt, null);
                }
            }
            catch (Exception ex)
            {
        ...
        // obtains _frmRules control range to be worked with
        ...
        // performs _frmRules parse of the string into an enum
        ...
        // determine if _frmRules string url is valid
        ...
        {
            if (_rebaseUrlsNeeded)
            {
                // review the anchors and remove any references to about:blank
                HtmlElementCollection anchors = _body.getElementsByTagName(ANCHOR_TAG);
            ...
            try
            {
                ...
                switch (command)
                {
                    ...
                    case INTERNAL_COMMAND_LISTORDERED:
                        // Ordered List
                        FormatList(HtmlListType.Ordenada);
                        break;
                    case INTERNAL_COMMAND_LISTUNORDERED:
                        // Unordered List
                        FormatList(HtmlListType.Desordenada);
                        break;
                    ...
                    case INTERNAL_COMMAND_INSERTTABLE:
                        // Display _frmRules dialog to enable the user to insert _frmRules table
                        TableInsertPrompt();
                        break;
                    case INTERNAL_COMMAND_TABLEPROPERTIES:
                        // Display _frmRules dialog to enable the user to modify _frmRules table
                        TableModifyPrompt();
                        break;
                    ...
                    case INTERNAL_COMMAND_INSERTIMAGE:
                        // Display _frmRules dialog to enable the user to insert _frmRules image
                        InsertImagePrompt();
                        break;
                    case INTERNAL_COMMAND_INSERTLINK:
                        // Display _frmRules dialog to enable user to insert the href
                        InsertLinkPrompt();
                        break;
                    case INTERNAL_COMMAND_INSERTTEXT:
                        // Display _frmRules dialog to enable user to insert the text
                        InsertTextPrompt();
                        break;
                    case INTERNAL_COMMAND_INSERTTEXTAREA:
                        // Display _frmRules dialog to enable user to insert the text area
                        InsertTextAreaPrompt(GetTextRange());
                        break;
                    case INTERNAL_COMMAND_INSERTCHECKBOXES:


                    case INTERNAL_COMMAND_INSERTSELECT:
                        InsertSelectPrompt(GetTextRange());
                        break;
                    case INTERNAL_COMMAND_INSERTHTML:
                        // Display _frmRules dialog to enable user to insert the html
                        InsertHtmlPrompt();
                        break;
                    case INTERNAL_COMMAND_FINDREPLACE:
                        // Display _frmRules dialog to enable user to perform find and replace operations
                        FindReplacePrompt();
                        break;
                    ...
                    case INTERNAL_TOGGLE_SCROLLBAR:
                        ...
                        this.ScrollBars = (_scrollBars == DisplayScrollBarOption.No ? DisplayScrollBarOption.Automatico : DisplayScrollBarOption.No);
                        break;
        ...
        // method to raise an event if _frmRules delegeate is assigned
        ...
        private void menuInsertTextArea_Click(object sender, System.EventArgs e)
        {
            ProcessCommand(INTERNAL_COMMAND_INSERTTEXTAREA);
        }
        private void menuInsertSelect_Click(object sender, EventArgs e)
        {
            ProcessCommand(INTERNAL_COMMAND_INSERTSELECT);
        }
        private void menuInsertCheckBoxes_Click(object sender, EventArgs e)
        {
            InsertCheckBoxesPrompt();
        }
        private void menuInsertRadioButtons_Click(object sender, EventArgs e)
        {
            InsertRadioButtonsPrompt();
        }
        private void menuEditar_Popup(object sender, EventArgs e)
        {
            GetTextRange();
        }
        ...
        public void AddTextArea(IHTMLTextAreaElement textArea)
        {
            AddTextArea(textArea, null);
        }
        public void AddTextArea(IHTMLTextAreaElement textArea, HtmlTextRange range)
        {
            if (null == range)
                range = GetTextRange();
        }
        ...
        public void AddTable(IHTMLTable table)
        {
            AddTable(table, null);
        }
        public void AddTable(IHTMLTable table, HtmlTextRange range)
        {
            if (null == range)
                range = GetTextRange();
        }
        ...
        public void AddSelect(IHTMLSelectElement select)
        {
            AddSelect(select, null);
        }
        public void AddSelect(IHTMLSelectElement select, IHTMLTxtRange range)
        {
            if (null == range)
                range = GetTextRange();
        }
        ...
        public void AddRadioButton(HTMLInputElement radioButton)
        {
            AddRadioButton(radioButton, null);
        }
        public void AddRadioButton(HTMLInputElement radioButton, IHTMLTxtRange range)
        {
            if (null == range)
                range = GetTextRange();

            ProcessRadioButton(range, radioButton);
        }
        ...
        public void AddCheckBox(HTMLInputElement checkBox)
        {
            AddCheckBox(checkBox, null);
        }
        public void AddCheckBox(HTMLInputElement checkBox, IHTMLTxtRange range)
        {
            if (range == null)
                range = GetTextRange();

            ProcessCheckBox(range, checkBox);
        }

        #endregion

        /// <summary>
        /// Salva el documento si se modifico , se usa cuando el control contenedor se esta cerrando.
        /// </summary>
        public void OnParentClose()
        {
            if (IsContentChanged())
            {
                if (MessageBox.Show("Se ha modificado el documento. ¿Desea guardar los cambios realizados?", "Guardar", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    SaveFile();
            }
        ...
        /// <summary>
        /// Busca en el HTML si se encuentra un elemento con el id especificado
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public Boolean ExistsElementId(String id)
        {
            ...
            return InnerHtml.ToLower().Contains(id.ToLower());
        }

        private void menuInsertRules_Click(object sender, EventArgs e)
        {
            try
            {
                FrmRules.Show();
        ...
        {
            if (HtmlParser.HasAsociatedDocuments(this.BodyHtml))
            {
                String head = HeadHtml;
                HtmlParser.AppendAsociatedScript(ref head);
                HeadHtml = head;
                String body = this.BodyHtml;
                HtmlParser.AppendAsociatedDocContainer(ref body);
                _parseHead = false;
                BodyHtml = body;
                _parseHead = true;
            }

            if (HtmlParser.HasRules(this.BodyHtml))
            {
                String head = HeadHtml;
                HtmlParser.AppendRuleScript(ref head);
                HeadHtml = head;

                String body = this.BodyHtml;
                HtmlParser.AppendRuleIdContainer(ref body);
                _parseHead = false;
                BodyHtml = body;
                _parseHead = true;
            }
            try
            {
                if (!BodyHtml.ToUpper().Contains("<FORM"))
                {
                    String body = BodyHtml;
                    Int32 formIndex = body.ToUpper().IndexOf("<BODY>") + "<body>".Length;
                    body = body.Insert(formIndex, "<form name=frmMain>");

                    formIndex = body.ToUpper().IndexOf("</BODY>");
                    body = body.Insert(formIndex, "</form>");
                    _parseHead = false;
                    BodyHtml = body;
                    _parseHead = true;
                }

                if (!BodyHtml.Contains("name=frmMain"))
                {
                    String body = BodyHtml;

                    Int32 formIndex = body.ToUpper().IndexOf("<FORM") + "<form".Length;
                    body = body.Insert(formIndex, " name=frmMain ");
                    _parseHead = false;
                    BodyHtml = body;
                    _parseHead = true;
                }
