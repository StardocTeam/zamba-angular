...
Namespace MonitorFactory
    ...
    Public Class MonitorProcessFactory
        ...
        Public Shared Function GetFolders(ByVal UserConfigMachineName As String, ConfiguredFoldersIdsForMachine As String) As DataSet
            Dim dsTemp As DataSet
            Dim Strselect As String

            If UserConfigMachineName = String.Empty Then
                Strselect = "SELECT * FROM IP_FOLDER WHERE Upper(NOMBREMAQUINA) = '" & Environment.MachineName.ToUpper & "' " & If(ConfiguredFoldersIdsForMachine <> String.Empty, "and id in (" & ConfiguredFoldersIdsForMachine & ")", String.Empty)
            Else
                Strselect = "SELECT * FROM IP_FOLDER WHERE Upper(NOMBREMAQUINA) = '" & UserConfigMachineName.ToUpper & "' " & If(ConfiguredFoldersIdsForMachine <> String.Empty, "and id in (" & ConfiguredFoldersIdsForMachine & ")", String.Empty)
            End If
