...
Public Class IntegrityFactory
    Public Shared Function getAllDrI() As Hashtable
        ...
        Dim fila As DataRow
        Dim DtId As Int32 = 0
        Dim Atributos As New Hashtable
        Dim DrI As DrI
        ...
        For Each fila In dsTemp.Tables(0).Rows
            Try
                If DtId = CType(fila.Item("doc_type_id"), Int32) Then
                    Try
                        Atributos.Add(Integer.Parse(fila.Item("index_id")), Integer.Parse(fila.Item("index_id")))
                    Catch
                    End Try
                Else
                    'Creo el DrI
            ...
            Try
                If iDocI > 0 Then
                    If dtId = iDocI Then
                        ...
                        If iIndice > 0 Then
                            Atributos.Add(iIndice, iIndice)
                        End If
                    Else
                        'Creo el doci
                        DocI = New DocI
                        DocI.DtId = dtId
                        DocI.Atributos = Atributos
                        'Agrego el DocI a la lista
                        listaDocI.Add(dtId, DocI)
                        'Creo la nueva lista de atributos
                        Atributos = New Hashtable
                        'Guardo el atributo de la fila actual
                        Try
                            iIndice = Integer.Parse(fila.Item("Columna"))
                            Atributos.Add(iIndice, iIndice)
                        Catch
