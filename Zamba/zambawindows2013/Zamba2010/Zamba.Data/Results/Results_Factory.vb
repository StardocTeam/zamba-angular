...
#Region "GetResults"
    ...
    Public Shared Function getResultsData(ByVal docTypeId As Int64, ByVal indexId As Int32, ByVal genIndex As List(Of ArrayList), Optional ByVal comparateValue As String = "", Optional ByVal searchValue As Boolean = True, Optional ByVal strRestricc As String = "") As DataSet
        ...
        'strSql.Append(Chr(34))
        'strSql.Append("Fecha Creación del Documento")
        'strSql.Append(Chr(34))
    ...
    Public Shared Function LoadFileFromDB(ByVal DocId As Long, ByVal DocTypeId As Long) As Byte()
        ...
        Catch ex As SqlException
            ...
            If ex.Message.Contains("Valor de tiempo de espera caducado") Then
                If DialogResult.Yes = MessageBox.Show("El archivo que va a abrir puede tardar varios minutos en abrirse por ser muy pesado. ¿Desea continuar?", "Atención",
    ...
    Public Shared Function getResultsAndPageQueryResults(ByVal PageId As Int16, ByVal PageSize As Int16, ByVal docTypeId As Int64, ByVal indexId As Int64, ByVal genIndex As List(Of ArrayList), Optional ByVal RestrictionAndSortExpression As String = "", Optional ByVal SymbolToReplace As String = "", Optional ByVal BySimbolReplace As String = "", Optional ByRef resultCount As Integer = 0) As DataTable
        ...
        For Each index As ArrayList In genIndex
            ...
            Else
                ...
                If indxType = IndexAdditionalType.AutoSustitución Or indxType = IndexAdditionalType.AutoSustituciónJerarquico Then
...
#Region "Indexs"
    ...
    ''' 	[Gaston]    18/05/2009	Modified    Se comento una validación para hacer posible la inserción de atributos con datos vacíos
    ''' </history>
    ''' -----------------------------------------------------------------------------
    <Obsolete("Metodo discontinuado", False)>
        ...
        Try
            ...
            Else ' Reindexa
                ...
                For i = 0 To _result.Indexs.Count - 1
                    ...
                    ' Se comento el código porque sino es imposible insertar un Atributo con valor vacío
    ...
    ''' <history>
    ''' 	[Tomás]     18/03/09    Created
    '''     [Tomas]     19/03/09    Modified    Se agrega la opción para Oracle
    ''' </history>
    ''' -----------------------------------------------------------------------------
    ...
    '''     [Tomás]     19/03/09    Modified    Se agrega la opción para Oracle
    ...
    Public Shared Sub UpdateRegisterDocument(ByRef Result As NewResult, ByVal ReindexFlag As Boolean, Optional ByVal isvirtual As Boolean = False)
        Try
            ...
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Actualizo el tamaño")
            Try
                If isvirtual = False Then
                    If Server.isOracle Then
                        ''Dim parNames() As String = {"VolumeId", "FileSize"}
    ...
    Private Function CreateInsertQuery(ByRef Result As NewResult, ByVal tableName As String, ByVal isVirtual As Boolean, ByVal fileLen As Decimal, ByVal userid As Int64) As String
        Try
            ...
            Query.Append(",' ")
            'Tomas: Se agrega validación en el caso de que el file este en nothing
    ...
    '
    ''' <summary>
    ''' Se creo este overload porque cuando se intentaba realizar una insercion en la base de un tipo alfanumérico,
...
#Region "Delete"
    ...
    ''' <param name="newRes"></param>
    ''' <param name="t"></param>
    ''' <remarks>Se utiliza para migrar documentos existentes en servidor a la base de datos sin límite de timeout</remarks>
...
#Region "WorkFlow"
    ...
    'Por cada WAIT ID llama a la función ZWFIIIndexsValidation, si todos los WI devuelven
    'verdadero en esa función, entonces esta función devuelve verdadero. Si alguno de low WI
    'devuelven False o hay algún error. Esta función devuelve false.[Alejandro].
    ...
    'Verifica si hay documentos en ZWFI esperando por el DocType pasado por parámetro.[Alejandro].
    ...
    'En cada row del DataTable updatea el InsertID especificado por parámetro.
    'llamando a la función ZWFIUpdateInsertID. [Alejandro].
    ...
    'Updatea el InsertID pasado por parámetro en el WAIT ID pasado por parámetro
...
#End Region
    ...
    ''' <summary>
    ''' Método que devuelve un dataset con el contenido de la tabla ZDocRelations
    ''' </summary>
    ''' <param name="id"></param>
