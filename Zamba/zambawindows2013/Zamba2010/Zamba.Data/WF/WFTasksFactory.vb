...
#Region "Get"
    ...
    Public Function GetTasksByStepandDocTypeId(ByVal stepId As Int64,
        ...
        If FilterString IsNot Nothing AndAlso Not String.IsNullOrEmpty(FilterString.Trim()) Then

            FilterString = FilterString.Replace("strTableT", strTableT)
            FilterString = FilterString.Replace("strTableI", strTableI)

            'If Server.isOracle Then
            FilterString = FilterString.Replace("strTable", "WFDOCUMENT")
            FilterString = FilterString.Replace("[", "")
            FilterString = FilterString.Replace("]", "")
            FilterString = FilterString.ToUpper.Replace("WFDOCUMENT.CHECKIN", """Fecha de ingreso""")
            ...
            FilterString = FilterString.Replace("SS.NAME", "wfstepstates.name")
            'End If

            Dim my_rx As Regex = New Regex("I[0-9]+")

            'Si la doc_i esta adentro
            If strselect.ToString.ToLower.Contains(strTableI.ToLower()) AndAlso my_rx.Match(FilterString).Success Then

                Dim FilterSimpleIndexsString As New StringBuilder
                'saco del filtro los indices comunes y armo string para where interno.
                Dim FiltersList As List(Of String) = FilterString.Split(New String() {" AND "}, StringSplitOptions.None).ToList()

                For Each filter As String In FiltersList
                    If my_rx.Match(filter).Success Then
                        If FilterSimpleIndexsString.Length Then
                            FilterSimpleIndexsString.Append(" AND ")
                        End If
                        'FilterSimpleIndexsString.Append(" ")
                        FilterSimpleIndexsString.Append(filter)
                        FilterString = FilterString.Replace(filter, String.Empty)
                        FilterString = FilterString.Replace("  ", " ")
                ...
                If FilterSimpleIndexsString.Length Then
                    FilterSimpleIndexsString = FilterSimpleIndexsString.Insert(0, If(strselect.ToString.Contains("WHERE"), " AND ", " WHERE "))
                    If FilterSimpleIndexsString.ToString.Trim.EndsWith("AND") Then
                        FilterSimpleIndexsString.Clear()
                        FilterSimpleIndexsString.Append(FilterSimpleIndexsString.ToString.Trim.Substring(0, FilterSimpleIndexsString.ToString.Trim.Length - "AND".Length))
                    End If
                    'Lo agrego a la consulta interna
                    strselect.Append(FilterSimpleIndexsString)

                    If FilterString.Trim.EndsWith("AND") Then
                        FilterString = FilterString.Trim.Substring(0, FilterString.Trim.Length - "AND".Length)
                    End If
                    If FilterString.Trim.StartsWith("AND") Then
                        FilterString = FilterString.Trim.Substring("AND".Length).Trim()
                    End If
                End If
            ...
            If Not String.IsNullOrEmpty(FilterString) Then
                If Not FilterString.ToLower.Trim.StartsWith("and") Then
                    FilterString = FilterString.Insert(0, If(strPOSTselect.ToString.Contains("WHERE"), " AND ", " WHERE "))
                End If
                strPOSTselect.Append(FilterString)
            End If
        ...
        Else
            ...
            dt = GetTaskTable(strselect, strPREselect, strPOSTselect, order, PageSize, LastPage, False, SearchType.GridResults, TaskQueryMode.WFStep, orderType, GroupByString, GroupByCount)
        End If
    ...
    Public Function GetTasksByWFandDocTypeId(workflowid As Int64,
        ...
        Else
            If dateDeclarationString.Length > 0 Then
                ...
                strselect.Insert(0, dateDeclarationString)
            End If
            Return GetTaskTable(strselect, strPREselect, strPOSTselect, "task_id asc", PageSize, LastPage, False, SearchType.GridResults, TaskQueryMode.WFTask, Nothing, String.Empty, GroupByCount)
        End If
    ...
    Public Function GetTasksByTasksIdsAndDocTypeId(ByVal tasksids As List(Of Int64), ByVal stepId As Int64, ByVal docTypeId As Int64, ByVal indexs As List(Of IIndex), ByVal WithGridRights As Boolean, ByVal FilterString As String, ByVal PageSize As Int32, ByVal LastPage As Int64, ByVal RestrictionString As String, ByVal DateDeclarationString As String, ByVal FlagAsRead As Boolean, Optional ByVal auIndex As List(Of IIndex) = Nothing, ByRef Optional GroupByCount As Hashtable = Nothing, Optional slstFiltersIndexsID As List(Of Long) = Nothing) As DataTable
        ...
        Return GetTaskTable(strselect, strPREselect, strPOSTselect, "task_id asc", PageSize, LastPage, False, SearchType.GridResults, TaskQueryMode.WFStep, Nothing, String.Empty, GroupByCount)
    ...
    Public Function GetTaskByTaskIdAndDocTypeId(ByVal taskid As Int64, ByVal stepId As Int64, ByVal docTypeId As Int64, ByVal indexs As List(Of IIndex), ByVal WithGridRights As Boolean, ByVal FilterString As String, ByVal RestrictionString As String, ByVal DateDeclarationString As String, ByVal FlagAsRead As Boolean, Optional ByVal auIndex As List(Of IIndex) = Nothing, ByRef Optional GroupByCount As Hashtable = Nothing, Optional slstFiltersIndexsID As List(Of Long) = Nothing) As DataTable
        ...
        Try
            ...
            Return GetTaskTable(strselect, strPREselect, strPOSTselect, "task_id asc", 1, 1, True, SearchType.OpenTask, TaskQueryMode.WFTask, Nothing, String.Empty, GroupByCount)
        Finally
            strselect = Nothing
            firsttask = Nothing
        End Try
        ...
        Try
            ...
            Else
                ...
                If SearchType <> SearchType.OpenTask Then
                    ...
                    If (GroupByString IsNot Nothing AndAlso GroupByString.Length > 0) Then 'Si hay agrupado

                        'Qiçuito la numeracion de la consulta para traer los grupos
                        Dim pattern0 As String = "(?i)(,){1}(\s)*?(wd){1}(.){1}(rnum){1}"
                        Dim pre As String = Regex.Replace(strPREQuery.ToString, pattern0, String.Empty)

                        Dim pattern1 As String = "(?i)(,){1}(\s)*?(ROW_NUMBER){1}(.|\s)*?(order){1}(\s)+?(by){1}(.|\s)*?(\)){1}(.|\s)*?(RNUM){1}"
                        Dim query As String = " from (" & Regex.Replace(strQuery.ToString, pattern1, String.Empty) & " ) wd "

                        Dim post As String = strPOSTQuery.ToString

                        Dim GroupQueryString As New StringBuilder
                        GroupQueryString.Append(String.Concat(pre, query, post))

                        Dim DsGroups As DataSet

                        Dim GroupByColumns As String = GroupByString.Replace(" ASC", "").Replace(" DESC", "").Replace(Chr(34), "").Trim()

                        Dim func = String.Empty
                        'En algun caso la col viene envuelta en una funcion
                        If GroupByColumns.Contains("(") AndAlso GroupByColumns.Contains(")") Then
                            func = GroupByColumns.Substring(0, GroupByColumns.IndexOf("("))
                            GroupByColumns = GroupByColumns.Replace(func, "").Replace("(", "").Replace(")", "").Trim()
                        End If
                        ...
                        If Not String.IsNullOrEmpty(func) Then
                            GroupByColumns = func & "(" & GroupByColumns & ")"
                        End If
                        ...
                        GroupQuery.Append(GroupQueryString.ToString())
                        ...
                        For Each value As String In currentgroup.Keys
                            ...
                            If Not String.IsNullOrEmpty(Group) AndAlso Group.Trim() <> String.Empty AndAlso Group <> "Default" And GroupByString IsNot Nothing AndAlso GroupByString.Length > 0 Then
                                ...
                                For Each ColumnDescriptor As String In Group.Split(Char.Parse(","))
                                    ...
                                    'Aca hay que obtener el nombre de la columna en vez del alias. Lo mismo esta resuelto en WFTaskBussines
                                    ColumnDescriptor = GridColumns.GetRightColumnByGroupDescriptor(ColumnDescriptor)
    ...
    ''' <param name="autoSubstitutionIndexes"></param>
    ...
    ''' </history>
    Private Sub GetCommonTaskStringBuilder(docTypeId As Long,
                                           WithGridRights As Boolean,
                                           indexs As List(Of IIndex),
                                           autoSubstitutionIndexes As List(Of IIndex),
                                           stepId As Long,
                                           workflowid As Int64,
                                           QueryMode As TaskQueryMode,
                                           RestrictionString As String,
                                           DateDeclarationString As String,
                                           CheckInColumnIsShortDate As Boolean,
                                           FlagAsRead As Boolean,
                                           wfstateID As Int64,
                                           searchType As SearchType,
                                           CurrentUserId As Int64,
                                           orderString As String,
                                           slstFiltersIndexsID As List(Of Long),
                                           ByRef strPREselect As StringBuilder,
                                           ByRef strselect As StringBuilder,
                                           ByRef strPOSTselect As StringBuilder)

        If slstFiltersIndexsID Is Nothing Then slstFiltersIndexsID = New List(Of Long)
        ...
        Dim noLock As String = If(Server.isSQLServer, " WITH (nolock)", String.Empty)

        Dim MainJoin As String
        Dim MainPOSTJoin As String

        If QueryMode = TaskQueryMode.WFTask AndAlso searchType = SearchType.OpenTask Then
            MainJoin = $" FROM wfdocument WD {noLock} "
            MainPOSTJoin = $" INNER JOIN {strTableI} I {noLock} ON WD.doc_id = I.doc_id INNER JOIN {strTableT} T {noLock} ON I.doc_id = T.doc_id "
        Else
            MainJoin = $" FROM wfdocument WD {noLock} INNER JOIN {strTableI} I {noLock} on WD.doc_id = I.doc_id "
            MainPOSTJoin = $" INNER JOIN {strTableT} T {noLock} on WD.doc_id = T.doc_id "
        End If
        ...
        If searchType = SearchType.GridResults OrElse searchType = SearchType.OpenTask Then
            ...
            strselect.Append(" wd.doc_Id, wd.doc_type_Id, wd.step_Id, wd.Do_State_ID, wd.IconId, wd.User_Asigned, wd.Exclusive,")
            If Server.isOracle Then strselect.Replace("Exclusive", "C_Exclusive")
            strselect.Append(" wd.Task_ID, wd.Task_State_ID, wd.work_id, wd.Tag,")
            ...
            If Server.isSQLServer AndAlso CheckInColumnIsShortDate Then
            ...
            strPREselect.Append($" wd.doc_id, wd.doc_type_id, wd.step_id, wd.do_state_id, wd.iconid, wd.user_asigned, {If(Server.isOracle, "wd.c_exclusive", "wd.exclusive")}, wd.task_state_id, wd.work_id, wd.tag, wd.remark")

            strPREselect.Append($", wd.{Chr(34)}Tarea{Chr(34)}, wd.task_id")
            ...
            If Not QueryMode = TaskQueryMode.WFTask AndAlso Not searchType = SearchType.OpenTask Then
            ...
            If WithGridRights Then
                For f = 0 To indexs.Count - 1
                    ...
                    If autoSubstitutionIndexes IsNot Nothing AndAlso autoSubstitutionIndexes.Contains(DirectCast(indexs(f), Index)) AndAlso Not DirectCast(indexs(f), Index).isReference Then
            ...
            Else
                For f = 0 To indexs.Count - 1
                    If Not indexs(f).isReference Then
                        IndexsString.Append(",I" & DirectCast(indexs(f), Index).ID)
                    End If
                Next
            End If
            ...
            If QueryMode = TaskQueryMode.WFTask AndAlso searchType = SearchType.OpenTask Then
                strPREselect.Replace("wd.*", IndexsString.ToString())
            Else
            ...
            If FlagAsRead Then strPREselect.Append(", ZDocReads.crdate As READDATE ")

            If QueryMode <> TaskQueryMode.WFTask AndAlso searchType <> SearchType.OpenTask Then
                strselect.Append(", ROW_NUMBER() OVER (ORDER  BY " & orderString & ") RNUM  ")
            End If

        Else 'Solo para obtener total
        ...
        Dim HasRestrictions As Boolean = RestrictionString IsNot Nothing AndAlso Not String.IsNullOrEmpty(RestrictionString.Trim)
        ...
        If searchType = SearchType.GridResults OrElse searchType = SearchType.OpenTask Then
            strPOSTselect.Append($" LEFT JOIN disk_Volume {noLock} ON disk_Vol_id = T.vol_id")
        End If
        ...
        If searchType = SearchType.GridResults OrElse searchType = SearchType.OpenTask OrElse searchType = SearchType.GridResultsCount Then
            ...
            strPOSTselect.Append($" left join wfstepstates {noLock} On do_state_id = doc_state_id")
            strPOSTselect.Append(" left join zvw_UserAndGroups uag On user_asigned = uag.id")
            strPOSTselect.Append($" left join wftask_states {noLock} On wftask_states.task_state_id = wd.Task_State_ID")

            'Verifica si debe obtener la marca de leido
            If FlagAsRead AndAlso searchType <> SearchType.WFStepCount Then
                strPOSTselect.Append($" left join ZDocReads On I.doc_id = ZDocReads.DOCID And ZDocReads.USERID = {CurrentUserId} ")
            End If
        ...
        If HasRestrictions OrElse searchType = SearchType.GridResults OrElse searchType = SearchType.OpenTask OrElse slstFiltersIndexsID.Count > 0 Then 'OrElse searchType = SearchType.GridResultsCount
            ...
            If autoSubstitutionIndexes IsNot Nothing Then
                For Each indice As IIndex In autoSubstitutionIndexes
                    If Not indice.isReference Then
                        If searchType <> SearchType.GridResultsCount OrElse (searchType = SearchType.GridResultsCount AndAlso slstFiltersIndexsID.Count > 0 AndAlso slstFiltersIndexsID.Contains(indice.ID)) Then
                            strPOSTselect.Append($" LEFT JOIN SLST_S{indice.ID} {noLock} ON I{indice.ID} = slst_s{indice.ID}.codigo")
                        End If
                    End If
                Next
            End If
        ...
        If Not String.IsNullOrEmpty(RestrictionString) AndAlso RestrictionString.Trim() <> String.Empty Then
            If Not strselect.ToString().ToLower().Contains("where") Then strselect.Append(" WHERE ")
