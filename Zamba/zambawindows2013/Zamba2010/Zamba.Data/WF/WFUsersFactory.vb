Public Class WFUsersFactory
    ...
    Public Shared Function GetUsersByStepID(ByVal stepId As Int64) As DataSet
        ...
        If (dstemp.Tables.Count > 0) Then
            ...
            dsFull.Tables(0).Merge(dstemp.Tables(0), True, MissingSchemaAction.Add)
        End If
    ...
    Public Shared Function GetDistinctUsersByStepID(ByVal stepId As Int64) As DataSet
        ...
        Dim Strselect As String = "select distinct (id), name from ( Select distinct usrtable.id,usrtable.name from usrtable,USR_RIGHTS where (aditional = " & stepId & " and usrtable.id=USR_RIGHTS.groupid) UNION select distinct(id), name from USRTABLE U INNER JOIN USR_R_GROUP R on u.id =r.USRID where r.GROUPID in ( Select distinct(id) from USRGROUP inner join usr_Rights on usr_Rights.groupid = USRGROUP.id where ADITIONAL = " & stepId & ") ) q order by name"
