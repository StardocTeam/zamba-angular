...
Public Class DataBaseIndexsFactory
    ...
    '                Dim j As Integer
    '                For j = 0 To atributos.Rows.Count - 1
    '                    'ME FIJO SI EL CAMPO PERTENECE AL INDICE Y LO CARGO A LA COLECCION CORRESPONDIENTE
    '                    If dsDocDIndex.Tables(0).Rows(i).Item("D" & atributos.Rows(j).Item("Index_Id")) Then
    '                        newdocd.Asigned_Columns.Add(New DocDIndex(atributos.Rows(j).Item("Index_Id"), atributos.Rows(j).Item("Index_Name")))
    '                    Else
    '                        newdocd.No_Asigned_Columns.Add(New DocDIndex(atributos.Rows(j).Item("Index_Id"), atributos.Rows(j).Item("Index_Name")))
    '                    End If
    '                Next
    ...
    '    If dsDocDIndex.Tables(0).Rows.Count > 0 Then
    '        Dim i As Integer
    '        'ARMO UNA COLECCION CON TODOS LOS OBJETOS DOCD QUE CONTIENEN LOS ATRIBUTOS DE BUSQUEDA
    '        For i = 0 To dsDocDIndex.Tables(0).Rows.Count - 1
    '            Dim newdocd As New DocD
    '            newdocd.Index_Id = dsDocDIndex.Tables(0).Rows(i).Item("Index_Id")
    '            newdocd.Index_Name = dsDocDIndex.Tables(0).Rows(i).Item("Index_Name")
    ...
    '            For j = 0 To atributos.Rows.Count - 1
    '                'ME FIJO SI EL CAMPO PERTENECE AL INDICE Y LO CARGO A LA COLECCION CORRESPONDIENTE
    '                If dsDocDIndex.Tables(0).Rows(i).Item("D" & atributos.Rows(j).Item("Index_Id").ToString) Then
    '                    newdocd.Asigned_Columns.Add(New DocDIndex(atributos.Rows(j).Item("Index_Id"), atributos.Rows(j).Item("Index_Name")))
    '                Else
    '                    newdocd.No_Asigned_Columns.Add(New DocDIndex(atributos.Rows(j).Item("Index_Id"), atributos.Rows(j).Item("Index_Name")))
    '                End If
    ...
    'Public Shared Function Create_New_Index(ByVal docId As Integer, ByVal Name As String, ByVal atributos As DataTable) As DocD
    ...
    '    '       Dim cols(atributos.Rows.Count + 2) As String
    ...
    '    Values.Append("',0")
    '    Dim i As Integer = 0
    '    For i = 0 To atributos.Rows.Count - 1
    '        Columns.Append(",D")
    '        Columns.Append(atributos.Rows(i).Item("Index_Id"))
    '        Values.Append(",0")
    '        'COMO EL OBJETO DOCD ES NUEVO NO TIENE CREADO el atributo
    '        newdocd.No_Asigned_Columns.Add(New DocDIndex(atributos.Rows(i).Item("Index_Id"), atributos.Rows(i).Item("Index_Name")))
    '    Next
    ...
    '    'Dim cols(atributos.Rows.Count + 1) As String
    ...
    '        Col.Append("D")
    '        Col.Append(atributos.Rows(i).Item("Index_Id"))
    '        StrUpdate.Append(",")
    '        StrUpdate.Append(Col)
    '        StrUpdate.Append(" = ")

    '        'cols.SetValue("I" & atributos.Rows(i).Item("Index_Id"), i + 2)
    ...
    Public Shared Sub Create_Database_Index(ByVal docd_obj As DocD, ByVal doc_dindex As Integer, ByVal Cluster As Boolean, ByVal Unico As Boolean)
        ...
        If Server.isOracle Then
            ...
            ' Tengo que averiguar si existen en Oracle los atributos Clusters     
    ...
    Public Shared Function VerificarServer() As Boolean
        'ORACLE no permite crear atributos Agrupados, entonces lo deshabilito
        If Server.isOracle Then
