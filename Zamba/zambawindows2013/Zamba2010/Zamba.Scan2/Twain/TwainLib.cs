...
public abstract class  Twain
	...
	internal Twain()
		...
		appid.Manufacturer		= "Stardoc";
		appid.ProductFamily		= "Zamba";
		appid.ProductName		= "Zamba.Scan";
	...
	public void initialize( IntPtr hwndp )
		{
		free();
    ...
    public virtual bool scan(bool showGUI, bool guiModal)
	{
	    TwRC rc;
	    CloseSrc();
	    ...
	    guif.ShowUI   = (short)(showGUI == true ? 1 : 0);
        guif.ModalUI = (short)(guiModal == true ? 1 : 0);
	...
	protected virtual ArrayList TransferPictures()
		{
		...
		do
			...
			pics.Add( hbitmap );
			this.imageScan(hbitmap, pxfr.Count);
        ...
        protected abstract void imageScan( IntPtr hbitmap, int imageCount );


	    protected TwainCommand PassMessage( ref Message m )
		{
	        if( srcds.Id == IntPtr.Zero )
	    ...
	    protected void CloseSrc()
		{
		    if( srcds.Id != IntPtr.Zero )
			{
			    ...
			    {
                /*
                    Thread t =  new Thread(new ThreadStart(this.endCloseSrc));
                    t.Start();
                    t.Join();
                  */
                GlobalLock        (this.appid.Id);
                GlobalLock        (srcds.Id);
                
                this.endCloseSrc   ();
                
                GlobalUnlock      (this.appid.Id);
                GlobalUnlock      (srcds.Id);
              }
			}
		}
		
		
		private void endCloseSrc() 
		{
            TwRC rc;
            TwUserInterface guif = new TwUserInterface();
