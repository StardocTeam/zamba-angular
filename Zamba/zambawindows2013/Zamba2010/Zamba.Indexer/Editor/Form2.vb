...
#Region "Interno"
    ...
    Private Sub GetThumbs1(ByVal I As Image)
        Try
            ...
            Invoke(New LockMeDelegate(AddressOf LockMe), New Object() {True})
            ...
            If iCount > 0 Then
                For x = 0 To iCount
                    ...
                    If bAbort Then
                        Exit Sub
                    End If
                    Invoke(New AddPictureToFlowPanel1Delegate(AddressOf AddPictureToFlowPanel1), New Object() {Pic})
                Next
        ...
        Finally
            Application.DoEvents()
            Invoke(New LockMeDelegate(AddressOf LockMe), New Object() {False})
        End Try
    ...
    Private Sub ToolStripButton3_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles ToolStripButton3.Click
        Try
            If zoomw > Zoomfactor Then zoomw = zoomw - Zoomfactor
            myResize()
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub ResizeT1()
        Try
            If Resizing1 = False Then
                Resizing1 = True
                Dim CB As New Drawing.Image.GetThumbnailImageAbort(AddressOf CB1)
                Dim CBData As System.IntPtr
                For Each P As Pic In FlowLayoutPanel1.Controls
                    '  P.OriginalImage.SelectActiveFrame(P.Dimension, P.Page)
                    Dim Size As New Size(zoomw, zoomh(P.OriginalImage))
    ...
    Private Sub ResizeT2()
        Try
            If Resizing2 = False Then
                Resizing2 = True
                Dim CB As New Drawing.Image.GetThumbnailImageAbort(AddressOf CB1)
                Dim CBData As System.IntPtr
                For Each p As Pic In FlowLayoutPanel2.Controls
                    '                    p.OriginalImage.SelectActiveFrame(p.Dimension, p.Page)
                    Dim Size As New Size(zoomw, zoomh(p.OriginalImage))
                    p.Invoke(New DAdjustSize(AddressOf AdjustSize), New Object() {p, Size})
                    Dim T As Image
    ...
    Private Shadows Sub MouseDown(ByVal o As Object, ByVal e As MouseEventArgs)
        If Dragging = False Then
            Dragging = True
            DraggingRepresentation.Location = e.Location
            DraggingRepresentation.Size = New Size(zoomtw, zoomth(DirectCast(o, PictureBox).Image))
            DraggingRepresentation.Image = DirectCast(o, PictureBox).Image
            DraggingRepresentation.Visible = True
            DraggingObject = DirectCast(o, PictureBox)
            DraggingImage = DirectCast(o, PictureBox).Image
    ...
    Private Sub DragDrop1(ByVal sender As Object, ByVal e As DragEventArgs) Handles FlowLayoutPanel2.DragDrop, FlowLayoutPanel1.DragDrop, DraggingRepresentation.DragDrop
        Try
            If Dragging = True Then
                Dragging = False
                DraggingRepresentation.Visible = False
                DirectCast(DestinationPanel, FlowLayoutPanel).Controls.Add(DraggingObject)
                ...
                If String.Compare(sender.GetType.Name, "FlowLayoutPanel", True) = 0 Then
                    ...
                    Try
                        If IsNothing(c) Then
                            Position = CInt(DirectCast(sender, FlowLayoutPanel).Controls.Count)
                        Else
                            Position = DirectCast(sender, FlowLayoutPanel).Controls.GetChildIndex(c)
                        End If
                    Catch ex As Exception
                    End Try
                    Try
                        DirectCast(DestinationPanel, FlowLayoutPanel).Controls.SetChildIndex(DraggingObject, Position)
                    Catch ex As Exception
                    End Try
                End If
            End If
    ...
    Private Sub DragEnter2(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles DraggingRepresentation.DragEnter
        e.Effect = DragDropEffects.All
    End Sub
    Private Sub DragOver1(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs) Handles FlowLayoutPanel1.DragOver, FlowLayoutPanel2.DragOver, ToolStripContainer1.DragOver, ZToolBar1.DragOver, ToolStripButton1.DragOver, MyBase.DragOver, DraggingRepresentation.DragOver
        Try
            If Dragging = True Then
                DraggingRepresentation.Location = New Point(e.X + 1, e.Y + 1)
            End If
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    ...
    Private Sub DragOver2(ByVal sender As Object, ByVal e As System.Windows.Forms.DragEventArgs)
        Try
            ...
            End If
            Try
                Position = DirectCast(sender, PictureBox).Parent.Controls.GetChildIndex(DirectCast(sender, PictureBox))
            Catch ex As Exception
            End Try
        Catch ex As Exception
            MsgBox(ex.ToString)
    ...
    Private Sub ToolStripComboBox1_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ToolStripComboBox1.SelectedIndexChanged
        Try
            If ToolStripComboBox1.Text = "Horizontal" Then
...
#Region "Metodos IPlugIn"
    ...
    Public Sub initialize(ByVal o As Object)
        Try
            LocalResult = o
            AddResult(DirectCast(o, Zamba.Core.TaskResult).FullPath)
        Catch ex As Exception
    ...
    Private Sub ToolStripButton4_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles ToolStripButton4.Click
        bAbort = True
        RaiseEvent CloseDocument(LocalResult)
    ...
    Private Sub SaveDocument()
        Try
            ...
            m_helper.PicArray = DirectCast(FlowLayoutPanel1.Controls, IList)
            m_helper.PathOriginal = m_FileResult
            m_path = m_helper.Save()
            RaiseEvent Save(LocalResult)
        Catch ex As Exception
            Throw New ArgumentException(ex.Message)
        End Try
    End Sub
