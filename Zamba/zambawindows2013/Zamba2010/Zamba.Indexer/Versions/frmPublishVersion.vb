...
Public Class frmPublishVersion
    Inherits ZForm
...
#Region " Constructor "
    Public Sub New(ByRef Publishresult As PublishableResult, ByVal result As Result, ByRef comment As String, ByRef fecha As String)
        ...
        IndexsIds.AddRange(IndexsBusiness.GetIndexsIdsByDocTypeId(Result.DocTypeId))
...
#End Region
    ...
    Private Sub BntPublish_Click(ByVal sender As System.Object, ByVal e As EventArgs) Handles BntPublish.Click
        Try
            ' Esto no sabemos lo que hace, Martin lo va a ver.
            Results_Business.SavePublish(_result, _Publishresult)

            '21/03/2017 - Ivan
            Dim convertToPDF As Boolean = False
            ' Si es office preguntar si quiere convertir a PDF
            If _result.FileType = FileType.WORD AndAlso MessageBox.Show("Desea convertir archivo a PDF?", "Publicar", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                convertToPDF = True
            End If
            ' Copiar e insertar nuevo Result con Version = 2
            Results_Business.PublishResultVersion(_result, convertToPDF)
            Close()
            ...
            If String.Compare(Membership.MembershipHelper.CurrentUser.eMail.Mail, String.Empty) <> 0 Then
                ...
                If validMails.Count > 0 Or externalMails.Count > 0 Then
                    ...
                    If validMails.Count = mails.Count OrElse MessageBox.Show("Hay usuarios sin cuentas de mail definidas, ¿Desea continuar el envio?", "Zamba", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = DialogResult.Yes Then
                        ...
                        Dim frm As New frmNotifyPublish(link, validMails, _result.FullPath)
                        frm.ShowDialog()
                        Close()
                    End If
                Else
    ...
    Private Sub PublishAndSendInternalMessage()
        Try
            ...
            If users.Count > 0 Then
                ...
                Dim frm As New frmNotifyPublish(link, users)
                frm.ShowDialog()
                Close()
            Else
