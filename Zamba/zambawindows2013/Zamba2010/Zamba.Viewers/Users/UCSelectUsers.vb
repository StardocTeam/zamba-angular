...
#Region "Eventos"
    ...
    Public Sub Aceptar()
        ...
        For Each u As IUser In usuariosAgregados
            If GroupId <> 0 Then
                NotifyBusiness.SetNewUserToNotify(TypeId, GroupId, u.ID)
            Else
                'For Each DocId As Long In docIds

                NotifyBusiness.SetNewUserToNotify(TypeId, docIds.Item(0), u.ID)
        ...
        For Each u As IUser In usuariosBorrados
            If GroupId <> 0 Then
                NotifyBusiness.DeleteUserToNotify(GroupId, u.ID)
            Else
                'For Each docid As Long In docIds
        ...
        For Each g As IUserGroup In gruposAgregados
            If GroupId <> 0 Then
                NotifyBusiness.SetNewUserGroupToNotify(TypeId, GroupId, g.ID)
            Else
                'For Each docid As Long In docIds
                NotifyBusiness.SetNewUserGroupToNotify(TypeId, docIds.Item(0), g.ID)
                'Next
            End If
        Next
        ...
        For Each g As IUserGroup In gruposBorrados
            If GroupId <> 0 Then
                NotifyBusiness.DeleteUserGroupToNotify(GroupId, g.ID)
            Else
                'For Each docid As Long In docIds
        ...
        For Each mail As String In extmails
            ...
            If GroupId <> 0 Then
                NotifyBusiness.SetNewMailToNotify(TypeId, GroupId, mail)
            Else
                'For Each docid As Long In docIds
                NotifyBusiness.SetNewMailToNotify(TypeId, docIds(0), mail)
                'Next
            End If
        Next
        ...
        Try
            ...
            Else
                ...
                If docIds.Count > 0 Then
                    idRecipesUsers = NotifyBusiness.GetGroupToNotifyUsers(TypeId, docIds)
            ...
            If docIds.Count > 0 Then
                idSelectedGroups = NotifyBusiness.GetGroupToNotifyGroups(TypeId, docIds)
            Else
