...
#Region "Atributos"
    ...
    Public Property NewRuleId() As Int64
#End Region

#Region "Constructores"
    Public Sub New(ByVal BaseNode As BaseWFNode, ByVal IL As Zamba.AppBlock.ZIconsList)
        Me.New(BaseNode, IL, False)
    End Sub

    Public Sub New(ByVal BaseNode As BaseWFNode, ByVal IL As Zamba.AppBlock.ZIconsList, ByVal blnFilterUIRules As Boolean)
        MyBase.New()
        InitializeComponent()
        ...
        For Each M As System.Type In a.GetTypes
            ...
            If (className.StartsWith("if") OrElse className.StartsWith("do")) And String.Compare(className, "ifbranch") <> 0 Then
                ...
                If Not filterUIRules Then
                    If nodeText = "Todas" OrElse
                        nodeText = mainCategory OrElse
                        nodeText = category OrElse
                        nodeText = subCategory Then
                        If rulesDescriptions.ContainsKey(description) = False Then rulesDescriptions.Add(description, New RulesDescription(description, help, category, ruleName, iconId, mainCategory, subCategory))
                    End If
                End If
            End If
        Next
            ...
            Try
                ...
                If frmName.ShowDialog() = DialogResult.OK Then
                    ...
                    If baseNode.NodeWFType = NodeWFTypes.TipoDeRegla AndAlso DirectCast(baseNode, RuleTypeNode).RuleParentType = TypesofRules.Eventos Then
                        frmEvents = New WFUserControl.frmchooseevent
                        frmEvents.ShowDialog()
                        TypeofRule = DirectCast(frmEvents.Selected, TypesofRules)
                    Else
                        TypeofRule = TypesofRules.Regla
                    End If

                    NewRuleId = WFRulesBusiness.Add(SelectedRuleClass, baseNode, ruleName, TypeofRule)
