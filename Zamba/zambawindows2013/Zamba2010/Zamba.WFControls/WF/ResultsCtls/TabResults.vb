...
Namespace WF.ResultsCtls
    ...
    Public Class TabResults
        ...
        Public Property LastSearchSelected() As ISearch

        Private Sub RefreshGrid()
            Try
                If LastSearchSelected Is Nothing Then
                    LastSearchSelected = LastSearch
                    If LastSearchSelected Is Nothing Then Exit Sub
                End If

                Dim WP As New BackgroundWorker
                AddHandler WP.DoWork, AddressOf RefreshGridHandler
                AddHandler WP.RunWorkerCompleted, AddressOf RefreshGridCompletedHandler
                WP.RunWorkerAsync(New With {LastSearchSelected, LastSearch})

            Catch ex As Exception
                ZClass.raiseerror(ex)
                ZTrace.WriteLineIf(ZTrace.IsError, "No se pudo actualizar grilla despues de cerrar tarea")
            End Try
        End Sub

        Private Sub RefreshGridHandler(sender As Object, e As DoWorkEventArgs)
            Try
                If e.Argument IsNot Nothing Then
                    Dim dt As DataTable = ModDocuments.DoSearchBySearch(e.Argument.LastSearchSelected)
                    e.Result = dt
                End If
            Catch ex As Exception
                ZClass.raiseerror(ex)
        ...
        Private Sub RefreshGridCompletedHandler(sender As Object, e As RunWorkerCompletedEventArgs)
            Try
                ...
                RaiseEvent SearchReload(dt.MinimumCapacity, LastSearch)
            Catch ex As Exception
                ZClass.raiseerror(ex)
                ZTrace.WriteLineIf(ZTrace.IsError, "No se pudo actualizar grilla despues de cerrar tarea")
            End Try
        ...
        Public Event SearchReload(ByVal ResultsCount As Integer, search As ISearch)
...
#Region "Show"
        ...
        Private Sub NewDocumentViewer(ByRef Result As Result, Optional ByVal ShowOriginal As Boolean = False)
            ...
            If Not IsOpened Then
                ...
                RemoveHandler UcViewer.CloseOpenTask, AddressOf RefreshGrid
                ...
                AddHandler UcViewer.CloseOpenTask, AddressOf RefreshGrid
            Else
