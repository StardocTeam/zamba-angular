...
#Region "TaskViewer"
        ...
        Public Sub ClosingTaskInTaskDetails(ztc As UCTaskViewer)
        ...
        Private Sub ShowDocumentInTaskViewerLazy(ByRef ParentTaskId As Int64, ByVal Result As IResult)
            ...
            Try
                ...
                If (IsNothing(ztc)) Then
                    ...
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "No estaba abierto el UCTaskViewer " & Now.ToString)
                    task = wftb.GetTaskByTaskIdAndDocTypeId(ParentTaskId, Result.DocTypeId, 0)
        ...
        Friend Sub CloseTaskViewer(ByVal _TaskResult As TaskResult, ByVal refreshGrid As Boolean)
            Try
                If Disposing = False Then
                    If refreshGrid = True AndAlso UserPreferences.getValue("ShowTaskTreeAndGrid", UPSections.UserPreferences, False) = True Then
...
#Region "TaskController"
        ...
        Private Sub LoadController()
            ...
            ControllerTask = New Controller(CurrentUserId, Me)
            ...
            RemoveHandler ControllerTask.tabResults.SearchReload, AddressOf UpdateNodeSearchCount
            ...
            AddHandler ControllerTask.tabResults.SearchReload, AddressOf UpdateNodeSearchCount
        End Sub
...
#End Region
        ...
        Private Sub UpdateNodeSearchCount(ByVal ResultsCount As Integer, search As ISearch)
            WFTreeControlForClient.RefreshSearchNode(ResultsCount, search)
        End Sub
        ...
        Friend Sub ShowController(ByRef Search As ISearch, ByVal query As String, ByVal queryCount As String)
            Try
                ...
                If ControllerTask.tabResults IsNot Nothing AndAlso Search IsNot Nothing Then
                    ControllerTask.tabResults.LastSearchSelected = Search
                End If
                ...
                If ControllerTask.tabResults.UCFusion2 IsNot Nothing Then
                    ...
                    If Not String.IsNullOrEmpty(query) Then
                        dt = Core.Search.ModDocuments.SearchRows(query, queryCount, 0)
                    Else
                        dt = Core.Search.ModDocuments.DoSearchBySearch(Search)
                    End If
                    'Llena la grilla con los resultados obtenidos
                    ControllerTask.tabResults.UCFusion2.FillResults(dt, Search)
                    WFTreeControlForClient.RefreshSearchNode(dt.MinimumCapacity, Search)
                End If
        ...
        Public Sub addTasksFromResults(ByVal data As DataTable, ByVal Search As ISearch, ByVal taskCount As Integer)
            If Search IsNot Nothing Then
                ...
                ElseIf Not WFTreeControlForClient.IsSearchNodeSelected(Search.Name) Then
                    ...
                    ControllerTask.tabResults.ShowFilesInFolder(data, If(Search IsNot Nothing, Search.Name, String.Empty), Search)
                    WFTreeControlForClient.RefreshSearchNode(data.MinimumCapacity, Search)
                Else
                    WFTreeControlForClient.RefreshSearchNode(data.MinimumCapacity, Search)
                End If
        ...
        Private Sub refreshTask(ByRef TV As UCTaskViewer, ByVal TaskResult As ITaskResult)
            If TV IsNot Nothing Then
                ...
                '' [Claudia - 30/10/2019] - Se actualiza la propiedad Text del DockTaskDetails (ZTaskContent) con el nuevo Name de TaskResult(ITaskResult)
                DockTaskDetails.Text = TaskResult.Name
...
#Region "Refresh"
        ...
        Public Function CloseTasks() As Boolean
            ...
            For Each _zZTaskContent As ZTaskContent In UcDocumentsParent.TabPages
                If Not _zZTaskContent.TaskViewer Is Nothing Then
                    If _zZTaskContent.TaskViewer.CloseTaskViewer(True) = False Then
                        Return False
                    End If
        ...
        Public Function CloseCurrentTask() As Boolean
            Try
                ...
                If UcDocumentsParent.SelectedTab IsNot Nothing Then
                    ...
                    ActiveTaskViewer.TaskViewer.CloseTaskViewer(True)
...
#End Region
        ...
        Public Sub OpenTask(ByVal Task As ITaskResult)
            ...
            If UserPreferences.getValue("ShowTaskTreeAndGrid", UPSections.UserPreferences, False) = True Then ControllerTask.UCTaskGrid.SelectTask(Task.TaskId)
            'Muestra la tarea
            ShowTask(Task)
        End Sub
        ...
        Private Sub ToolBar1_ButtonClick(ByVal sender As System.Object, ByVal e As ToolStripItemClickedEventArgs) Handles ToolBar1.ItemClicked
            ...
            If Not (TypeOf (e.ClickedItem) Is GenericRuleButton) Then
                ...
                Try
                    ...
                    _tasks = wftb.GetTasksByTaskIdsAndDocTypeIdAndStepId(tasksids, DirectCast(ControllerTask.UCTaskGrid.GridView.cmbDocType, System.Windows.Forms.ToolStripComboBox).ComboBox.SelectedValue, stepid)
                ...
                For Each RuleRow As DsRules.WFRulesRow In WFStep.DSRules.WFRules.Rows
                    ...
                    If Rule.ParentType = TypesofRules.AccionUsuario AndAlso RuleEnabled = True AndAlso Rule.Enable = True Then
                        ...
                        Try
                            ...
                            userActionName = WFB.GetUserActionName(Rule.ID, Rule.WFStepId, Rule.Name, True).ToUpper
                            WFB = Nothing
                        Catch ex As Exception
                            ZClass.raiseerror(ex)
