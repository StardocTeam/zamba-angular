...
#Region "Get"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function NewResult(ByVal r As DataRow, ByVal s As WFStep, Optional ByVal inThread As Boolean = False) As TaskResult
            Try
                ...
                If Not IsNothing(Doctype) Then
                    ...
                    If Server.isOracle Then
                        Result = New TaskResult(s.ID, r.Item(GridColumns.TASK_ID_COLUMNNAME), r.Item(GridColumns.DOC_ID_COLUMNNAME), Doctype, ResultName, r.Item(GridColumns.ICONID_COLUMNNAME), r.Item("C_Exclusive"), Task_State_ID, Doctype.Indexs, s.InitialState)
                    Else
                        Result = New TaskResult(s.ID, r.Item(GridColumns.TASK_ID_COLUMNNAME), r.Item(GridColumns.DOC_ID_COLUMNNAME), Doctype, ResultName, r.Item(GridColumns.ICONID_COLUMNNAME), r.Item("Exclusive"), Task_State_ID, Doctype.Indexs, s.InitialState)
                    End If
                    ...
                    End If
                    If Not IsDBNull(r(GridColumns.ORIGINAL_FILENAME_COLUMNNAME)) Then
                        Result.OriginalName = r.Item(GridColumns.ORIGINAL_FILENAME_COLUMNNAME)
                    End If
                    ...
                    Else
                        ...
                        For i = 0 To Result.Indexs.Count - 1
                            Try
                                Dim IndexColumn As String = "I" & DirectCast(Result.Indexs(i), Index).ID
                                If r.Table.Columns.Contains(IndexColumn) = False Then
                                    IndexColumn = DirectCast(Result.Indexs(i), Index).Name
                                End If

                                If r.Table.Columns.Contains(IndexColumn) = True Then
                                    If Not IsDBNull(r(IndexColumn)) Then
                                        DirectCast(Result.Indexs(i), Index).Data = r(IndexColumn).ToString
                                        DirectCast(Result.Indexs(i), Index).DataTemp = r(IndexColumn).ToString
        ...
        Public Function GetTasksByStepandDocTypeId(ByVal stepId As Int64,
                                                   ...
                                                   Optional ByVal wfstateID As Int64 = 0,
                                                   Optional ByVal GroupByString As String = "",
                                                   Optional ByRef GroupByCount As Hashtable = Nothing,
                                                   Optional ByRef slstIndexsIds As List(Of Long) = Nothing
                                                   ) As DataTable
            ...
            Try
                ...
                Dim filterElements As List(Of IFilterElem)
                ...
                If slstIndexsIds Is Nothing Then
                    slstIndexsIds = New List(Of Long)
                End If
                ...
                If searchType <> SearchType.WFStepCount Then
                    If FC IsNot Nothing Then
                        ...
                        filters = FC.GetFiltersString(filterElements, False, isCaseSensitive, slstIndexsIds)
                    End If
                End If

                '(pablo) ZUserConfig Option - Converts the DateTime type column into a Date Column
                If UserPreferences.getValue("CheckInColumnShortDateFormat", UPSections.UserPreferences, "True") Then
                    checkInColumnIsShortDate = True
                End If

                'Obtencion del string de restricciones
                sbColumnCondition = New StringBuilder
                sbDateDeclaration = New StringBuilder
                ...
                Dim VerAsignadosAOtros As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosAOtros, stepId)
                Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosANadie, stepId)

                Dim wftf As New WFTasksFactory

                orderType = GetOrderDataType(Order)
                Order = GetOrderString(Order)
                GroupByString = GetGroupString(GroupByString)
                ...
                If WithGridRights Then
                    ...
                    If searchType <> SearchType.WFStepCount Then
                        ...
                        If UserPreferences.getValue("ShowIndexsOnGrid", UPSections.UserPreferences, "True") Then
                        ...
                        dt = wftf.GetTasksByStepandDocTypeId(stepId,
                                                                 ...
                                                                 orderType, GroupByString, GroupByCount, slstIndexsIds)
        ...
        Private Function GetOrderDataType(Order As String) As String
            Dim orderType As String
            If Not String.IsNullOrEmpty(Order) Then
                Dim col As String = GridColumns.GetColumnByOrderString(Order)
                If GridColumns.IsIndex(col) Then
                    Dim currentIndex As IIndex = IndexsBussinesExt.getIndex(IndexsBusiness.GetIndexIdByName(col), True)
        ...
        ''' <summary>
        ''' Obtiene el ordenamiento para la consulta segun las columnas de la grilla-
        ''' </summary>
        ''' <param name="Order"></param>
        ''' <returns></returns>
        Private Function GetOrderString(Order As String) As String

            Dim _order As String = String.Empty

            If Not String.IsNullOrEmpty(Order) Then

                For Each orderString As String In Order.Split(",")

                    Dim column = GridColumns.GetColumnByOrderString(orderString)
                    Dim orderDirection = GridColumns.GetOrderByOrderString(orderString)

                    If GridColumns.IsIndex(column.Replace("""", String.Empty)) Then

                        _order &= "I" & IndexsBusiness.GetIndexIdByName(column.Replace("""", String.Empty)) & " " & orderDirection

                    Else

                        If Server.isOracle Then
                            If column.Replace("""", String.Empty).Equals(GridColumns.NOMBRE_DOCUMENTO_COLUMNNAME) Then

                                _order &= "to_char(" & column & ")"
                                _order = _order.Replace(Chr(34), Chr(39))
                            Else
                                _order &= GridColumns.GetWFColumnNameByAliasName(column)
                            End If
                        Else
                            _order &= GridColumns.GetWFColumnNameByAliasName(column)
                        End If
                ...
                If _order.Trim.EndsWith(",") Then
                    _order = _order.Substring(0, _order.LastIndexOf(","))
                End If
        ...
        ''' <summary>
        ''' Devuelve el mismo string de ordenamiento, con el nombre de la columna en la bd en vez de el alias de la grilla
        ''' Se usa para los indices que son slst nada mas
        ''' </summary>
        ''' <param name="Groups"></param>
        ''' <returns></returns>
        Private Function GetGroupString(Groups As String) As String

            Dim _groups As New StringBuilder

            If Not String.IsNullOrEmpty(Groups) Then

                For Each group As String In Groups.Split(",")

                    If Not String.IsNullOrEmpty(group) Then

                        Dim column = GridColumns.GetColumnByOrderString(group)
                        Dim orderDirection = GridColumns.GetOrderByOrderString(group)

                        If Not GridColumns.IsIndex(column.Replace("""", String.Empty)) AndAlso Server.isOracle Then
                            If column.Replace("""", String.Empty).Equals(GridColumns.NOMBRE_DOCUMENTO_COLUMNNAME) Then
                                column = "to_char(" & column & ")"
                            End If
                        End If

                        _groups.Append(column)
                        _groups.Append(" ")
                        _groups.Append(orderDirection)
                        _groups.Append(",")
            ...
            If _groups.ToString.Trim.EndsWith(",") Then
                '_groups.Clear()
                _groups.Replace(_groups.ToString, _groups.ToString.Substring(0, _groups.ToString.LastIndexOf(",")))
            End If
        ...
        Public Function GetTasksByWFandDocTypeId(workflowid As Int64, ByVal stepId As Int64, ByVal DocTypeId As Int64, ByVal WithGridRights As Boolean, ByVal CurrentUserID As Int64, ByRef FC As FiltersComponent, ByVal LastDocId As Int64, ByVal PageSize As Int32, ByVal searchType As SearchType, Optional ByVal wfstateID As Int64 = 0, ByRef Optional GroupByCount As Hashtable = Nothing, Optional ByRef slstIndexsIds As List(Of Long) = Nothing) As DataTable
            ...
            Try
                ...
                'Obtencion del string de restricciones
                ColumCondstring = New StringBuilder
                dateDeclarationString = New StringBuilder
                Dim wftsbs As New WFTaskBusiness
                ...
                If WithGridRights Then
                    ...
                    Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosANadie, stepId)
                    Dim wftf As New WFTasksFactory
                    dt = wftf.GetTasksByWFandDocTypeId(workflowid, DocTypeId, indexsaux, WithGridRights, FilterString, ColumCondstring.ToString(), dateDeclarationString.ToString(), LastDocId, PageSize, CheckInColumnIsShortDate, autosustindex, searchType, VerAsignadosAOtros, VerAsignadosANadie, False, wfstateID, Membership.MembershipHelper.CurrentUser, GroupByCount, slstIndexsIds)
                    wftf = Nothing
                Else
                    indexs.AddRange(DocTypesBusiness.GetDocType(DocTypeId, True).Indexs)
                    Dim VerAsignadosAOtros As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosAOtros, stepId)
                    Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosANadie, stepId)
                    Dim wftf As New WFTasksFactory
                    dt = wftf.GetTasksByWFandDocTypeId(workflowid, DocTypeId, indexs, WithGridRights, FilterString, ColumCondstring.ToString(), dateDeclarationString.ToString(), LastDocId, PageSize, CheckInColumnIsShortDate, Nothing, searchType, VerAsignadosAOtros, VerAsignadosANadie, False, wfstateID, Membership.MembershipHelper.CurrentUser, GroupByCount, slstIndexsIds)
                    wftf = Nothing
                End If
                Return dt
            Catch ex As Exception
        ...
        Public Shared Function GetStepsTasksCountByStepsandDocTypeId(ByVal stepIds As List(Of Int64), ByVal DocTypeId As Int64) As DataTable
            ...
            Try
                ...
                Dim wftsbs As New WFTaskBusiness
                wftsbs.CompleteRestrictionString(ColumCondstring, dateDeclarationString, DocTypeId, Membership.MembershipHelper.CurrentUser.ID, True)
                wftsbs = Nothing
                ...
                Dim flagAsRead As Boolean = RightsBusiness.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.FlagAsRead, DocTypeId)
                ...
                SyncLock (Cache.DocTypesAndIndexs.hsRestrictionsStrings)
                    ...
                    If useCache Then
                        If Not Cache.DocTypesAndIndexs.hsRestrictionsStrings.Contains(CurrentUserID & "-" & DocTypeId) Then
                            indRestriction = RestrictionsMapper_Factory.getRestrictionIndexs(CurrentUserID, DocTypeId, useCache)
        ...
        '''        [Javier]   06/10/10  Modified    Se agregan sentencias para obtener los strings de restricciones sobre docs
        ''' </history>
        Public Function GetTasksByTaskIdsAndDocTypeIdAndStepId(ByVal taskIds As List(Of Int64),
                                                                      ByVal docTypeId As Int64,
            ...
            Dim dt As DataTable
            Dim sbColumnCondition As New StringBuilder
            Dim sbDateDeclaration As New StringBuilder
        ...
        Public Shared Function GetAllTasksByDocId(ByVal docId As Int64, ByVal doctypeid As Int64, ByVal PageSize As Int32) As Generic.List(Of ITaskResult)
            ...
            For Each id As DataRow In TaskId.Tables(0).Rows
                AssociatedTask.Add(WFTB.GetTaskByTaskIdAndDocTypeId(id("task_id"), doctypeid, PageSize))
...
#Region "AddtoWF"
        ...
        Public Shared Sub ExecuteCheckInRulesFromInsert(ByVal InsertedWFIdsAndInitialStepsAndTasksResults As ArrayList, ByVal OpenTaskAfterInsert As Boolean)
            If Not IsNothing(InsertedWFIdsAndInitialStepsAndTasksResults) AndAlso InsertedWFIdsAndInitialStepsAndTasksResults.Count = 2 Then
                ...
                wfRules = WFRulesBusiness.GetInstanceRules(InsertedWFIdsAndInitialStepsAndTasksResults(0), TypesofRules.ValidacionEntrada, False)
                ...
                wfRules = WFRulesBusiness.GetInstanceRules(InsertedWFIdsAndInitialStepsAndTasksResults(0), TypesofRules.Entrada, False)
                If wfRules IsNot Nothing Then
                    For Each r As WFRuleParent In wfRules
                        TasksResults = WFRB.ExecutePrimaryRule(r, TasksResults, Nothing)
                    Next
                ...
                wfRules = WFRulesBusiness.GetInstanceRules(InsertedWFIdsAndInitialStepsAndTasksResults(0), TypesofRules.Insertar, True)
                If wfRules IsNot Nothing Then
                    For Each r As WFRuleParent In wfRules
        ...
        Public Shared Sub AddResultsToWorkFLow(ByVal TResults As ArrayList, ByVal WF As WorkFlow, ByVal p_WStep As WFStep, Optional ByVal OpenTaskAfterInsert As Boolean = True)
            ...
            For Each Result As Core.TempTaskResult In TResults
                Dim t As New TaskResult(p_WStep.ID, CoreData.GetNewID(IdTypes.Tasks), Result.Result.ID, Result.Result.DocType, Result.Result.Name, Result.Result.IconId, 0, CInt(Result.TaskState), Result.Result.Indexs, p_WStep.InitialState, Result.AsignedToId)
                t.WorkId = WF.ID
                t.CheckIn = Now
                TasksResults.Add(t)
                WFTaskBusiness.LogCheckIn(t)
        ...
        ''' </history>
        ''' <remarks></remarks>
        Public Sub OpenTask(ByVal result As Zamba.Core.ITaskResult, ByVal params As Hashtable)
            Try
        ...
        ''' </history>
        Public Sub AddResultsToWorkFLowSinceRuleADDTOWF(ByVal Results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal wfId As Integer, ByVal p_WStep As WFStep, ByVal initialStateId As Int32, ByVal OpenTaskAfterInsert As Boolean)
            Dim TasksResults As New System.Collections.Generic.List(Of Core.ITaskResult)
            Dim wf As WorkFlow = WFBusiness.GetWorkFlow(wfId)
...
#Region "Indexs & DocType"
        ...
        Friend Shared Sub FillIndexs(ByRef Result As TaskResult, Optional ByVal inThread As Boolean = False)
            Try
                ...
                Result.Indexs = ZCore.FilterIndex(CInt(Result.DocTypeId))
                'Results_Factory.FillIndexData(Result)
                Results_Business.CompleteDocument(Result, inThread)
            Catch ex As Exception
                ZClass.raiseerror(ex)
            End Try
        End Sub
        ...
        Private Shared Sub FillDocType(ByRef Result As TaskResult)
            Try
                ...
                If DTs.Contains(Result.DocTypeId) = False Then
                    Result.Parent = DocTypesBusiness.GetDocType(Result.DocTypeId, True)
                    DTs.Add(Result.Parent.ID, Result.Parent)
                Else
                    Result.Parent = DirectCast(DTs(Long.Parse(Result.DocTypeId.ToString())), IZambaCore)
                    If IsNothing(Result.Parent) Then
                        Result.Parent = DirectCast(DTs(Result.DocTypeId), IZambaCore)
                    End If
                End If
            Catch ex As Exception
                ZClass.raiseerror(ex)
        ...
        Friend Shared Function HaveRightToView(ByRef wfstep As WFStep, ByVal AsignedToId As Int64, ByVal CurrentUserId As Int64) As Boolean
            Try
                If AsignedToId = 0 Then
                    'Asignado a nadie
                    If Not UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosANadie, wfstep.ID) Then
                        Return False
                    End If
                ElseIf AsignedToId = CurrentUserId Then
                    'Asignado a uno
                    If Not UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.Use, wfstep.ID) Then
                        Return False
                    End If
                Else
                    'Asignado a otro
                    If Not UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.VerAsignadosAOtros, wfstep.ID) Then
                        Return False
                    End If
                End If
                Return True
            ...
            Try
                ...
                For Each Result As ITaskResult In Results
                    ...
                    If Resultsaux.Contains(Result) Then
                        ...
                        Result.State = newWFStep.InitialState
        ...
        Public Shared Sub Asign(ByRef result As ITaskResult, ByVal asignedToId As Int64, ByVal asignedById As Int64, ByVal logAction As Boolean, ByVal RefreshUI As Boolean)
            Try
                ...
                If logAction Then
                    ...
                    WFTasksFactory.LogAsignedTask(result.TaskId, result.Name, result.DocTypeId, result.DocType.Name, result.StepId, result.State.Name, asignedToName, result.WorkId, Membership.MembershipHelper.CurrentUser.Name, stepname, workflowname)
        ...
        Private Shared Sub RefreshAsignedDate(ByVal t As TaskResult, ByVal r As DsDocuments.DocumentsRow)
            Try
                If IsDBNull(r("Date_Asigned_By")) Then
                    ...
                    End If
                Else
                    Dim DateAsignedBy As Date = r("Date_Asigned_By")
...
#Region "Load"
        Public Shared Sub LoadMonitor(ByVal WF As WorkFlow, ByVal TreeView As RadTreeView)
            Try
                ...
                For Each s As WFStep In WF.Steps.Values
                    Try
                        Dim StepNode As New MonitorStepNode((s))
...
#Region "Nodes"
        ...
        Private Shared Sub UpdateNodesTasksCountExtracted(ByVal n As RadTreeNode)
            DirectCast(n, MonitorStepNode).UpdateNodeText()
        End Sub

        Public Shared Sub UpdateNodesTasksCount(ByVal WFNode As WFNode)
            Try
                For Each n As RadTreeNode In WFNode.Nodes
...
#Region "Logs"
        ...
        Public Shared Sub LogUserAction(ByVal taskID As Int64, ByVal taskName As String, ByVal docTypeId As Int64, ByVal docTypeName As String, ByVal stepId As Int32, ByVal state As String, ByVal workflowId As Int32, ByVal accionDeUsuario As String)
            Dim stepname As String = WFStepBusiness.GetStepNameById(stepId)
            If (stepname Is Nothing) Then
                stepname = "sin Etapa"
            End If
            Dim workflowname As String = WFBusiness.GetWorkflowNameByWFId(workflowId)
            If (workflowname Is Nothing) Then
                workflowname = "sin workflow"
            End If
...
#End Region
        ...
        Public Shared Function GetTaskHistory(ByVal taskId As Int64, ByVal doctypeId As Long, Optional ByVal fc As IFiltersComponent = Nothing) As DataSet
            ...
            If fc IsNot Nothing Then
                ...
                Filters = CType(fc, FiltersComponent).GetFiltersString(filterElements, False, isCaseSensitive, Nothing)
            End If
...
#End Region
        ...
        Private Shared Sub CreateTasksRestrictionsWhere(ByRef sbValue As StringBuilder, ByVal Indexs As List(Of IIndex), ByRef i As Int64, ByRef FlagCase As Boolean, ByRef ColumCondstring As StringBuilder, ByRef First As Boolean, ByRef dateDeclarationString As StringBuilder)
            ...
            If sbValue.Length <> 0 OrElse Indexs(i).[Operator].ToLower = "es nulo" Then
                ...
                Select Case Indexs(i).[Operator]
                    Case "="
                        Op = "="
                        If FlagCase = True AndAlso (Indexs(i).Type = IndexDataType.Alfanumerico OrElse Indexs(i).Type = IndexDataType.Alfanumerico_Largo) AndAlso (IsNumeric(Results_Business.ReplaceChar(mainValue, String.Empty)) = False) Then
                            ColumCondstring.Append(" (lower(" & indexColName & ") " & Op & " " & mainValue & ")")
                        ElseIf (Indexs(i).Type = IndexDataType.Numerico OrElse Indexs(i).Type = IndexDataType.Numerico_Largo OrElse Indexs(i).Type = IndexDataType.Numerico_Decimales OrElse Indexs(i).Type = IndexDataType.Moneda) Then
                            ColumCondstring.Append(" (" & indexColName & " " & Op & " " & mainValue.ToString().Replace("'", "") & ")")
                        Else
                    ...
                    Case "Entre"
                        ...
                        Try
                            ...
                            Select Case Indexs(i).Type
                                ...
                                Case 7, 8
                                    If Not String.IsNullOrEmpty(Indexs(i).Data2) Then
                                        FlagCase = True
                                        tempValue = "'" & LCase(Indexs(i).Data2) & ""
                                        Data2Added = True
                                    End If
                            End Select
                        Catch ex As Exception
                        ...
                        End If
                    Case "Contiene"
                        If FlagCase = True AndAlso (IsNumeric(Results_Business.ReplaceChar(Replace(Trim(mainValue), "'", ""), String.Empty)) = False) Then
                        ...
                        For x = 0 To SomeValues.Length - 1
                            ...
                            If IsNothing(Val) = False AndAlso Not String.IsNullOrEmpty(Val.Trim) Then
                                If i = 0 AndAlso x = 0 Then
                                    If FlagCase = True AndAlso (IsNumeric(Results_Business.ReplaceChar(Val.Replace("'", ""), String.Empty)) = False) Then
                                ...
                                ElseIf x > 0 Then
                                    If String.IsNullOrEmpty(somestring) Then
                                        If FlagCase = True AndAlso (IsNumeric(Results_Business.ReplaceChar(Replace(Val, "'", "").Trim, String.Empty)) = False) Then
                    ...
                    Case "Distinto"
                        ...
                        somestring = Nothing
                    Case "Dentro"
                        If FlagCase = True AndAlso (IsNumeric(Results_Business.ReplaceChar(mainValue, String.Empty)) = False) Then
                            Select Case Indexs(i).Type
                    ...
                    Case "SQL Sin Atributos"
                        ColumCondstring.Append(" (" & mainValue & ")")
                    Case "SQL"
                        ColumCondstring.Append(" (" & indexColName & " in (" & mainValue & "))")
                End Select
                Op = Nothing
        ...
        '''     [Marcelo]   30/11/2009  Modified    Si el documento esta en un solo wf lo devuelve, sino filtra por workid
        ''' </history>
        Public Function GetTaskByDocId(ByVal docId As Int64, Optional ByVal WorkId As Int64 = 0) As ITaskResult
            ...
            If Not IsNothing(ds) And ds.Tables.Count = 1 And ds.Tables(0).Rows.Count > 0 Then
                If ds.Tables(0).Rows.Count = 1 Or WorkId = 0 Then
                    Return GetTask(ds.Tables(0).Rows(0)(0))
                Else
                    ds.Tables(0).Select("Work_id=" + WorkId)
                    If ds.Tables(0).Rows.Count > 0 Then
                        Return GetTask(ds.Tables(0).Rows(0)(0))
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetTask(ByVal taskId As Int64, Optional ByVal WfStep As IWFStep = Nothing, Optional ByVal useRestrictions As Boolean = True) As ITaskResult
            ...
            If Not IsNothing(ds) And ds.Tables.Count = 1 And ds.Tables(0).Rows.Count > 0 Then
                ...
                Tasks = GetTasksByTaskIdsAndDocTypeIdAndStepId(tasksids, ds.Tables(0).Rows(0)("DOC_TYPE_ID"), ds.Tables(0).Rows(0)("step_id"), useRestrictions)
        ...
        Public Shared Function LockTask(ByVal taskId As Int64, ByRef currentLockedUser As String) As Boolean
            ...
            Else
                If Server.isOracle Then
                    currentLockedUser = Server.Con.ExecuteScalar(CommandType.Text, String.Format("select (apellido || ',' || nombres) as Usuario from usrtable where id = (select c_exclusive from wfdocument where task_id = {0})", taskId))
                    Return False
                Else
                    currentLockedUser = Server.Con.ExecuteScalar(CommandType.Text, String.Format("select (apellido + ',' + nombres) as Usuario from usrtable where id = (select exclusive from wfdocument where task_id = {0})", taskId))
                    Return False
                End If
