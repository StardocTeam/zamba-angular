...
Namespace DocTypes
        ...
        Public Shared Function IsValidDiskVolume(ByVal DiskGroupId As Int32) As Boolean
            ...
            Try
                ...
                For i = 0 To Vols.Tables(0).Rows.Count - 1
                    Try
                        Dim VolId As Int64 = Vols.Tables(0).Rows(i).Item("DISK_VOLUME_ID")
                        Dim VolType As VolumeTypes = VolumesBusiness.GetVolumeType(VolId)
                        If VolType = VolumeTypes.Disco_Rigido Then
                            volPath = VolumesFactory.RetrieveVolumePath(VolId)
                            'chequeo el volumen
    ...
    Public Class DocTypeDataBase
        ...
        Public Shared Sub AddColumn(ByVal doctype As Zamba.Core.DocType, ByVal IndexIdArray As ArrayList, ByVal IndexTypeArray As ArrayList, ByVal IndexLenArray As ArrayList)
            DocTypesFactory.AddColumn(doctype.ID, IndexIdArray, IndexTypeArray, IndexLenArray)
        End Sub
    ...
    Public Class DocType
        ...
        Shared Sub CreateView(DocTypeId As Long)
            Dim Cr As CreateTables = Server.CreateTables
            Try
                'agrego la tabla del documento
                Cr.CreateView(DocTypeId)
            Catch ex As Exception
                Throw New Exception("Ocurrió un error al crear las tablas. " & ex.ToString, ex)
            End Try
        End Sub
...
Namespace MonitorBusiness
    Public Class MonitorProcessBusiness
        Public Shared Function GetFolders(ByVal UserConfigMachineName As String, serviceID As Int64) As DataSet
            Dim ConfiguredFoldersIdsForMachine As String = UserPreferences.getValueForMachine("ConfiguredFoldersIdsForMachine" & serviceID, UPSections.MonitorPreferences, "", False)
            Return MonitorProcessFactory.GetFolders(UserConfigMachineName, ConfiguredFoldersIdsForMachine)
        End Function
    End Class
End Namespace