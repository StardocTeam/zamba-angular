...
Public Class ProcessFactory
    ...
    Public Shared Sub SaveProcess(ByVal Process As Process)
        ...
        Try
            ...
            & Process.ID & ",'" & Process.Name.Trim & "','" & Process.Path.Trim & "','" & Process.Caracter.Trim & "'," & Process.DocType.ID & ", " & Process.Move & ", " & Process.Verify & ", " & Process.FlagAceptBlankData & ", " & Process.FlagBackUp & ", " & Process.FlagDelSourceFile & ", " & Process.FlagSourceVariable & ", " & Process.FlagMultipleFiles & ",'" & Process.MultipleCaracter.Trim & "','" & Process.BackUpPath.Trim & "',0," & Process.CheckBatch & ")"
    ...
    Public Shared Sub SaveEditedProcess(ByVal Process As Process)
        ...
        strupdate = "UPDATE Ip_Type SET IP_NAME = '" & Process.Name.Trim & "', IP_Path = '" & Trim(Process.Path) & "', IP_Chr = '" & Trim(Process.Caracter) & "',IP_DOCTYPEID = " & Process.DocType.ID & ", IP_MOVE = " & Process.Move & ", IP_VERIFY = " & Process.Verify & ", IP_ACEPTBLANK = " & Process.FlagAceptBlankData & ",IP_BACKUP = " & Process.FlagBackUp & ", IP_DELSOURCE = " & Process.FlagDelSourceFile & ", IP_SOURCEVARIABLE = " & Process.FlagSourceVariable & ", IP_MULTIPLEFILES = " & Process.FlagMultipleFiles & ", IP_MULTIPLECHR = '" & Process.MultipleCaracter & "', IP_BACKUPPATH = '" & Process.BackUpPath & "' where (IP_ID = " & Process.ID & ")"
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Shared Function ShowProcessHistory(ByVal ProcessId As Int32) As DataSet
        Dim DsProcessHistory As New DataSet
        If Server.isOracle Then
            Dim dstemp As DataSet
            ''Dim parNames() As String = {"ProcessID", "io_cursor"}
            ...
            dstemp.Tables(0).TableName = DsProcessHistory.Tables(0).TableName
    ...
    Public Shared Sub DelProcess(ByVal ProcessId As Integer)
        Try
            ...
            UserBusiness.Rights.SaveAction(ProcessId, ObjectTypes.ModuleImport, RightsType.Delete, "Eliminando proceso")
        Catch ex As Exception
    ...
    Public Shared Function Get_DocTypes() As ArrayList
        ...
        Dim dt As DataTable = DSTEMP.Tables("Doc_Type")
    ...
    Public Shared Function GetProcess(ByVal ProcessId As Integer) As Process
        ...
        DSTEMP1.Tables(0).TableName = Table
        ...
        DSTEMP2 = Server.Con.ExecuteDataset(CommandType.Text, StrSelect)
        DSTEMP2.Tables(0).TableName = Table
        DSTEMP2.Merge(DSTEMP1)

        Dim Process As Process = ProcessFactory.GetTempProcess(DSTEMP1, 0)
        Dim i As Integer
        For i = 0 To DSTEMP1.Tables("IP_INDEX").Rows.Count - 1
            Process.Index.Add(DSTEMP1.Tables("IP_INDEX").Rows(i).Item("Index_ID"))
        Next
        Return Process
    End Function
    Public Shared Function GetProcessIndexData(ByVal ProcessId As Int32) As DataSet
        Dim strSelect As String = "SELECT * FROM IP_INDEX where (IP_ID = " & ProcessId & ") ORDER BY INDEX_ORDER ASC"
        Dim DsTemp As DataSet
        DsTemp = Server.Con.ExecuteDataset(CommandType.Text, strSelect)
        DsTemp.Tables(0).TableName = "IP_INDEX"
        Return DsTemp
    End Function
    ...
    End Sub
    Public Shared Function GetTempProcess(ByVal DsProcess As DataSet, ByVal Index As Int32) As Process
        Dim Process As New Process
        Process.ID = DsProcess.Tables("IP_TYPE")(Index)("IP_ID")
        Process.Name = DsProcess.Tables("IP_TYPE")(Index)("IP_NAME")
        Process.Caracter = DsProcess.Tables("IP_TYPE")(Index)("IP_CHR")
        Dim DocType As DocType

        DocType = DocTypesBusiness.GetDocType(CInt(DsProcess.Tables("IP_TYPE")(Index)("IP_DOCTYPEID")), True)

        Process.DocType = DocType
        Process.Path = DsProcess.Tables("IP_TYPE")(Index)("IP_PATH")
        Try
            If IsDBNull(DsProcess.Tables("IP_TYPE")(Index)("IP_GROUP")) Then
                Process.BackUpPath = 0
            Else
                Process.BackUpPath = DsProcess.Tables("IP_TYPE")(Index)("IP_BACKUPPATH")
            End If
        Catch
            Process.BackUpPath = 0
        End Try
        Try
            If IsDBNull(DsProcess.Tables("IP_TYPE")(Index)("IP_GROUP")) Then
                Process.IP_GROUP = 0
            Else
                Process.IP_GROUP = DsProcess.Tables("IP_TYPE")(Index)("IP_GROUP")
            End If
        Catch
            Process.IP_GROUP = 0
        End Try
        Process.Move = DsProcess.Tables("IP_TYPE").Rows(Index)("IP_MOVE")
        Process.Verify = DsProcess.Tables("IP_TYPE")(Index)("IP_VERIFY")
        Process.FlagAceptBlankData = DsProcess.Tables("IP_TYPE")(Index)("IP_ACEPTBLANK")
        Process.FlagBackUp = DsProcess.Tables("IP_TYPE")(Index)("IP_BACKUP")
        Process.FlagDelSourceFile = DsProcess.Tables("IP_TYPE")(Index)("IP_DELSOURCE")
        Process.FlagSourceVariable = DsProcess.Tables("IP_TYPE")(Index)("IP_SOURCEVARIABLE")
        Process.FlagMultipleFiles = DsProcess.Tables("IP_TYPE")(Index)("IP_MULTIPLEFILES")
        If Process.FlagMultipleFiles = True Then
            If Not IsNothing(DsProcess.Tables("IP_TYPE")(Index)("IP_MULTIPLECHR")) Then Process.MultipleCaracter = DsProcess.Tables("IP_TYPE")(Index)("IP_MULTIPLECHR")
        End If
        Return Process
    End Function
    Public Shared Function GetProcessByProcessListId(ByVal ProcessListId As Int64) As DataSet
        ...
        Return DSTEMP
    End Function
    Public Shared Function GetAllProcess() As DataSet
    ...
    Public Shared Function getInsertedFiles(ByVal doc_id As String, ByVal ds3 As DataSet) As DataSet
            ...
            For ind = 1 To 20
                ...
                If d.Exists = True Then
                    ...
                    For Each f As IO.FileInfo In files
                        Dim row As DataRow = ds3.Tables("VolFiles").NewRow()
                        row("FileName") = f.FullName
                        row("Offset") = d.Name
                        row("Doc_id") = f.Name.Split("")(0)
                        ds3.Tables("VolFiles").Rows.Add(row)
                        Try
        ...
        Return ds3
    End Function
    Public Shared Function CheckExist(ByVal row As DataRow, Optional ByVal doc_type_id As Integer = 55) As Integer
    ...
    Public Shared Function getDocumentInfo(ByVal doc_id As Integer) As DataSet
        ...
        Try
            ...
            strb.Append("select i.doc_id IDs, i29 Por,i30 Fecha ,i31 Para,i32 CC,i33 BCC,i34 Asunto,i16 Cliente ,i18 Poliza, dg.disk_group_id Grupo ,disk_group_name NombreGrupo, vol_id Volume, dv.disk_vol_path Path ,doc_file Files,offset Offset,")
    ...
    Public Shared Function GetProcessByUserId(ByVal usrid As Int64) As ArrayList
        ...
        If Server.isOracle Then
            ...
            'Dim parTypes() As Object = {13, 5}
            Dim parValues() As Object = {Membership.MembershipHelper.CurrentUser.ID, 2}
