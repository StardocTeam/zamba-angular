...
#End Region
    ...
    Public Shared Sub MoveDocument(ByRef Result As NewResult, ByVal TempVolId As Int32, ByVal TempVolOffSet As Int32)
        Dim Table As String = Zamba.Core.Results_Business.MakeTable(Result.DocTypeId, Results_Factory.TableType.Document)
        Dim Fi As IO.FileInfo
        ...
        End Try
        Dim TempVolPath As String = VolumesFactory.RetrieveVolumePath(TempVolId).Trim

        Try
            Fi = New IO.FileInfo(TempVolPath & "\" & Result.DocTypeId & "\" & TempVolOffSet & "\" & Result.FileName.Trim)
            If Fi.Exists Then
                Fi.CopyTo(Result.Volume.path.Trim & "\" & Result.DocTypeId & "\" & Result.Volume.offset & "\" & Result.FileName.Trim, True)
