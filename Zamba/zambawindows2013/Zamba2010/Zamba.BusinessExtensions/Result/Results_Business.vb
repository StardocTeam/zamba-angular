...
#Region "Extensiones"
    ...
    Public Function GetFileIcon(ByVal File As String) As Int32
        If File Is Nothing OrElse File = String.Empty Then
            Return 30
        Else
            Try
    ...
    Public Shared Function GetPublishDate(docid As Long) As String
        Try
            Dim publishDate As Object = Results_Factory.GetPublishDate(docid)
            If Not IsDBNull(publishDate) AndAlso Not String.IsNullOrEmpty(publishDate) Then
    ...
    Friend Shared Sub UpdateResultIcon(resultId As Int64, docTypeId As Int64, IconID As Int32)
        Results_Factory.UpdateFileIcon(resultId, docTypeId, IconID)
    End Sub

    ''' <summary>
    ''' Se fija si una version esta publicada
    ''' </summary>
    ''' <param name="docid"></param>
    ''' <returns></returns>
    Public Shared Function IsResultPublished(docTypeId As Long, docid As Long) As Boolean
        Try
            Dim value As Integer = Results_Factory.GetVersionValue(docTypeId, docid)
    ...
    Public Shared Function GetPublisherUser(docid As Long) As String
        Try
            Dim userName As Object = Results_Factory.GetUserPublisher(docid)
            If Not IsDBNull(userName) AndAlso Not String.IsNullOrEmpty(userName) Then
...
#Region "GetResults"
    ...
    Public Shared Sub CompleteDocument(ByRef _Result As Result, ByVal dr As DataRow)
        Try
            If dr.ItemArray.Contains(GridColumns.DISKGROUPIDCOLUMNNAME) Then
                _Result.Disk_Group_Id = CInt(dr(GridColumns.DISKGROUPIDCOLUMNNAME))
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.PLATTER_ID_COLUMNNAME) Then
                _Result.Platter_Id = CInt(dr(GridColumns.PLATTER_ID_COLUMNNAME))
            End If
        Catch ex As Exception
        End Try

        Try
            If dr.ItemArray.Contains(GridColumns.DOC_FILE_COLUMNNAME) Then
                _Result.Doc_File = dr(GridColumns.DOC_FILE_COLUMNNAME).ToString()
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.OFFSET_COLUMNNAME) Then
                _Result.OffSet = CInt(dr(GridColumns.OFFSET_COLUMNNAME))
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.SHARED_COLUMNNAME) Then
                If CInt(dr(GridColumns.SHARED_COLUMNNAME)) = 1 Then
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.VER_PARENT_ID_COLUMNNAME) Then
                _Result.ParentVerId = CInt(dr(GridColumns.VER_PARENT_ID_COLUMNNAME))
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.VERSION_COLUMNNAME) Then
                _Result.HasVersion = CInt(dr(GridColumns.VERSION_COLUMNNAME))
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.ROOTID_COLUMNNAME) Then
                _Result.RootDocumentId = CInt(dr(GridColumns.ROOTID_COLUMNNAME))
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.ORIGINAL_FILENAME_COLUMNNAME) Then
                _Result.OriginalName = dr(GridColumns.ORIGINAL_FILENAME_COLUMNNAME).ToString()
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.NUMERO_DE_VERSION_COLUMNNAME) Then
                _Result.VersionNumber = CInt(dr(GridColumns.NUMERO_DE_VERSION_COLUMNNAME))
            End If
        Catch ex As Exception
        ...
        Try
            If dr.ItemArray.Contains(GridColumns.DISK_VOL_PATH_COLUMNNAME) Then
                _Result.DISK_VOL_PATH = dr(GridColumns.DISK_VOL_PATH_COLUMNNAME).ToString()
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        If dr.ItemArray.Contains(GridColumns.CRDATE_COLUMNNAME) Then
            If Not IsDBNull(dr(GridColumns.CRDATE_COLUMNNAME)) Then
                If Not IsNothing(dr(GridColumns.CRDATE_COLUMNNAME)) Then
                    If Not String.IsNullOrEmpty(dr(GridColumns.CRDATE_COLUMNNAME).ToString()) Then
                        Try
                            If Not IsNothing(_Result) Then
                                _Result.CreateDate = DateTime.Parse(dr(GridColumns.CRDATE_COLUMNNAME).ToString())
                            End If
                        Catch ex As Exception
                            ZClass.raiseerror(ex)
                        End Try
...
#Region "Insert & Save Document"
    ...
    Public Function GetNewNewResult(ByVal DocType As DocType, Optional ByVal _UserId As Int32 = 0, Optional ByVal File As String = "", Optional ByVal loadedVolume As Boolean = True) As NewResult
        Dim newResult As New NewResult(File)
        newResult.UserID = _UserId
        If IsNothing(DocType) = False Then
            newResult.DocType = DocType
            newResult.DocTypeId = DocType.ID

            If loadedVolume Then
                Try
            ...
            LoadIndexs((newResult))
        End If
        Return newResult
    End Function
    Public Sub ChangeDocTypeToNewResult(ByVal DocType As DocType, ByRef NewResult As NewResult, Optional ByVal loadedVolume As Boolean = True)
        NewResult.DocType = DocType
        NewResult.DocTypeId = DocType.ID
    ...
    Public Function GetSearchResult(ByVal DocType As DocType, Optional ByVal _UserId As Int32 = 0, Optional ByVal File As String = "") As NewResult
        Dim newResult As New NewResult(File)
        newResult.DocType = DocType
        newResult.DocTypeId = DocType.ID
        newResult.UserID = _UserId
    ...
    Public Shared Function CloneResult(ByVal originalResult As Result,
                                       ...
                                       Optional ByVal setDiskGroupIdToZero As Boolean = False,
                                       Optional ByVal isPublishing As Boolean = False) As NewResult
        ...
        ClonedResult.DocTypeId = originalResult.DocTypeId
        ...
        If isPublishing Then
            ClonedResult.VersionNumber = originalResult.VersionNumber
            ClonedResult.ParentVerId = originalResult.ParentVerId
            ClonedResult.RootDocumentId = originalResult.RootDocumentId
        ElseIf FlagInsertar Then
        ...
        Else
            ...
            ClonedResult.VersionNumber = GetNewVersionID(originalResult.RootDocumentId, originalResult.DocTypeId, originalResult.ID)
        ...
        Return ClonedResult
    End Function
    Public Shared Sub CloneIndexs(ByRef r As Result, ByVal DocType As DocType)
        r.Indexs.Clear()
...
#Region "Indexs"
    ...
    Public Shared Sub LoadIndexs(ByRef result As IResult)
        ...
        result.DocType.Indexs = ZCore.FilterIndex(result.DocTypeId)
    ...
    ''' <remarks></remarks>
    Public Sub CompleteIndexData(ByRef _result As Result, Optional ByVal inThread As Boolean = False)
    ...
    Public Shared Sub FillIndexData(ByRef _result As TaskResult, Optional ByVal inThread As Boolean = False)
        ...
        Try
            If _result.Indexs.Count = 0 Then
                _result.Indexs = ZCore.FilterIndex(_result.DocTypeId)
            End If
            con = Server.Con(False)
    ...
    Public Shared Function ValidateIndexDatabyRights(ByVal _result As Result) As Boolean
        If UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.ViewRightsByIndex, _result.DocTypeId) Then
            Dim IRI As Hashtable = UserBusiness.Rights.GetIndexsRights(_result.DocTypeId, Membership.MembershipHelper.CurrentUser.ID, True, True)
    ...
    Private Function CheckRequiredIndexs(ByRef result As NewResult) As Boolean
    ...
    Public Shared Function InsertDocument(ByRef newResult As NewResult,
                                          ByVal move As Boolean,
                                          Optional ByVal ReindexFlag As Boolean = False,
                                          Optional ByVal Reemplazar As Boolean = False,
                                          Optional ByVal showQuestions As Boolean = True,
                                          Optional ByVal IsVirtual As Boolean = False,
                                          Optional ByVal IsReplica As Boolean = False,
                                          Optional ByVal hasName As Boolean = False,
                                          Optional ByVal RefreshWFAfterInsert As Boolean = True,
                                          Optional ByVal openTask As Boolean = False,
                                          Optional ByVal OpenDocument As Boolean = False,
                                          Optional ByVal ParentTaskId As Int64 = 0,
                                          Optional ByVal DoAutoComplete As Boolean = True,
                                          Optional ByVal RealFileExtension As String = "",
                                          Optional ByVal OriginalFileNameToKeep As String = "") As InsertResult
        ...
        Try
            SyncLock Reg
                ...
                End If
                Return Reg.Insert(newResult, move, ReindexFlag, Reemplazar, showQuestions, IsVirtual, IsReplica, hasName, False, RefreshWFAfterInsert, OpenDocument, ParentTaskId,, DoAutoComplete, RealFileExtension, OriginalFileNameToKeep)
            End SyncLock
        Catch ex As Exception
            ZClass.raiseerror(ex)
    ...
    Public Shared Function InsertDocumentNew(ByRef newResult As NewResult, ByVal move As Boolean, Optional ByVal ReindexFlag As Boolean = False, Optional ByVal Reemplazar As Boolean = False, Optional ByVal showQuestions As Boolean = True, Optional ByVal IsVirtual As Boolean = False, Optional ByVal IsReplica As Boolean = False, Optional ByVal hasName As Boolean = False, Optional ByVal OpenDocumentAfterInsert As Boolean = False, Optional ByVal ParentTaskId As Int64 = 0) As InsertResult
        ...
        Try
            SyncLock Reg
                ...
                Return Reg.InsertNew(newResult, move, ReindexFlag, Reemplazar, showQuestions, IsVirtual, IsReplica, hasName)
            End SyncLock
        Catch ex As Exception
            ZClass.raiseerror(ex)
            Return InsertResult.NoInsertado
    ...
    Private Function GetResultName(ByRef result As IResult) As String
        Dim DTB As New DocTypesBusiness
        Try
            Dim AutoName As String = ZCore.GetDocTypeAutoName(result.DocTypeId)
            Return DTB.GetAutoName(AutoName, DocTypesBusiness.GetDocTypeName(result.DocTypeId, True), result.CreateDate, result.EditDate, result.Indexs).Trim()
        Catch ex As Exception
            ZClass.raiseerror(ex)
            Return DTB.GetDocTypeName(result.DocTypeId, True)
        Finally
            DTB = Nothing
        End Try
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Insert(ByVal name As String, ByVal binaryDocument As Byte(), ByVal fileExtension As String, ByVal docTypeId As Int64, ByVal indexs As DataTable, ByVal DontOpenTaskAfterInsertInDoGenerateCoverPage As Boolean, ByRef transaction As Transaction) As Int64
        ...
        Try
            If Not String.IsNullOrEmpty(fileExtension.Trim()) Then
                temporaryPath = EnvironmentUtil.GetTempDir("\Temp").FullName
            ...
            Using CurrentNewResult As NewResult = GetNewResult(docTypeId, documentPath, False)
                    ...
                    For Each CurrentIndexRow As DataRow In indexs.Rows
                        ...
                        For Each CurrentIndex As IIndex In CurrentNewResult.Indexs
                            If CurrentIndex.ID = CurrentIndexRow.Item("IndexId") Then
                                CurrentIndex.Data = CurrentIndexRow.Item("IndexValue")
                ...
                SyncLock Reg
                    ...
                    If String.IsNullOrEmpty(fileExtension.Trim()) Then
                        Reg.Insert(CurrentNewResult, False, False, False, False, True, False, False, True, True, False, 0, transaction)
                    Else
                        Reg.Insert(CurrentNewResult, True, False, False, False, False, False, False, True, True, False, 0, transaction)
                    End If
                End SyncLock
    ...
    Public Shared Function ValidateIndexData(ByVal _newresult As NewResult) As Boolean
        ...
        Dim IRI As Hashtable = UserBusiness.Rights.GetIndexsRights(_newresult.DocTypeId, Membership.MembershipHelper.CurrentUser.ID, True, True)
    ...
    Public Shared Function ValidateIndexDataFromDoctype(ByVal _newresult As NewResult) As Boolean
        ...
        Dim dsIndexproperty As DataSet = DocTypesBusiness.GetIndexsProperties(_newresult.DocTypeId)
    ...
    Private Sub AutocompleteIndexsNewDocument(ByRef newResult As NewResult)
        ...
        Try
            ...
            indexTemp = AutoCompleteBarcode_FactoryBusiness.getIndexKeys(newResult.DocTypeId)
            ...
            If Not indexTemp Is Nothing AndAlso indexTemp.Count > 0 Then
                ...
                AC = AutoCompleteBarcode_FactoryBusiness.GetComplete(Int32.Parse(newResult.DocTypeId), indexTemp(0).ID) 'Obtiene una instancia del Objeto AutoComplete
                ...
                If Not IsNothing(AC) Then 'Siempre AC deberia ser una instancia
                    ...
                    If Not IsNothing(AC.CompleteInsert(newResult, indexTemp, Nothing, True)) AndAlso newResult.ID > 0 Then
                        'se cambio por save modified
                        Dim rstBuss As New Results_Business()
                        rstBuss.SaveModifiedIndexData(newResult, True, True)
                        rstBuss = Nothing
                    End If
    ...
    ''' </history>
    Public Function Insert(ByRef newResult As NewResult,
                           ByVal move As Boolean,
                           Optional ByVal reIndexFlag As Boolean = False,
                           Optional ByVal reemplazarFlag As Boolean = False,
                           Optional ByVal showQuestions As Boolean = True,
                           Optional ByVal isVirtual As Boolean = False,
                           Optional ByVal isReplica As Boolean = False,
                           Optional ByVal hasName As Boolean = False,
                           Optional ByVal throwEx As Boolean = False,
                           Optional ByVal RefreshWFAfterInsert As Boolean = True,
                           Optional ByVal OpenDocumentAfterInsert As Boolean = False,
                           Optional ByVal ParentTaskId As Int64 = 0,
                           Optional ByRef transaction As Transaction = Nothing,
                           Optional ByVal DoAutocomplete As Boolean = True,
                           Optional ByVal RealFileExtension As String = "",
                           Optional ByVal OriginalFileNameToKeep As String = "") As InsertResult
        ...
        Try
            ...
            If newResult.ID = 0 Then newResult.ID = CoreData.GetNewID(IdTypes.DOCID, 0)

            newResult.Name = GetResultName(newResult)
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Nombre: " & newResult.Name & " id: " & newResult.ID)
            ...
            If isVirtual = False OrElse isReplica OrElse newResult.Volume.Type <> VolumeTypes.DataBase Then 'Intento cargar un volumen para el documento, si no se puede se genera una excepcion
                LoadVolume(newResult)
                Dim VB As New VolumesBusiness()
                newResult.NewFile = VB.VolumePath(newResult.Volume, newResult.DocTypeId) & "\" & newResult.ID & If(RealFileExtension.Length > 0, RealFileExtension, newResult.Extension)
                If RealFileExtension <> String.Empty OrElse newResult.Extension <> String.Empty Then
                    If Directory.Exists(newResult.Volume.path) = False Then
                        Directory.CreateDirectory(newResult.Volume.path)
                    End If
                End If
            ...
            If isVirtual = False Or isReplica Then 'Copio fisicamente el archivo
                ...
                If newResult.Volume.Type = VolumeTypes.DataBase Then
                    ...
                    If move Then
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "Moviendo el documento (Al ser base de datos se elimina del origen): " & newResult.File & " a " & newResult.NewFile)
                        'File.Delete(newResult.File)
                    End If
                Else
                    If OriginalFileNameToKeep.Length > 0 Then
                        newResult.OriginalName = OriginalFileNameToKeep
                    End If
                    If move Then
                        Try
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "Moviendo el documento: " & newResult.File & " a " & newResult.NewFile)
                            MoveFile(newResult)
                        Catch ex As Exception
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "Error al Mover - Copiando el documento: " & newResult.File & " a " & newResult.NewFile)
                            ZClass.raiseerror(ex)
                            copyFile(newResult)
                        End Try
                    Else
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "Copiando el documento: " & newResult.File & " a " & newResult.NewFile)
                        copyFile(newResult)
                    End If
                End If
            ...
            If isVirtual And isReplica = False Then
                ...
                'ElseIf OriginalFileNameToKeep.Length > 0 Then
                '    newResult.Disk_Group_Id = -1
            Else
            ...
            Dim WFIds As ArrayList = DocTypesBusiness.GetDocTypeWfIds(newResult.DocTypeId)
            Dim ResultsArray As New ArrayList(1)

            'Objeto para realizar la inserción en una transaccion
            Dim Transact As Transaction
            If transaction Is Nothing Then
                Transact = New Transaction(Server.Con(False))
            Else
                Transact = transaction
            End If

            Dim IsInWorkflow As Boolean = False
            Try
                Try
                    Dim RF As New Results_Factory
                    RF.RegisterDocumentWithAlfanumericIndex(newResult, reIndexFlag, isVirtual, Transact, Membership.MembershipHelper.CurrentUser.ID)
                    RF = Nothing
                ...
                If newResult.Volume.ID <> 0 Then
                    ...
                    If newResult.Volume.path = String.Empty Then
                        newResult.Volume.path = volFactory.GetVolumenPathByVolId(newResult.Volume.ID)
                    End If
                    newResult.DISK_VOL_PATH = newResult.Volume.path
                    volFactory = Nothing
                Else
                ...
                For Each wfId As String In WFIds
                    ...
                    IsInWorkflow = True
                Next
                If (Boolean.Parse(UserPreferences.getValueForMachine("QuequeInIndexerNewDocuments", UPSections.Indexer, False, True))) Then
                ...
                Try
                    GenerateThumbs(newResult.FullPath, newResult.ID, newResult.DocTypeId)
                Catch ex As Exception
                    ZClass.raiseerror(ex)
                End Try
            ...
            If (Boolean.Parse(UserPreferences.getValueForMachine("IsBulkInsertSaveHistory", UPSections.Indexer, True, True)) = False) Then
                UserBusiness.Rights.SaveAction(newResult.ID, ObjectTypes.Documents, RightsType.Create, "Se crea el documento " & newResult.Name & "(" & newResult.ID & ")")
            End If
            ...
            Try
                If Not IsNothing(InsertedWFIdsAndInitialStepsAndTasksResults) Then
                    For Each A As ArrayList In InsertedWFIdsAndInitialStepsAndTasksResults
                        WFTaskBusiness.ExecuteCheckInRulesFromInsert(A, OpenDocumentAfterInsert)
                    Next
                End If
                If RightsBusiness.GetUserRights(ObjectTypes.ModuleWorkFlow, RightsType.Use) = False Then
                    If (OpenDocumentAfterInsert) Then
                        If IsInWorkflow Then
                            RaiseEvent ResultInserted(newResult, 0)
                        Else
                            RaiseEvent ResultInserted(newResult, ParentTaskId)
                        End If
                    End If
                End If
            Catch ex As Exception
    ...
    Public Function InsertNew(ByRef newResult As NewResult, ByVal move As Boolean, Optional ByVal reIndexFlag As Boolean = False, Optional ByVal reemplazarFlag As Boolean = False, Optional ByVal showQuestions As Boolean = True, Optional ByVal isVirtual As Boolean = False, Optional ByVal isReplica As Boolean = False, Optional ByVal hasName As Boolean = False) As InsertResult
        Dim NewResultStatus As InsertResult = InsertResult.NoInsertado
        ...
        If isVirtual = False Or isReplica Then 'Intento cargar un volumen para el documento, si no se puede se genera una excepcion
            LoadVolume(newResult)
            Dim VB As New VolumesBusiness()
            newResult.NewFile = VB.VolumePath(newResult.Volume, newResult.DocTypeId) & "\" & newResult.ID & newResult.Extension
            VB = Nothing
            newResult.OffSet = newResult.Volume.offset
            If hasName = False Then
        ...
        Try
            ...
            If CheckRequiredIndexs(newResult) Then
                Dim RF As New Results_Factory
                RF.RegisterDocumentWithAlfanumericIndex(newResult, reIndexFlag, isVirtual)
                RF = Nothing
            End If
        ...
        newResult.DISK_VOL_PATH = VolumesFactory.GetVolume(newResult.Volume.ID).path ' Hernan
    ...
    End Function
    Public Shared Function findIn(ByVal Indexs As List(Of IIndex), ByVal pIndex As Index) As Index
    ...
    ''' 	[Gaston]	11/09/2008	Modified    Se agrego el evento que permite refrescar el o los workflows
    ''' </history>
    Public Function UpdateInsert(ByRef Result As NewResult, ByVal move As Boolean, Optional ByVal ReindexFlag As Boolean = False, Optional ByVal Reemplazar As Boolean = False, Optional ByVal showQuestions As Boolean = True, Optional ByVal IsVirtual As Boolean = False, Optional ByVal addToWF As Boolean = True) As InsertResult

        Dim insertresult As InsertResult = InsertResult.NoInsertado

        'Obtengo el auto nombre
        Try
            Dim autonamecode As String = ZCore.GetDocTypeAutoName(Result.DocTypeId)
            If Result.Indexs.Count > 0 Then
                Result.Name = GetResultName(Result)
            End If
        Catch ex As Exception
        ...
        If IsVirtual = False Then
            LoadVolume(Result)
            Dim VB As New VolumesBusiness()
            Result.NewFile = VB.VolumePath(Result.Volume, Result.DocTypeId) & "\" & Result.ID & Result.Extension
            VB = Nothing
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Result ID=" & Result.ID)
        ...
        Catch ex As Exception
            ZTrace.WriteLineIf(ZTrace.IsWarning, "Error al guardar la doc i Buscando si es un reemplazo " & ex.ToString)
        ...
        If move = True AndAlso insertresult <> InsertResult.NoInsertado Then
            ...
            End Try
        End If
        Result.DISK_VOL_PATH = VolumesFactory.GetVolume(Result.Volume.ID).path ' Hernan
        ...
        Try
            RaiseEvent ResultInserted(Result, 0)
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
        ...
        Try
            If addToWF = True Then
                ...
                Dim WFIds As ArrayList = DocTypesBusiness.GetDocTypeWfIds(Result.DocTypeId)
    ...
    Private Sub UpdateRegisterDocument(ByRef Result As NewResult, ByVal reIndexFlag As Boolean, Optional ByVal isvirtual As Boolean = False)
        ...
        Try
            ...
            If dr IsNot Nothing AndAlso Not dr.IsClosed Then
                While dr.Read
                    ...
                    End Try
                    Try
                        If Not IsDBNull(dr.Item(13)) Then _Result.OriginalName = dr.Item(13)
                    Catch ex As Exception
                        ZClass.raiseerror(ex)
                    End Try
                    Try
                    ...
                    For i = 0 To _Result.Indexs.Count - 1
                        Try
                            If _Result.Indexs(i).isReference = False Then

                                If Not IsDBNull(dr.GetValue(dr.GetOrdinal("I" & DirectCast(_Result.Indexs(i), Index).ID))) Then
                                    DirectCast(_Result.Indexs(i), Index).Data = dr.GetValue(dr.GetOrdinal("I" & DirectCast(_Result.Indexs(i), Index).ID)).ToString
                                    DirectCast(_Result.Indexs(i), Index).DataTemp = DirectCast(_Result.Indexs(i), Index).Data
                                    'Si el atributo es de tipo Sustitucion
                                    If DirectCast(_Result.Indexs(i), Index).DropDown = IndexAdditionalType.AutoSustitución _
                                    Or DirectCast(_Result.Indexs(i), Index).DropDown = IndexAdditionalType.AutoSustituciónJerarquico Then
                                        'Se carga la descripcion de Atributo
...
#Region "GetNewResult"
    ...
    Public Function GetNewResult(ByVal docTypeId As Int64, ByVal File As String, Optional ByVal loadVolume As Boolean = True) As NewResult
    ...
    Public Shared Function GetTaskResult(ByVal docId As Int64, ByVal docTypeId As Int64)
    ...
    Public Shared Function GetNewNewResult(ByVal docTypeId As Long) As NewResult
        Dim Result As New NewResult()
        Result.DocType = DocTypesBusiness.GetDocType(docTypeId, True)
        Result.Indexs = IndexsBusiness.GetIndexsSchemaAsListOfDT(docTypeId, False)
        '        LoadIndexs((Result))
    ...
    Public Shared Function GetResultRow(ByVal docId As Int64, ByVal docTypeId As Int64) As DataRow
        ...
        If Cache.DocTypesAndIndexs.hsRestrictionsIndexs.ContainsKey(restrictionKey) = False Then
            Cache.DocTypesAndIndexs.hsRestrictionsIndexs.Add(restrictionKey, RestrictionsMapper_Factory.getRestrictionIndexs(Membership.MembershipHelper.CurrentUser.ID, docTypeId, True))
        End If
...
#Region "Tasks"
    Public Shared Function GetNewTaskResult(ByVal DocId As Int64, ByVal DocType As DocType) As TaskResult
        ...
        Next
        CompleteDocument((Result), False)
        Return Result
    End Function
#End Region

#Region "Volume"
    Public Sub LoadVolume(ByRef result As NewResult)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Obteniendo Lista de Volumenes para Entidad: " & result.DocTypeId)
        result.VolumeListId = VolumesBusiness.GetVolumeListId(result.DocTypeId)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Lista de Volumenes: " & result.VolumeListId)

        result.DsVols = VolumesBusiness.GetActiveDiskGroupVolumes(result.VolumeListId)
        If (result.DsVols IsNot Nothing AndAlso result.DsVols.Tables.Count > 0 AndAlso result.DsVols.Tables(0).Rows.Count() > 0) Then
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Cantidad de Volumenes: " & result.DsVols.Tables(0).Rows.Count().ToString)
        Else
            ZTrace.WriteLineIf(ZTrace.IsError, "No hay volumenes disponibles para la Entidad: " & result.DocTypeId.ToString)
            ZClass.raiseerror(New Exception("No hay volumenes disponibles para la Entidad: " & result.DocTypeId.ToString))
        End If
        result.Volume = VolumesBusiness.LoadVolume(result.DocTypeId, result.DsVols)
        If result.Volume.path = String.Empty Then
            ZTrace.WriteLineIf(ZTrace.IsError, "El Path del volumen esta vacio con Id: " & result.Volume.ID.ToString)
            ZClass.raiseerror(New Exception("El Path del volumen esta vacio con Id: " & result.Volume.ID.ToString))
        End If
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Path del Volumen: " & result.Volume.path)
    End Sub
    Public Shared Sub LoadVolumeForResult(ByRef result As Result)
        result.Disk_Group_Id = VolumesBusiness.GetVolumeListId(result.DocTypeId)
    End Sub
#End Region
...
#Region "Delete"
    ...
    Public Shared Sub Delete(ByRef Result As Result, Optional ByVal delfile As Boolean = True, Optional ByVal saveAction As Boolean = True)
        If Result.Indexs.Count = 0 Then
            Result.Indexs = ZCore.FilterIndex(Result.DocTypeId)
        End If
...
#Region "DOCFILE"
    ...
    ''' <param name="result"></param>
    ''' <remarks></remarks>
    Public Sub copyFile(ByRef result As NewResult)
        ...
        Try
            ...
            If fi.Exists Then
                ...
                If String.IsNullOrEmpty(result.NewFile) = False Then
                    Dim newfile As New FileInfo(result.NewFile)
                    If newfile.Directory.Exists = False Then
        ...
        End Try
    End Sub
    Private Sub MoveFile(ByRef newResult As NewResult)
        Dim fi As FileInfo = Nothing
        Try
            fi = New FileInfo(newResult.File)
            If fi.Exists Then
                Dim dir As DirectoryInfo = New FileInfo(newResult.NewFile).Directory
    ...
    End Sub
    'Public Sub ReplaceDocument(ByRef Result As Result, ByVal NewDocumentFile As String, ByVal ComeFromWF As Boolean)
    '    Dim Fi As New FileInfo(Result.FullPath)
    '    Dim NewFi As New FileInfo(NewDocumentFile)
    '    Dim NewName As String = Result.ID ' Results_Factory.MakefileName()
    '    If Directory.Exists(Fi.DirectoryName) = False Then
    ...
    '    Results_Factory.ReplaceDocument(Result, Result.Doc_File, ComeFromWF, DsVols)
    '    Results_Factory.InsertIndexerState(Result.DocTypeId, Result.ID, 0, Nothing)

    'End Sub
    Public Sub ReplaceDocument(ByRef Result As IResult, ByVal NewDocumentFile As String, ByVal ComeFromWF As Boolean)
        ...
        Try
            ...
            If Volume IsNot Nothing AndAlso Volume.Type = VolumeTypes.DataBase Then
                ...
                RF.ReplaceDocument(Result, NewFi, Result.Doc_File, ComeFromWF)
            ...
            ElseIf Volume IsNot Nothing Then
                ...
                Dim NewFullDirectory As String = VB.VolumePath(Volume, Result.DocType.ID)
                If (Directory.Exists(NewFullDirectory) = False) Then
                    Directory.CreateDirectory(NewFullDirectory)
                End If

                Dim NewFullFileName As String = VB.VolumePath(Volume, Result.DocType.ID) & "\" & Result.ID & NewFi.Extension
                NewFi.CopyTo(NewFullFileName, True)
            ...
            UB.Rights.SaveAction(Result.ID, ObjectTypes.Documents, RightsType.Edit, Result.Name)
            UB = Nothing
...
#Region "Interfaz WS"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetWebDocFileWS(ByVal docTypeId As Long, ByVal docId As Long, ByVal userId As Long) As Byte()
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function CopyBlobToVolumeWS(ByVal docId As Long, ByVal docTypeId As Long) As Boolean
        Dim wsFactory As New WSResultsFactory()
    ...
    Public Function GetBlob(ByVal docId As Long, ByVal docTypeId As Long, ByVal userid As Long) As Byte()
        Dim wsFactory As New WSResultsFactory()
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function InsertDocFileWS(ByVal docId As Long, ByVal docTypeId As Long, ByVal fileBytes As Byte(), ByVal incomingFile As String, ByVal userId As Long) As Boolean
        Dim wsFactory As New WSResultsFactory()
        Dim returnVal As Boolean = False
        Try
...
#Region "Versioning"
    ...
    Public Shared Function InsertNewVersion(ByVal OriginalResult As Result, ByVal Comment As String) As Result
        ...
        setParentVersion(OriginalResult.DocTypeId, OriginalResult.ID)
    ...
    Public Shared Sub PublishResultVersion(ByVal resultToPublish As Result, ByVal convertToPDF As Boolean)
        Try
            Dim fileName As String = resultToPublish.FullPath

            If convertToPDF Then
                fileName = fileName.Substring(0, fileName.LastIndexOf(".")) & ".pdf"
                Dim sp As SpireTools = New SpireTools()
                sp.ConvertWordToPDF(resultToPublish.FullPath, fileName)
            End If

            Dim clonedResultToPublish As Result = CloneResult(resultToPublish, fileName, False, False, False, True)
            clonedResultToPublish.HasVersion = 2


            For index As Integer = 0 To clonedResultToPublish.Indexs.Count - 1
                Dim value As Object = clonedResultToPublish.Indexs.Item(index).Data
                clonedResultToPublish.Indexs.Item(index).DataTemp = value
            Next

            InsertDocument(CType(clonedResultToPublish, NewResult), False, False, False, False, False)
    ...
    Public Shared Function PublishableResult(ByVal result As Result) As PublishableResult
        ...
        PublishResult.DocType.ID = result.DocTypeId
    ...
    Public Shared Sub SavePublish(ByVal result As Result, ByVal Publishresult As PublishableResult)
        ...
        If hash.Count > 0 Then
            ...
            If indexsToModify.Count > 0 Then
                rstBuss.SaveModifiedIndexData(Publishresult, True, True, indexsToModify)
            Else
                rstBuss.SaveModifiedIndexData(Publishresult, True, True)
            End If
            rstBuss = Nothing
        End If
...
#Region "AutoName"
    ...
    ''' </history>
    Public Sub UpdateAutoName(ByRef Result As IResult, Optional ByVal ChNomC As Boolean = False)
        Dim AutoNameCode As String
        Dim DocTypeName As String

        If Not Result.DocType Is Nothing AndAlso Result.DocType.AutoNameCode.Length = 0 Then
            AutoNameCode = DocTypesBusiness.GetDocType(Result.DocTypeId, True).AutoNameCode
        Else
            AutoNameCode = Result.DocType.AutoNameCode
        End If
        If Not Result.DocType Is Nothing AndAlso Result.DocType.Name.Length = 0 Then
            DocTypeName = DocTypesBusiness.GetDocTypeName(Result.DocTypeId, True)
        Else
        ...
        If Not Result.Indexs Is Nothing AndAlso Result.Indexs.Count > 0 Then
            If Not ChNomC OrElse Result.Name = String.Empty Then
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Modificando Autonombre de ResultId " & Result.ID)
                Result.Name = GetResultName(Result)
...
#Region "Link"
    Public Shared Function GetLinkFromResult(ByRef Result As Result) As String
        Dim Link As String
        Link = "Zamba:\\DT=" & Result.DocTypeId & "&DOCID=" & Result.ID
    ...
    <PropiedadesType(Propiedades.PropiedadPublica), PropiedadesReturnType(New String() {""}), PropiedadesParam(New String() {"EntidadId,DocId,TextoBoton"})>
    Public Shared ReadOnly Property GetWebLinkToDocument(docTypeId As Int64, docId As Int64, ButtonText As String) As String
        Get
            Dim sBuilder As New StringBuilder()
            'http://localhost/Zamba.Web/Views/WF/TaskViewer.aspx?taskid=886149&docid=18153908&mode=&UserID=10084#Zamba/
            Dim webPath As String = ZOptBusiness.GetValue("ThisDomain")
            sBuilder.Append("<a href=")
            sBuilder.Append(Chr(34))
            sBuilder.Append(webPath)
            sBuilder.Append(String.Format("/Views/WF/TaskSelector.ashx?docid={0}&doctypeid={1}", docId, docTypeId))
            sBuilder.Append(Chr(34))
            sBuilder.Append(String.Format(" role='button' class='btn btn-primary'  >{0}</a>", ButtonText))
...
#Region "Creation Info"
    ...
    Public Shared Function HasForms(ByVal Result As Result) As Boolean
        If FormBusiness.GetShowAndEditFormsCount(Result.DocTypeId) > 0 Then
            Return True
        Else
            Return False
        End If
    ...
    Public Shared Function HasFormsExtended(ByVal Result As Result) As Boolean
        If FormBusiness.GetShowAndEditFormsCountExtended(Result.DocTypeId) > 0 Then
            Return True
        Else
            Return False
        End If
...
#End Region
    ...
    Public Shared Function getDate() As DateTime
        Dim fecha As DateTime = Server.Con.ExecuteScalar(CommandType.Text, "Select getDate()")
        Return fecha
    End Function
    ...
    ''' </remarks>
    Public Shared Function CopyFileToTemp(ByVal res As Result, ByVal rootPath As String, ByVal destPath As String) As String
        'Verifica si el volumen es de tipo base de datos o si todavia no se inserto el documento
        If (res.Disk_Group_Id > 0 AndAlso (ZCore.filterVolumes(res.Disk_Group_Id) IsNot Nothing AndAlso ZCore.filterVolumes(res.Disk_Group_Id).Type = VolumeTypes.DataBase)) OrElse Not File.Exists(res.RealFullPath) Then
            ...
            If IsNothing(res.EncodedFile) Then
                ...
                If (File.Exists(rootPath)) Then
                    ...
                    Dim newPDFFile As String
                    If (res.IsImage) AndAlso res.IsTif = False Then
                        newPDFFile = Path.Combine(GetTempDir("\OfficeTemp").FullName, Path.GetFileName(res.FullPath).ToLower().Replace(".jpg", ".pdf").Replace(".bmp", ".pdf").Replace(".png", ".pdf").Replace(".gif", ".pdf").Replace(".jpeg", ".pdf").Replace(".tif", ".pdf").Replace(".ico", ".pdf"))
                        If File.Exists(newPDFFile) = False Then
                            Dim ST As New Zamba.FileTools.SpireTools()
                            ST.ConvertImageToPDF(destPath, newPDFFile)
                        End If
                    End If


                    If (res.IsTif) Then
                        newPDFFile = Path.Combine(GetTempDir("\OfficeTemp").FullName, Path.GetFileName(res.FullPath).ToLower().Replace(".tiff", ".pdf").Replace(".tif", ".pdf"))
                        If File.Exists(newPDFFile) = False Then
                            Dim ST As New Zamba.FileTools.SpireTools()
                            ST.ConvertTIFFToPDF(destPath, newPDFFile)
                        End If
                    End If
                    If newPDFFile IsNot Nothing Then
                        destPath = newPDFFile
                    End If
                    'Lo codifica
                    res.EncodedFile = FileEncode.Encode(newPDFFile)
            ...
            Dim DuplicateBlobAndVolumeFiles As Boolean = False
            Boolean.TryParse(ZOptBusiness.GetValue("DuplicateBlobAndVolumeFiles"), DuplicateBlobAndVolumeFiles)

            duplicate = DuplicateBlobAndVolumeFiles

            If duplicate AndAlso res.EncodedFile IsNot Nothing AndAlso File.Exists(res.RealFullPath) = False Then
                FileEncode.Decode(res.RealFullPath, res.EncodedFile)
            End If
        Else
    ...
    Public Function UpdateMonitorInsert(ByRef Result As NewResult, Optional ByVal Reemplazar As Boolean = False) As InsertResult
        Dim insertresult As InsertResult = InsertResult.NoInsertado
    ...
    Public Shared Sub ConvertToPdfFile(ByRef Result As Result, ByVal pdfFolderPath As String, ByRef CantPdfs As Int32)
        ...
        Try
            ...
            If PathFile.ToUpper.EndsWith("TIF") Then
                ...
                If iCount = 1 Then
                    ...
                    pe = New ceTe.DynamicPDF.PageElements.Image(PathFile, 0.0, 0.0, 100)
                    ...
                    NumMagicoW = Img2.HorizontalResolution / pe.HorizontalDpi
                    NumMagicoH = Img2.VerticalResolution / pe.VerticalDpi
                ...
                Else
                    ...
                    For j As Int32 = 0 To iCount - 1
                        ...
                        p = New ceTe.DynamicPDF.Page
                        pe = New ceTe.DynamicPDF.PageElements.Image(Path, 0.0, 0.0, 100)
        ...
        Catch ex As Exception
            ...
            Try
                ...
                fTiff = New ceTe.DynamicPDF.Imaging.TiffFile(New System.IO.FileInfo(PathFile).Directory.FullName & "\" & New System.IO.FileInfo(PathFile).Name & "TEMP.TIF")
...
#End Region
    ...
    ''' </history>
    Public Sub InsertSearchIndexData(ByVal result As IResult, Optional ByRef t As Transaction = Nothing)
    ...
    Public Function IndexFile(ByVal filePath As String, docId As Integer, docTypeid As Integer, serviceId As Int64) As Integer
        ...
        Try
            ...
            Dim IndexResult As IndexedState
            Dim thumbGenerated As Boolean

            Dim mustGenerateThumb As Boolean = True
            If serviceId > 0 Then
                mustGenerateThumb = ServiceBusiness.getValue(serviceId, "GenerateThumb")
            End If

            If ServiceBusiness.getValue(serviceId, "IndexContent") OrElse mustGenerateThumb Then
            ...
            If filePath.Length > 0 AndAlso File.Exists(filePath) Then
                ...
                IndexResult = IndexFile(filePath, docId, docTypeid, body, res, IsBlob, serviceId)

                If mustGenerateThumb Then
                    thumbGenerated = GenerateThumbs(filePath, docId, docTypeid)
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, IIf(thumbGenerated, "Generacion thumb exitosa", "Error al generar thumb"))
            ...
            If thumbGenerated Then
                Select Case IndexResult
                    Case IndexedState.IndicesyContenido
                        Return IndexedState.Indexado
                    Case IndexedState.IndicesSolamenteIndexado
                        Return IndexedState.Indicesythumbs
                    Case IndexedState.SoloContenido
                        Return IndexedState.ThumbsyContenido
                    Case IndexedState.IndexadoNoRequerido
                        Return IndexedState.SoloThumbs
                End Select
            Else
                Return IndexResult
            End If
    ...
    Public Shared Function GenerateThumbs(img As Image, docId As Integer, docTypeid As Integer) As Boolean
        Try
            Dim exist As Boolean = Server.Con.ExecuteScalar(CommandType.Text, String.Format("SELECT COUNT(1) FROM ZTHUMB WHERE DOC_TYPE_ID = {0} AND DOC_ID= {1}", docTypeid, docId))
            If Not exist Then
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Generando thumb de: " & docId.ToString())

                Dim thumb As String = Thumbs.Base64.Image(img, docTypeid, docId)
                ...
                If thumb IsNot String.Empty Then
                    ...
                    If Server.isOracle Then
                        ...
                        If thumb.Length > 25001 Then
                            hasta = 25001
                        Else
                            hasta = thumb.Length - 1
                        End If
                        Dim distThumb1 As String = thumb.Substring(0, hasta)

                        If thumb.Length > 25001 AndAlso thumb.Length > 50000 Then
                            hasta = 25000
                        Else
                            hasta = thumb.Length - 25001
                        End If
                        Dim distThumb2 As String = thumb.Substring(25001, hasta)

                        If thumb.Length > 50001 Then
                            hasta = thumb.Length - 50001
                        End If

                        Dim distThumb3 As String = thumb.Substring(50001, hasta)
                    ...
                    Else
                        Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("INSERT INTO ZTHUMB (DOC_TYPE_ID, DOC_ID,THUMB) VALUES({0},{1},'{2}')", docTypeid, docId, thumb))
                    End If
                    'Else
                    '    ZTrace.WriteLineIf(ZTrace.IsVerbose, "Actualizando registro de thumb: docTypeid " & docTypeid & " docId " & docId)
                    '    Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("UPDATE ZTHUMB SET THUMB = '{0}' WHERE DOC_TYPE_ID = {1} AND DOC_ID = {2}", thumb, docTypeid, docId))
                End If
            End If
        Catch ex As Exception
            Throw ex
    ...
    Private Shared Function GenerateThumbs(filePath As String, docId As Integer, docTypeid As Integer) As Boolean
        ...
        Try
            Dim StoreInDB As Boolean
            Boolean.TryParse(ZOptBusiness.GetValueOrDefault("ThumbStoreInDB", "false"), StoreInDB)


            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Generando thumb de: " + filePath)

            Dim thumb As String = Thumbs.Base64.ConvertFromPath(filePath, docTypeid, docId)
            ...
            If StoreInDB Then
                ...
                End If
            Else
                If Not String.IsNullOrEmpty(thumb) Then 'AndAlso File.Exists(thumb)
                    processState = True
                Else
                    processState = False
                End If
    ...
    Private Function IndexFile(filePath As String, docId As Integer, docTypeid As Integer, ByRef body As String, ByRef res As IResult, IsBlob As Boolean, serviceid As Int64) As IndexedState
        ...
        Dim InsertoIndices As Boolean
        Dim insertoContenido As Boolean

        Dim IndexMsgFiles As Boolean = UserPreferences.getValueForMachine("IndexMsgFiles", UPSections.WFService, True)
        ...
        Else
            ...
            Try
                ...
                'Indexo el valor de los índices
                If ServiceBusiness.getValue(serviceid, "IndexAttribute") OrElse serviceid = 0 Then
                    InsertSearchIndexData(res, Nothing)
                    InsertoIndices = True
                End If
                ...
                If ServiceBusiness.getValue(serviceid, "IndexContent") Then
                    ...
                    If extension.ToLower.Contains(".doc") OrElse extension.ToLower.Contains(".docx") OrElse extension.ToLower.Contains(".dot") Then
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Procesando archivo Word")

                        Dim st As New SpireTools()
                        body = st.GetTextFromDoc(filePath)

                        st = Nothing

                    ElseIf extension.ToLower.Contains(".xls") OrElse extension.ToLower.Contains(".xlsx") Then
                    ...
                    ElseIf extension.ToLower.Contains(".pdf") Then
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Procesando archivo PDF")

                        'Dim ITS As New ITextSharp()
                        'body = ITS.GetTextFromPDF(filePath)
                        'ITS = Nothing

                        Dim ocr As New SpireOCR()
                        body = ocr.GetTextFromPDF(filePath, docId)
                        ocr = Nothing

                    ElseIf extension.ToLower.Contains(".png") OrElse extension.ToLower.Contains(".jpg") OrElse extension.ToLower.Contains(".gif") OrElse extension.ToLower.Contains(".tif") OrElse extension.ToLower.Contains(".tiff") OrElse extension.ToLower.Contains(".bmp") OrElse extension.ToLower.Contains(".__1") Then
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Procesando archivo PDF")

                        Dim ocr As New SpireOCR()
                        body = ocr.GetTextFromImage(filePath, docId)
                        ocr = Nothing


                    ElseIf extension.ToLower.Contains(".msg") AndAlso IndexMsgFiles Then
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Procesando archivo MSG")

                        Dim interopB As New Zamba.FileTools.Interop()
                        interopB.ExtractTextFromMsgAttach(filePath, body)
                        interopB = Nothing

                        'Dim st As New Zamba.FileTools.SpireTools()
                        'body = st.GetTextFromMsg(filePath)

                    ElseIf extension.ToLower.Contains(".ppt") OrElse extension.ToLower.Contains(".pps") OrElse extension.ToLower.Contains(".pptx") OrElse extension.ToLower.Contains(".ppsx") Then
                        ...
                        Dim st As New Zamba.FileTools.SpireTools()
                        body = st.GetTextFromPowerPoint(filePath)

                    ElseIf extension.ToLower.Contains(".txt") OrElse extension.ToLower.Contains(".rtf") Then
                    ...
                    insertoContenido = True
                End If

                If InsertoIndices AndAlso insertoContenido Then
                    Return IndexedState.IndicesyContenido
                ElseIf InsertoIndices Then
                    Return IndexedState.IndicesSolamenteIndexado
                ElseIf insertoContenido Then
                    Return IndexedState.SoloContenido
                Else
                    Return IndexedState.IndexadoNoRequerido
                End If
    ...
    Public Shared Function InsertSearchData(ByVal body As String, ByVal docID As Int64, ByVal docTypeID As Int64) As Boolean
        ...
        Try
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Indexer Palabras: " & body)
            data = ReplaceChar(body)
            data = TextTools.ReemplazarAcentos(data)
            data = data.ToLower()
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Indexer Palabras parcial: " & data)
            ...
            If data.Length > 0 Then
                Dim RF As New Results_Factory
                RF.InsertSearchIndexDataService(data, docTypeID, docID, 0, Nothing)
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        Finally
    ...
    End Function
    Public Shared Function ReplaceChar(body As String, character As String) As String
        Dim data As New StringBuilder
        _unicodeCategory = ZOptBusiness.GetValue("GetUnicodeCategory")
        ...
        While Letters.MoveNext
            If EvaluateIsNotValidChar(Letters.Current) Then
                If data.Length > 0 AndAlso data.Chars(data.Length - 1) <> character Then
