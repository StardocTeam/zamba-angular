...
Public Class TextoInteligente
        ...
        If SmartText.Contains("<<") Then
            ...
            If Not IsNothing(OnlySpecifiedIndexsids) Then
                If OnlySpecifiedIndexsids.Count > 0 Then
                    ...
                    rstBuss.SaveModifiedIndexData(Result, True, True, OnlySpecifiedIndexsids)
                    rstBuss = Nothing
                End If
            End If
            Return True
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GetValueFromZvarOrSmartText(ByVal inputStr As String, ByVal taskResult As ITaskResult) As String
        ...
        If (inputStr.ToLower().Contains(">>.<<")) AndAlso taskResult IsNot Nothing Then
            stringToReturn = Zamba.Core.TextoInteligente.ReconocerCodigo(stringToReturn, taskResult).Trim()
            ...
            For actualLevel As Integer = 1 To levels
                ...
                If actualLevel = levels Then
                    For Each objectProperty As PropertyInfo In InitialObject.GetType.GetProperties
                        If objectProperty IsNot Nothing AndAlso Not String.IsNullOrEmpty(objectProperty.Name) AndAlso String.Compare(objectProperty.Name, Propiedad, True) = 0 Then
                            ...
                            If Not IsNothing(Parametro) Then
                                ...
                                If String.Compare("VARIABLES", Propiedad, True) = 0 Then
                                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Es Variable")
                                    DirectCast(InitialObject, TaskResult).Variables(Parametro) = resultado
                                Else
                                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Es Atributo")
                                    Dim indice As IIndex = DirectCast(InitialObject, TaskResult).IndexByName(Parametro)
                                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Atributo: " & indice.Name)
                                    If String.Compare(indice.Data, resultado, True) <> 0 Then
                                        ...
                                        ZTrace.WriteLineIf(ZTrace.IsInfo, "Id de Atributo:" & indice.ID)
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Shared Function ReconocerCodigo(ByVal codedText As String, ByVal TaskResult As ITaskResult, Optional ByVal TraceChanges As Boolean = True) As String

        If codedText.Length = 0 Then Return codedText

        If (codedText.Contains("<<") = False AndAlso codedText.ToLower().Contains("zamba.") = False) Then
            Return codedText
        End If
        ...
        'codedText = codedText.Replace("<<Zamba>>", "<<Tarea>>")
        ...
        Dim listao As System.Text.RegularExpressions.MatchCollection
        listao = Regex.Matches(codedText, "[^\ \{{]+|\{{([^\}}]*)\}}|}}([^{{]*)", RegexOptions.ExplicitCapture)

        'Se quita una barra para expresion regular para marsh
        'Dim listao As System.Text.RegularExpressions.MatchCollection
        'listao = Regex.Matches(codedText, "[^\\ \{{]+|\{{([^\}}]*)\}}|}}([^{{]*)", RegexOptions.ExplicitCapture)

        For Each RegexMatch As System.Text.RegularExpressions.Match In listao ' codedText.Split(" ")
            Try
                Dim CurrentWord As String = RegexMatch.Value
                ...
                If bolIndex = False Then
                    If CurrentWord.Contains("<<") = True Then
                        ZTrace.WriteLineIf(TraceChanges, "Palabra a decodificar: " & CurrentWord)
                        ...
                        If lista.Count > 0 Then
                            ZTrace.WriteLineIf(TraceChanges, "Expresion Regular: " & lista(0).Value)
                            ...
                            ZTrace.WriteLineIf(TraceChanges, "Texto Decodificado: " & DecodedText.ToString())
                        End If
                        '     Dim lista As MatchCollection = Regex.Matches(Codigo, "(\<.+\>\.\<.+\>)+", RegexOptions.ExplicitCapture)
                        '   DecodedText.Append(Search(CurrentWord))
                    ElseIf CurrentWord.ToLower().Contains("zamba.") = True Then
                        CurrentWord = CurrentWord.Replace("{", "").Replace("}", "")
                        ZTrace.WriteLineIf(TraceChanges, "Palabra a decodificar: " & CurrentWord)
                        If CurrentWord.Split(".")(0).ToLower() = "zamba" Then
                            If CurrentWord.Split(".")(1).ToLower() = "tarea" Then
                                DecodedText.Append(Search(CurrentWord.Replace("zamba.", "").Replace("Zamba.", "").Replace("ZAMBA.", "").Replace(".", "|"), TaskResult))
                            Else
                                DecodedText.Append(Search(CurrentWord.Replace("zamba.", "").Replace("Zamba.", "").Replace("ZAMBA.", "").Replace(".", "|")))
                            End If
                            ZTrace.WriteLineIf(TraceChanges, "Texto Decodificado: " & DecodedText.ToString())
                        End If
                    Else
                ...
                Else
                    ...
                    If count1 = count2 Then
                        ...
                        If partialText.Contains("<<") = True Then
                            ZTrace.WriteLineIf(TraceChanges, "Palabra a decodificar: " & partialText)
                            Dim lista As System.Text.RegularExpressions.MatchCollection
                            ...
                            If lista.Count > 0 Then
                                ...
                                ZTrace.WriteLineIf(TraceChanges, "Texto Parcial: " & DecodedText.ToString())
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Shared Function ReconocerCodigoAsHashTB(ByVal codedText As String, ByVal TaskResult As ITaskResult, ByVal newResult As IResult) As Hashtable
            ...
            If bolIndex = False Then
                If CurrentWord.Contains("<<") = True Then
                    ...
                    If lista.Count > 0 Then
                        ...
                        Dim values() As String = codedText.Replace(">>.<<", "|").Replace("<<", "").Replace(">>", "").Split("|")
                        If lista(0).Value.Split(".")(0).ToLower() = "<<tarea>>" Then
                            DecodedText.Append(Search(lista(0).Value, newResult))
                            If Cambios.ContainsKey(lista(0).Value) = False Then
                                Cambios.Add(lista(0).Value, Search(lista(0).Value, newResult))
                            Else
                                Cambios(lista(0).Value) = Search(lista(0).Value, newResult)
                            End If
                        Else
                            DecodedText.Append(Search(lista(0).Value))
                            If Cambios.ContainsKey(lista(0).Value) = False Then
                            ...
                            Else
                                Cambios(lista(0).Value) = Search(lista(0).Value)
                            End If
                        End If
                        DecodedText.Append(CurrentWord.Substring(lista(0).Index + lista(0).Length, CurrentWord.Length - (lista(0).Index + lista(0).Length)))
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Shared Function ReconocerCodigo(ByVal codedText As String, ByVal TaskResult As ITaskResult, ByVal prevtext As String, Optional ByVal TraceChanges As Boolean = True) As String
    ...
    Public Shared Function Search(ByVal codedText As String, Optional ByVal InitialObject As Object = Nothing) As String
        ...
        If codedText.ToLower().Trim = "<<tarea>>.<<fullpath>>" OrElse codedText.ToLower().Trim = "tarea.fullpath" Then
            If Not IsNothing(InitialObject) AndAlso TypeOf (InitialObject) Is Result Then
                If DirectCast(InitialObject, Result).Disk_Group_Id <> 0 AndAlso ZCore.filterVolumes(DirectCast(InitialObject, Result).Disk_Group_Id).Type = VolumeTypes.DataBase Then
        ...
        Try
            ...
            For actualLevel As Integer = 1 To levels
                ...
                If actualLevel = levels Then
                    ...
                    For Each objectProperty As PropertyInfo In InitialObject.GetType.GetProperties
                        If Not IsNothing(objectProperty) Then
                            ...
                            If String.Compare(objectProperty.Name, Propiedad, True) = 0 Then
                                ...
                                Else
                                    'Si es solo un propiedad entra aca
                                    Dim finalobject As Object = objectProperty.GetValue(InitialObject, Nothing)
                                    If finalobject IsNot Nothing Then
                                        FinalValue = finalobject.ToString()
                                    Else
                                        FinalValue = String.Empty
                                    End If
                                End If
    ...
    Public Shared Sub AddNewItems(ByRef txtTextBoxAEvaluar As TextBox, ByRef lstListaDeSugerencias As ListBox)
        Try
            ...
            If String.Compare(TextoDesde, "<<", True) = 0 Then
                lstListaDeSugerencias.Items.Clear()
                AgregarAllFromAssembly(lstListaDeSugerencias)
                Exit Sub
            End If
        ...
        For Each indice As Int64 In atributos.Keys
            For Each index2 As Index In newres.Indexs
                If index2.ID = indice Then
                    ...
                    Dim value As String
                    value = ReconocerCodigo(atributos(indice), Result)
                    ...
                    '    ZTrace.WriteLineIf(ZTrace.IsInfo, "Atributo de Sustitucion - Intentando recuperar descripcion")
                    '    index2.dataDescription = AutoSubstitutionBusiness.getDescription(value, index2.ID, False, index2.Type)
                    '    ZTrace.WriteLineIf(ZTrace.IsInfo, "Valor descripcion: " & index2.dataDescription)
                    'End If
                    'ZTrace.WriteLineIf(ZTrace.IsInfo, "Valor asignado")

                    Dim strABuscar As String
                    If TypeOf (obj) Is DataSet Then
                        strABuscar = "<<Tarea>>.<<TablaIndice(" + index2.Name + ")>>"
                        Office.OfficeInterop.FindAndReplaceInWord(word, strABuscar, DirectCast(obj, DataSet).Tables(0))
                        'Else
                        '    strABuscar = "<<Tarea>>.<<Indice(" + index2.Name + ")>>"
