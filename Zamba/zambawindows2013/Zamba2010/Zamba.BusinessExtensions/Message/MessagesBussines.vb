...
Public Class MessagesBusiness
    ...
    Public Shared Function EmbedImages(ByRef HTML As String, ByRef ImagesPaths As ArrayList)
        ...
        While currentindex <> -1
            ...
            srcstartindex = TempHTML.IndexOf("src=" & Chr(34), currentindex, StringComparison.CurrentCultureIgnoreCase) + 5
    ...
    Public Shared Function GetNewFile(ByVal fileFullPath As String, ByVal fileName As String) As String
        Try
            ...
            If Not Directory.Exists(EnvironmentUtil.GetTempDir(String.Empty).FullName & officeTemp) Then
                Directory.CreateDirectory(EnvironmentUtil.GetTempDir(String.Empty).FullName & officeTemp)
            End If

            newFileExtension = Path.GetExtension(fileFullPath)
            newFileName = EnvironmentUtil.GetTempDir(String.Empty).FullName & officeTemp & fileName

            'Se verifica si la extensión del archivo existe y si es la misma que el archivo de origen.
            If String.Compare(Path.GetExtension(newFileName).ToLower(), newFileExtension.ToLower()) <> 0 Then
            ...
            File.Copy(fileFullPath, newFileName, True)
            Return newFileName
        Catch ex As Exception
            raiseerror(ex)
            Return Nothing
    ...
    Public Overloads Shared Sub AutoMail_SMTP(ByVal automail As AutoMail, ByVal currentResult As Result, ByVal addDocument As Boolean, ByVal AddLink As Boolean, ByVal AddIndexs As Boolean, ByVal askConfirm As Boolean, ByVal smtp As SMTP_Validada, ByVal indexsNames As List(Of String))
        ...
        body.Append(TextoInteligente.ReconocerCodigo(automail.Body, currentResult))

        If AddIndexs Then
            Dim TextoIndices As New StringBuilder
            TextoIndices.Append("Atributos: ")
            TextoIndices.AppendLine()

            If Not IsNothing(indexsNames) AndAlso indexsNames.Count > 0 Then
                For Each index As Index In currentResult.Indexs
        ...
        Try
            Dim sUseWebService As String = UserPreferences.getValueForMachine("WinUseWSSendMail", UPSections.Mail, "False")
    ...
    Public Overloads Shared Sub AutoMail_SMTP(ByVal automail As AutoMail, ByVal smtpAutenticado As SMTP_Validada)
        If IsNothing(smtpAutenticado) Then
            Try
                Dim m As IMailMessage = GetMessage(MailTypes.NetMail)
                ...
                m.Body = TextoInteligente.ReconocerCodigo(automail.Body.Trim, Nothing)
    ...
    Public Shared Function GetAutomailByName(ByVal Name As String) As AutoMail
        ...
        Try
            ir = Server.Con.ExecuteReader(CommandType.Text, strselect)
    ...
    Public Shared Function SendMail(ByVal para As String, ByVal CC As String, ByVal CCO As String, ByVal Asunto As String, ByVal Body As String, ByVal isBodyHtml As Boolean, Optional ByVal AttachFileNames As List(Of String) = Nothing, Optional ByVal ImagesToEmbedPaths As List(Of String) = Nothing) As Boolean
        ...
        If Not Membership.MembershipHelper.CurrentUser Is Nothing Then
            ...
            Dim rw As DataRow
            Correo = UserBusiness.Mail.FillUserMailConfig(UserId)
    ...
    Public Shared Sub SendMail(ByVal eMailType As MailTypes,
        ...
        If useWebService Then
            ...
            Dim wsAttaches As List(Of DataWS.WSResults.BlobDocument) = GetBlobFilesAttaches(attachFileNames)
    ...
    Public Shared Sub SendMail(ByVal para As String, ByVal CC As String, ByVal CCO As String, ByVal Asunto As String, ByVal Body As String, ByVal isBodyHtml As Boolean, ByVal addLinks As Boolean, ByVal attachResult As Result, Optional ByVal AttachFileNames As List(Of String) = Nothing)
        ...
        If addLinks Then
            ...
            Dim attachResultLink As String = Results_Business.GetHtmlLinkFromResult(_attachResult.DocTypeId, _attachResult.ID, "Acceso al documento.")
            ...
            If Not IsNothing(ZOptBusiness.GetValue("WebAccessDocIdPath")) Then
                ...
                sBuilder.Append(ZOptBusiness.GetValue("WebAccessDocIdPath") & "?docid=" & _attachResult.ID & "&doctid=" & _attachResult.DocTypeId)
                sBuilder.Append(Convert.ToChar(34))
                sBuilder.Append(">")
    ...
    ''' </history>
    ''' -----------------------------------------------------------------------------
    Public Shared Function GetMessage(ByVal mt As MailTypes) As IMailMessage
        Select Case mt
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Shared Function GetBlobFilesAttaches(ByVal attachFileNames As List(Of String)) As List(Of DataWS.WSResults.BlobDocument)
        'Si hay paths para levantar
        If attachFileNames IsNot Nothing AndAlso attachFileNames.Count > 0 Then
            Dim blobAttach As DataWS.WSResults.BlobDocument
            Dim filenfo As FileInfo
            Dim attachesBytes As Byte()

            Try
                Dim wsAttaches As New List(Of DataWS.WSResults.BlobDocument)
                ...
                For Each sFile As String In attachFileNames
                    If Not String.IsNullOrEmpty(sFile) Then
                        ...
                        If filenfo IsNot Nothing AndAlso filenfo.Exists Then
                            ...
                            blobAttach = New DataWS.WSResults.BlobDocument()
    ...
    Private Shared Sub QuequeMail(conf As ISendMailConfig)
        ...
        Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("Insert into ZQM (UserId, Mail, State) values ({0},{1},0)", conf.UserId, MailSerialized))
    ...
    Public Sub PeekQuequedMail(QuequedState As QuequedStates)
        Dim Id As String = Server.Con.ExecuteScalar(CommandType.Text, String.Format("select top 1 Id from ZQM where state = {0}", QuequedState))
        Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("update  ZQM set state = 1 where id = {0}", Id))
        Dim MailSerialized As String = Server.Con.ExecuteScalar(CommandType.Text, String.Format("select Mail from ZQM where id = {0}", Id))
        ...
        If Not MailSerialized Is Nothing Then
            ...
            If SendMail(conf.MailTo, conf.Cc, conf.Cco, conf.Subject, conf.Body, conf.IsBodyHtml, conf.AttachFileNames, conf.ImagesToEmbedPaths) Then
                Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("update  ZQM set state = 2 where id = {0}", Id))
            Else
                Server.Con.ExecuteNonQuery(CommandType.Text, String.Format("update  ZQM set state = 3 where id = {0}", Id))
            End If
    ...
    Public Shared Function GetNewMailAndSaveHistory(ByVal docId As Long, ByVal docTypeId As Long, Optional ByVal _params As Hashtable = Nothing, Optional ByVal _automaticSend As Boolean = False) As Boolean
        Try
            Dim outlookInstance As OutlookInterop = SharedOutlook.GetOutlook()
            outlookInstance.SendMail(outlookInstance.GetNewMailItem(_params), _automaticSend)
            SaveParamInDB(outlookInstance.LastMailItemTo, outlookInstance.LastMailItemSubject, docId, docTypeId)
    ...
    Public Shared Sub SearchMailAndSaveAsync(sender As Object, e As DoWorkEventArgs)
        Try
            ...
            Dim outlookInstance As OutlookInterop = e.Argument
            ...
            If Not IsNothing(mails) AndAlso mails.Rows.Count > 0 Then
                ...
                If outlookInstance Is Nothing Then
                    If SharedOutlook.IsOutlookRunning() Then
                ...
                Dim _pathDirectory As String = EnvironmentUtil.GetTempDir("\IndexerTemp").FullName
                Dim NuevoFile As New FileInfo(FileBusiness.GetUniqueFileName(_pathDirectory, "OutlookMail", ".msg"))
    ...
    Private Shared Sub SaveMailItem(ByVal outlookInstance As OutlookInterop, ByVal mailItemFind As Microsoft.Office.Interop.Outlook.MailItem, ByVal nameFile As String, ByVal docId As Long, ByVal docTypeId As Long)
        Try
            ...
            UserBusiness.Rights.SaveAction(0, ObjectTypes.ModuleMail, RightsType.EnviarPorMail, "Se ha enviado un mail desde el documento " & docId & "(dtid=" & docTypeId & ") con asunto '" & m_Subject & "'")

            SaveHistory(m_TO, m_CC, m_BCC, m_Subject, m_Body, m_Attachs, docId, docTypeId, nameFile)
    ...
    Private Shared Sub SaveParamInDB(ByVal toSend As String, ByVal subject As String, ByVal docId As Integer, ByVal docTypeId As Integer)
        Try
            ...
            If Server.isOracle Then
                strQuery.Append(If(subject IsNot Nothing, subject.ToString, OutlookInterop.SUBJECT_EMPTY.ToString()))
            Else
    ...
    Public Function ExtractAttachsAsFiles(MsgFile As String, newFilesDirectory As String, extensions As List(Of String)) As List(Of [String])
        Dim Attachs As New List(Of [String])()

        Dim myOutlook As OutlookInterop = SharedOutlook.GetOutlook()

        Dim currentMail As Microsoft.Office.Interop.Outlook.MailItem = myOutlook.GetMailItemFromFile(MsgFile, False, FormWindowState.Minimized, False)
        ...
        If currentMail IsNot Nothing Then
            ...
            For Each att As Microsoft.Office.Interop.Outlook.Attachment In currentMail.Attachments
                Try
                    Dim AllowedExtension As Boolean = False
                    If extensions.Count > 0 Then
                        For Each ext As String In extensions
                            If att.FileName.ToLower.EndsWith(ext.ToLower) Then
                    ...
                    If AllowedExtension Then
                        Dim storeName As [String] = Path.Combine(newFilesDirectory, att.FileName)
                        Dim i As Int64 = 0
                ...
                End Try
            Next
            currentMail.Close(Microsoft.Office.Interop.Outlook.OlInspectorClose.olDiscard)

        End If
        Return Attachs
    End Function

    Public Function ExtractAttachsAsFilesWithSpire(MsgFile As String, newFilesDirectory As String, extensions As List(Of String)) As List(Of String)

        Dim _attachs As New List(Of String)
        Dim Attachs As New Dictionary(Of String, Stream)()

        Dim SpEmail As New SpireEmail
        ZTrace.WriteLineIf(ZTrace.IsInfo, String.Format("Extrayendo adjuntos del msg: {0}", MsgFile))
        Attachs = SpEmail.GetEmailAttachs(MsgFile)
    ...
    Public Function ExtractFirstAttachsInMail(MsgFile As String, newFilesDirectory As String, extensions As List(Of String)) As String
        Dim Attachs As New List(Of [String])()

        Dim myOutlook As OutlookInterop = SharedOutlook.GetOutlook()

        Dim TempfileDirectory As DirectoryInfo = EnvironmentUtil.GetTempDir("OfficeTemp")
        If TempfileDirectory.Exists = False Then
            TempfileDirectory.Create()
        End If
        Dim fi As New FileInfo(MsgFile)
        Dim newFileName As String = Path.Combine(TempfileDirectory.FullName, fi.Name)
        Dim i As Int64 = 0
        While File.Exists(newFileName)
            i += 1
            newFileName = Path.Combine(TempfileDirectory.FullName, Path.GetFileNameWithoutExtension(fi.Name) + "(" & i & ")" + Path.GetExtension(fi.Name))
        End While

        File.Copy(MsgFile, newFileName, True)

        Dim currentMail As Microsoft.Office.Interop.Outlook.MailItem = myOutlook.GetMailItemFromFile(newFileName, False, FormWindowState.Minimized, False)
        ...
        If currentMail IsNot Nothing Then
            ...
            For Each att As Microsoft.Office.Interop.Outlook.Attachment In currentMail.Attachments
                Try
                    ...
                    If extensions.Count > 0 Then
                        For Each ext As String In extensions
                            If att.FileName.EndsWith(ext) Then
                                AllowedExtension = True
                            End If
                        Next
                    Else
                    ...
                    If AllowedExtension Then
                        ...
                        While File.Exists(storeName)
                            x += 1
                            storeName = Path.Combine(newFilesDirectory, Path.GetFileNameWithoutExtension(att.FileName) & "(" & x & ")" & Path.GetExtension(att.FileName))
                        End While
                        If storeName.Length > 254 Then
                            storeName = storeName.Substring(0, 254 - Path.GetExtension(att.FileName).Length) + Path.GetExtension(att.FileName)
                        End If
    ...
    Public Function ExtractFirstAttachsInMailWithSpire(MsgFile As String, newFilesDirectory As String, extensions As List(Of String)) As String
    ...
    Public Function CountAttachsAsFiles(MsgFile As String, extensions As List(Of String)) As Int64
        Dim Attachs As New List(Of [String])()

        Dim myOutlook As OutlookInterop = SharedOutlook.GetOutlook()
        Dim newfileDirectory As DirectoryInfo = EnvironmentUtil.GetTempDir("OfficeTemp")
        If newfileDirectory.Exists = False Then
            newfileDirectory.Create()
        End If
        Dim fi As New FileInfo(MsgFile)
        Dim newFileName As String = Path.Combine(newfileDirectory.FullName, fi.Name)
        Dim i As Int64 = 0
        While File.Exists(newFileName)
            i += 1
            newFileName = Path.Combine(newfileDirectory.FullName, Path.GetFileNameWithoutExtension(fi.Name) & "(" & i & ")" + Path.GetExtension(fi.Name))
        End While
        File.Copy(MsgFile, newFileName, True)
        Dim currentMail As Microsoft.Office.Interop.Outlook.MailItem = myOutlook.GetMailItemFromFile(newFileName, False, FormWindowState.Minimized, False)
    ...
    Public Function CountAttachsAsFilesWithSpire(MsgFile As String, extensions As List(Of String)) As Int64

        Dim attachsCount As Long
        Dim SpireMail As New SpireEmail

        Try

            If Not extensions.Count > 0 Then

                attachsCount = SpireMail.GetEmailAttachsCount(MsgFile)

            Else

                Dim attachs As List(Of String) = SpireMail.GetEmailAttachsNames(MsgFile)
