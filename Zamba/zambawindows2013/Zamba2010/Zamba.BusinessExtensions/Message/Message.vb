...
#Region "Propiedades"
    ...
    Public Property Read() As Boolean Implements IInternalMessage.Read
        Get
            If _Read = False Then
                ...
                For Each d In Destinatarios
                    If d.UserID = _User.ID Then 'Me.Owner_User_ID Then
        ...
        Set(ByVal Value As Boolean)
            ...
            _Read = Value
            For Each d In Destinatarios
                If d.UserID = Owner_User_ID Then
                    d.Readed = Value
                End If
            Next
    ...
    Public ReadOnly Property CC() As ArrayList Implements IInternalMessage.CC
        Get
            ...
            For i = 0 To Destinatarios.Count - 1
                dest = DirectCast(Destinatarios(i), Destinatario)
    ...
    Public Property DeStr() As String Implements IInternalMessage.DeStr
        Get
            Return UserGroupBusiness.GetUserorGroupNamebyId(De)
        End Get
        Set(ByVal value As String)
            '_DeStr = value
        End Set
...
#End Region
    ...
    Public Function getResend() As IInternalMessage Implements IInternalMessage.getResend
        ...
        msg.Body = ControlChars.NewLine & ControlChars.NewLine & ControlChars.NewLine & "Mensaje Enviado el " & Fecha.ToShortDateString & " por:" & UserName & ControlChars.NewLine & "Para:" & ToStr & ControlChars.NewLine & "Con Copia a:" & CCStr & ControlChars.NewLine & Body
        msg.Subject = "RV:" & Subject
        'msg.Destinatarios = Me.ResendDest
        msg.AttachList = AttachList
        msg.De = Owner_User_ID
        Return msg
    End Function
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ...
    Public Function GetResponse() As IInternalMessage Implements IInternalMessage.GetResponse
        ...
        Owner_User_ID = Convert.ToInt32(_User.ID)
        Dim msg As New InternalMessage(Owner_User_ID)
        msg.Body = ControlChars.NewLine & ControlChars.NewLine & ControlChars.NewLine & "Mensaje Enviado el " & Fecha.ToShortDateString & " por:" + UserName + ControlChars.NewLine + "Para:" & ToStr & ControlChars.NewLine & "Con Copia a:" & CCStr & ControlChars.NewLine & Body
        ...
        dest.Add(New Destinatario(UserBusiness.GetUserById(De), MessageType.MailTo))
    ...
    Public Function getResponseAll() As IInternalMessage Implements IInternalMessage.getResponseAll
        ...
        Owner_User_ID = Convert.ToInt32(_User.ID)
        Try
            msg.Destinatarios.Add(New Destinatario(UserBusiness.GetUserById(De), MessageType.MailTo))
            msg.Body = ControlChars.NewLine & ControlChars.NewLine & ControlChars.NewLine & "Mensaje Enviado el " & Fecha.ToShortDateString & " por:" + UserName + ControlChars.NewLine + "Para:" & ToStr & ControlChars.NewLine & "Con Copia a:" & CCStr & ControlChars.NewLine & Body
            msg.Subject = "RE:" & Subject
            msg.AttachList = New ArrayList
            msg.De = Owner_User_ID
            ...
            'msg.RemoveMyDest()
            Dim name As String = ""
            msg.Destinatarios.AddRange(getResponseAllDestino(name))
            msg.UserName = name
            Return msg
        ...
        Try
            For Each des In Destinatarios
                If des.Type <> MessageType.MailCCO Then
                    If des.UserID <> Owner_User_ID Then
                        result.Add(des)
                    Else
    ...
    Public Sub Send() Implements IInternalMessage.Send
        Id = Zamba.Core.CoreBusiness.GetNewID(IdTypes.INTERNALMAIL)
        De = Convert.ToInt32(Membership.MembershipHelper.CurrentUser.ID) 'Id Usuario que envia mensaje
    ...
    Public Sub Delete() Implements IInternalMessage.Delete
        If Owner_User_ID = De Then
            DeleteMessageSended()
        Else
            DeleteMessageRecived()
        End If
    End Sub
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ...
    Public Sub SetAsRead() Implements IInternalMessage.SetAsRead
        Try
            MessagesFactory.SetAsRead(Id.ToString, _User.ID)
        Catch ex As Exception
            Zamba.Core.ZClass.raiseerror(ex)
        End Try
    End Sub
...
#Region "Send"
    ...
    Private Sub RegisterDestinatarios()
        Try
            For Each item As Object In Destinatarios
                Try
                    Select Case item.GetType().FullName
                        ...
                        Case "Zamba.Controls.Destinatario"
                            ...
                            InternalMessage.RegisterMailToUser(Convert.ToInt32(Id), Convert.ToInt32(user.UserID), user.Type, user.UserName)
                            user = Nothing
                            item = Nothing
    ...
    Private Sub RegisterAllAttach()
        ...
        Try
            ...
            For i = 0 To _attachList.Count - 1
                Dim attachment As Object = _attachList(i)
...
#Region "Delete"
    Public Sub DeleteMessageRecived() Implements IInternalMessage.DeleteMessageRecived
        Try
            MessagesFactory.DeleteMessageRecived(Id, _User.ID)
        Catch ex As Exception
            Zamba.Core.ZClass.raiseerror(ex)
        End Try
    End Sub
...
#End Region
    ...
    Public Sub addDest(ByVal id As Integer, ByVal User_Name As String, ByVal desttype As MessageType) Implements IInternalMessage.addDest
        ...
        Dim dest As New Destinatario(id.ToString(), Integer.Parse(User_Name), desttype.ToString())
        Destinatarios.Add(dest)
    End Sub
    Public Sub addDest(ByVal dest As IDestinatario) Implements IInternalMessage.addDest
        If dest.UserID = Owner_User_ID Then
            _Read = dest.Readed
        End If
        Destinatarios.Add(dest)
    End Sub
