...
Public Class DocTypesBusiness
    ...
    Public Shared Sub GetEditRights(ByVal DocType As DocType)
        Try
            If DocType.RightsLoaded = False Then
                DocType.IsReadOnly = Not UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.Edit, DocType.ID)
                ...
                If IgnoreIsReIndex = False Then
                    DocType.IsReindex = UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.ReIndex, DocType.ID)
                Else
                    DocType.IsReindex = True
                End If
                DocType.IsShared = UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.Share, DocType.ID)
    ...
    Public Shared Function GetDocTypesArrayList(Optional ByVal bolAddBlank As Boolean = False) As ArrayList
        Dim dsDocType1 As DataSet = DocTypesFactory.GetDocTypesArrayList
        ...
        For i = 0 To dsDocType1.Tables("DOC_TYPE").Rows.Count - 1
            Dim DocType As New DocType(Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("DOC_TYPE_ID")), dsDocType1.Tables("DOC_TYPE").Rows(i)("DOC_TYPE_NAME"), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("FILE_FORMAT_ID")), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("DISK_GROUP_ID")), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("THUMBNAILS")), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("ICON_ID")), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("OBJECT_TYPE_ID")), dsDocType1.Tables("DOC_TYPE").Rows(i)("AUTONAME"), dsDocType1.Tables("DOC_TYPE").Rows(i)("AUTONAME"), Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("DOCCOUNT")), 0, Convert.ToInt32(dsDocType1.Tables("DOC_TYPE").Rows(i)("DOCUMENTALID")))
    ...
    Public Shared Function GetDocType(ByVal DocTypeId As Int64, Optional ByVal useCache As Boolean = True) As DocType
        SyncLock (Cache.DocTypesAndIndexs.hsDocTypes)
...
#Region "Update"
    ...
    Public Shared Sub UpdateDocType(ByVal DocType As DocType)
        ...
        'Grabo el Log
        UserBusiness.Rights.SaveAction(DocType.ID, ObjectTypes.DocTypes, RightsType.Edit, DocType.Name)
    End Sub
#End Region
...
#Region "WF"
    ...
    Public Shared Function GetDocTypeWfIds(ByVal docTypeId As Int64, Optional ByVal WithCache As Boolean = True) As ArrayList
        SyncLock (Cache.DocTypesAndIndexs.hsWFDocTypes)
            If Not WithCache Then Return DocTypesFactory.GetDocTypeWfIds(docTypeId, WithCache)
            If Not Cache.DocTypesAndIndexs.hsWFDocTypes.ContainsKey(docTypeId) Then
                Cache.DocTypesAndIndexs.hsWFDocTypes.Add(docTypeId, DocTypesFactory.GetDocTypeWfIds(docTypeId, WithCache))
            End If
                ...
                Else
                    If String.IsNullOrEmpty(CurrentIndex.dataDescriptionTemp) Then
                        If String.IsNullOrEmpty(CurrentIndex.dataDescription) Then
                            If CurrentIndex.DropDown = IndexAdditionalType.AutoSustitución AndAlso CurrentIndex.Data.Length > 0 Then
...
#End Region
    ...
    Public Shared Sub AddColumn(ByVal DocTypeId As Int64, ByVal IndexIdArray As ArrayList, ByVal IndexTypeArray As ArrayList, ByVal IndexLenArray As ArrayList)
        DocTypesFactory.AddColumn(DocTypeId, IndexIdArray, IndexTypeArray, IndexLenArray)
    End Sub
    Public Shared Sub AddColumn(docTypeId As Int64, indexId As Long, indexType As IndexDataType, indexLen As Integer)
        ...
        IndexTypeArray.Add(indexType)
        IndexLenArray.Add(indexLen)
        DocTypesBusiness.AddColumn(docTypeId, IndexIdArray, IndexTypeArray, IndexLenArray)
    End Sub

    ''' <summary>
    ''' Agrega un Atributo a un Entidad. Agregar la tabla para observaciones
    ''' </summary>
    ''' <param name="doctype"></param>
    ''' <param name="IndexIdArray"></param>
    ''' <param name="IndexTypeArray"></param>
    ...
    Public Shared Sub AddTable(ByVal DocTypeId As Int64, ByVal IndexIdArray As ArrayList, ByVal IndexTypeArray As ArrayList, ByVal IndexLenArray As ArrayList)
        DocTypesFactory.AddTable(DocTypeId, IndexIdArray, IndexTypeArray, IndexLenArray)
    End Sub
    ...
    Public Shared Function GetDocTypesbyUserRightsOfView(ByVal UserId As Int64, ByVal RightType As Zamba.Core.RightsType) As List(Of IDocType)
        SyncLock (Cache.DocTypesAndIndexs.dicDocTypeByRight)
            If Not Cache.DocTypesAndIndexs.dicDocTypeByRight.ContainsKey(RightType) Then
                Dim DocTypes As List(Of IDocType) = DocTypesFactory.GetDocTypesbyUserRightsOfView(UserId, RightType)
...
#Region "AutoSetupHelper"
    ...
    Public Function AsignIndex(ByVal DocTypeName As String, ByVal IndexId As Int64) As Int64
        ...
        Try
            ...
            DocTypesBusiness.AddColumn(DocType.ID, IndexIdArray, IndexTypeArray, IndexLenArray)
...
#End Region
    ...
    Public Class IndexReferences
        ...
        ''' </history>
        Public Shared Function SaveIndexReference(ByRef docTypeId As Int64, ByRef indexId As Int64, ByRef server As String, ByRef dataBase As String,
    ...
    Public Shared Function CheckColumn(docTypeId As Int64, indexId As Int64, indexType As IndexDataType, indexLen As Integer) As Boolean
        Return DocTypeFactoryExt.CheckColumn(docTypeId, indexId, indexType, indexLen)
    End Function
End Class
