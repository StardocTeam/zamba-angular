...
#Region "Get"
    ...
    Public Shared Function GetUsersArrayList(LazyLoad As Boolean) As ArrayList
        Return UserFactory.GetUsersArrayList(LazyLoad)
    End Function
    ...
    Public Shared Function GetUserByname(ByVal name As String) As IUser
        ...
        For Each u As IUser In Cache.UsersAndGroups.hsUserTable.Values
            If String.Compare(u.Name.Trim, name, True) = 0 Then
                Return u
            End If
        Next
    ...
    ''' <summary>
    ''' Recibe un grupo y completa los grupos a los que pertenece para Active Directory
    ''' </summary>
    ''' <param name="user"></param>
    ''' <returns></returns>
    Public Shared Function CompleteUserWithAD(ByRef user As User)
        Try
            Dim adlogin As ADResources = New ADResources()
            Dim roles As List(Of String) = adlogin.GetRolesForUser(user.Name)

            If roles IsNot Nothing AndAlso roles.Count > 0 Then

                Dim groupNames As List(Of String) = UserGroupBusiness.GetUserGroupsNamesByUserId(user.ID)

                'Asignar grupos
                For Each rol As String In roles
                    If rol.ToLower.Contains("zamba") Then
                        If Not groupNames.Contains(rol) Then
                            Dim groupId As Long = UserGroupBusiness.GetUserorGroupIdbyName(rol, False)
                            If groupId > 0 Then
                                Dim usrGroup As UserGroup = UserGroupBusiness.GetUserGroupAsUserGroup(groupId)
                                UserGroupBusiness.AssignUser(user, usrGroup)
                            Else
                                ZTrace.WriteLineIf(ZTrace.IsInfo, String.Format("El grupo ""{0}"" no existe en Zamba", rol))
                            End If
                        End If
                    End If
                Next
                ...
                For Each group As UserGroup In user.Groups
                    If group.Name.ToLower.Contains("zamba") Then
                        If Not roles.Contains(group.Name) Then
                            UserGroupBusiness.RemoveUser(group, user)
                        End If
                    End If
                Next

                user.Groups.Clear()
                user.Groups = UserGroupBusiness.getUserGroups(user.ID, True)
    ...
    Public Shared Function ValidateDataBase() As Boolean
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                'Valido SERVER
                If array(0).ToString().ToLower().Contains("www.stardoc") OrElse array(0).ToString().ToLower().Contains("yoda") OrElse array(0).ToString().ToLower().Contains("buesrvsql04.boston.ad") OrElse array(0).ToString().ToLower().Contains("obi-wan") OrElse array(0).ToString().ToLower().Contains("server2008") Then
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Stardoc")
                    Return True
                End If


                Dim FirstStringConn As String = Encryption.DecryptString(dv.ToTable().Rows(0).Item("value").ToString(), key, iv).ToLower()
                Dim CompleteStringConn As String = array(0).ToString().ToLower()
...
#End Region
    ...
    Public Shared Function GetNewUser(ByVal sessionName As String, ByVal userName As String, ByVal userLastName As String) As IUser
        ...
        UserBusiness.Rights.SaveAction(user.ID, ObjectTypes.Users, RightsType.Create, "Usuario creado: " & user.Name & " (" & user.ID & ")")
    ...
    Public Shared Function IsWindowsUserAndPreference() As Boolean
        ...
        Try
            ...
            End If
            flagIsWUserPreferenceTrue = Boolean.Parse(UserPreferences.getValue("LoadWindowsUserOnSOStart", UPSections.UserPreferences, False))
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
    ...
    Public Shared Function IsWUPreferenceAndExistUser() As Boolean
        Try
            ...
            If Boolean.Parse(UserPreferences.getValueForMachine("LoadWindowsUserOnSOStart", UPSections.UserPreferences, False)) Then

                If GetUserID(Environment.UserName.Trim()) <> 0 Then
                ...
                Else
                    ...
                    Dim adLogin As ADResources = New ADResources()
                    Dim usrName As String = Environment.UserName.Trim()

                    'Si usuario esta en AD, crear nuevo usuario y retornar true
                    If adLogin.ADIsActive() AndAlso adLogin.ADValidation(usrName) Then

                        Dim UserProperties As Dictionary(Of String, Object) = adLogin.GetUserProperties(usrName)
                        Dim newuser As IUser = New User()
                        newuser.Name = usrName
                        newuser.Password = MembershipHelper.GenerateRandomPassword()

                        If UserProperties IsNot Nothing AndAlso UserProperties.Values.Count > 0 Then
                            If UserProperties.ContainsKey("NOMBRE") Then
                                newuser.Nombres = UserProperties("NOMBRE").ToString()
                            Else
                                newuser.Nombres = usrName
                            End If
                            If UserProperties.ContainsKey("APELLIDO") Then
                                newuser.Apellidos = UserProperties("APELLIDO").ToString()
                            Else
                                newuser.Apellidos = usrName
                            End If

                            Dim mailPort As Int16 = Short.Parse(ZOptBusiness.GetValueOrDefault("DefaultMailPort", "25"))
                            Dim smtpProvider As String = ZOptBusiness.GetValueOrDefault("DefaultMailSMTPProvider", "mx04.main.pseguros.com")
                            Dim enablessl As Boolean = Boolean.Parse(ZOptBusiness.GetValueOrDefault("DefaultMailEnableSsl", "True"))

                            If UserProperties.ContainsKey("EMAIL") Then
                                newuser.eMail = New Correo With
                                    {
                        ...
                        newuser.ID = CoreData.GetNewID(IdTypes.USERTABLEID)
                        AddUser(newuser)
...
#End Region
    ...
    Public Class Rights
        ...
        Public Shared Function GetIndexsRights(ByVal DocTypeId As Int64, ByVal GID As Int64, ByVal addOwnerGroup As Boolean, ByVal WithCache As Boolean) As Hashtable
            Try
                If WithCache AndAlso Cache.DocTypesAndIndexs.hsSpecificIndexsRights.ContainsKey(DocTypeId & "§" & GID) Then
                    Return Cache.DocTypesAndIndexs.hsSpecificIndexsRights(DocTypeId & "§" & GID)
                Else
                    Dim DV As DataView
                    Dim dt As DataTable
                    ...
                    For i As Int64 = 0 To Indexs.Count - 1
                        ...
                        Next
                        If (hashIndexRights.ContainsKey(Indexs(i).ID) = False) Then
                            hashIndexRights.Add(Indexs(i).ID, IR)
                        End If
                    Next
                    If Not Cache.DocTypesAndIndexs.hsSpecificIndexsRights.ContainsKey(DocTypeId & "§" & GID) Then
                        Cache.DocTypesAndIndexs.hsSpecificIndexsRights.Add(DocTypeId & "§" & GID, hashIndexRights)
                    Else
                        Cache.DocTypesAndIndexs.hsSpecificIndexsRights(DocTypeId & "§" & GID) = hashIndexRights
                    End If
        ...
        ''' </history>
        ''' -----------------------------------------------------------------------------
        Public Shared Function GetIndexsRights(ByVal DocTypeIds As Generic.List(Of Int64), ByVal GID As Int64, ByVal addOwnerGroup As Boolean, ByVal WithCache As Boolean) As Hashtable
            Try

                If WithCache AndAlso Cache.DocTypesAndIndexs.hsSpecificIndexsRights.ContainsKey(String.Join("-", DocTypeIds) & "§" & GID) Then
                    Return Cache.DocTypesAndIndexs.hsSpecificIndexsRights.Item(String.Join("-", DocTypeIds) & "§" & GID)
                Else
                    ...
                    dt = RightFactory.GetIndexsRights(DocTypeIds, GroupsUsersIds)
                    ...
                    Cache.DocTypesAndIndexs.hsSpecificIndexsRights.Add(String.Join("-", DocTypeIds) & "§" & GID, hashIndexRights)
                    Indexs = Nothing
                    Indexstemp = Nothing
                    Return hashIndexRights
                End If
        ...
        Public Shared Function GetIndexRightValue(ByVal doctypeid As Int64, ByVal IndexId As Int64, ByVal Userid As Int32, ByVal RightType As RightsType) As Boolean
            ...
            If (doctypeid <> 0 AndAlso IndexId <> 0) AndAlso (Zamba.Core.UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.ViewRightsByIndex, doctypeid) = False) Then
                Return True
            End If
        ...
        Public Shared Sub SetIndexRights(ByVal DocTypeId As Int64, ByVal GID As Int64, ByVal IndexId As Int64, ByVal _RightType As RightsType, ByVal Value As Boolean)
            Try
                Dim righttypeid As Integer = _RightType
        ...
        ''' <summary>
        ''' Establece permiso para ver una columna (de las fijas) en grilla de asociados.
        ''' </summary>
        ''' <param name="groupOrUserID">Id de Grupo o Usuario</param>
        ''' <param name="DocTypeID">Id de tipo de documento</param>
        ''' <param name="DocTypeParentID">Id de tipo de documento del padre</param>
        ''' <param name="Column">Nombre de la columna a la que se le establece el permiso</param>
        ''' <param name="value">Si se puede ver o no</param>
        Public Shared Sub SetAssociateGridColumnsRight(groupOrUserID As Int64, DocTypeID As Int64, DocTypeParentID As Int64, Column As String, value As Boolean)
            Try
                If value Then
                    RightFactory.SetAssociateGridColumnsRight(groupOrUserID, DocTypeID, DocTypeParentID, Column)
                Else
                    RightFactory.RemoveAssociateGridColumnsRight(groupOrUserID, DocTypeID, DocTypeParentID, Column)
                End If
            Catch ex As Exception
                raiseerror(ex)
            End Try
        ...
        ''' <returns></returns>
        Public Shared Function GetAssociateGridColumnsRight(groupOrUserID As Int64, DocTypeID As Int64, DocTypeParentID As Int64, withCache As Boolean) As DataTable
            Try
                Dim dt As DataTable

                'RightFactory.GetAssociateGridColumnsRight(groupOrUserID, DocTypeID, DocTypeParentID)

                If withCache Then
                    If Not Cache.DocTypesAndIndexs.hsAssociatedIndexsRights.ContainsKey(groupOrUserID & "§" & DocTypeID & "§" & DocTypeParentID) Then
                        Cache.DocTypesAndIndexs.hsAssociatedIndexsRights.Add(groupOrUserID & "§" & DocTypeID & "§" & DocTypeParentID, RightFactory.GetAssociateGridColumnsRight(groupOrUserID, DocTypeID, DocTypeParentID))
                    End If
                    dt = Cache.DocTypesAndIndexs.hsAssociatedIndexsRights.Item(groupOrUserID & "§" & DocTypeID & "§" & DocTypeParentID)
                Else
                    dt = RightFactory.GetAssociateGridColumnsRight(groupOrUserID, DocTypeID, DocTypeParentID)
                    If Not Cache.DocTypesAndIndexs.hsAssociatedIndexsRights.ContainsKey(groupOrUserID & "§" & DocTypeID & "§" & DocTypeParentID) Then
                        Cache.DocTypesAndIndexs.hsAssociatedIndexsRights.Add(groupOrUserID & "§" & DocTypeID & "§" & DocTypeParentID, dt)
                    End If
                End If
        ...
        Public Shared Function GetAssociateGridColumnsRight(groupOrUserID As Int64, DocTypeID As Int64, DocTypeParentID As Int64, column As String, withCache As Boolean) As Boolean
            Try
                Dim dt As DataTable
        ...
        Public Shared Function ValidateLogIn(ByVal User As String, ByVal Password As String, ByVal clientType As ClientType) As IUser
            ...
            Dim ActualUserToValidate As IUser = UserFactory.validateUser(User, Password)


            If IsNothing(ActualUserToValidate) Then
                Throw New Exception("El usuario ingresado es incorrecto")
            ElseIf String.Compare(ActualUserToValidate.Password, Password) = 0 Then
                If ActualUserToValidate.State <> 0 Then
            ...
            MembershipHelper.SetCurrentUser(ActualUserToValidate)

            Dim Rights As DataSet = RightFactory.GroupRights(Membership.MembershipHelper.CurrentUser.ID)
            ...
            If currentUserVersion.Length = 0 Then
                currentUserVersion = UpdaterBusiness.GetVersion("Zamba.Cliente.exe")
            End If
        ...
        Public Shared Function ValidateLogIn(ByVal ID As Int32, ByVal clientType As ClientType) As IUser
            Dim ActualUserToValidate As IUser = UserFactory.validateUser(ID)

            If IsNothing(ActualUserToValidate) Then
                Throw New Exception("El usuario ingresado es incorrecto")
            ElseIf ActualUserToValidate.State <> 0 Then
                Throw New Exception("El usuario esta bloqueado, por favor contacte a su administrador de sistema")
            End If
            ...
            Else
                ...
                Dim currentUserVersion As String = UpdaterFactory.GetVersionByEstreg()
                If currentUserVersion.Length = 0 Then
                    currentUserVersion = UpdaterBusiness.GetVersion(Application.ExecutablePath)
                End If
