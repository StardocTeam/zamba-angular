...
Public Class AutoSubstitutionBusiness
    ...
    Public Shared Function GetIndexData(ByVal Indexid As Int64, ByVal Reload As Boolean) As DataTable
        SyncLock (Cache.DocTypesAndIndexs.hsIndexsDT)
            If Reload OrElse Not Cache.DocTypesAndIndexs.hsIndexsDT.ContainsKey(Indexid) Then
                Dim OrderByDescripcion As Boolean = Boolean.Parse(UserPreferences.getValue("OrderListsByDescription", UPSections.UserPreferences, "True"))
    ...
    Public Shared Function getDescription(ByVal Code As String, ByVal IndexId As Int64, Optional ByVal inThread As Boolean = False, Optional ByVal indexType As IndexDataType = IndexDataType.Numerico, Optional ByVal returnEmpty As Boolean = False) As String
        ...
        Dim dtSust As DataTable = GetIndexData(IndexId, False)

        Dim dv As New DataView(dtSust)
        dv.RowFilter = "codigo='" & Code & "'"
        Dim dt2 As DataTable = dv.ToTable()

        If dt2.Rows.Count > 0 AndAlso IsDBNull(dt2.Rows(0)("Descripcion")) = False Then
        ...
        If Cache.DocTypesAndIndexs.hsIndexsDT.ContainsKey(IndexId) Then
            ...
            If dt2.Rows.Count > 0 Then
                ...
                If IsDBNull(dt2.Rows(0)) = False Then
        ...
        Else
            If inThread = False Then
                ...
                If Not IsNothing(dr) AndAlso IsDBNull(dr("Descripcion")) = False Then
    ...
    Public Shared Function getCodigo(ByVal Code As String, ByVal IndexId As Int64, Optional ByVal inThread As Boolean = False, Optional ByVal indexType As IndexDataType = IndexDataType.Numerico, Optional ByVal returnEmpty As Boolean = False) As String

        Dim dtSust As DataTable = GetIndexData(IndexId, False)

        Dim dv As New DataView(dtSust)
        dv.RowFilter = "Descripcion='" & Code & "'"
        Dim dt2 As DataTable = dv.ToTable()

        If dt2.Rows.Count > 0 AndAlso IsDBNull(dt2.Rows(0)("Codigo")) = False Then
            Dim Codigo As String = dt2.Rows(0)("Codigo")
            Return Codigo
        Else
            Return String.Empty
        End If
        ...
        Else
            If inThread = False Then
                Dim dr As DataRow = AutoSubstitutionDataFactory.getCodeRow(IndexId, Code)
                If Not IsNothing(dr) AndAlso IsDBNull(dr("Codigo")) = False Then
                    dtSust.Rows.Add(dr.ItemArray)
                    Return dr("Codigo")
                Else
                    If returnEmpty Then
    ...
    Private Shared Sub setDescriptionToHash(ByVal values As Object)
        ...
        Dim dr As DataRow = AutoSubstitutionDataFactory.getDescriptionRow(indexId, code)
        Dim dt As DataTable = GetIndexData(indexId, False)
        dt.Rows.Add(dr.ItemArray)
    End Sub


    'Quita un item de sustitucion de acuerdo a su codigo
    Public Shared Sub RemoveItems(ByVal codeList As List(Of String), ByVal indexId As Integer)
