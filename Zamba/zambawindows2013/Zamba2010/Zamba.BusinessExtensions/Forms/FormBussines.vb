...
Public Class FormBusiness
    ...
    Public Shared Function GetForms(ByVal DocTypeId As Int64, ByVal FormType As FormTypes, ByVal UseCache As Boolean) As List(Of ZwebForm)

        SyncLock Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType
            If Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType.ContainsKey(DocTypeId & "-" & DirectCast(FormType, Int32)) = False OrElse UseCache = False Then
                Dim forms As New List(Of ZwebForm)
                forms.AddRange(FormFactory.GetForms(DocTypeId, FormType))
                If Not forms Is Nothing Then
                    SyncLock Cache.DocTypesAndIndexs.hsForms
                        For Each form As ZwebForm In forms
                            If Cache.DocTypesAndIndexs.hsForms.ContainsKey(form.ID) = False Then
                        ...
                        Next
                    End SyncLock
                    If Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType.ContainsKey(DocTypeId & "-" & DirectCast(FormType, Int32)) = False Then Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType.Add(DocTypeId & "-" & DirectCast(FormType, Int32), forms)
                    Return forms
                Else
                    Return Nothing
                End If
            Else
                If Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType.ContainsKey(DocTypeId & "-" & DirectCast(FormType, Int32)) = True Then
                    Return Cache.DocTypesAndIndexs.hsFormsByEntityIdAndType(DocTypeId & "-" & DirectCast(FormType, Int32))
                Else
                    Return Nothing
                End If
            End If
    ...
    Public Shared Sub DeleteForm(ByVal Form As ZwebForm)
        ...
        UserBusiness.Rights.SaveAction(Form.ID, ObjectTypes.FormulariosElectronicos, RightsType.Delete, "Se eliminó el formulario: " & Form.Name & "(" & Form.ID & ")")
...
#Region "InsertVirtualForm"
    Public Shared Function CreateVirtualDocument(ByVal docTypeId As Long) As NewResult
        ...
        Results_Business.LoadIndexs((InsertedResult))
...
#End Region
    ...
    Public Shared Function DataTableToJson(Data As DataTable) As String
