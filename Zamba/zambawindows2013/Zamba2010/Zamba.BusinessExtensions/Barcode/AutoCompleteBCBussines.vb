...
#End Region
    ...
    Public Shared Function ExecuteAutoComplete(ByRef docresult As IResult, ByRef ind As IIndex, ByRef frmGrilla As Form, ByVal completaData As Boolean) As Hashtable
        ...
        If Not IsNothing(AC) Then
            Dim indexKeysTemp As ArrayList
            Try
                'Obtiene el campo IndexKey relacionado con AutoComplete del primer documento.
                indexKeysTemp = AutoCompleteBarcode_FactoryBusiness.getIndexKeys(docresult.DocType.ID)


                ZTrace.WriteLineIf(ZTrace.IsInfo, "Obteniendo los datos para autocompletar en los atributos...")
                For i As Int32 = 0 To indexKeysTemp.Count - 1
                    indexKeysTemp.Item(i).DataTemp = Core.Results_Business.findIn(docresult.Indexs, indexKeysTemp.Item(i)).DataTemp
                Next

                Return AC.Complete(docresult, indexKeysTemp, frmGrilla, completaData)
            Finally
                AC.Dispose()
                AC = Nothing
            End Try
    ...
    ''' </history>
    Public Function Complete(ByRef Result1 As Result, ByRef indexKeysTemp As ArrayList, ByRef frmGrilla As Form, ByVal completaData As Boolean) As Hashtable
        Dim modifiedIndex As Hashtable = New Hashtable()
        Dim ds As DataSet = Data.BarcodeFactory.GetSentencia(docTypeId, indexKeysTemp)
        ...
        ElseIf ds.Tables(0).Rows.Count > 1 AndAlso Not frmGrilla Is Nothing Then
            ...
            fila = DirectCast(frmGrilla, IfrmGrilla).Id
        Else
            fila = 0
        End If
                    ...
                    For Each ind As Index In Result1.Indexs
                        ...
                        If ind.ID = r.Item(0) Then
                            ...
                            Try
                                ...
                                If Not IsDBNull(ds.Tables(0).Rows(fila)(r(3).substring(1, r(3).ToString.Length - 2))) Then
                                    col = r(3).substring(1, r(3).ToString.Length - 2)

                                    If completaData = True Then
                                        ind.Data = ds.Tables(0).Rows(fila)(col)
                                    End If
                                    ind.DataTemp = ds.Tables(0).Rows(fila)(col)
    ...
    Public Function CompleteInsert(ByRef Result1 As Result, ByRef indexKeysTemp As ArrayList, ByRef frmGrilla As Form, ByVal completaData As Boolean) As Hashtable
        Dim modifiedIndex As Hashtable = New Hashtable()
        Dim ds As DataSet = Data.BarcodeFactory.GetSentencia(docTypeId, indexKeysTemp)
