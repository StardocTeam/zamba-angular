...
Public Class PlayDOSELECT
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        Try
            Trace.WriteLineIf(ZTrace.IsVerbose, "Cantidad de results a ejecutar: " & results.Count)
            Trace.WriteLineIf(ZTrace.IsVerbose, "Tipo de ejecucion: " & _myRule.ExecuteType)
    ...
    Private Sub executeQuery(ByVal result As TaskResult)
        strselect = _myRule.SQL
        strselect = strselect.Replace("&lt;", "<")
        strselect = strselect.Replace("&gt;", ">")
        strselect = strselect.Replace("&lt", "<")
        strselect = strselect.Replace("&gt", ">")
        strselect = strselect.Replace(Chr(13), String.Empty)
        strselect = strselect.Replace(Chr(10), String.Empty)


        strselect = WFRuleParent.ReconocerVariables(strselect)
        strselect = strselect.Replace(">>,", ">>Ð,").Replace(">><<,", ">>Ð<<")


        If IsNothing(result) AndAlso strselect.ToLower.Contains("<<tarea>>") Then
        Else

            If strselect.Contains("Ð") Then
                Dim newstrselect As New StringBuilder
                For Each CurrentWord As String In strselect.Split("Ð")
                    newstrselect.Append(TextoInteligente.ReconocerCodigo(CurrentWord, result))
                Next
    ...
    Private Sub ExecuteDataset(ByVal strquery As String)
                ...
                For Each pvalue As Object In params(3)
                    If Not IsDBNull(pvalue) Then
                        If Not VariablesInterReglas.ContainsKey(_myRule.HashTable & ".Param" & i) Then
                            VariablesInterReglas.Add(_myRule.HashTable & ".Param" & i, pvalue.ToString, False)
                        Else
        ...
        If Ds IsNot Nothing Then
            Trace.WriteLineIf(ZTrace.IsVerbose, "Resultado Dataset Count: " & Ds.Tables(0).Rows.Count)
        ...
        If Not IsNothing(Ds) Then
            If Not VariablesInterReglas.ContainsKey(_myRule.HashTable & ".Count") Then
                VariablesInterReglas.Add(_myRule.HashTable & ".Count", Ds.Tables(0).Rows.Count, False)
            Else
    ...
    Private Sub ExecuteScalar(ByVal strquery As String, ByVal r As TaskResult, ByRef fullSelect As String)
        ...
        If strquery.StartsWith("@") Then
            ...
            Trace.WriteLineIf(ZTrace.IsVerbose, "Consulta: " & strquery)
        End If
        ...
        If TypeOf (tempResultado) Is DBNull Then
            ...
            Trace.WriteLineIf(ZTrace.IsVerbose, "Resultado es DBNull")
            resultado = String.Empty
        ...
        Else
            ...
            Trace.WriteLineIf(ZTrace.IsVerbose, "Guardando en variable: " & _myRule.HashTable)

            If VariablesInterReglas.ContainsKey(_myRule.HashTable) = False Then
                VariablesInterReglas.Add(_myRule.HashTable, resultado, False)
            Else
