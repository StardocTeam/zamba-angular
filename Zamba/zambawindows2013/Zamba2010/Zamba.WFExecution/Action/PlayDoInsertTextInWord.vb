        ...
        Try
            ...
            If results.Count > 0 Then
                For Each r As ITaskResult In results
                    ...
                    ZTrace.WriteLineIf(ZTrace.IsInfo, String.Format("Ruta del documento : {0}", _wordPath))
                    ...
                    If File.Exists(_wordPath) Then
                        ...
                        If Path.GetExtension(newfile).ToLower = ".rtf" Then
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "El documento es .rtf")
                            Dim rtf As New FrmRtf()
                            rtf.InsertHeaderIntoRtf(newfile, wordTextVar.ToString(), False, Nothing)
                            rtf.Dispose()
                            rtf = Nothing
                        Else
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "El documento es word")
