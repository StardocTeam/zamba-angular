            ...
            If Not IsNothing(value) Then
                ...
                Else
                    For Each r As TaskResult In results
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "Utilizar documentos asociados...")
                        If myRule.UseAsocDoc = False Then
                            ...
                            If TypeOf value Is String Then
                                value = TextoInteligente.ReconocerCodigoAsObject(myRule.valueList, r)
                            End If
                        Else
                            ZTrace.WriteLineIf(ZTrace.IsInfo, " Verdadero.")
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "Obteniendo documentos asociados...")
                            Dim Asociated As ArrayList = DocAsociatedBusiness.getAsociatedResultsFromResult(r)
                            ...
                            If Asociated.Count > 0 Then
                                If myRule.idDocTypeAsoc = 0 Then
                                    If TypeOf value Is String Then
                                        value = TextoInteligente.ReconocerCodigoAsObject(myRule.valueList, Asociated(0))
                                    End If
                                Else
                                    For Each asoc As Result In Asociated
                                        If asoc.DocTypeId = myRule.idDocTypeAsoc Then
                                            value = TextoInteligente.ReconocerCodigoAsObject(myRule.valueList, asoc)
                                            Exit For
                                        End If
                                    Next
                                End If
