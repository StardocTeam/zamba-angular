...
Public Class PlayDOSHOWTABLE
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal myRule As IDOShowTable) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Try
            ...
            If myRule.VarSource.ToLower().Contains("zvar(") Then
                myRule.VarSource = myRule.VarSource.Trim().Remove(0, 5).Replace(")", String.Empty)
            End If
            ...
            Else
                ...
                grid.BackColor = Drawing.Color.LightSteelBlue
                grid.Dock = DockStyle.Fill
                grid.ForeColor = Drawing.Color.FromArgb(76, 76, 76)
                grid.TabIndex = 0
                grid.WithExcel = True
                grid.UseColor = False
                grid.AlwaysFit = True
                ...
                'Botón Aceptar.
                RemoveHandler btnOk.Click, AddressOf CloseForm
                AddHandler btnOk.Click, AddressOf CloseForm
                ...
                RemoveHandler frm.Shown, AddressOf FixFormSize
                ...
                RemoveHandler btnCancel.Click, AddressOf CancelChildRules
                ...
                frm.Width = 900
                frm.Height = Screen.PrimaryScreen().WorkingArea.Height - 50
                ...
                If results.Count > 0 Then
                    _data = GetDataTable(data, myRule, results(0))
                Else
                    _data = GetDataTable(data, myRule, Nothing)
                End If
                _data.MinimumCapacity = _data.Rows.Count
                ...
                If frm.DialogResult = DialogResult.OK AndAlso (Not dataIsEmpty) AndAlso (Not justShow) Then
                        ...
                        If (dt.Rows.Count = 1) Then
                            For Each col As DataColumn In dt.Columns
                                ...
                                If VariablesInterReglas.ContainsKey(ColVariable) = False Then
                                    VariablesInterReglas.Add(ColVariable, dt.Rows(0)(col.ColumnName).ToString(), False)
                                Else
                    ...
                    Else
                        If myRule.GetSelectedCols.Split(",").Length = 1 AndAlso dt.Rows.Count = 1 Then
                            ...
                            ZTrace.WriteLineIf(ZTrace.IsInfo, "Se ha seleccionado un registro. El dato obtenido será escalar. Guardando el valor a la variable inter regla.")
        ...
        If grid IsNot Nothing Then
            grid.NewGrid.BestFitColumns(BestFitColumnMode.HeaderCells)
        End If
    ...
    Private Sub CancelChildRules(ByVal sender As Object, ByVal e As EventArgs)
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Cerrando el formulario y cancelando ejecución de reglas hijas.")
        frm.DialogResult = DialogResult.Cancel
    End Sub
    ...
    Private Sub FixFormSize(ByVal sender As Object, ByVal e As EventArgs)
        ...
        'grid.FixColumns()
        grid.NewGrid.AutoScroll = True
        grid.NewGrid.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.None
        grid.NewGrid.BestFitColumns(BestFitColumnMode.AllCells)
        ...
        If frmHeight > 0 AndAlso frmHeight < pcHeight Then
        ...
        If frmWidth < frm.Width Then
            grid.NewGrid.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.Fill
        End If
        grid.BringToFront()
    ...
    Private Function GetDataTable(ByRef data As Object, ByVal myRule As IDOShowTable, ByVal result As IResult) As DataTable
        ...
        ElseIf (TypeOf data Is DataSet) Then
            If IsNothing(data) OrElse Not DirectCast(data, DataSet).Tables.Count > 0 OrElse DirectCast(data, DataSet).Tables(0).Rows.Count = 0 Then
                Return Dt
            Else
        ...
        If showCheckColumn Then
            ...
            If String.IsNullOrEmpty(myRule.CheckedItems) = False And String.IsNullOrEmpty(myRule.CheckedItemsColumn) = False Then
                ...
                If Not IsNothing(result) Then
                    strColNumber = TextoInteligente.ReconocerCodigo(myRule.CheckedItemsColumn, result)
                End If
                ...
                If Int32.TryParse(strColNumber, intColNumber) Then
                    ...
                    If Not IsNothing(result) Then
                        strColNumber = TextoInteligente.ReconocerCodigo(strColNumber, result)
                    End If
    ...
    Private Function GetRowValues(ByRef row As GridViewRowInfo, ByVal columnsStructure As DataColumnCollection) As Object()
        ...
        For i As Int32 = 0 To row.Cells.Count - 1
            ...
            If IsDBNull(row.Cells(i).Value) Then
                itemArray(i) = Nothing
            Else
    ...
    Public Sub ShowTaskOfDT() Implements IGrid.ShowTaskOfDT
        If FiltersChanged Then
            Dim CurrentUserID As Long = Membership.MembershipHelper.CurrentUser.ID
            Dim FilterString As String = FiltersComponent.GetFiltersString(Fc.GetLastUsedFilters(0, CurrentUserID, FilterTypes.Document), False, False, Nothing)
            Dt.CaseSensitive = False
            Dim DV As New DataView(Dt)
            DV.RowFilter = FilterString
            Dim data As DataTable = DV.ToTable
            data.MinimumCapacity = data.Rows.Count
