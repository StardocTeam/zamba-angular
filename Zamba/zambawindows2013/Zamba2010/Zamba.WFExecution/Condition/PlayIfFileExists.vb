...
Public Class PlayIfFileExists
    ...
    Private Function SearchFile() As Boolean
        Try
            If Not fileDirectory Is Nothing AndAlso fileDirectory.Trim.Length > 0 Then
                Dim MainDirectory As New DirectoryInfo(fileDirectory)
        ...
        Catch ex As Exception
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Nombre de archivo incorrecto. " & ex.Message)
        End Try
    End Function
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal ifType As Boolean) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        If Not IsNothing(results(0)) AndAlso fileName.Contains("<<") AndAlso fileName.Contains(">>") Then
            fileName = TextoInteligente.ReconocerCodigo(fileName, results(0))
        End If

        fileDirectory = myRule.SearchPath
        If fileDirectory.Contains("zvar") Then
            fileDirectory = WFRuleParent.ReconocerVariables(fileDirectory)
        End If
        If Not IsNothing(results(0)) AndAlso fileDirectory.Contains("<<") AndAlso fileDirectory.Contains(">>") Then
            fileDirectory = TextoInteligente.ReconocerCodigo(fileDirectory, results(0))
        End If

        'Verificacion de archivo
        If SearchFile() = ifType Then
            Return results
        Else
