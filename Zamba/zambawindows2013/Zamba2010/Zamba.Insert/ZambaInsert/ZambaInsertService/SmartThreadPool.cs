...
using Zamba.Core;
using Zamba.ThreadPool.Internal;
...
namespace Zamba.ThreadPool
{
	...
	public class SmartThreadPool : IWorkItemsGroup, IDisposable
	{
		...
		/// </summary>
		private string _name = "Zamba.ThreadPool";
		...
		private void IncrementWorkItemsCount()
		{
			...
			//Trace.WriteLineIf(ZTrace.IsInfo,"WorkItemsCount = " + _currentWorkItemsCount.ToString());
			if (count == 1) 
			{
				//Trace.WriteLineIf(ZTrace.IsInfo,"STP is NOT idle");
		...
		private void DecrementWorkItemsCount()
		{
			...
			if (count == 0) 
			{
				//Trace.WriteLineIf(ZTrace.IsInfo,"STP is idle");
