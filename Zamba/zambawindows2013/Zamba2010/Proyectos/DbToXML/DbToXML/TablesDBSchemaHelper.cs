...
namespace DbToXML
{
    class TablesDBSchemaHelper
    {
        ...
        #region Constructor
        public TablesDBSchemaHelper() { }
        public TablesDBSchemaHelper(SqlConnection connection) : this(connection, null) { }
        public TablesDBSchemaHelper(SqlConnection connection, SqlTransaction transaction)
        {
        ...
        {
            string sql = GetCreateSQL(schema, primaryKeys);
        ...
        {
            string sql = GetCreateFromDataTableSQL(table);
        ...
        public static string GetCreateSQL(DataTable schema, int[] primaryKeys)
        {
        ...
        public static string GetCreateFromDataTableSQL(DataTable table)
        {
        ...
        public static string SQLGetType(object type, int columnSize, int numericPrecision, int numericScale)
        {
            switch (type.ToString())
            {
                ...
                case "System.Decimal":
                    if (numericScale > 0)
                        return "Decimal (" + ((numericPrecision == -1) ? "18" : numericPrecision.ToString()) + "," + numericScale + ")";
                    else if (numericPrecision > 10)
                        return "BIGINT  (" + ((numericPrecision == -1) ? "18" : numericPrecision.ToString()) + "," + numericScale + ")";
                    else
                        return "Numeric  (" + ((numericPrecision == -1) ? "18" : numericPrecision.ToString()) + ")";
        ...
        {
            return SQLGetType(column.DataType, column.MaxLength, 18, 2);
        }
        #endregion

        internal Boolean TableExist(DataTable CurrentTable)
        {
            string sql = GetExistTableSQL(CurrentTable.TableName);
            ...
            Object otablecount = cmd.ExecuteScalar();
            Int32 tablecount;
            if (otablecount != null && Int32.TryParse(otablecount.ToString(), out tablecount) && tablecount > 0) return true; else return false;
        }

        private string GetExistTableSQL(string _tableName)
        {
            return "SELECT count(1) FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[" + _tableName + "]') AND type = 'U'";
        }
    }
}
