...
Imports MIS.EN.Exchange
Imports MIS.EN.Exchange.Entities
Imports System.Net

Public Class frm2003
    ...
    'abre los contactos pero no el address book
    Private Sub test()
        Dim mapiNamespace As Outlook.NameSpace = ol.GetNamespace("MAPI")

        Dim mapiFolder As Outlook.MAPIFolder = mapiNamespace.GetDefaultFolder(Outlook.OlDefaultFolders.olFolderContacts)

        mapiFolder.Display()
    End Sub

    Private Sub testExchange()

        Dim credentials As NetworkCredential
        Dim userMailBox As Uri
        Dim useFormsBasedAuthentication As Boolean
        Dim useIsaServer2006 As Boolean

        Dim username As String = "arey"
        Dim password As String = "Lok2j34$"
        Dim domain As String = ""
        Dim userMail As String = "http://192.168.1.70"

        credentials = New NetworkCredential(username, password, domain)
        userMailBox = New Uri(userMail)

        'If inside the firewall, always set these to false
        useFormsBasedAuthentication = False
        useIsaServer2006 = False

        Dim store As ExchangeStore = New ExchangeStore(credentials, userMailBox, useFormsBasedAuthentication, useIsaServer2006)

        Dim contacts As IList(Of Contact) = store.GetContacts()

        For Each contact As Contact In contacts
            Debug.Write("Name:  {0}", contact.Name)
        Next

        Debug.Write("Total Contacts:  " + contacts.Count)
    ...
    '            'Si no tiene direccion es una lista de distribucion
    ...
    ''Devuelve la direccion de email del contacto
    ...
    ''Convierte la direccion de email de exchagne a SMTP
    ...
    '    Dim oMsg As Outlook.MailItem = ol.CreateItem(Outlook.OlItemType.olMailItem)
    '    oMsg.Attachments.Add("c:\bootex.log")
    '    oMsg.Display(True)
