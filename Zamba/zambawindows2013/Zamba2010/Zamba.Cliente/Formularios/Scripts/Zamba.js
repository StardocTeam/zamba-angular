...
function LoadWinWeb() {
    ...
    $('body').on("shown.bs.collapse", "#toolbarTabTasksList", function () {
        setTabsHeight();
    });
    $('body').on("hidden.bs.collapse", "#toolbarTabTasksList", function () {
        setTabsHeight();
    });
    $('body').on('click', '#btnTabSearch, #anchorTabTasks, #liInsert', function (event) {
        setTabsHeight();
    });
    $('body').on('click', '#btnCloseSidePanel, #btnOpenSidePanel, #btnSidePanelTasksList, #btnCloseSidePanelTasksList', function (e) {
        toggleSidePanel(e);
    });
    $('body').on('click', '#toolbarTabTasksList, #ContentPlaceHolder_pnlHomeButtons, .dropdown-toggle', function (event) {
...
function SetIndexPnlVisibility(obj, divDoc, panel2Indices) {
    ...
    if ($(obj).hasClass("Expand")) {
        ...
        $("#separator").animate({
            width: $(divIndice).width()
        }, "slow");
        $(divDoc).animate({
            width: a + 16 + scrollDifference
        }, "slow");
...
var TaskOptions = {
    SetFav: function (btn) {
        ...
        $(btn).attr({
            "isset": !setFav,
            "title": setFav ? "Marcar como favorito" : "Desmarcar favorito"
        });
    ...
    SetImportant: function (btn) {
        ...
        $(btn).attr({
            "isset": !setImp,
            "title": setImp ? "Marcar como importante" : "Desmarcar importante"
        });
        $(btn).children("span").removeClass()
...
function AddDynamicsTags() {
    ...
    if ($('form').length == 1) {
        ...
        $('form').each(function (index, item) {
            MainForm = item;
            if ($(item).attr('id') == false || $(item).attr('id') == '' || $(item).attr('id') == 'frmMain') {
                $(item).attr('id', 'MainForm');
...
function CheckUserTimeOut() {
    ...
    if (CurrentUserId != null && ConnectionId != null) {
        $.ajax({
            ...
            success: function (data) {
                if (data.d == true) {
                    parent.$('#openModalTimeout').modal();
                    $('#openModalTimeout').draggable();
                    ConnectionId = 0;
                    CurrentUserId = 0;
                } else {
                    return false;
...
function PrintBarcode(path, CopiesCount, Width, Height) {
    ...
    $.ajax({
        type: "POST",
        url: ZambaWebRestApiURL + '/Tasks/GeneratePdfCoverPage?' + jQuery.param({
            TempPath: path,
...
function AssociatedIndexsModalselectBtn(sender) {
    ...
    for (i = 0; i < NewAssociatedDocIds.length; i++) {
        $.ajax({
            type: "POST",
            url: ZambaWebRestApiURL + '/Tasks/UpdateTaskIndex?' + jQuery.param({
                DoCTypeId: NewAssociatedEntityId,
                DocId: NewAssociatedDocIds[i],
                IndexId: NewAssociatedIndexIds,
                Data: IndexListSelectedValue
            }),
            contentType: "application/json; charset=utf-8",
            async: false,
            success: function (data) {
...
function FuncionDropzone(indexIds, entityId, newDocIds) {
    ...
    if ($('#listaID')[0].childElementCount == 0) {
        ...
        $.ajax({
            type: "POST",
            url: ZambaWebRestApiURL + '/Tasks/GetAsociatedIndexsDropzon?' + jQuery.param({
                indexId: indexIds,
                entityId: entityId
            }),
            contentType: "application/json; charset=utf-8",
