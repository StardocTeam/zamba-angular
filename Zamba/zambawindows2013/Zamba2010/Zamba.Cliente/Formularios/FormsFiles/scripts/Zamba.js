...
function cargaSync() {
    ...
    function cargaMeta() {
        for (i = 0; i <= referenceFilesMeta.length; i++) {
            $('head').append('<meta http-equiv=' + referenceFilesMeta[i]);
        }
    }
    ...
    function cargaCss() {
        for (i = 0; i < referenceFilesCss.length; i++) {
            $('head').append('<link rel= "stylesheet" href= "' + referenceFilesCss[i] + '" type= "text/css" />');
        }
    }
...
function AddFilter(indexid, codecolumn) {
    ...
    var $btnfilter = $('<img id="table_filter_' + btnid + '" onclick="CreateTable(this,' + codecolumn + ');" style="height: 16px; margin-left:10px;" alt="Filtrar" />');
