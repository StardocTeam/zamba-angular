...
#End Region
    ...
    Public Function ConvertDate(ByVal datetime As String, Optional ByVal ResolveDate As Boolean = True) As String Implements IConnection.ConvertDate
        If datetime.Trim = String.Empty Then Return "null"
        If ResolveDate Then
...
#Region "ExecuteNonQuery"
    Public Overloads Function ExecuteNonQuery(ByVal spName As String, ByVal ParametersNames() As Object, ByVal parameterstypes As Object, ByVal parametersValues() As Object) As Integer Implements IConnection.ExecuteNonQuery
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteNonQuery(spName, ParametersNames, parameterstypes, parametersValues)
            Else
                Throw ex
            End If
        Catch ex As Exception
    ...
    Public Overloads Function ExecuteNonQuery(ByVal spName As String, ByVal ParamArray parameterValues() As Object) As Integer Implements IConnection.ExecuteNonQuery
        ...
        Try
            ...
            If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
                commandParameters = SqlHelperParameterCache.GetSpParameterSet(CN, ConString, spName)
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteNonQuery(spName, parameterValues)
            Else
                Throw ex
            End If
        Catch ex As Exception
   ...
   ByVal ParamArray commandParameters() As IDbDataParameter) As Integer Implements IConnection.ExecuteNonQuery
    ...
    Public Overloads Function ExecuteNonQuery(ByVal transaction As IDbTransaction, ByVal spName As String, ByVal ParamArray parameterValues() As Object) As Integer Implements IConnection.ExecuteNonQuery
        ...
        Dim commandParameters As IDbDataParameter()
        If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
            commandParameters = SqlHelperParameterCache.GetSpParameterSet(CN, transaction, transaction.Connection.ConnectionString, spName)
            AssignParameterValues(commandParameters, parameterValues)
            State = IConnection.ConnectionStates.Ready
...
#Region "ExecuteDataset"
    ...
    ByVal commandText As String, ByVal ParamArray commandParameters() As IDbDataParameter) As DataSet Implements IConnection.ExecuteDataset
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteDataset(commandType, commandText, commandParameters)
            ElseIf ex.Number = 11 And RetryCount <= 3 Then
                RetryCount += 1
                ExecuteDataset(commandType, commandText, commandParameters)
            Else
                RetryCount = 0
                Throw ex
            End If
    ...
    ByVal ParamArray parameterValues() As Object) As DataSet Implements IConnection.ExecuteDataset
        ...
        Try
            ...
            If Not (parameterValues Is Nothing) AndAlso parameterValues.Length > 0 Then
                commandParameters = SqlHelperParameterCache.GetSpParameterSet(CN, CN.ConnectionString, spName)
                AssignParameterValues(commandParameters, parameterValues)
                State = IConnection.ConnectionStates.Ready
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                ...
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteDataset(spName, parameterValues)
            Else
                Throw ex
            End If
        Catch ex As Exception
    ...
    ByVal connectionOwnership As SqlConnectionOwnership) As IDataReader Implements IConnection.ExecuteReader
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                ...
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteReader(transaction, commandType, commandText, commandParameters, connectionOwnership)
            Else
                Throw ex
            End If
        Catch ex As Exception
...
#Region "ExecuteScalar"
   ...
   ByVal ParamArray commandParameters() As IDbDataParameter) As Object Implements IConnection.ExecuteScalar
        ...
        Try
            ...
            cmd.CommandTimeout = CN.ConnectionTimeout
            _command = cmd
            If timeout <> 0 Then _command.CommandTimeout = timeout
   ...
   ByVal ParamArray parameterValues() As Object) As Object Implements IConnection.ExecuteScalar
        ...
        Catch ex As System.Data.SqlClient.SqlException
            ...
            If ex.Number = 11 OrElse ex.Number = 3113 OrElse ex.Number = 12571 OrElse ex.Number = 53 OrElse ex.Number = 4060 OrElse ex.Number = 18456 Then
                ...
                CN = Server.ConnectionIsBroken(ex).CN
                Return ExecuteScalar(spName, parameterValues)
            Else
                Throw ex
            End If
        Catch ex As Exception
...
Public Class SQL7CreateTables
    ...
    Public Shadows Sub AddDocsTables(ByVal DocTypeId As Int64) Implements CreateTables.AddDocsTables
        ...
        Try
            ...
            End Try
            Try
                Dim Str As String = "CREATE TABLE [DOC_I" & strDocTypeId & "] ([lupdate] [datetime],[crdate] [datetime] ,[Fecha] smalldatetime default(GetDate()),[DOC_ID] [numeric]  primary key (doc_id)  foreign key(doc_id) references doc_t" & strDocTypeId & "(doc_id)ON DELETE CASCADE) ;"
                Server.Con.ExecuteNonQuery(CommandType.Text, Str)
            Catch ex As Exception
            End Try

            Dim Strt As String = "CREATE TRIGGER 	tu_i" & strDocTypeId & "	ON	doc_i" & strDocTypeId & "	FOR UPDATE AS BEGIN update	doc_i" & strDocTypeId & " set lupdate=getdate() where doc_id in (select doc_id from inserted) END"
            Server.Con.ExecuteNonQuery(CommandType.Text, Strt)

            Strt = "CREATE TRIGGER 	ti_" & strDocTypeId & " ON doc_i" & strDocTypeId & "	FOR insert AS BEGIN update doc_i" & strDocTypeId & " set lupdate=getdate(),crdate=getdate() where doc_id in (select doc_id from inserted) END"
            Server.Con.ExecuteNonQuery(CommandType.Text, Strt)

            'str = "CREATE TABLE [DOC_D" & strDocTypeId & "] ([INDEX_ID] [numeric] NOT NULL, [INDEX_NAME] [nvarchar] (30) PRIMARY KEY, [INDEX_CREATED] [bit] NOT NULL) "
            'Server.Con.ExecuteNonQuery(CommandType.Text, str)
            ...
            If sql_version.StartsWith("Microsoft SQL Server  2000") Then
                Strt = "CREATE TABLE [DOC_B" & DocTypeId & "] ([DOC_ID] [numeric] PRIMARY KEY, [DOCFILE] [Image] NOT NULL, [ZIPPED] [int] NOT NULL default (0)) "
            Else
                'sql 2005/2008
                Strt = "CREATE TABLE [DOC_B" & DocTypeId & "] ([DOC_ID] [numeric] PRIMARY KEY, [DOCFILE] [varbinary] (MAX) NOT NULL, [ZIPPED] [int] NOT NULL default (0)) "
            End If
            Server.Con.ExecuteNonQuery(CommandType.Text, Strt)

            Strt = "ALTER TABLE [DOC_B" & strDocTypeId & "] ADD CONSTRAINT FK_DOCT" & strDocTypeId & "_DOCB FOREIGN KEY (DOC_ID) REFERENCES [DOC_T" & strDocTypeId & "](DOC_ID)ON DELETE CASCADE"
            Server.Con.ExecuteNonQuery(CommandType.Text, Strt)
    ...
    Public Shadows Sub CreateTextIndex(ByVal DocTypeId As Int64, ByVal IndexId As Int64) Implements CreateTables.CreateTextIndex
        ...
        Dim str As String = "CREATE TABLE [DOC_X" & DocTypeId & "] ([DOC_ID] [int], [ID] [int]) ;"
        Server.Con.ExecuteNonQuery(CommandType.Text, str)
        str = "CREATE INDEX [IX_DOC_X" & DocTypeId & "] ON [DOC_X" & DocTypeId & "]([ID]) "
        Server.Con.ExecuteNonQuery(CommandType.Text, str)
