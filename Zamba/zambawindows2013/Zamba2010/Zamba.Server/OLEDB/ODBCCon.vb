...
Public Class odbc7CreateTables
    ...
    Public Shadows Sub AddDocsTables(ByVal DocTypeId As Int64) Implements CreateTables.AddDocsTables
        Try
            ...
            str = "CREATE TABLE [DOC_I" & DocTypeId & "] ([lupdate] [datetime],[crdate] [datetime] ,[Fecha] smalldatetime default(GetDate()),[DOC_ID] [numeric] foreign key(doc_id) primary key (doc_id) references doc_t" & DocTypeId & "(doc_id)ON DELETE CASCADE) ;"
            Server.Con.ExecuteNonQuery(CommandType.Text, str)
...
#End Region
    ...
    Public Sub AddObsTable(DocTypeId As Long, IndexIdArray As ArrayList, IndexTypeArray As ArrayList, IndexLenArray As ArrayList) Implements CreateTables.AddObsTable
        Try
            Dim str As String = "CREATE TABLE ZOBS_" & DocTypeId & "_" & IndexTypeArray(0) & "(ID NUMBER NOT NULL,DOC_ID NUMBER,USER_ID varchar2(200),DateObs DATE,VALUE varchar2(200),PRIMARY KEY (ID))"
            Server.Con.ExecuteNonQuery(CommandType.Text, str)
        Catch ex As Exception
            Zamba.AppBlock.ZException.Log(ex)
            Throw
