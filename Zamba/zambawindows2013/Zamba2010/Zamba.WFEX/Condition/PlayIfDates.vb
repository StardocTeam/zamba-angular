...
Public Class PlayIfDates
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal ifType As Boolean) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Ingreso en el PLAY de IfDates")
        Select Case myRule.MiFecha
            Case Result.DocumentDates.Creacion
                For Each TR As TaskResult In results
                    If validarFecha(TR, myRule) = ifType Then
                        Trace.WriteLineIf(ZTrace.IsInfo, "Resultado: Verdadero")
                        SL.Add(TR)
                    Else
                        Trace.WriteLineIf(ZTrace.IsInfo, "Resultado: Falso")
                    End If
                Next
