    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        For Each CurrentResult As ITaskResult In results
            ...
            Try
                ...
                If UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, ObjectTypes.DocTypes, RightsType.Create, myRule.AsociatedDocType.ID) = False Then
                    Trace.WriteLineIf(ZTrace.IsWarning, "No se pudo crear un entidad asociado porque no posee los permisos requeridos.")
                Else
                    result.DocType = DirectCast(myRule.AsociatedDocType, Core.DocType)
                    result.DocTypeId = result.DocType.ID
                    Trace.WriteLineIf(ZTrace.IsInfo, "Creando la entidad " & result.DocType.Name)
                    ...
                    Trace.WriteLineIf(ZTrace.IsVerbose, "Obteniendo schema de Entidad: " & result.DocTypeId)
                    result.DocType.Indexs.AddRange(IndexsBusiness.GetIndexsSchemaAsListOf(result.DocTypeId, False))
                    ...
                    Trace.WriteLineIf(ZTrace.IsVerbose, "Iterando por Atributos, Cantidad: " & result.Indexs.Count)
                    ...
                    For I As Int32 = 0 To result.Indexs.Count - 1
                        ...
                        Else
                            Trace.WriteLineIf(ZTrace.IsVerbose, "no tiene atributos especificos se busca si la tarea padre tiene un atributo en común, Cantidad: " & CurrentResult.Indexs.Count)
                    ...
                    Trace.WriteLineIf(ZTrace.IsInfo, "El documento se configuró con éxito")
            ...
            Catch ex As Exception
                Trace.WriteLineIf(ZTrace.IsError, "Error al configurar el documento")
                Throw ex
            Finally
