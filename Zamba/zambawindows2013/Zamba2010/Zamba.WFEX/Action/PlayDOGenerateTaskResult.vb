...
Public Class PlayDOGenerateTaskResult
    ...
    Private Function createResult(ByVal tipo As Int16) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Select Case tipo
          ...
          Dim task As ITaskResult = WFTaskBusiness.GetTaskByDocId(newRes.ID)
                                If Not IsNothing(task) Then
                                    Trace.WriteLineIf(ZTrace.IsVerbose, "Se adquiere la tarea del WF: " & task.WorkId)
                                Else
                                    Trace.WriteLineIf(ZTrace.IsVerbose, "La tarea no se encuentra en ningun WF, se agrega una tarea vacia")
                                    task = New TaskResult()
                                    task.ID = newRes.ID
                    ...
                    Select Case Results_Business.InsertDocument(newRes, False, False, False, False, isvirtual)
                        Case InsertResult.Insertado
                            ...
                            If newRes.ID > 0 And Me.MyRule.addCurrentwf = False Then
                                ...
                                arrayNewRes.Add(newRes)
                             Else
                                Trace.WriteLineIf(ZTrace.IsVerbose, "No se adjunta al WF actual, debido a que esta activa la opcion de no adjuntar")
