...
namespace HtmlRichText
{
	...
	{ 
        ...
        /// <summary>
        /// Convierte la unidad puntos a la unidad html (1 al 7).
        /// </summary>
        /// <returns>Html size unit</returns>
        private int ConvertToHtmlSize(float points)
        {
            int htmlSize = Convert.ToInt32(points);
				...
				for (i = 0; i < k; i++)
				{
					...
					if (bHTML)
					{
						...
						if ((strFont != strfname) || (crFont != cf.crTextColor) || (yHeight != cf.yHeight))
						{
							...
							// html size px (Approximately)
                            int fsize = ConvertToHtmlSize(this.SelectionFont.Size);
							...
							mfr.nLen = 0;
							mfr.nType = uMyREType.U_MYRE_TYPE_TAG;
                            mfr.strValue = "<font face=\"" + strFont + "\" color=\"" + strcolor + "\" size=\"" + fsize.ToString() + "\">";
						...
						else
						{
							...
							if (nEnd > nStart)
							{
								if ((nEnd - nStart) > 0)
								{
									...
									else if ((strTag.Length > 5) && (strTag.Substring(0, 5) == "<div"))
									{
										...
										int nFace = strTag.IndexOf("font-family:");
										if (nFace > 0)
										{
										...
										int nSize = strTag.IndexOf("font-size:");
										if (nSize > 0)
										{
										...
										int nColor = strTag.IndexOf("font-color:");
										if (nColor > 0)
										{
