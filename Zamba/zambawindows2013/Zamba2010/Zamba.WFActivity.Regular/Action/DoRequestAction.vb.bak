...
<RuleMainCategory("Workflow"), RuleCategory("Reglas"), RuleSubCategory(""), RuleDescription("Solicitar Tarea"), RuleHelp("Permite solicitar una o más tareas a un usuario de forma remota"), RuleFeatures(False)> <Serializable()> _
Public Class DoRequestAction
    Inherits WFRuleParent
    Implements IDoRequestAction, IDisposable, IRuleValidate
...
#End Region
    ...
    Private Function SendMailToUserAssigned(ByRef results As List(Of ITaskResult), ByRef CurrentRequestAction As RequestAction) As List(Of ITaskResult)
        ...
        For Each CurrentTaskResult As ITaskResult In results
            ...
            If (UserBusiness.GetUserNamebyId(CurrentTaskResult.AsignedToId) <> String.Empty) Then
            ...
            Else
                ...
                CurrentRequestAction.UsersIds.AddRange(UserGroupBusiness.GetUsersIds(CurrentTaskResult.AsignedToId, True))
            End If

            CurrentRequestAction.Tasks.Add(New RequestActionTask(CurrentTaskResult.TaskId, CurrentTaskResult.StepId))
            Trace.WriteLineIf(ZTrace.IsInfo, "Desde SendMailToUserAssigned invoko a RequestActionBusiness.Notify(CurrentRequestAction, _message, _subject, _linkMail) 'RULE_NAME")
