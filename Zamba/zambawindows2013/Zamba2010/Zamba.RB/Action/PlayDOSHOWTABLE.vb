...
Public Class PlayDOSHOWTABLE
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal myRule As IDOShowTable) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Try
            ...
            Else
                ...
                grid.ShowGroupBtn(False)
                grid.Name = "grid"
                grid.OutLookGrid.MultiSelect = myRule.SelectMultiRow
                ...
                If Not dataIsEmpty Then
                    ...
                    If showCheckColumn Then
                        grid.OutLookGrid.Columns("zcheck").Visible = False
                        grid.OutLookGrid.Columns("zcheck").HeaderText = "     "
                    End If
                    ...
                    If Not String.IsNullOrEmpty(myRule.ShowColumns) AndAlso Not dataIsEmpty Then
                        ...
                        For Each col As Windows.Forms.DataGridViewColumn In grid.OutLookGrid.Columns
                            col.Visible = False
                        Next
                        ...
                        For Each col As String In cols
                            If IsNumeric(col) Then
                                grid.OutLookGrid.Columns(Integer.Parse(col) - 1).Visible = True
                            Else
                                grid.OutLookGrid.Columns(col).Visible = True
                            End If
                        Next
                        cols = Nothing
                    End If
                    ...
                    If showCheckColumn Then
                        grid.OutLookGrid.Columns("zcheck").Visible = True
                        RemoveHandler grid.OnDoubleClick, AddressOf CheckSelectedRows
                        AddHandler grid.OnRowClick, AddressOf ClearSelection
                    ElseIf Not IsNothing(grid.OutLookGrid.Rows) AndAlso grid.OutLookGrid.Rows.Count > 0 Then
                        grid.OutLookGrid.Rows(0).Selected = True
                    End If
    ...
    Private Sub ClearSelection(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            grid.OutLookGrid.ClearSelection()
    ...
    Private Sub FixFormSize(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If grid.OutLookGrid.Rows.Count > 0 Then
            Dim dgv As DataGridView = grid.OutLookGrid
    ...
    Private Sub CheckSelectedRows(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If showCheckColumn Then
            Dim grid As Zamba.Grid.OutlookGrid = DirectCast(sender, GroupGrid).OutLookGrid
    ...
    Private Function GetCleanDataTable(ByRef grid As GroupGrid) As DataTable
        ...
        If showCheckColumn Then
            For Each row As DataGridViewRow In grid.OutLookGrid.Rows
        ...
        If IsNothing(dtClean.Rows) OrElse dtClean.Rows.Count = 0 Then
            For Each row As DataGridViewRow In grid.OutLookGrid.SelectedRows
                Dim drClean As DataRow = dtClean.NewRow
                drClean.ItemArray = GetRowValues(row, dtClean.Columns)
                dtClean.Rows.Add(drClean)
            Next
    ...
    Private Function HasRowsSelected() As Boolean
        Dim grid As DataGridView = DirectCast(frm.Controls("grid"), GroupGrid).OutLookGrid
