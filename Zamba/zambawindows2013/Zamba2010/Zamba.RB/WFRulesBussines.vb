...
#Region "Get"
    ...
    Public Shared Function GetIcon(ruleEnable As Boolean, ruleClass As String, DisableChildRules As Boolean, RefreshTask As Boolean, IconId As Int32) As Icons
        ...
        ElseIf IconId > 0 AndAlso IconId <> 31 Then
            Return DirectCast(IconId, Icons)
        Else
            Return GetRuleIconBasedOnClass(ruleClass)
        End If
    End Function

    Public Shared Function GetRuleIconBasedOnClass(RuleClass As String)
        If RuleClass.ToLower.Contains("excel") Then
            Return Icons.Excel2
        ElseIf RuleClass.ToLower.Contains("word") Then
            Return Icons.Word
        ElseIf RuleClass.ToLower.Contains("explorer") Then
            Return Icons.IE2
        ElseIf RuleClass.ToLower.Contains("mail") Then
            Return Icons.Mail
        ElseIf RuleClass.ToLower.Contains("outlook") Then
            Return Icons.Outlook2007
        ElseIf RuleClass.ToLower.Contains("pdf") Then
            Return Icons.Pdf
        ElseIf RuleClass.ToLower.Contains("asign") Then
            Return Icons.User
        ElseIf RuleClass.ToLower.Contains("user") Then
            Return Icons.User
        ElseIf RuleClass.ToLower.Contains("group") Then
            Return Icons.User
        Else
    ...
    Public Shared Function GetRuleStepId(ByVal ruleId As Int64) As Int64
        Return WFRulesFactory.GetRuleStepId(ruleId)
    End Function
    ...
    Public Function ExecutePrimaryRule(ByRef rule As WFRuleParent, ByVal results As List(Of ITaskResult), ByVal refreshTasks As List(Of Int64)) As List(Of ITaskResult)
        Try
            ...
            'UserBusiness.Rights.SaveAction(rule.ID, ObjectTypes.ModuleWorkFlow, RightsType.ExecuteRule, "usuario Ejecuto la regla " & rule.Name)

            For j As Int16 = 0 To refreshTasks.Count - 1
                Trace.WriteLineIf(ZTrace.IsInfo, "Actualizando Tarea: " & refreshTasks(j))
                Trace.WriteLineIf(ZTrace.IsInfo, "Obteniendo la tarea de la BD")
                ...
                If Not IsNothing(task) Then
                    'todo verificar si este codigo es correcto
                    'task.UserRules = refreshTasks(j).UserRules.Clone()
                    ZClass.HandleModule(ResultActions.RefreshIndexs, task)
                Else
                    Trace.WriteLineIf(ZTrace.IsInfo, "Tarea inexistente")
                End If
            Next
    ...
    Public Sub ExecuteZopenRules(ByVal TasksDT As DataTable, ByVal StepId As Int64)
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Obtengo regla del Evento Abrir Zamba")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Ejecuto tareas")

        For Each CurrentRow As DataRow In TasksDT.Rows
            Try
                Dim task As TaskResult = WFTaskBusiness.NewResult(CurrentRow, Wfstep)
    ...
    Private Shared Function GetInstanceRuleById(ByVal p_iRuleId As Int64, ByVal p_WfStepID As Int64, ByVal useCache As Boolean, ByVal ruleInstanceList As List(Of Int64)) As IWFRuleParent
        ...
        Try
                    ...
                    Else
                        If Cache.Workflows.hsSteps.ContainsKey(p_WfStepID) Then
                            ...
                            If ruleRow IsNot Nothing Then
                                rule = ReturnInstanceRule(DsRules.WFRules, DsRules.WFRuleParamItems, ruleRow, p_WfStepID, ruleRow.ParentType, ruleInstanceList, Nothing)
            ...
            Trace.WriteLineIf(ZTrace.IsVerbose, "Búsqueda de la regla " & p_iRuleId)
            Dim r As DsRules.WFRulesRow = i_dsRule.WFRules.Rows.Find(p_iRuleId)
            If r IsNot Nothing Then
                Trace.WriteLineIf(ZTrace.IsVerbose, "Se encontro la regla con id " & p_iRuleId & " se pide la instancia de la misma")
                rule = ReturnInstanceRule(i_dsRule.WFRules, i_dsRule.WFRuleParamItems, r, p_WfStepID, r.ParentType, ruleInstanceList, rule)
                If Not Cache.Workflows.HSRules.Contains(p_iRuleId) Then Cache.Workflows.HSRules.Add(rule)
            End If
        ...
        Catch ex As Exception
            Trace.WriteLineIf(ZTrace.IsInfo, "Error al instanciar la regla  " & ex.ToString)
            ZClass.raiseerror(ex)
    ...
    Public Shared Function GetInstanceRules(ByVal stepID As Int64, ByVal rulesType As Core.Enumerators.TypesofRules, ByVal isEvent As Boolean) As IWFRuleParent()
        ...
        If Not Cache.Workflows.HSRulesByStepAndType.ContainsKey(key) Then
            Cache.Workflows.HSRulesByStepAndType.Add(key, WFRulesFactory.GetRulesIdByStep(stepID, CInt(rulesType), isEvent))
        End If
        ...
        If ruleIds IsNot Nothing AndAlso ruleIds.Rows.Count > 0 Then
            ...
            For i As Int32 = 0 To ruleIds.Rows.Count - 1
                Dim ruleInstancesList As New List(Of Int64)
                rules(i) = GetInstanceRuleById(CLng(ruleIds.Rows(i)(0)), stepID, True)
        ...
        SyncLock Cache.Workflows.hsStepsRulesDS
            If useCache Then
                If Cache.Workflows.hsStepsRulesDS.Contains(p_WFstepID) = False Then
                    Trace.WriteLineIf(ZTrace.IsInfo, "Cargo la definicion de las reglas de la etapa: " & p_WFstepID)
    ...
    Public Shared Function GetIsRuleEnabled(ByVal UserRules As Hashtable, ByVal Rule As IWFRuleParent) As Boolean
        ...
        If Rule.Enable Then
            ...
            Else
                'Obtiene el estado
                Return WFRulesBusiness.GetRuleEstado(Rule.ID, True)
            End If
    ...
    ''' <returns>Boolean</returns>
    ''' <remarks></remarks>
    Public Shared Function GetRuleEstado(ByVal p_iRuleId As Int64, ByVal withcache As Boolean) As Boolean
        If withcache Then
            If Not Cache.Workflows.hsRulesEnable.ContainsKey(p_iRuleId) Then
                Cache.Workflows.hsRulesEnable.Add(p_iRuleId, WFRulesFactory.GetRuleStateById(p_iRuleId))
            End If
            Return Cache.Workflows.hsRulesEnable(p_iRuleId)
        Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Sub InsertUsersToNotifyAboutRuleExecution(ByVal ruleid As Int64, ByVal RuleSection As RuleSectionOptions, ByVal _RulePreference As RulePreferences, ByVal DestType As Int32, ByVal items As Generic.List(Of String))
...
#Region "Add"
    Public Shared Function Add(ByVal ruleName As String, ByVal baseNode As BaseWFNode, ByVal RuleNameFromUser As String, ByVal typeofRule As TypesofRules) As Int64
        If GetRuleParentType(baseNode) = TypesofRules.Salida AndAlso ruleName.IndexOf("distribuir") <> -1 Then
        ...
        Else
            ...
            UserBusiness.Rights.SaveAction(rule.ID, ObjectTypes.WFRules, Zamba.Core.RightsType.Create, "Creación de regla: '" & rule.Name & "' en Etapa '" & rule.WFStepId & "'")
    ...
    ''' <summary>
    ''' Devuelve 1 listado de reglas segun sus ids
    ''' </summary>
    ''' <param name="ruleAndStepId"></param>
    ''' <param name="ruleDirectoryPath"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    'Public Shared Function GetRulesByID(ByVal ruleAndStepId As Dictionary(Of Int64, Int64), ByVal ruleDirectoryPath As String) As List(Of IWFRuleParent)
    '    If IsNothing(ruleAndStepId) OrElse String.IsNullOrEmpty(ruleDirectoryPath) Then
    '        Return Nothing
    '    End If

    '    Dim Rules As New List(Of IWFRuleParent)(ruleAndStepId.Count)

    '    Dim RuleName As String = Nothing
    '    Dim CurrentRule As IWFRuleParent = Nothing

    '    Dim StepId, RuleId As Int64

    '    For Each CurrentItem As KeyValuePair(Of Int64, Int64) In ruleAndStepId
    '        RuleId = CurrentItem.Key
    '        StepId = CurrentItem.Value

    '        RuleName = GetRuleClass(CurrentItem.Key)

    '        If Not String.IsNullOrEmpty(RuleName) Then
    '            'InstanceRule(CurrentRule, nothing ,GetRuleClass(ruleId),ruleId,RuleName,getruletype(

    '            CurrentRule = GetRule(RuleName, StepId, ruleDirectoryPath)
    '            CurrentRule.ID = CurrentItem.Key
    '            CurrentRule.Name = GetRuleName(CurrentItem.Key)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    'Public Shared Function GetRulesByID(ByVal rulesId As List(Of Int64), ByVal ruleDirectoryPath As String) As List(Of IWFRuleParent)
    '    If IsNothing(rulesId) OrElse String.IsNullOrEmpty(ruleDirectoryPath) Then
    '        Return Nothing
    '    End If

    '    Dim Rules As New List(Of IWFRuleParent)(rulesId.Count)

    '    Dim RuleName As String = Nothing
    '    Dim CurrentRule As IWFRuleParent = Nothing

    '    Dim stepId As Int64
    '    For Each ruleid As Int64 In rulesId
    '        If stepId = 0 Then
    '            stepId = WFRulesBusiness.GetWFStepIdbyRuleID(ruleid)
    '        End If
    '        RuleName = GetRuleClass(ruleid)
    ...
    '            CurrentRule.Name = GetRuleName(ruleid)
    '            Rules.Add(CurrentRule)
    '        End If
    '    Next
    ...
    'Private Shared Function GetRule(ByVal name As String, ByVal stepid As Int64, ByVal ruleDirectoryPath As String) As IWFRuleParent

    '    Dim RuleAssembly As Assembly = Assembly.LoadFile(ruleDirectoryPath & "\Zamba.WFActivity.Regular.dll")
    '    Dim RuleType As Type = RuleAssembly.GetType("Zamba.WFActivity.Regular." & name, True, True)
    '    Dim RuleConstructor As ConstructorInfo = RuleType.GetConstructors.GetValue(0)

    '    Dim ConstructorParamenters(RuleConstructor.GetParameters.Length - 1) As Object

    '    ConstructorParamenters(0) = CoreData.GetNewID(IdTypes.WFRule) 'Primeros dos parametros Standarts del Ctor
    '    ConstructorParamenters(1) = "Regla" 'todo: ver de preguntar por el nombre o tomar el nombre basico de maskname
    '    ConstructorParamenters(2) = stepid
    '    FillArgs(ConstructorParamenters, RuleConstructor, TypesofRules.Regla)

    '    Dim Rule As WFRuleParent = Activator.CreateInstance(RuleType, ConstructorParamenters)
    '    Rule.ID = CoreData.GetNewID(IdTypes.WFRule)

    '    RuleAssembly = Nothing
    '    RuleType = Nothing
    '    RuleConstructor = Nothing
    ...
    Private Shared Function GetNewRule(ByVal BaseNode As BaseWFNode, ByVal RuleNameFromUser As String, ByVal RuleName As String, ByVal TypeOfRule As TypesofRules) As WFRuleParent
        Dim tt As Assembly = GetWFActivityRegularAssembly()
        ...
        Try
            ...
            If NewNode IsNot Nothing Then
                If NewRule.RuleClass.ToUpper.StartsWith("IF") AndAlso NewRule.RuleClass.ToLower() <> "ifbranch" AndAlso (Not IsNothing(NewNode) OrElse myBaseNode.Text.ToLower.CompareTo("eventos de zamba") = 0) Then
                    Dim rule As WFRuleParent = DirectCast(GetIfBranch(True, NewRule.WFStepId, NewRule), IWFRuleParent)
                    AddRule(NewNode, rule)
                    WFRulesBusiness.UpdateParamItem(rule, 0, "True")
                    Dim rule2 As WFRuleParent = DirectCast(GetIfBranch(False, NewRule.WFStepId, NewRule), IWFRuleParent)
    ...
    Public Shared Function addRules(ByVal intTarea As Int32, ByVal strName As String, ByVal WFStep1 As WFStep, ByVal WFStep2 As WFStep) As Int32
        Try
            Dim NewRule As WFRuleParent
            NewRule = GetNewRules(WFStep1, "DoDistribuir", strName)
            NewRule.ParentType = intTarea
    ...
    ''' </history>
    <Obsolete("Usa solo el Id y recibe toda la entidad")> _
    ...
    Private Shared Sub FillArgsValues(ByVal p As ParameterInfo, ByRef o As Object, ByVal value As Object)
        ...
        Select Case Type.GetTypeCode(p.ParameterType)
            ...
            Case TypeCode.Boolean
                ...
                If Servers.Server.isOracle AndAlso value.GetType() Is GetType(String) Then
                    If (String.IsNullOrEmpty(value)) Then
                        value = "0"
                    ElseIf (String.IsNullOrEmpty(value.Trim())) Then
                        value = "0"
                    ElseIf ((value <> "0") And (value <> "1") And (value.ToUpper() <> "TRUE") And _
...
#Region "Remove"
    ...
    Shared NodesToRemove As New List(Of TreeNode)
        ...
        Try
            ...
            If TypeOf myBaseNode Is RuleNode Then
                ...
                If ask Then
                    ...
                    dr = MessageBox.Show("Presione SI para eliminar toda la cadena de reglas." & vbCrLf & _
                                                          "Presione NO para eliminar solo la regla seleccionada." & vbCrLf & _
                                                          "Presione CANCELAR para no eliminar reglas.", "Eliminar", _
                ...
                If Not ask OrElse dr <> DialogResult.Cancel Then
                    ...
                    If nodeToRemove.Nodes.Count > 0 Then
                        If askForAllRules = False OrElse dr = DialogResult.No Then
                            ...
                            If nodeToRemove.RuleClass.StartsWith("If") AndAlso Not nodeToRemove.RuleClass.StartsWith("IfB") Then
                                For Each n As TreeNode In nodeToRemove.Nodes
                                    NodesToRemove.Add(n)
                                Next
                            End If
                    ...
                    If NodesToRemove.Count > 0 Then
                        Dim N As TreeNode = NodesToRemove(0)
            ...
            ElseIf TypeOf myBaseNode Is RuleTypeNode Then
                ...
                End If
                myBaseNode.TreeView.Nodes.Remove(myBaseNode)
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
    ...
    Shared Sub RemoveNodesFromTree(ByRef mybasenode As BaseWFNode, ByRef _RuleNode As RuleNode, Optional ByVal deleteChilds As Boolean = True)
        Try
            ...
            If (mybasenode.Parent.Nodes.Count = 1 AndAlso _RuleNode.ParentType = TypesofRules.Eventos) OrElse _
                (_RuleNode.ParentType = TypesofRules.AccionUsuario And deleteChilds) Then
                mybasenode.Parent.Remove()
            Else
                mybasenode.TreeView.Nodes.Remove(_RuleNode)
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
    ...
    ''' </history>
    Shared Sub CleanUnusedEventNodes(ByRef NodoEventos As TreeNode)
        Try
            For Each nodo As TreeNode In NodoEventos.Nodes
                If nodo.Nodes.Count = 0 Then
                    NodoEventos.Nodes.Remove(nodo)
                End If
            Next
        Catch ex As Exception
            ZClass.raiseerror(ex)
    ...
    Private Shared Sub RemoveRule(ByRef ruleNode As RuleNode)
        ...
        UserBusiness.Rights.SaveAction(ruleNode.RuleId, ObjectTypes.WFRules, Zamba.Core.RightsType.Edit, "Eliminación de regla: '" & ruleNode.RuleName & " (" & ruleNode.RuleId & ")")
...
#Region "COPY, CUT AND PASTE"
        ...
        Try
                ...
                If generalRulesNode IsNot Nothing Then
                    ...
                    ElseIf cuttedNode.Parent IsNot Nothing Then
                        If cuttedNode.Parent.Parent IsNot Nothing AndAlso cuttedNode.Parent.Parent.Text = "Eventos de Zamba" Then
                            Dim eventNode As TreeNode = cuttedNode.Parent.Parent
                            cuttedNode.Parent.TreeView.Nodes.Remove(cuttedNode)
            ...
            Else
                ...
                While Not (IsNothing(generalRulesNode.Parent))
                    For Each TreeNode As TreeNode In generalRulesNode.Nodes
                        If TreeNode.Text.StartsWith("Reglas Generales") Then
                            generalRulesNode = TreeNode
                            Exit While
                        End If
                    Next
            ...
            If TypeOf myBaseNode Is RuleTypeNode Then
                If Not isCopyNode Then
                    ...
                    If GetRuleParentType(myBaseNode) = TypesofRules.AccionUsuario AndAlso myBaseNode.Nodes.Count > 0 Then
                        ...
                        For Each TreeNode As TreeNode In DirectCast(myBaseNode.Parent, EditStepNode).Nodes
                            If (String.Compare(TreeNode.Text, "Acción de Usuario") = 0) Then
                                ' Se agrega la regla que se acaba de cortar a la acción de usuario
                                myRuleNode.Parent.Nodes.Remove(myRuleNode)
                                'RemoveNodesFromTree(myBaseNode, myRuleNode)
        ...
        Try
            ...
            For i As Int32 = eventNodes.Nodes.Count - 1 To eventNodes.Nodes.Count - 1 Step -1
                ...
                If TypeOf (eventNodes.Nodes(i)) Is RuleNode Then
                    ...
                    For Each nodo As TreeNode In eventNodes.Nodes
                    ...
                    If contieneEvento Then
                        ...
                        Dim Node As TreeNode = eventNodes.Nodes(eventoIndex)
                    ...
                    Else
                        ...
                        Dim Node As New TreeNode(rule.RuleType.ToString.Trim)
                        Node.NodeFont = New Font(FontFamily.GenericSansSerif, 7, FontStyle.Regular)
                        Node.ImageIndex = 28
    ...
    Private Shared Sub DuplicateRule(ByVal ruleNode As RuleNode, ByVal ruleFather As Object)
        ...
        If (rule.ParentType = TypesofRules.Eventos AndAlso _
            TypeOf (ruleFather) Is RuleTypeNode AndAlso _
             DirectCast(ruleFather, RuleTypeNode).RuleParentType <> TypesofRules.Eventos) OrElse _
        ...
        If ruleNode.ParentType = TypesofRules.Eventos OrElse _
            (TypeOf (ruleFather) Is RuleTypeNode AndAlso _
    ...
    Private Shared Function insertDuplicateRule(ByRef duplicateRuleId As Long, ByRef ruleType As TypesofRules, ByRef originalRule As WFRuleParent, ByVal ruleFather As BaseWFNode) As DsRules.WFRuleParamItemsRow()
        ...
        Try
            ...
            If (TypeOf ruleFather Is RuleNode) Then
                ...
                Try
                    WFRulesFactory.InsertRule(duplicateRuleId, DirectCast(ruleFather, RuleNode).RuleId, DirectCast(ruleFather, RuleNode).WFStepId, _
                                              originalRule.Name, ruleType, DirectCast(ruleFather, RuleNode).RuleType, originalRule.RuleClass, originalRule.Enable, _
                                              originalRule.Version)
                Catch ex As Exception
                    WFRulesFactory.InsertRule(duplicateRuleId, DirectCast(ruleFather, RuleNode).RuleId, DirectCast(ruleFather.Parent, EditStepNode).WFStep.ID, _
                    originalRule.Name, ruleType, DirectCast(ruleFather, RuleNode).RuleType, originalRule.RuleClass, originalRule.Enable, _
                    originalRule.Version)
                End Try

            Else
                WFRulesFactory.InsertRule(duplicateRuleId, 0, DirectCast(ruleFather.Parent, EditStepNode).WFStep.ID, _
                                          originalRule.Name, ruleType, DirectCast(ruleFather, RuleTypeNode).RuleParentType, originalRule.RuleClass, originalRule.Enable, _
                                          originalRule.Version)
            End If
...
#Region "Load"
    Public Shared Sub LoadRules(ByVal wf As WorkFlow, ByVal treeView As TreeView, ByVal LoadTreePanel As Boolean)
        Try
    ...
    Private Shared Function AddRuleNode(parentNode As BaseWFNode, _
                                        DSRules As DsRules, _
                                        loadPanel As Boolean, _
                                        rule As WFRuleParent, _
                                        parentId As Int64) As RuleNode
        Return AddRuleNode(parentNode, _
                           DSRules, _
                           loadPanel, _
                           rule.ID, _
                           rule.Name, _
    ...
    Private Shared Function AddRuleNode(parentNode As BaseWFNode, _
                                        ...
                                        ruleClass As String, _
                                        ruleEnable As Boolean, _
                                        ruleType As TypesofRules, _
                                        ruleParentId As Int64, _
                                        ruleParentType As TypesofRules, _
                ...
                If DTRuleOptions.Rows.Count > 0 Then
                    For Each o As DataRow In DTRuleOptions.Rows
                        Select Case o("ObjectId")
                            Case 0
                                ruleEnable = DirectCast(o("ObjValue"), Decimal)
                            Case 59
                    ...
                    Next
                End If
                icon = GetIcon(ruleEnable, ruleClass, DisableChildRules, RefreshTask, IconId)
            End If
    ...
    Private Shared Sub AddEventRuleNode(parentNode As BaseWFNode, _
                                        dsRules As DsRules, _
                                        rule As WFRuleParent, _
                                        ruleParentId As Int64)
        AddEventRuleNode(parentNode, _
                         dsRules, _
                         rule.ID, _
                         rule.Name, _
                         rule.RuleClass, _
    ...
    Private Shared Sub AddEventRuleNode(parentNode As BaseWFNode, _
        ...
        Try
            ...
            'En caso de existir se agrega a ese nodo, en caso de no 
            'existir, lo crea y luego se agrega al nodo.
            Dim eventNode As TreeNode
            If contieneEvento Then
                eventNode = parentNode.Nodes(eventoIndex)
                eventNode.Nodes.Add(DirectCast(ruleNode, TreeNode))
            Else
                eventNode = New TreeNode(ruleType.ToString)
        ...
        End Try
    End Sub
    Private Shared Sub AddEventRuleNode(ByVal parentNode As BaseWFNode, _
        ...
        Try
            ruleTypeName = ruleNode.RuleType.ToString()
            ...
            If contieneEvento Then
                ...
                EventNode.Nodes.Add(DirectCast(ruleNode, TreeNode))
            Else
                Dim EventNode As New TreeNode(ruleTypeName)
                EventNode.NodeFont = New Font(FontFamily.GenericSansSerif, 7, FontStyle.Regular)
                EventNode.ImageIndex = 28
                EventNode.SelectedImageIndex = 28
                parentNode.Nodes.Add(EventNode)
    ...
    Public Shared Sub LoadMonitorRules(ByRef wfstep As WFStep, ByVal TreeView As TreeView)
        Try
            TreeView.Nodes.Clear()
            Dim wf As IWorkFlow
...
#Region "Application"
        ...
        Try
            ...
            If Not Rule Is Nothing Then
                ...
                If Rule.IsUI AndAlso Membership.MembershipHelper.ClientType = ClientType.Service Then
                    Trace.WriteLineIf(ZTrace.IsInfo, "Se deshabilita la regla " + Rule.ID.ToString + " por tener UI y estar ejecutandose desde un servicio")
                    Rule.Enable = False
                End If
                ...
                For Each subRule As DataRow In dtSubRules.Select("ParentId = " & Rule.ID)
                    Try
                        If ruleInstanceList.Contains(subRule("ID")) = False Then
                            ...
                            End If
                        Else
                            Trace.WriteLineIf(ZTrace.IsInfo, "La regla " & subRule("ID") & " ya se encuentra en el workflow. Verificar en WFRules para el parentID " & Rule.ID)
                        End If
                    Catch ex As Exception
                        ZClass.raiseerror(ex)
                ...
                If Rule.RuleClass = "DoExecuteRule" Then
                    ...
                    Try
                        If ruleInstanceList.Contains(RuleToExecuteId) = False Then
                            ...
                            End If
                        Else
                            Trace.WriteLineIf(ZTrace.IsInfo, "La regla " & RuleToExecuteId & " ya se encuentra en el workflow. Verificar en WFRules para el parentID " & Rule.ID)
                        End If
                    Catch ex As Exception
                        ZClass.raiseerror(ex)
                        Trace.WriteLineIf(ZTrace.IsInfo, ex.ToString())
                ...
                End If
            Else
                Trace.WriteLineIf(ZTrace.IsInfo, "La regla " & r.Item("Id").ToString & " no pudo ser instanciada")
            End If
    ...
    Shared RulesAssembly As Assembly
    Private Shared Function GetWFActivityRegularAssembly() As Assembly
    ...
    Private Shared Sub InstanceRule(ByRef rule As WFRuleParent, ByVal paramItems As DataRow(), ByVal ruleClass As String, ByVal ruleId As Int64, ByVal ruleName As String, ByVal ruleType As TypesofRules, ByVal stepId As Int64)
        ...
        Try
            tt = GetWFActivityRegularAssembly()
            t = tt.GetType("Zamba.WFActivity.Regular." & ruleClass, True, True)
            ...
            If Not c Is Nothing Then
                ...
                If Not t Is Nothing Then
                    'Trace.WriteLineIf(ZTrace.IsInfo,"Instanciando" & ruleClass)
                    ...
                    Next
                Else
                    Trace.WriteLineIf(ZTrace.IsInfo, "Tipo no reconocido: " & ruleClass)
                End If
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
...
#End Region
    ...
    Public Shared Function recoverItemSelectedThatCanBe_StateOrUserOrGroup(ByVal stepid As Int64, ByVal ruleId As Int64, ByVal forceHashReload As Boolean) As RulePreferences
        ...
        Try
            ...
            dv = New DataView(dt)
            dv.RowFilter = "ruleid = " & ruleId.ToString & " And SectionId = " & RuleSectionOptions.Habilitacion & " And (ObjectId = " & _
