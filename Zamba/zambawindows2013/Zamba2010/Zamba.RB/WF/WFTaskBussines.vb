...
#Region "Get"
        ...
        Public Shared Function NewResult(ByVal r As DataRow, ByVal s As WFStep, Optional ByVal inThread As Boolean = False) As TaskResult
            Try
                Dim Doctype As DocType = DocTypesBusiness.GetDocType(CInt(r.Item("DOC_TYPE_ID")), True)
                ...
                If Not IsNothing(Doctype) Then
                    ...
                    If IsDBNull(r.Item("Name")) = False Then
                        ResultName = r.Item("Name")
                    End If
                    ...
                    If Server.isOracle Then
                        Result = New TaskResult(s.ID, CLng(r.Item("Task_ID")), CLng(r.Item("Doc_ID")), Doctype, ResultName, CInt(r.Item("IconId")), CInt(r.Item("C_Exclusive")), DirectCast(Task_State_ID, Zamba.Core.TaskStates), Doctype.Indexs, s.InitialState)
                    Else
                        Result = New TaskResult(s.ID, CLng(r.Item("Task_ID")), CLng(r.Item("Doc_ID")), Doctype, ResultName, CInt(r.Item("IconId")), CInt(r.Item("Exclusive")), DirectCast(Task_State_ID, Zamba.Core.TaskStates), Doctype.Indexs, s.InitialState)
                    End If
                    ...
                    'If Result.AsignedToId <> 0 AndAlso Result.TaskState <> Zamba.Core.TaskStates.Asignada Then
                    '    Result.TaskState = Zamba.Core.TaskStates.Asignada
                    'End If
        ...
        Public Function GetTasksByStepIdDataTable(ByVal stepId As Int64, ByVal WithRights As Boolean, ByVal CurrentUserId As Int64, ByVal LastDocId As Int64, ByVal PageSize As Int32) As DataTable
            ...
            For Each docid As Int64 In DTList
                DtTasks.Merge(GetTasksByStepandDocTypeId(stepId, docid, WithRights, CurrentUserId, fc, LastDocId, PageSize, False, Nothing))
            Next
        ...
        ''' </history>
        Public Function GetTasksByStepandDocTypeId(ByVal stepId As Int64, _
                                                   ByVal docTypeId As Int64, _
                                                   ByVal WithGridRights As Boolean, _
                                                   ByVal CurrentUserID As Int64, _
                                                   ByRef FC As FiltersComponent, _
                                                   ByVal LastDocId As Int64, _
                                                   ByVal PageSize As Int32, _
                                                   ByVal GetCount As Boolean, _
                                                   ByVal ZambaColumns As Dictionary(Of String, String), _
            ...
            Try
                If FC IsNot Nothing Then
                    ...
                    If ZambaColumns IsNot Nothing Then
                        If ZambaColumns.ContainsKey("Tarea") = False Then
                            ZambaColumns.Add(strTable, "Tarea")
                        Else
                            ZambaColumns("Tarea") = strTable & ".NAME"
                        End If
                    End If

                    filterElements = FC.GetLastUsedFilters(docTypeId, CurrentUserID, FilterTypes.Task)
                    filters = FC.GetFiltersString(filterElements, False, ZambaColumns)
                End If
                ...
                wftsbs.getRestrictionString(sbColumnCondition, sbDateDeclaration, docTypeId, CurrentUserID, True)
                wftsbs = Nothing
                ...
                If WithGridRights Then
                    ...
                    If CBool(UserPreferences.getValue("ShowIndexsOnGrid", Sections.UserPreferences, "True")) Then
                        indexs.AddRange(DocTypesBusiness.GetDocType(docTypeId, True).Indexs.ToArray(GetType(IIndex)))
                        ...
                        Next
                    End If
                    Dim VerAsignadosAOtros As Boolean = RightsBusiness.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosAOtros, stepId)
                    Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosANadie, stepId)
                    Dim wftf As New WFTasksFactory
                    dt = wftf.GetTasksByStepandDocTypeId(stepId, docTypeId, indexsAux, WithGridRights, filters, sbColumnCondition.ToString(), sbDateDeclaration.ToString(), LastDocId, PageSize, checkInColumnIsShortDate, autoSustIndex, GetCount, ZambaColumns, VerAsignadosAOtros, VerAsignadosANadie, False, wfstateID, Membership.MembershipHelper.CurrentUser)
                    wftf = Nothing
                Else
                    indexs.AddRange(DocTypesBusiness.GetDocType(docTypeId, True).Indexs.ToArray(GetType(IIndex)))
                    Dim VerAsignadosAOtros As Boolean = RightsBusiness.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosAOtros, stepId)
                    Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosANadie, stepId)
                    Dim wftf As New WFTasksFactory
                    dt = wftf.GetTasksByStepandDocTypeId(stepId, docTypeId, indexs, WithGridRights, filters, sbColumnCondition.ToString(), sbDateDeclaration.ToString(), LastDocId, PageSize, checkInColumnIsShortDate, Nothing, GetCount, Nothing, VerAsignadosAOtros, VerAsignadosANadie, False, wfstateID, Membership.MembershipHelper.CurrentUser)
                    wftf = Nothing
                End If
                Return dt
        ...
        Public Function GetTasksByWFandDocTypeId(workflowid As Int64, ByVal stepId As Int64, ByVal DocTypeId As Int64, ByVal WithGridRights As Boolean, ByVal CurrentUserID As Int64, ByRef FC As FiltersComponent, ByVal LastDocId As Int64, ByVal PageSize As Int32, ByVal GetCount As Boolean, ByVal ZambaColumns As Dictionary(Of String, String), Optional ByVal wfstateID As Int64 = 0) As DataTable
            ...
            Try
                If (Not FC Is Nothing) Then
                    ...
                    FilterString = FC.GetFiltersString(Filters, False, ZambaColumns)
                End If
                ...
                Dim wftsbs As New WFTaskBusiness
                wftsbs.getRestrictionString(ColumCondstring, dateDeclarationString, DocTypeId, CurrentUserID, True)
                wftsbs = Nothing
                ...
                Else
                    ...
                    Dim VerAsignadosANadie As Boolean = RightsBusiness.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosANadie, stepId)
                    Dim wftf As New WFTasksFactory
                    dt = wftf.GetTasksByWFandDocTypeId(workflowid, DocTypeId, indexs, WithGridRights, FilterString, ColumCondstring.ToString(), dateDeclarationString.ToString(), LastDocId, PageSize, CheckInColumnIsShortDate, Nothing, GetCount, Nothing, VerAsignadosAOtros, VerAsignadosANadie, False, wfstateID, Membership.MembershipHelper.CurrentUser)
                    wftf = Nothing
                End If
                Return dt
            Finally
        ...
        Public Shared Function GetStepsTasksCountByStepsandDocTypeId(ByVal stepIds As List(Of Int64), ByVal DocTypeId As Int64) As DataTable
            ...
            Try
                ...
                Dim wftsbs As New WFTaskBusiness
                wftsbs.getRestrictionString(ColumCondstring, dateDeclarationString, DocTypeId, Zamba.Membership.MembershipHelper.CurrentUser.ID, True)
                wftsbs = Nothing
        ...
        ''' </history>
        Private Sub getRestrictionString(ByRef ColumCondstring As StringBuilder, ByRef dateDeclarationString As StringBuilder, ByVal DocTypeId As Int64, ByVal CurrentUserID As Int64, ByVal useCache As Boolean)
            Dim Valuestring As New System.Text.StringBuilder
            Dim indRestriction() As Index
            'Opcion para cargar las restricciones cada vez que se llama al metodo
            Dim dinamycRestriciton As String = ZOptBusiness.GetValue("UseDinamycRestriccions")

            SyncLock (Cache.DocTypesAndIndexs.hsRestrictionsStrings)
                'SI se usa cache y no se deben recargar las retricciones
                If useCache AndAlso (String.IsNullOrEmpty(dinamycRestriciton) OrElse Not Boolean.Parse(dinamycRestriciton)) Then
                    If Cache.DocTypesAndIndexs.hsRestrictionsStrings.Contains(CurrentUserID & "-" & DocTypeId) = False Then
                        Cache.DocTypesAndIndexs.hsRestrictionsStrings.Add(CurrentUserID & "-" & DocTypeId, RestrictionsMapper_Factory.getRestrictionIndexs(CurrentUserID, DocTypeId))
                    End If
            ...
            Dim i As Integer
            For i = 0 To indRestriction.Length - 1
        ...
        Public Function GetTaskByTaskIdAndDocTypeIdAndStepId(ByVal taskId As Int64, ByVal DocTypeId As Int64, ByVal WFStepId As Int64, ByVal PageSize As Int32) As ITaskResult Implements IWFTaskBusiness.GetTaskByTaskIdAndDocTypeIdAndStepId
            ...
            Try
                ...
                ColumCondstring = New System.Text.StringBuilder
                dateDeclarationString = New System.Text.StringBuilder
                getRestrictionString(ColumCondstring, dateDeclarationString, DocTypeId, Membership.MembershipHelper.CurrentUser.ID, True)
        ...
        '''        [Javier]   06/10/10  Modified    Se agregan sentencias para obtener los strings de restricciones sobre docs
        ''' </history>
        Public Shared Function GetTasksByTaskIdsAndDocTypeIdAndStepId(ByVal taskIds As List(Of Int64), _
                                                                      ByVal docTypeId As Int64, _
            ...
            If useRestrictions Then
                Dim wftsbs As New WFTaskBusiness
                wftsbs.getRestrictionString(sbColumnCondition, sbDateDeclaration, docTypeId, Membership.MembershipHelper.CurrentUser.ID, True)
                wftsbs = Nothing
            End If
...
#Region "AddtoWF"
            ...
            For Each Result As Result In Results
                ...
                If WFBusiness.ValidateDocIdInWF(Result.ID, WF.ID) = False Then
                    If IsNothing(WF.InitialStep) = False Then
                        Dim t As New TaskResult(WF.InitialStep.ID, CoreData.GetNewID(IdTypes.Tasks), Result.ID, Result.DocType, _
                                                Result.Name, Result.IconId, 0, Zamba.Core.TaskStates.Desasignada, _
                                                Result.Indexs, Result.DISK_VOL_PATH, Result.Parent.ID, Result.OffSet, Result.Doc_File, _
        ...
        Public Shared Function AddNewResultsToWorkFLow(ByVal TResults As ArrayList, ByVal WFId As Int64, Optional ByVal OpenTaskAfterInsert As Boolean = True) As System.Collections.Generic.List(Of Core.ITaskResult)
            Dim TasksResults As New System.Collections.Generic.List(Of Core.ITaskResult)
            Trace.WriteLineIf(ZTrace.IsInfo, "Obtengo el WF")
            Dim wf As WorkFlow = WFBusiness.GetWorkFlow(WFId)
            Dim currentStep As WFStep = Nothing

            Trace.WriteLineIf(ZTrace.IsInfo, "Recorro los newres")
            For Each newRes As NewResult In TResults
                Trace.WriteLineIf(ZTrace.IsInfo, "Obtengo la etapa inicial")
                currentStep = WFStepBusiness.GetStepById(wf.InitialStepIdTEMP)
                Trace.WriteLineIf(ZTrace.IsInfo, "Contruyo el ITaskResult")
                ...
                t.File = newRes.File
                TasksResults.Add(t)
                Trace.WriteLineIf(ZTrace.IsInfo, "Inserto la tarea en el WF")
            ...
            If Not IsNothing(wfRules) Then
                For Each r As IWFRuleParent In wfRules
                    Trace.WriteLineIf(ZTrace.IsInfo, "Ejecuto regla de validacion de entrada con id " & r.ID)
            ...
            If Not IsNothing(wfRules) Then
                For Each r As IWFRuleParent In wfRules
                    Trace.WriteLineIf(ZTrace.IsInfo, "Ejecuto regla de entrada con id " & r.ID)
                    Dim list As List(Of ITaskResult) = WFRB.ExecutePrimaryRule(r, TasksResults, Nothing)
                    list = Nothing
                Next
            End If

            Trace.WriteLineIf(ZTrace.IsInfo, "Retorno las tareas insertadas")
        ...
        Public Shared Sub OpenTask(ByVal result As Zamba.Core.ITaskResult, ByVal params As Hashtable)
            Try
                If params.Item("UseCurrentTask") Then
                    Trace.WriteLineIf(ZTrace.IsInfo, "Abriendo la tarea - TaskId = " & result.ID.ToString)
                ...
                Else
                    ...
                    If params.Item("TaskID") <> String.Empty Then
                        ...
                        Trace.WriteLineIf(ZTrace.IsInfo, "Abriendo la tarea - TaskId = " & taskId.ToString)
                        Dim ds As DataSet = WFTasksFactory.GetTaskByTaskId(taskId)
                        If Not IsNothing(ds) AndAlso ds.Tables(0).Rows.Count > 0 Then
                            result = GetTaskByTaskIdAndDocTypeId(taskId, ds.Tables(0).Rows(0)("doc_type_id"), 0)
                            RaiseEvent ShowTask(result, True)
                        Else
                            Trace.WriteLineIf(ZTrace.IsInfo, "NO se ha encontrado la tarea - TaskId = " & taskId.ToString)
                        End If
                    ElseIf params.Item("DocIDs") <> String.Empty Then
                        ...
                        Trace.WriteLineIf(ZTrace.IsInfo, "Abriendo la tarea - DocId = " & params.Item("DocIDs").ToString)
                        If Not IsNothing(ds) AndAlso ds.Tables(0).Rows.Count > 0 Then
                            Dim t As Int32
                        ...
                        Else
                            Trace.WriteLineIf(ZTrace.IsInfo, "NO se ha encontrado la tarea - DocId = " & params.Item("DocID").ToString)
                        End If
                    End If
                End If
            Catch ex As Exception
        ...
        ''' </history>
        Public Shared Sub AddResultsToWorkFLowSinceRuleADDTOWF(ByVal Results As System.Collections.Generic.List(Of Core.ITaskResult), ByVal wfId As Integer, ByVal p_WStep As WFStep, ByVal initialStateId As Int32, Optional ByVal RuleId As Int64 = -1, Optional ByVal OpenTaskAfterInsert As Boolean = True)
            Dim TasksResults As New System.Collections.Generic.List(Of Core.ITaskResult)
            Dim wf As WorkFlow = WFBusiness.GetWorkFlow(wfId)
            Dim RuleConfig As DataSet = WFRulesBusiness.GetRuleParamItems(RuleId, True)
...
#Region "Indexs & DocType"
        Friend Shared Sub FillIndexsAndDocType(ByRef Result As TaskResult)
            Try
                FillDocType(Result)
                Result.Indexs = FilterIndex(CInt(Result.DocType.ID))
        ...
        Friend Shared Function HaveRightToView(ByRef wfstep As WFStep, ByVal AsignedToId As Int64, ByVal CurrentUserId As Int64) As Boolean
            Try
                If AsignedToId = 0 Then
                    'Asignado a nadie
                    If Not UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosANadie, CInt(wfstep.ID)) Then
                        Return False
                    End If
                ElseIf AsignedToId = CurrentUserId Then
                    'Asignado a uno
                    If Not UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.Use, CInt(wfstep.ID)) Then
                        Return False
                    End If
                Else
                    'Asignado a otro
                    If Not UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, Zamba.Core.ObjectTypes.WFSteps, Zamba.Core.RightsType.VerAsignadosAOtros, CInt(wfstep.ID)) Then
                        Return False
                    End If
                End If
                Return True
...
#Region "Remove"
        ...
        Public Shared Sub Remove(ByRef result As ITaskResult, ByVal deleteDocument As Boolean, ByVal CurrentUserId As Int64, ByVal DeleteFile As Boolean)
            Try
                ...
                WFTaskBusiness.LogCheckOut(result.TaskId)
            ...
            End Try
        End Sub
        'Private Shared Sub RefreshRemove(ByVal KeysToRemove As ArrayList, ByVal s As WFStep)
                ...
                For Each Result As ITaskResult In Results
                    ...
                    If Resultsaux.Contains(Result) Then
                        ...
                        If Resultsaux.Contains(Result) Then
                            ...
                            '            Trace.WriteLineIf(ZTrace.IsInfo, "Ejecuto las reglas de ENTRADA")
                            '            Resultsaux = WFRB.ExecutePrimaryRule(r, Resultsaux, Nothing)
                            '        End If
                            '    Next
                            'End If
...
#Region "Assign"
        ...
        Public Shared Function GetTaskUserNamebyId(ByVal UserId As Int32) As String
            Return UserBusiness.GetUserNamebyId(UserId)
        End Function
        ...
        Public Shared Sub Asign(ByRef result As ITaskResult, ByVal asignedToId As Int64, ByVal asignedById As Int64, ByVal logAction As Boolean, ByVal RefreshUI As Boolean)
            Try
                ...
                If logAction Then
                    Dim asignedToName As String = UserGroups.GetUserorGroupNamebyId(result.AsignedToId)
                    If String.IsNullOrEmpty(asignedToName) Then
                        asignedToName = "Ninguno"
                    End If

                    WFTasksFactory.LogAsignedTask(result.TaskId, result.Name, result.DocType.ID, result.DocType.Name, result.StepId, result.State.Name, asignedToName, result.WorkId, Membership.MembershipHelper.CurrentUser.Name)
        ...
        Public Shared Sub Asign(ByVal taskID As Int64, ByVal asignedToUserId As Int64, ByVal asignedByUserId As Int64, ByVal asignedDate As Date)
            Try
                Dim asignedToUserName As String = UserBusiness.GetUserNamebyId(asignedToUserId)
                If String.IsNullOrEmpty(asignedToUserName) Then asignedToUserName = "Sin Asignar"
                WFTasksFactory.UpdateAssign(taskID, asignedToUserId, asignedByUserId, asignedDate, TaskStates.Asignada)
                WFTasksFactory.LogAsignedTask(taskID, asignedToUserName)
            Catch ex As Exception
                ZClass.raiseerror(ex)
            End Try
        End Sub
        ...
        Public Shared Sub Asign(ByVal taskID As Int64, ByVal taskName As String, ByVal docTypeId As Int64, ByVal docTypeName As String, ByVal stepId As Int32, ByVal stateId As Int32, ByVal userName As String, ByVal workflowId As Int32, ByVal asignedToUserId As Int64, ByVal asignedByUserId As Int64, ByVal asignedDate As Date)
            WFTasksFactory.UpdateAssign(taskID, asignedToUserId, asignedByUserId, asignedDate, TaskStates.Asignada)
            WFTasksFactory.LogAsignedTask(taskID, taskName, docTypeId, docTypeName, stepId, stateId, userName, workflowId, Membership.MembershipHelper.CurrentUser.Name)
        End Sub
...
#Region "UnAssign"
        ...
        Public Shared Sub UnAssign(ByRef Result As ITaskResult, ByVal asignedByUserId As Int64)
            Try
                ...
                WFTasksFactory.UpdateAssign(Result)
                WFTasksFactory.LogAsignedTask(Result)
        ...
        Public Shared Sub UnAssign(ByVal taskId As Int64, ByVal asignedToUserId As Int64, ByVal asignedToUserName As String, ByVal asignedByUserId As Int64, ByVal asignedDate As Date)
            Try
                WFTasksFactory.UpdateAssign(taskId, asignedToUserId, asignedByUserId, asignedDate, DirectCast(TaskStates.Desasignada, Int32))
                WFTasksFactory.LogAsignedTask(taskId, asignedToUserName)
            Catch ex As Exception
                ZClass.raiseerror(ex)
            End Try
...
#Region "Estado de Tarea"
        ...
        Public Shared Sub Derivar(ByVal Task As ITaskResult, ByVal stepId As Int64, ByVal asignedToId As Int64, ByVal asignedToName As String, ByVal asignedBy As Int64, ByVal asignedDate As Date, ByVal selecCarp As Boolean, ByVal CurrentUserId As Int64)
            ...
            Try
                ...
                'WFTaskBusiness.Distribute(Task, stepId, CurrentUserId)
                WFTaskBusiness.Asign(Task.TaskId, asignedToId, asignedBy, asignedDate)
                'WFTasksFactory.UpdateTaskState(Task.ID, DirectCast(TaskStates.Asignada, Int32))
            Catch ex As Exception
                ZClass.raiseerror(ex)
            End Try
        End Sub
...
#Region "ChangeExpireDate"
        ...
        Public Shared Sub ChangeExpireDate(ByRef Result As TaskResult, ByVal NuevaFecha As DateTime)
            Try
                ...
                WFTaskBusiness.LogChangeExpireDate(Result)
                RaiseEvent AsignedAndExpireDate(Result)
            Catch ex As Exception
                ZClass.raiseerror(ex)
            End Try
        ...
        'ML: Metodo no utilizado
        'Friend Shared Sub ChangeExpireDateWithoutLogAction(ByRef Result As TaskResult, ByVal NuevaFecha As DateTime)
        '    Try
        '        Result.ExpireDate = NuevaFecha
        ...
        'ML:Metodo no utilizado
        'Private Shared Sub RefreshChangeExpireDate(ByVal t As TaskResult, ByVal r As DsDocuments.DocumentsRow)
        '    Try
...
#Region "Logs"
        ...
        Public Shared Sub LogUserAction(ByVal taskID As Int64, ByVal taskName As String, ByVal docTypeId As Int64, ByVal docTypeName As String, ByVal stepId As Int32, ByVal state As String, ByVal workflowId As Int32, ByVal accionDeUsuario As String)
                WFTasksFactory.LogUserAction(taskID, taskName, docTypeId, docTypeName, stepId, state, workflowId, accionDeUsuario, Membership.MembershipHelper.CurrentUser.Name)
        End Sub
              ...
              Public Shared Sub LogOtherActions(ByVal taskId As Int64, ByVal taskName As String, ByVal docTypeId As Int64, ByVal docTypeName As String, ByVal stepId As Int32, ByVal state As String, ByVal workflowId As Int32, ByVal comment As String)
                WFTasksFactory.LogOtherActions(taskId, taskName, docTypeId, docTypeName, stepId, state, workflowId, comment, Membership.MembershipHelper.CurrentUser.Name)
        End Sub
        ...
        Public Shared Sub LogAction(ByVal taskID As Int64, ByVal action As String)
            WFTasksFactory.LogAction(taskID, action, Membership.MembershipHelper.CurrentUser.Name)
        End Sub
        ...
        Public Shared Sub LogCheckIn(ByVal TaskResult As TaskResult, Optional ByRef t As Transaction = Nothing)
            ...
            If TaskResult.StepId <> 0 Then
                wfstep = WFStepBusiness.GetStepById(TaskResult.StepId)
                WFTasksFactory.LogCheckIn(TaskResult.ID, TaskResult.Name, TaskResult.DocTypeId, TaskResult.DocType.Name, wfstep.ID, TaskResult.State.Name, TaskResult.WorkId, TaskResult.CheckIn, t, Membership.MembershipHelper.CurrentUser.Name)
        ...
        ''' <summary>
        ''' Loguea el egreso de la tarea
        ''' </summary>
        ''' <param name="TaskID"></param>
        ''' <remarks></remarks>
        Public Shared Sub LogCheckOut(ByVal TaskID As Int64)
            WFTasksFactory.LogCheckOut(TaskID)
        End Sub

        Public Shared Sub LogStart(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogStartTask(TaskResult)
        End Sub

        Public Shared Sub LogFinish(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogFinishTask(TaskResult, Membership.MembershipHelper.CurrentUser)
        End Sub

        Public Shared Sub LogFinish(ByVal taskId As Int64)
            WFTasksFactory.LogFinishTask(taskId)
        End Sub

        Public Shared Sub LogChangeStepState(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogChangeStepState(TaskResult)
        End Sub

        Public Shared Sub LogChangeStepState(ByVal TaskResult As TaskResult, ByRef transact As Transaction)
            WFTasksFactory.LogChangeStepState(TaskResult, transact)
        End Sub

        Public Shared Sub LogChangeStepState(ByVal taskId As Int64)
            WFTasksFactory.LogChangeStepState(taskId)
        End Sub

        Public Shared Sub LogReject(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogRejectTask(TaskResult, Membership.MembershipHelper.CurrentUser)
        End Sub

        Public Shared Sub LogReject(ByVal taskId As Int64)
            WFTasksFactory.LogRejectTask(taskId)
        End Sub

        Public Shared Sub LogView(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogViewTask(TaskResult, Membership.MembershipHelper.CurrentUser)
        End Sub

        Public Shared Sub LogChangeExpireDate(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogChangeExpireDate(TaskResult)
        End Sub

        Public Shared Sub LogChangeExpireDate(ByVal taskID As Int64, ByVal taskName As String, ByVal docTypeId As Int64, ByVal docTypeName As String, ByVal stepId As Int32, ByVal stateId As Int32, ByVal workflowId As Int32)
            WFTasksFactory.LogChangeExpireDate(taskID, taskName, docTypeId, docTypeName, stepId, stateId, workflowId, Membership.MembershipHelper.CurrentUser.Name)
        End Sub

        Public Shared Sub LogAsignedTask(ByVal TaskResult As TaskResult)
            WFTasksFactory.LogAsignedTask(TaskResult)
        End Sub
...
#End Region
        ...
        Public Shared Function GetTaskHistory(ByVal taskId As Int64) As DataSet
            Return WFTasksFactory.GetTaskHistoryByResultId(taskId)
        End Function
        ...
        Public Shared Function GetTaskAprobementsHistory(ByVal TaskId As Int64) As DataSet
            'todo RequestAction
            If Boolean.Parse(UserPreferences.getValue("UseZAPROB", Sections.WorkFlow, False)) = True Then
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function GetTaskDs(ByVal taskId As Int64) As DataSet
            Return WFTasksFactory.GetTaskByTaskId(taskId)
        End Function
...
#Region "ExecuteEventRules"
        Public Sub ExecuteEventRules(ByVal TaskResult As ITaskResult, ByVal withcache As Boolean, ByVal ruleType As TypesofRules)
            Trace.WriteLineIf(ZTrace.IsInfo, "Ejecutando reglas de atributos")
...
#End Region
        ...
        ''' </history>
        Private Shared Sub CreateTasksRestrictionsWhere(ByRef sbValue As StringBuilder, ByVal Indexs() As Index, ByRef i As Int64, ByRef FlagCase As Boolean, ByRef ColumCondstring As StringBuilder, ByRef First As Boolean, ByRef dateDeclarationString As StringBuilder)
        ...
        Public Function SetLastUpdate(ByVal Result As ITaskResult, ByVal comment As String, ByVal saveHistory As Boolean) As Int64 Implements IWFTaskBusiness.SetLastUpdate
            ...
            If saveHistory = True Then
                WFTasksFactory.LogAction(Result.TaskId, Result.Name, Result.DocTypeId, Result.DocType.Name, Result.StepId, Result.State.Name, Result.WorkId, comment, Membership.MembershipHelper.CurrentUser.Name)
            End If
            Return newsID
        End Function
        ...
        Public Function GetTaskCount(ByVal stepId As Int64, ByVal WithGridRights As Boolean, ByVal CurrentUserID As Int64) As Int32
            ...
            For Each docid As Int64 In DTList
                taskCount += Int32.Parse(GetTasksByStepandDocTypeId(stepId, docid, WithGridRights, CurrentUserID, Nothing, Int32.MaxValue, 0, True, Nothing).Rows(0)(0).ToString)
            Next
        ...
        Public Shared Function ValidateVar(ByVal r As ITaskResult, ByVal TxtVar As String, ByVal Operador As String, ByVal Value As String) As Boolean
            ...
            Trace.WriteLineIf(ZTrace.IsInfo, "Validando Variable")
            ...
            Trace.WriteLineIf(ZTrace.IsInfo, "Variable original: " & zvar)
            zvar = Zamba.Core.TextoInteligente.ReconocerCodigo(zvar, r)
            Trace.WriteLineIf(ZTrace.IsInfo, "Variable pasada por textointeligente: " & zvar)
            zvar = zvar.Trim()
            ...
            Trace.WriteLineIf(ZTrace.IsInfo, "Variable Obtenida: " & zValue)
            ...
            Trace.WriteLineIf(ZTrace.IsInfo, "Variable a comparar Obtenida: " & rulevalue)
        ...
        Public Function GetTaskIdsByStepId(stepId As Int64) As List(Of Int64)
            ...
            For i As Int32 = 0 To dt.Rows.Count - 1
                taskIds.Add(CLng(dt.Rows(i)(0)))
            Next

            Return taskIds
        End Function
     
        Public Shared Function LockTask(ByVal taskId As Int64) As Boolean
