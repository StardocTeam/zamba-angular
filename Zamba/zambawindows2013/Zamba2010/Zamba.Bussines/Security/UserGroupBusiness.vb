...
#Region "Funciones Publicas"
    ...
    Public Shared Function GetUserGroupsIds(ByVal usrid As Integer) As ArrayList
        If Not Cache.UsersAndGroups.hsUserGroupsIds.ContainsKey(usrid) Then
    ...
    ''' </History>
    Public Shared Function GetUserorGroupIdbyName(ByVal userGroupName As String) As Int64
...
#End Region
    ...
    Public Shared Function RemoveUser(ByVal ug As IUserGroup, ByVal u As IUser) As Boolean
        If ug.Users.Contains(u) Then
            ...
            UserBusiness.Rights.SaveAction(u.ID, ObjectTypes.UserGroups, RightsType.Edit, "Se ha quitado el usuario " & u.Name & "(" & u.ID & ") del grupo " & ug.Name & "(" & ug.ID & ")")
            ug.Users.Remove(u)
            Return True
        Else
            Return False
