        ...
        If PermitedConections(type) > 0 Then
            ...
            If (actCon >= PermitedConections(type)) Then
                ...
                If (actCon >= PermitedConections(type)) And (forceConnection = False) Then
                    ...
                    If (type = 0) And (UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, ObjectTypes.ModuleWorkFlow, Zamba.Core.RightsType.Use)) Then
                        Return -1
                    End If
            ...
            End If
            ElseIf (type = 0) And (UserBusiness.Rights.GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser, ObjectTypes.ModuleWorkFlow, Zamba.Core.RightsType.Use)) Then
                Return -1
            Else
                Return 0
        End If
...
#Region "Generación de la conexión"
    ...
    Private Shared Function returnNewConId(ByRef userId As Int32, ByRef winUser As String, ByRef winPC As String, ByRef timeout As Int16, ByVal type As Int32) As Int32
        Try
            ...
            makeNewConnection(userId, winUser, winPC, lastConID, type, timeout)
            Return (lastConID)
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
    ...
    ''' </history>
    Private Shared Sub makeNewConnection(ByVal userId As Integer, ByVal winUser As String, ByVal winPC As String, ByVal actCon As Integer, ByVal type As Int32, ByVal TimeOut As Int16)
        Try
            UcmFactory.MakeNewConnection(userId, winUser, winPC, actCon, type, TimeOut)
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
    End Sub
...
#Region "Otros"
    ...
    Public Shared Function UpdateOrInsertActionTime(ByVal TimeOut As Integer, ByVal serviceType As ServiceTypes) As Integer
        ...
        If Membership.MembershipHelper.CurrentUser.WFLic = True Then
            UpdateOrInsertActionTime(Membership.MembershipHelper.CurrentUser.ID, winusername, machinename, Membership.MembershipHelper.CurrentUser.ConnectionId, TimeOut, 1)
        Else
            UpdateOrInsertActionTime(Membership.MembershipHelper.CurrentUser.ID, winusername, machinename, Membership.MembershipHelper.CurrentUser.ConnectionId, TimeOut, 0)
        End If
    End Function
#End Region
