...
Public Class ZCore
    ...
    Private Shared DsCore As New DsCore
...
#Region "Volumes"
    ...
    Public Shared Function filterVolumes(ByVal VolumeId As Int32) As Volume
        ...
        Else
            ...
            Else
                Dim DvVolumes As New DataView(DsCore.Disk_Volume)
...
#Region "Atributos"
    ...
    Public Shared Function FilterIndex(ByVal SelectedDocTypes As ArrayList) As Zamba.Core.Index()
        ...
        Try
            LoadCore()
            DvIndexR.Table = DsCore.INDEX_R_DOC_TYPE
            ...
            DvIndex.Table = DsCore.DOC_INDEX
            Dim x As Int32
    ...
    Private Shared Function FillIndex() As Boolean
        If FlagIndexLoaded = False Then
            DsCore.Merge(Zamba.Core.IndexsBusiness.GetIndex().DOC_INDEX)
            ...
            If DsCore.DOC_INDEX.Count > 0 Then
...
#Region "DocTypes"
    Public Shared Function FilterDocTypes(ByVal SelectedIndexs() As Zamba.Core.Index) As Zamba.Core.DocType()
        ...
        Try
            DvIndexR.Table = DsCore.INDEX_R_DOC_TYPE
            DvDocType.Table = DsCore.DOC_TYPE
    ...
    Public Shared Sub FilterCurrentDocTypes(ByVal SelectedDocTypes As ArrayList)
        ...
        Try
            ...
            For i = 0 To DsCore.DOC_TYPE.Count - 1
                Dim z As Int32
                For z = 0 To DocTypeCount
                    If DsCore.DOC_TYPE(i).DOC_TYPE_ID = DvDocType(z).Item("DOC_TYPE_ID") Then
                        Permitted = True
                        Exit For
                    End If
                Next
                If Permitted = False Then DsCore.DOC_TYPE(i).Delete()
                Permitted = False
            Next
    ...
    Private Shared Sub FillDSDocTypes()
        ...
        ZCore.FilterCurrentDocTypes(UserBusiness.Rights.GetAditional(ObjectTypes.DocTypes, Zamba.Core.RightsType.View))
    End Sub
    Private Shared Sub FillDocTypesChilds()
        ...
        If Membership.MembershipHelper.CurrentUser IsNot Nothing Then
            ...
            dstemp.Tables(0).TableName = DsCore.INDEX_R_DOC_TYPE.TableName
            DsCore.Merge(dstemp)
        End If
    End Sub
    Private Shared Sub FillDocTypes()
        Dim ArrDocTypes As ArrayList
        ArrDocTypes = DocTypesFactory.GetDocTypesbyUserRightsOfView(Membership.MembershipHelper.CurrentUser.ID, Zamba.Core.RightsType.View)
...
#Region "System"
    ...
    Public Shared Sub StartTrace(ByVal moduleName As String)
        Try
            ...
            Dim level As Int32
            Try
                level = Int32.Parse(UserPreferences.getValueForMachine("TraceLevel", Zamba.Sections.UserPreferences, "4"))
            Catch
                level = 4
            End Try
