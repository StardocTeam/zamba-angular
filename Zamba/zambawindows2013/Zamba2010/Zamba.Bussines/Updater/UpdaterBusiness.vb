...
Public Class UpdaterBusiness
    ...
    Public Shared Sub FinalizarUpdate()
        Try
            If IO.File.Exists(Application.StartupPath & "\initialexecute.dat") Then
                Dim sr As New IO.StreamReader(Application.StartupPath & "\initialexecute.dat")
    ...
    Public Shared Function IsLastestVersion() As Boolean
        ...
        Try
            Dim CurrentUser As IUser = Zamba.Membership.MembershipHelper.CurrentUser
    ...
    Public Shared Function IsNewHost() As Boolean
        Dim CurrentUser As IUser = Zamba.Membership.MembershipHelper.CurrentUser
        If String.Compare(GetVersionByEstreg(CurrentUser), "Desconocida") = 0 Then
            Return True
        Else
            Return False
        End If
    ...
    Public Shared Sub SetEstreg()
        Dim id As String = GetMaxIdFromEstreg() + 1
        Dim version As String = GetVersion(Application.StartupPath & "\Zamba.Cliente.exe").Replace(".", String.Empty)

        ZTrace.WriteLineIf(ZTrace.IsInfo, "# Registrando el nuevo puesto de usuario:")
        ZTrace.WriteLineIf(ZTrace.IsInfo, "- ID: " & id.ToString)
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "- Versión de Zamba: " & version)

        UpdaterFactory.SetEstreg(id, Environment.MachineName, Environment.UserName, version)
        ZTrace.WriteLineIf(ZTrace.IsInfo, "# Puesto registrado de manera correcta.")
    End Sub
    ...
    ''' <remarks></remarks>
    Public Shared Function GetVersionByEstreg(ByVal WinUser As IUser) As String
        Return GetVersionByEstreg(Environment.MachineName, WinUser)
    End Function

    '''<summary>Obtiene la versión de la tabla ESTREG donde el nombre de máquina pasado
    '''por parámetro.</summary>
    '''<history>[Alejandro]</history>
    Public Shared Function GetVersionByEstreg(ByVal strMachineName As String, ByVal WinUser As IUser) As String
        ...
        Try
            ...
            If String.IsNullOrEmpty(sVersion) OrElse IsDBNull(sVersion) Then
            ...
            Else
                ...
                If WinUser IsNot Nothing Then UpdaterFactory.UpdateLastVerified(strMachineName, WinUser)
            End If
        Catch ex As Exception
            ZClass.raiseerror(ex)
        End Try
