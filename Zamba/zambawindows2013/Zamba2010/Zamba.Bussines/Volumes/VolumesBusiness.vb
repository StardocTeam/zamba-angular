...
#Region "GetVolumeData"
    ...
    & " Disk_Vol_Size_Len,Disk_Vol_State,Disk_Vol_LstOffset,Disk_Vol_Files,Disk_Vol_Id from Disk_Volume  WHERE Disk_Vol_id > 0 order by Disk_Vol_Name"
     ...
     & " Disk_Vol_Size_Len,Disk_Vol_State,Disk_Vol_LstOffset,Disk_Vol_Files,Disk_Vol_Id from Disk_Volume WHERE Disk_Vol_id < 0 and DISK_VOL_NAME = '" & VolName & "' order by Disk_Vol_Name"
        ...
        Dim DsVolume As New DsVolume
        Dstemp.Tables(0).TableName = DsVolume.Disk_Volume.TableName
        DsVolume.Merge(Dstemp)

        Dim Qrows As Int32 = DsVolume.Disk_Volume.Count
        Dim i As Int32
        For i = 0 To Qrows - 1
            Dim Vol As New Volume
            Vol.Name = DsVolume.Disk_Volume(i).DISK_VOL_NAME
            Vol.Size = DsVolume.Disk_Volume(i).Disk_Vol_Size
            Vol.Type = DsVolume.Disk_Volume(i).Disk_Vol_Type
            Vol.copy = DsVolume.Disk_Volume(i).Disk_Vol_Copy
            Vol.path = DsVolume.Disk_Volume(i).Disk_Vol_Path
            Vol.sizelen = DsVolume.Disk_Volume(i).Disk_Vol_Size_Len
            Vol.state = DsVolume.Disk_Volume(i).Disk_Vol_State
            Vol.offset = DsVolume.Disk_Volume(i).Disk_Vol_LstOffset
            Vol.Files = DsVolume.Disk_Volume(i).Disk_Vol_Files
            Vol.Id = DsVolume.Disk_Volume(i).Disk_Vol_Id
            Volumes.Add(Vol)
        Next
        Return Volumes
    End Function
    ...
    Public Shared Function GetVolume(ByVal volumeId As Int32) As IVolume
        Dim Strselect As String = "select Disk_Vol_Name,Disk_Vol_Size,Disk_Vol_Type,Disk_Vol_Copy,Disk_Vol_Path, Disk_Vol_Size_Len,Disk_Vol_State,Disk_Vol_LstOffset,Disk_Vol_Files,Disk_Vol_Id from Disk_Volume  WHERE Disk_Vol_id = " & volumeId.ToString() & " order by Disk_Vol_Name"
    ...
    Public Shared Function GetTemporalVolume(ByVal docTypeId As Int64) As IVolume
        ...
        Try
            If Server.isOracle Then
                ...
                Dr = con.ExecuteReader(CommandType.Text, Strselect)
            Else
                Dr = con.ExecuteReader("ZSP_VOLUME_100_GetTemporalVolume", New Object() {docTypeId})
            End If
        ...
        Return Vol
    End Function
    'Public Shared Function GetTemporalVolume(ByVal docTypeId As Int64, Optional ByRef t As Transaction = Nothing) As IVolume
    '    Dim Strselect As String = "select Disk_Vol_Name,Disk_Vol_Size,Disk_Vol_Type,Disk_Vol_Copy,Disk_Vol_Path, Disk_Vol_Size_Len,Disk_Vol_State,Disk_Vol_LstOffset,Disk_Vol_Files,Disk_Vol_Id from Disk_Volume WHERE Disk_Vol_ID < 0 and DISK_VOL_NAME = '" & System.Environment.MachineName & "' order by Disk_Vol_Name"
    ...
    '            Dr = con.ExecuteReader(CommandType.Text, Strselect)
    '        Else
    '            Dr = t.Con.ExecuteReader(t.Transaction, CommandType.Text, Strselect)
    '        End If
...
#Region "Document Insert"
    ...
    'Public Shared Function GetVolumeListId(ByVal docTypeId As Int64, Optional ByRef t As Transaction = Nothing) As Int32
    '    'TODO Falta que esta lista ya este cargada
    '    'recupero el id de la lista de volumen para el tipo de doc

    '    Return RetrieveDiskGroupId(docTypeId, t)
    'End Function
    Public Shared Function GetVolumeListId(ByVal docTypeId As Int64) As Int32
        Return RetrieveDiskGroupId(docTypeId)
    End Function
    'Public Shared Function RetrieveDiskGroupId(ByVal DocTypeId As Integer, Optional ByRef t As Transaction = Nothing) As Integer
    '    Try
    '        If Server.isOracle Then
    ...
    '            Else
    '                Return t.Con.ExecuteScalar(t.Transaction, CommandType.Text, Strselect)
    '            End If
    '        Else
    ...
    '            Else
    '                Return t.Con.ExecuteScalar(t.Transaction, "zsp_Volume_100_RetrieveDiskGroupId", parameters)
    '            End If
    '        End If
    ...
    'Public Shared Function GetVolumes(ByVal VolumeListId As Int32, Optional ByRef t As Transaction = Nothing) As DataSet
    '    'recupero un dataset con la lista de volumenes
    '    Return VolumeListsBusiness.GetActiveDiskGroupVolumes(VolumeListId, t)
    'End Function
    Public Shared Function GetVolumes(ByVal VolumeListId As Int32) As DataSet
        'recupero un dataset con la lista de volumenes
        Return VolumeListsBusiness.GetActiveDiskGroupVolumes(VolumeListId)
    End Function
    ' Private Shared Function VerificoCantidadVolumenesYCargoelprimero(ByVal DsVols As DataSet) As Volume
    '    'me fijo si hay volumenes disponibles
    '    Dim Volume As New Volume
    '    If DsVols.Tables(0).Rows.Count > 0 Then
    '        Volume.Id = DsVols.Tables(0).Rows(0).Item("DISK_VOLUME_ID")
    '        Return Volume
    '    Else
    '        Return Nothing
    '    End If
    'End Function
    'Public Shared Function GetVolumeData(ByRef volume As Volume, Optional ByRef t As Transaction = Nothing) As IVolume
    '    'obtengo la data para el volumen seleccionado
    '    volume = GetVolumeData(volume.ID, t)
    ...
    Public Shared Function GetOffSet(ByRef Volume As IVolume) As Int32
        ...
        Try
            DSize = CDec(Volume.sizelen) / 1024
            If (DSize / (Volume.Size / 20)) + 1 <= Volume.offset Then
...
#End Region
    ...
    Public Shared Sub Fill(ByRef instance As IVolume)
