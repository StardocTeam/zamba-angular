...
Namespace WF.ResultsCtls
    ...
    Public Class TabResults
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal _CurrentUserId As Int64)
        ...
        Private Sub LoadGridResults(ByVal CurrentUserID As Int64)
            ...
            If IsNothing(UCFusion2) OrElse UCFusion2.IsDisposed Then
                UCFusion2 = New UCFusion2(UCFusion2.Modes.Results, CurrentUserID, Nothing)
                RemoveHandler UCFusion2.currentContextMenu.ItemClicked, AddressOf currentContextMenuClick
                AddHandler UCFusion2.currentContextMenu.ItemClicked, AddressOf currentContextMenuClick
            ...
            Try
                ...
                If (asocResults IsNot Nothing) Then
                    If TabDocAsociated.Controls.Count = 0 Then
                        ...
                        Dim grdDocsAsoc As New UCFusion2(UCFusion2.Modes.AsociatedResults, CurrentUserId, Result)
                        RemoveHandler grdDocsAsoc.currentContextMenu.ItemClicked, AddressOf currentContextMenuClick
                        AddHandler grdDocsAsoc.currentContextMenu.ItemClicked, AddressOf currentContextMenuClick
                    ...
                    Case "IRAWORKFLOW"
                        Try
                            If (UCFusion2.SelectedResults(False, UCFusion2.GetSelectedRows()).Count > 0) Then
                                ...
                                Dim Task As ITaskResult = WFTaskBusiness.GetTaskByDocId(Result.ID)
...
#End Region
        ...
        Private Sub ShowTaskResult(ByVal docId As Int64, ByVal docTypeId As Int64)
            ...
            Dim Task As ITaskResult = WFTaskBusiness.GetTaskByDocId(docId)
            If Not IsNothing(Task) Then
                If UserBusiness.Rights.GetUserRights(Membership.MembershipHelper.CurrentUser, ObjectTypes.WFSteps, RightsType.Use, Task.StepId) = True Then
                    RaiseEvent ShowTask(Task.TaskId, Task.StepId, Task.DocTypeId)
                Else
