...
namespace Zamba.PrintTreeView
{
    public class PrintHelper : Zamba.Core.IPrintTreeViewHelper, IDisposable
    {
        ...
        public PrintHelper()
        {
            ...
            this._printDoc.Disposed += new EventHandler(_disposed);
        }
        ...
        public void PrintPreviewTree(RadTreeView tree, string title)
        {
            this._title = title;
            PrepareTreeImage((RadTreeView)tree);
            pp = new PrintPreviewDialog();
            pp.FormClosed += new FormClosedEventHandler(_formClosed);
        ...
        public void PrintTree(RadTreeView tree, string title)
        {
            this._title = title;
            PrepareTreeImage((RadTreeView)tree);
        ...
        /// <summary>
        /// Guarda el WF en un BMP
        /// </summary>
        /// <param name="tree"></param>
        /// <param name="FileName"></param>
        ///
        public void SaveTreeBitmap(RadTreeView tree, string FileName)
        {
        ...
        /// <param name="tree"></param>
        /// 
        private void PrepareTreeImage(RadTreeView tree)
        {
            ...
            int height = tree.Nodes[0].TreeViewElement.Bounds.Height;
            this._nodeHeight = height;
            int width = tree.Nodes[0].TreeViewElement.Bounds.Right;
            RadTreeNode node = tree.Nodes[0].NextVisibleNode;

            while (node != null)
            {
                height += node.TreeViewElement.Bounds.Height;
                if (node.TreeViewElement.Bounds.Right > width)
                {
                    width = node.TreeViewElement.Bounds.Right;
                }
            ...
            //BorderStyle tempBorder = tree.TreeViewElement.BorderStyle;
            //bool tempScrollable = tree.Scrollable;
            RadTreeNode selectedNode = tree.SelectedNode;
