...
function DecimalCheck(e){
	...
	//Si son caracteres invalidos se cancela el evento
	if(key!=8 && key!=0 && key!=46 && (key<48 || key>57)){
		...
		//Guarda el valor del control en variable
		var text = $('#' + e.target.id).val() + String.fromCharCode(key);

		//Si el campo no es numérico, por haber introducido 2 comas, quito una
		if(isNaN(text)){
			return false;
		} else {
			//Valido que tenga solo 2 decimales
			if(text.indexOf(".") != -1 && text.indexOf(".") < text.length - 3)
				return false;
		}
	}
}
...
function PorcentualCheck(e){
	...
	if(key!=8 && key!=0 && key!=46 && (key<48 || key>57)){
		...
		if(isNaN(text)){
			...
			//Valido que tenga solo 2 decimales
			if(text > 100 || (text.indexOf(".") != -1 && text.indexOf(".") < text.length - 3))
				return false;
		}
	}
}
