...
namespace Zamba.Grid
{
    ...
    /// </summary>
    public interface IOutlookGridGroup : IComparable, ICloneable, IDisposable
    {
        /// <summary>
    ...
    /// </summary>
    public class OutlookgGridDefaultGroup : IOutlookGridGroup, IDisposable  
    {
        ...
        public virtual string Text
        {
            get
            {
                if (text != null)
                {
                    if (text.StartsWith("Documento:") == true)
                    {
                        return string.Format("{0}({1})", text.Replace("Documento:",""), itemCount == 1 ? "1 item" : itemCount.ToString() + " items");
                    }
                }
                if (column == null)
