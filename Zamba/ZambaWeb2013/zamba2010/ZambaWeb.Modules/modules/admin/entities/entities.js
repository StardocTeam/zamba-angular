...
//datos
//Función que se debe sobreescribir para poder obtener los resultados por fuera
var searchResults;
var urlServer = ZambaWebRestApiURL + "/Search/";
var iMsgTxt = [];
            ...
            controller: [
                ...
                function ($scope, $http, $attrs, $element, $timeout, $filter) {
                    ...
                    $scope.doSearch = function () {
                        ...
                        if (!$p.length || $p == undefined) return;
                        window.wait();
                                ...
                                for (var i = 0; i <= $li.length - 1; i++) {
                                    ...
                                    if ($input.css("display") != 'none' && $input.val().length >= 1) {
                                        for (var j = 0; j <= item.indexes.length - 1; j++) {
                                            var liId = $input.parent().prop('tagName') == "LI" ? $input.parent().attr("id") :
                                                $input.parent().parent().attr("id");
                                            if (liId== item.indexes[j].id) {
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                element.bind('mouseleave', function (e) {
                    // e.stopPropagation();
                    if ($(this).children(".typeahead").val()=="")
            ...
            link: function (scope, element, attrs) {
                ...
                element.bind('keydown', function (e) {
                    ...
                    $.ajax({
                        ...
                        url: urlServer + 'SuggestionsByindex',
                        success: function (result) {
                            for (var i = 0; i <= result.length - 1; i++)
                                data.push(result[i].word);
                        },
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                element.bind('keypress', function (e) {
                    ...
                    if ($li.length == 0) $li = $tp.parent().children("ul").children("li");
                    if ($li.length == 0) $li = $tp.parent().parent().children("ul").children("li");
...
window.wait = function () {
    waitingDialog.show('Procesando su b&uacute;squeda');
}
