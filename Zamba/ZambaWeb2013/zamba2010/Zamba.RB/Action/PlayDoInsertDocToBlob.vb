...
Public Class PlayDoInsertDocToBlob
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo valores en la consulta")
        executeDoctype = WFRuleParent.ReconocerVariables(_myRule.DocTypeID)
        If Not IsNothing(results(0)) Then
            executeDoctype = Zamba.Core.TextoInteligente.ReconocerCodigo(executeDoctype, results(0))
        End If
        Trace.WriteLineIf(ZTrace.IsInfo, "DocTypeID: " & executeDoctype)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo valores en la consulta")
        executeDocId = WFRuleParent.ReconocerVariables(_myRule.DocID)
        If Not IsNothing(results(0)) Then
            executeDocId = Zamba.Core.TextoInteligente.ReconocerCodigo(executeDocId, results(0))
        End If
        Trace.WriteLineIf(ZTrace.IsInfo, "DocID: " & executeDocId)
        ...
        If IsNumeric(executeDocId) AndAlso IsNumeric(executeDoctype) Then
            ...
            If res IsNot Nothing Then
                ...
                Else
                    Trace.WriteLineIf(ZTrace.IsInfo, "El archivo o no se encontro o estaba vacio")
                End If
            Else
