...
Public Class PlayDoReadInboxMail
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Try
            Trace.WriteLineIf(ZTrace.IsInfo, "Entrando en ejecucion de DoReadInboxMail")

            Me._myRule = DiscoverRuleParams(Me._myRule, results(0))

            Trace.WriteLineIf(ZTrace.IsInfo, "Instanciando Pop3 Client")
            _client = New ZPop3Client(_myRule.Pop3Server, _myRule.Pop3Port, _myRule.Pop3User, _myRule.Pop3Password, _myRule.Pop3EnableSSL)
            Trace.WriteLineIf(ZTrace.IsInfo, "Pop3 Client instanciado, conectando")
            _client.Connect()
            Trace.WriteLineIf(ZTrace.IsInfo, "Conectado")
            Trace.WriteLineIf(ZTrace.IsInfo, "Obteniendo lista de emails")
            ...
            If obteinedMails IsNot Nothing AndAlso obteinedMails.Count > 0 Then
                Trace.WriteLineIf(ZTrace.IsInfo, "Cantidad de mails obtenidos: " & obteinedMails.Count)
                exportFolder = New DirectoryInfo(_myRule.PathToExport)

                If exportFolder.Exists Then
                    ExportMails(obteinedMails, exportFolder)
                Else
                    Trace.WriteLineIf(ZTrace.IsInfo, "No existe la ruta de exportacion de mails")
                End If

                VariablesInterReglas.Add(_myRule.Zvarname, obteinedMails, False)
            Else
                Trace.WriteLineIf(ZTrace.IsInfo, "No se encontraron mails para ese rango de fechas")
            End If

            Trace.WriteLineIf(ZTrace.IsInfo, "Terminando ejecucion de DoReadInboxMail")
    ...
    Private Function DiscoverRuleParams(ByVal ruleToDiscover As IDoReadInboxMail, ByVal res As ITaskResult) As IDoReadInboxMail
        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo variables de regla")

        Dim ruleToReturn As IDoReadInboxMail = ruleToDiscover

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo Pop3Server")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.Pop3Server)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo Pop3Port")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.Pop3Port)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo Pop3User")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.Pop3User)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo pathToExport")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.PathToExport)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo zvarname")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.Zvarname)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo startDate")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.StartDate)

        Trace.WriteLineIf(ZTrace.IsInfo, "Reconociendo endDate")
        ...
        Trace.WriteLineIf(ZTrace.IsInfo, "Valor: " & ruleToReturn.EndDate)
    ...
    Private Sub ExportMails(ByVal obteinedMails As List(Of IDownloadedEmailHeader), ByVal exportFolder As DirectoryInfo)
        Trace.WriteLineIf(ZTrace.IsInfo, "Exportando mails a HTML")
        ...
        Try
            ...
            If fiTemplate.Exists Then
                ...
                Next
            Else
                Trace.WriteLineIf(ZTrace.IsInfo, "No se encontro el template para exportar los mails")
            End If
        Finally
            fiTemplate = Nothing
            fiExport = Nothing
