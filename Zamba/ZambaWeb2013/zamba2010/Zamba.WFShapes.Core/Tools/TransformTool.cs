...
{
public class TransformTool : AbstractTool, IMouseListener
    {
        ...
        double ox = 0, oy = 0; //the origin vector where the scaling has its start
        //the scale with which the entities will be scaled
        double scale = 0, scalex = 0, scaley = 0;
        private TransformTypes transform;
        Point origin = Point.Empty; //dummy which is calculated in function of the current resizing location
        #endregion
        ...
        public void MouseDown(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && Enabled && !IsSuspended)
            {
                ...
                {
                    Point gripPoint = new Point(0, 0);
                    //let the tracker tell us if anything was hit
                    if (this.Controller.View.Tracker != null)
                    {
                        gripPoint = this.Controller.View.Tracker.Hit(e.Location);
                    }
                    Cursor c = null;
                    changing = false;
