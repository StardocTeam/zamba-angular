    ...
    Public Shared Function getAllValuesDB(ByVal UserId As Int64) As DataTable
        Try
            ...
            If Server.isOracle Then
                ...
                End If
            Else
                Dim ds As DataSet = Server.Con.ExecuteDataset(CommandType.Text, String.Format("SELECT userid userid, name name, section section, value value from ZUserConfig  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where userid = {0} union ALL  SELECT userid userid, name name, section section, value value from ZUserConfig d  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & "  where not exists( select 1 from ZUserConfig l  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where d.name = l.name and d.section = l.section and l.userid = {0} ) and (d.userid in ( select inheritedusergroup from group_r_group " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & "  where usergroup = {0})  or d.userid in (  Select groupid from usr_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usrid= {0})  or d.userid in ( select inheritedusergroup from group_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usergroup in ( select groupid from usr_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usrid = {0})) ) union ALL  SELECT userid userid, name name, section section, value value from ZUserConfig d  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & "  where not exists(  select 1 from ZUserConfig l " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & "  where d.name = l.name and d.section = l.section and (l.userid = {0}  or (l.userid in ( select inheritedusergroup from group_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usergroup = {0})  or l.userid in (  Select groupid from usr_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usrid= {0})  or l.userid in ( select inheritedusergroup from group_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usergroup in ( select groupid from usr_r_group  " & If(Server.isSQLServer, " WITH (NOLOCK) ", " ") & " where usrid = {0})) ) )) and d.userid = 0", UserId))
