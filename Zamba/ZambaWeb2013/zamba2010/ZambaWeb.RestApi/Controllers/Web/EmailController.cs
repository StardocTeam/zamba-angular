            ...
            try
            {
                ...
                if (emailData.isDomail == "true" && VariablesInterReglas.ContainsKey("rutaDocumento"))
                {
                    ...
                    for (int i = 0; i < emailData.Idinfo.Count; i++)
                    {
                        ...
                        SZOptBusiness ZO = new SZOptBusiness(); exportPath = Email_Factory.CreateExportFolder(Convert.ToInt32(emailData.Idinfo[i].DocId));
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Ruta exportacion: " + exportPath); string MailTo = emailData.MailTo.ToString();
                        string cc = emailData.CC.ToString();
                        long DocID = emailData.Idinfo[i].DocId; var from = ZO.GetValue("WebView_FromSMTP");
