...
namespace Zamba.SetupLibrary
{
    ...
    public class HelperSecurity
    {
        ...
        public void SetSecurityAddin(string ruta)
        {
            ...
            if (lista != null)
            {
                ...
                foreach (string f in lista)
                {
                    ...
                    registerAssembly(Path.GetFileName(f), gaspolPath, gacUtilPath);
                }
            }

            registerAssembly("ExportaOutlook.dll", gaspolPath, gacUtilPath);
            registerAssembly("SandBar.dll", gaspolPath, gacUtilPath);

            /*registerAssembly(DDL_NAME_EXPORTAOUTLOOK, gaspolPath, gacUtilPath);
            registerAssembly("Zamba.Core.Dll", gaspolPath, gacUtilPath);
            registerAssembly("Zamba.ICore.Dll", gaspolPath, gacUtilPath);
            registerAssembly("Zamba.Bussines.Dll", gaspolPath, gacUtilPath);
            registerAssembly("Zamba.Viewers.Dll", gaspolPath, gacUtilPath);
            registerAssembly("Zamba.WinUI.Dll", gaspolPath, gacUtilPath);*/

            HelperTrace.WriteFormatEndMethod("SetSecurityAddin");
        }

        private void registerAssembly(string assemblyName, string gaspolPath, string gacUtilPath)
        {
            List<SecurityAddin> lista = new List<SecurityAddin>();
            SecurityAddin sec = getSecurityAddin();
            sec.path = gaspolPath;
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_CASPOLADDIN_SETSECURITYADDIN, Path.Combine(sec.currentTargetPath, assemblyName));
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);

            HelperTrace.WriteAnswerFormat("add OK");
            sec = getSecurityAddin();
            sec.path = gacUtilPath;
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_GACUTILADDIN_SETSECURITYADDIN, Path.Combine(sec.currentTargetPath, assemblyName)); ;
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
            HelperProcess hp = new HelperProcess();
        ...
        private void UnregisterAssembly(string assemblyName, string gaspolPath, string gacUtilPath)
        {
            try
            {
            List<SecurityAddin> lista = new List<SecurityAddin>();
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_GACUTILADDIN_REMOVESECURITYADDIN, Path.Combine(sec.currentTargetPath, assemblyName)); ;
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
            HelperProcess hp = new HelperProcess();
        ...
        public void RemoveSecurityAddin(string ruta)
        {
            ...
            if (lista != null)
            {
                ...
                foreach (string f in lista)
                {
                    HelperTrace.WriteAnswerFormat("Registrando DLL " + f);
                    HelperTrace.WriteAnswerFormat("Registrando DLL Nombre: " + Path.GetFileName(f));
                    UnregisterAssembly(Path.GetFileName(f), gaspolPath, gacUtilPath);
                }
            }
            UnregisterAssembly("ExportaOutlook.dll", gaspolPath, gacUtilPath);
            UnregisterAssembly("SandBar.dll", gaspolPath, gacUtilPath);
