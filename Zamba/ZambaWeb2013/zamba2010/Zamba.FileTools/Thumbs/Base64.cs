...
namespace Zamba.Thumbs
{
    public class Base64
    {
        ...
        public static string PDF(string path, string thumbPath)
        {
            string base64 = string.Empty;
            try
            {
                var zfp = new Zamba.FileTools.PDFThumb();
                base64 = zfp.GenerateThumb(path, thumbPath);
            }
            catch (Exception ex)
            {
                ZTrace.WriteLineIf(ZTrace.IsError, "Error al generar Thumb " + ex.Message);
        ...
        public static string ConvertFromPath(string path, string thumbPath)
        {
