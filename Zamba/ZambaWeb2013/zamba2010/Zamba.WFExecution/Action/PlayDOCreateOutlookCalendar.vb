...
Public Class PlayDOCreateOutlookCalendar
    ...
    Public Function Play(ByVal results As List(Of ITaskResult)) As List(Of Core.ITaskResult)
        ...
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Play de la DOCreateOutlookCalendar")

        pathDirectory = Zamba.Membership.MembershipHelper.AppTempDir("\IndexerTemp").FullName
    ...
    '    ZTrace.WriteLineIf(ZTrace.IsInfo, "Ejecutando la regla para la tarea " & res.Name)
    ...
    '            toMails = VarInterReglas.ReconocerVariablesValuesSoloTexto(toMails)
    '        End If

    '        If Not String.IsNullOrEmpty(Me._myRule.Organizer) Then
    '            organizer = TextoInteligente.ReconocerCodigo(Me._myRule.Organizer, res)
    '            organizer = VarInterReglas.ReconocerVariablesValuesSoloTexto(organizer)
    '        End If
    ...
    '    IcsResult.FolderId = res.FolderId

    '    Results.Add(IcsResult)

    '    If UserBusiness.CurrentUser.eMail.Type = MailTypes.OutLookMail Then
    ...
    '        '    frmMessage = clsMessages.getMailForm()
