...
Public Class PlayDoGenerateOutlook
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        Try
            ...
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Play de la DOGenerateOutlook")
            ...
            For Each r As TaskResult In results
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Se van a reemplazar las variables")
                Me.Params.Clear()
                Me.Params = Me.ReemplazarVariables(r)
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Variables reemplazadas")
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Fin Evento")
                ...
                If (Me.resultAux.PrintName = "Cancel") Then
                    ...
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, "Cancelado")
                    Me.resultsAux.Add(r)
                Else
    ...
    Public Function ReemplazarVariables(ByVal res As TaskResult) As Hashtable
        ...
        Try
            Try
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo Asunto: " & Me.Asunto)
                Me.Asunto = Zamba.Core.TextoInteligente.ReconocerCodigo(Me.Asunto, res)
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Asunto1: " & Me.Asunto)
                ...
                End If
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Asunto2: " & Me.Asunto)
            Catch ex As Exception
            ...
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Para: " & Me.Para)
            Try
                Dim R As String
                Me.CC = Zamba.Core.TextoInteligente.ReconocerCodigo(Me._myRule.CC, res)
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "CC1: " & Me.CC)
                ...
                If IsNothing(ValorVariable) = False Then
                    ...
                    ElseIf IsNumeric(ValorVariable) Then
                        ...
                        Zamba.Core.UserBusiness.Mail.FillUserMailConfig(u.ID)
                        R = u.eMail.Mail
                        Me.CC = Me.CC.Replace("zvar(" & Variable & ")", R)
                    ...
                    End If
                End If
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo CC2: " & Me.CC)
            Catch ex As Exception
                ZTrace.WriteLineIf(ZTrace.IsVerbose, ex.ToString())
                ZClass.raiseerror(ex)
            End Try
            ...
            Try
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo CCO1: " & Me.CCO)
                ...
                If IsNothing(ValorVariable) = False Then
                    ...
                    End If
                End If
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo CCO2: " & Me.CCO)
            Catch ex As Exception
                ZTrace.WriteLineIf(ZTrace.IsVerbose, ex.ToString())
                ZClass.raiseerror(ex)
            End Try
            ...
            Try
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo Body1: " & Me.Body)
                Me.Body = VarInterReglas.ReconocerVariables(Me.Body)
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo Body2: " & Me.Body)
            Catch ex As Exception
                ZTrace.WriteLineIf(ZTrace.IsVerbose, ex.ToString())
                ZClass.raiseerror(ex)
            End Try
            ...
            If Me._myRule.ReplyMail AndAlso Not String.IsNullOrEmpty(Me._myRule.ReplyMsgPath) Then
                Try
                    ...
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo ReplyMsgPath 1: " & Me.replyMsgPath)
                    Me.replyMsgPath = VarInterReglas.ReconocerVariables(Me.replyMsgPath)
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, "Reconociendo ReplyMsgPath 2: " & Me.replyMsgPath)
        ...
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook To: " & Me.Para)
        Params.Add(OutlookMailParameters.TO, Me.Para)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook CC: " & Me.CC)
        Params.Add(OutlookMailParameters.CC, Me.CC)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook CCO: " & Me.CCO)
        Params.Add(OutlookMailParameters.BCC, Me.CCO)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook Subject: " & Me.Asunto)
        Params.Add(OutlookMailParameters.SUBJECT, Me.Asunto)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook Body: " & Me.Body)
        Params.Add(OutlookMailParameters.HTML_BODY, Me.Body)
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook Imagenes: " & Me._myRule.PathImages)
        ...
        ZTrace.WriteLineIf(ZTrace.IsVerbose, "DOGenerateOutlook ReplyMsgPath: " & Me.replyMsgPath)
...
#Region "Mensajes"
    ...
    Private Function CompleteHtmlFile(ByRef r As Result) As Boolean
        ...
        If Not Me.GetHtml(r) Then
            Return False
        End If
        r.HtmlFile = Zamba.Membership.MembershipHelper.AppTempDir("\temp").FullName & "\" & r.Name & "-temp" & (New Random).Next(1, 9999).ToString & ".html"
        ...
        If File.Exists(FB.GetShowAndEditForms(Int32.Parse(r.DocType.ID.ToString))(0).Path.Replace(".html", ".mht")) Then
            Try
                Using write As New StreamWriter(r.HtmlFile.Substring(0, r.HtmlFile.Length - 4) & "mht")
                    write.AutoFlush = True
                    Dim reader As New StreamReader(FB.GetShowAndEditForms(Int32.Parse(r.DocType.ID.ToString))(0).Path.Replace(".html", ".mht"))
    ...
    Public Function GetHtml(ByRef r As Result) As Boolean
        Try
            If Boolean.Parse(UP.getValue("PreviewFormInDoGenerateOutlook", Sections.UserPreferences, "True")) Then
    ...
    Public Sub EnviarMail(ByRef results() As Result, ByVal Params As Hashtable) 'FORM QUE PERMITE MANDAR UN MAIL
        If (New RightsBusiness().GetUserRights(Zamba.Membership.MembershipHelper.CurrentUser.ID, ObjectTypes.Documents, RightsType.EnviarPorMail)) Then
            ...
            If Not IsNothing(results(0)) Then
                ...
                indexerTemp = Zamba.Membership.MembershipHelper.AppTempDir("\IndexerTemp").FullName & "\"
                ...
                If SharedOutlook.GetOutlook().GetNewMailItem(Params) Then
                    Try
    ...
    Public Function SaveHistory(ByVal Params As Hashtable, ByVal DocId As Int64, ByVal DocTypeId As Int64, ByVal ExportPath As String)
        ...
        MessagesBusiness.SaveHistory(_to, _cc, _cco, _subject, _body, _attachs, DocId, DocTypeId, Zamba.Membership.MembershipHelper.CurrentUser.ID, ExportPath)
    End Function

    ''' <summary>
    ''' Completa los indices del documento con los valores del mail
    ''' </summary>
    ...
    Private Sub FillIndexWithMailProperties(ByRef _result As Zamba.Core.Result, ByVal _params As Hashtable)
        ...
        Try
            ...
            Results_Business.SaveModifiedIndexData(_result, True, True, modifiedIndex, Nothing)
        Finally
    ...
    Private Function getMailValue(ByVal MailValueName As String, ByVal _params As Hashtable) As String
        ...
        Select Case MailValueName
            ...
            Case "UsuarioZamba"
                value = Zamba.Membership.MembershipHelper.CurrentUser().Name
            Case "Codigo"
