...
#Region "Constantes"
        ...
        Const tagHtmlFORM As String = "form"
        Const tagHtmlACTION As String = "action"
        ...
        Const replazarAtributoActionRegex As String = "action=[^\s=]*"
...
#End Region
        ...
        Public Shared Sub replazarAtributoSrc(ByRef tag As String, ByVal path As String)
            ...
            If tag IsNot Nothing Then
                ...
                If path.ToLower().StartsWith(MembershipHelper.Protocol) Then
                    formatPath = " src=""" & path & """ "
                Else
        ...
        ''' <summary>
        ''' Reemplaza el atributo action de un tag action de un form
        ''' </summary>
        ''' <param name="tag">tag html</param>
        ''' <param name="action">action del formulario</param>
        ''' <history>
        ''' 	[AlejandroR]	21/04/2011	Created    
        ''' </history>
        Public Shared Sub replazarAtributoAction(ByRef tag As String, ByVal action As String)

            If Not IsNothing(tag) Then

                action = "action=""" & action & """>"""

                If tag.Contains(tagHtmlACTION) Then

                    tag = RemoveValueFromProperty(tag, tagHtmlACTION)

                    Dim regex As String = replazarAtributoActionRegex
                    Dim options As System.Text.RegularExpressions.RegexOptions = ((System.Text.RegularExpressions.RegexOptions.IgnorePatternWhitespace Or System.Text.RegularExpressions.RegexOptions.Multiline) _
                                Or System.Text.RegularExpressions.RegexOptions.IgnoreCase)
                    Dim reg As System.Text.RegularExpressions.Regex = New System.Text.RegularExpressions.Regex(regex, options)
                    'Se reemplaza la propiedad action del form
                    tag = reg.Replace(tag, action)
                Else
                    'determina si el tag  esta escrito de esta forma < /> o
                    '<></>
                    If tag.Substring(tag.Length - 2, 1) = "/" Then
        ...
        ''' 	[osanchez]	07/04/2009	Created    
        ''' </history>
        Public Shared Function ParseHtml(ByVal html As String, ByVal item As String) As MatchCollection
        ...
        ''' 	[osanchez]	07/04/2009	Created    
        ''' </history>
        Public Shared Function actualizarHtml(ByVal tags As List(Of IDtoTag), ByVal html As String) As String
        ...
        ''' <summary>
        ''' Busca el tag html form, Si existe retorna true 
        ''' </summary>
        ''' <param name="item">coincidencia de expresion regular</param>
        ''' <returns>True si existe el tag form</returns>
        ''' <history>
        ''' 	[AlejandroR]	21/04/2011	Created    
        ''' </history>
        Public Shared Function buscarHtmlForm(ByVal item As Match) As Boolean
            If item.Groups(1).Value.ToLower() = tagHtmlFORM Then
                Return True
            Else
                Return False
            End If
