...
#Region "Notify"
    ...
    Public Shared Sub Notify(ByVal userId As Int64, ByVal requestUserId As Int64, ByVal ruleIds As List(Of Int64), ByVal taskAndStepIds As Dictionary(Of Int64, Int64), ByVal message As String, ByVal title As String, ByVal linkMail As String)
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Notificando a UserId: " & userId)

        CurrentRequestAction.UsersIds.Add(userId)
        CurrentRequestAction.RequestUserId = requestUserId
        ZTrace.WriteLineIf(ZTrace.IsInfo, "RequestUserId: " & requestUserId)
        ...
        For Each CurrentRuleId As Int64 In CurrentRequestAction.RulesIds
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Pidiendo nombre de regla para RuleId: " & CurrentRuleId)
            Dim WFRulesBusiness As New WFRulesBusiness
            RuleName = WFRulesBusiness.GetRuleNameById(CurrentRuleId)
            WFRulesBusiness = Nothing
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Nombre de regla para RuleId: " & CurrentRuleId & " es " & RuleName)
    ...
    Public Shared Sub Notify(ByVal request As RequestAction, ByVal message As String, ByVal title As String, ByVal linkMail As String)
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "cantidad de reglas a ejecutar que recibe la regla: " & request.RulesIds.Count)
        Dim WFRulesBusiness As New WFRulesBusiness
        For Each CurrentRuleId As Int64 In request.RulesIds
            ZTrace.WriteLineIf(ZTrace.IsInfo, "RuleId: " & CurrentRuleId)
            RuleName = WFRulesBusiness.GetRuleNameById(CurrentRuleId)
            ZTrace.WriteLineIf(ZTrace.IsInfo, "RuleName: " & RuleName)
            RulesIdsAndName.Add(CurrentRuleId, RuleName)
        Next
        WFRulesBusiness = Nothing
    ...
    Private Shared Sub Notify(ByVal request As RequestAction, ByVal rulesIdsAndName As Dictionary(Of Int64, String), ByVal message As String, ByVal title As String, ByVal linkMail As String)
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Notificando")
        request.RequestActionId = Insert(request)

        ZTrace.WriteLineIf(ZTrace.IsInfo, "UserBusiness.GetUsersWithMailsNames(request.UsersIds)")
        Dim UB As New UserBusiness
        Dim Users As List(Of User) = UB.GetUsersWithMailsNames(request.UsersIds)
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Cantidad de Usuarios con Mails " & Users.Count)
        ...
        For Each CurrentUser As User In Users
            ...
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Enviando Mail a " & CurrentUser.Name)
            ZTrace.WriteLineIf(ZTrace.IsInfo, "Mail del from " & CurrentUser.eMail.Mail)
            ZTrace.WriteLineIf(ZTrace.IsInfo, "URL de requestAction " & request.ServerLocation)
            SendMail(CurrentUser.eMail.Mail, CurrentUser.ID, message, title)
        Next
    ...
    ''' <param name="title"></param>
    ''' <remarks></remarks>
    Private Shared Sub SendMail(ByVal mailAddress As String, ByVal userId As Int64, ByVal message As String, ByVal title As String)
        Dim mail As New SendMailConfig
        mail.MailTo = mailAddress
        mail.Subject = title
        mail.Body = message
        mail.IsBodyHtml = True
        mail.LinkToZamba = True
        mail.UserId = userId
