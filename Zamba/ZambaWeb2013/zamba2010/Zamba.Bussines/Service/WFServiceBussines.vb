            ...
            Try
                ...
                SyncLock (WfServ)
                    Try
                        ...
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "# Comienzo de un nuevo ciclo de ejecución de reglas de actualización (" & DateTime.Now & ")")
                        ...
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "# Finaliza el ciclo de ejecución. Tiempo de ejecucion: " & (DateTime.Now - WfServ.DateLastRefresh).ToString)
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "_______________________________________________________________________________________________________")
                    Catch ex As Exception
                        ZClass.raiseerror(ex)
                    End Try
            ...
            Try
                ...
                SyncLock (WfServ)
                    Try
                        ...
                        RaiseEvent ShowMsg("__________________________________________________________________")
                        RaiseEvent ShowMsg("Comienza un nuevo ciclo de ejecución de Servicios")
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "_______________________________________________________________________________________________________")
                        ZTrace.WriteLineIf(ZTrace.IsInfo, "# Comienzo de un nuevo ciclo de ejecución de Servicios (" & DateTime.Now & ")")
    ...
    Public Sub StartService() Implements IService.StartService
        Try
            ...
            If _WFServiceServicesExecute = True Then
                Me.TimerServices = New ZTimer(CBTimerS, Me, 0, 60000, _TimeStartT, _TimeEndT)
            End If
    ...
    Public Sub StopService() Implements IService.StopService
        ...
        Try
            ...
            If TimerServices Is Nothing = False Then
                Me.TimerServices.Pause()
                Me.TimerServices.Dispose()
                Me.TimerServices = Nothing
            End If
            Me.CheckSState = ServiceState.Stopped
            CBTimerS = Nothing
            SService = Nothing
        ...
        ZTrace.WriteLineIf(ZTrace.IsInfo, "Tiempo de detención: " & DateTime.Now)
    End Sub
#End Region
                    ...
                    Try
                        ...
                        If Not IsNothing(currentWFStep.DSRules) Then
                            ...
                            For Each R As DataRow In currentWFStep.DSRules.WFRules.Select("ParentType = 8")
                                ...
                                If WFRulesBusiness.IsExecutionTaskByTask(wfStep.Key, rule.ID, True) Then
                                    ...
                                    For i = 0 To taskIds.Count - 1
                                        ...
                                        Try
                                            ...
                                            If WFTaskBusiness.LockTask(taskIds(i), currentLockedUser) Then
    ...
    Public Sub ExecuteService(ByVal wfService As WFService, ByVal userId As Long)
        ...
        Else
            Try
                ...
                UpdateBusiness.CheckLastVersionAtServer()
    ...
    Private Sub InitializeWF(ByVal WFService As WFService)
        For Each WFId As Int64 In WFService.WFIds
            ...
            For Each Wfstep As KeyValuePair(Of Int64, String) In lstWFs.Item(WFId)(1)
                ...
                Catch ex As StackOverflowException
                    Zamba.Core.ZClass.raiseerror(ex)
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Ocurrió un error al cargar la etapa.")
                Catch ex As Exception
                    Zamba.Core.ZClass.raiseerror(ex)
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Ocurrió un error al cargar la etapa.")
                End Try
    ...
    Public Sub ExecuteService(ByVal WFService As WFService, ByVal CurrentUserId As Int64)
        ...
        For Each ar As Object In lstWFs.Values
            ...
            For Each Wfstep As KeyValuePair(Of Int64, String) In lstSteps
                ...
                Catch ex As StackOverflowException
                    Zamba.Core.ZClass.raiseerror(ex)
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Ocurrió un error en la ejecución de la tarea.")
                Catch ex As Exception
                    Zamba.Core.ZClass.raiseerror(ex)
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Ocurrió un error en la ejecución de la tarea.")
                End Try
