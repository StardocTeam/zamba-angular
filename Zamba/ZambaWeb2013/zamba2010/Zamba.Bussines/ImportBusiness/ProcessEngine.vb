...
Imports Zamba.Core.ProcessFactory
...
#Region "InitialProcess"
    ...
    Public Sub InitialProcess(ByRef Process As Process)
        ...
        Try
            ...
            UserBusiness.Rights.SaveAction(Process.ID, Zamba.ObjectTypes.ModuleImport, RightsType.Execute, "Proceso ejecutado: " & Process.Name)
            ProcessFinalized()
...
#Region "ProcessEvents"
    ...
    Private Sub WriteLog(ByVal Msg As String)
        Try
            Dim LogWriter As New IO.StreamWriter(Process.History.LOGFILE, True, Text.Encoding.Default)
            Try
...
#Region "Startprocess"
    Private Sub StartProcess()
        Try
            EventOCurred("Proceso: " & Me.Process.Name)
            ...
            Catch ex As Exception
                ...
                Exit Sub
            End Try
            'Obtengo los Datos de los indices del  Proceso
            ...
            Try
                ...
                Me.PostProcessEjecutados = Process_Business.PostProcess(Files, Me.Process.ID)
            Catch ex As Exception
                Me.ErrorOcurred(ex.ToString)
            End Try
        Finally
    ...
    Private Sub GetFileToProcess()
        Select Case Process.type
            Case ProcessTypes.Common
                If Me.Process.FlagSourceVariable = True Then
                    Try
    ...
    Private Shared Function OpenFileDialog() As String
        ...
        Try
            ...
            Else
                If New IO.FileInfo(FileDialog.FileName.Trim).Exists Then
    ...
    Private Shared Sub CheckFiles(ByVal File As String)
        If New IO.FileInfo(File).Exists = False Then Throw New Exception("El Archivo especificado no existe: " & File)
    End Sub
    ...
    Private Sub CreateTempFile()
        Try
            Select Case Process.type
                Case ProcessTypes.Common
                    Dim FiOriginal As New IO.FileInfo(Process.History.Path)
    ...
    Private Function CheckFileHash() As Boolean
        If Process.AskConfirmations = True AndAlso ProcessHistories.ProcessIsReady(Me.Process) = True Then
    ...
    Private Function CreateDocument() As Boolean
        Try
            ...
            If IsNothing(Document) Then Document = RB.GetNewNewResult(Me.Process.DocType)
            Document.Parent = Me.Process.DocType
...
#Region "PreProcess"
    Private Function PreProcess() As ArrayList
        ...
        Select Case Process.type
            Case ProcessTypes.Common
                FileList.Add(Me.Process.History.TEMPFILE)
                Try
                    Dim PreProcessArray As ArrayList = PreProcessFactory.getPreprocess(Process.ID)
            ...
            Case ProcessTypes.ContinueProcess
                FileList.Add(Me.Process.History.TEMPFILE)
            Case ProcessTypes.ProcessErrors
                FileList.Add(Me.Process.History.ERRORFILE)
            Case ProcessTypes.Test
                FileList.Add(Me.Process.History.TEMPFILE)
                Try
                    Dim PreProcessArray As ArrayList = PreProcessFactory.getPreprocess(Process.ID)
...
#Region "FinalProcess"
        ...
        Try
            If Not IsNothing(PreProcessedFilesList) Then
                For FileCounter = 0 To PreProcessedFilesList.Count - 1
                    Fi = New IO.FileInfo(PreProcessedFilesList(FileCounter))
    ...
    Private Function InformFilesToProcess() As Boolean
        If Me.Process.UserId <> 9999 AndAlso Process.AskConfirmations Then
    ...
    Private Sub Import(ByVal x As String, Optional ByVal Test As Boolean = False)
        ...
        FilesCount = 0
        Dim xsplit = x.Split(Me.Process.Caracter.Trim)
            ...
            Select Case IndexId.ToUpper
                ...
                Case "RUTACOMPLETA"
                    If Me.Process.FlagMultipleFiles = True Then
                        MultipleFiles.Clear()
                        Try
                            Dim FilesSplit() = xsplit(ProcessIndexCounter).split(Me.Process.MultipleCaracter.Trim)
                            Me.FilesCount += FilesSplit.Length
                ...
                Case Else
                    ...
                    Try
                        ...
                        Else
                            ...
                            Dim IndexValidated As String = Index.ValidateIndexTypeData(Data, _
                            CInt(DirectCast(Document.Indexs(Arrayindex), Index).Type))
        ...
        'Creo el documento
        Try
            'TODO Falta un solo proceso para insertar y salvar
            ...
            If Me.Process.FlagMultipleFiles = True Then
                ...
                For i = 0 To MultipleFiles.Count - 1
                    ...
                    If Document.File.Trim <> "" Then
                        ...
                        If New IO.FileInfo(Document.File).Exists = False Then
                            Process.History.Result = Zamba.Core.Results.Con_Errores
                    ...
                    If Document.File.Trim <> "" Then
                        Try
                            ...
                            If Process.type <> ProcessTypes.Test Then
                                Document.Id = 0
                                Result = New Results_Business().Insert(Document, False, False, Me.Process.Replace, Process.AskConfirmations)
                                Me.Hash.Add(Me.Document.Id, x)
                            Else
            ...
            Else
                Try
                    ...
                    If IO.File.Exists(Document.File) = False Then
                    ...
                    If Process.type <> ProcessTypes.Test Then
                        Document.Id = 0
                        Result = New Results_Business().Insert(Document, False, False, Me.Process.Replace, Process.AskConfirmations)
                        If Me.Document.ID <> 0 Then Me.Hash.Add(Me.Document.ID, x)
                    ...
                    Select Case Result
                        ...
                        Case InsertResult.ErrorIndicesIncompletos
                            Throw New Exception("Error faltan indices obligatorios")
                        Case InsertResult.ErrorIndicesInvalidos
                            Throw New Exception("Error indices con datos invalidos")
                    End Select
                Catch ex As Exception
                    ErrorOcurred(ex.tostring)
                    Process.History.Result = Zamba.Core.Results.Con_Errores
        ...
        If Process.CheckBatch = True Then
            ...
            Me.CheckAllInsertedDocuments(Me.Document.DocType.Id)
            RaiseEvent LogMsg("Verificación finalizada")
        End If
    End Sub
    Private Sub CheckAllInsertedDocuments(ByVal doctypeid As Int32)
        ...
        Try
            For i = 0 To Hash.Count - 1
                ...
                If c <> 1 Then
                    Me.Import(Hash(docid))
                End If
                Me.Hash.Remove(docid)
            Next
        Catch ex As Exception
        End Try
    End Sub
        ...
        Try
            If Process.type <> ProcessTypes.Test Then
                If Process.Move = True Then
                    Dim Fi As New IO.FileInfo(Document.OriginalName)
                    Fi.Delete()
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub DefineResult()
        If Me.Process.History.ErrorFiles > 0 Then
            'Con Errores
            If Me.FlagGoOn = True Then
                'Sin Cancelacion
                Me.Process.History.Result = Zamba.Core.Results.Con_Errores
            Else
                'Con Cancelacion
                Me.Process.History.Result = Zamba.Core.Results.Cancelado_y_Errores
            End If
        Else
            'Sin Errores
            If Me.FlagGoOn = True Then
                ...
                If Process.History.ProcesedFiles > 0 Then
                    Me.Process.History.Result = Zamba.Core.Results.Correctamente
                Else
                    Me.Process.History.Result = Zamba.Core.Results.Sin_procesar
                End If
            Else
                'Con Cancelacion
                Me.Process.History.Result = Zamba.Core.Results.Cancelado
            End If
        End If
    ...
    Private Sub FinalMessages()
        Try
            EventOCurred("Proceso: " & Me.Process.Name & " finalizado: " & Process.History.Result.ToString)
            ...
            If Me.PostProcessEjecutados.Count > 0 Then
                Dim I As Int32
                For I = 0 To Me.PostProcessEjecutados.Count - 1
                    EventOCurred("Post Proceso Ejecutado. Nombre: " & Me.PostProcessEjecutados(I))
                Next
            End If
...
#End Region
    ...
    Public Shared Function preProcessThisFile(ByVal proId As Int32, ByVal filein As String) As String
        ...
        Dim pre As ArrayList = PreProcessFactory.getPreprocess(proId)
        Dim files As New ArrayList
        files.Add(filein)
    ...
    Private Sub Informar()
        Try
            ...
            If IO.File.Exists(".\Notify.xml") Then
