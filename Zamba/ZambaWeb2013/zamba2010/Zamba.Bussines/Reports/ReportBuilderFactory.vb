...
Public Class ReportBuilderFactory
    ...
    ''' </history>
    Public Function RunQueryBuilderReporteGeneral(ByVal id As Int32, ByVal vars As Hashtable, Optional currentTask As ITaskResult = Nothing) As DataSet
        ...
        Try
            ...
            Else
                ZTrace.WriteLineIf(ZTrace.IsInfo, "Id de Reporte: " & id)
                'Execute the query
                ...
                ZTrace.WriteLineIf(ZTrace.IsInfo, "Query de Reporte: " & sql)
                ...
                If Not sql.ToLower.Trim.StartsWith("exec ") Then
                    ...
                    'Next
                    Dim vi As New VariablesInterReglas
                    If vars IsNot Nothing Then
                        For Each key As String In vars.Keys
                            If VariablesInterReglas.ContainsKey(key) Then
                                VariablesInterReglas.Item(key) = vars(key)
                            Else
                                VariablesInterReglas.Add(key, vars(key))
                            End If
                        Next
                    End If
                    sql = vi.ReconocerVariablesValuesSoloTexto(sql)
                    sql = TextoInteligente.ReconocerCodigo(sql, currentTask)
                    ...
                    ZTrace.WriteLineIf(ZTrace.IsInfo, "Query Final de Reporte: " & sql)
