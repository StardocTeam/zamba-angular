        ...
        private void InitDataTable()
        {
            ...
            foreach (DataRow r in table.Rows) {
                ...
                {                 
                    ...
                    if (r[i].GetType().ToString() == "System.DateTime" &&
                        this.ShortDateFormat ) {
                            //(pablo) add validations for the userConfig option 'CheckInColumnShortDateFormat' 07/01/2011
                            if (Columns[i].ToString() != Zamba.Core.UserPreferences.getValue("ColumnNameCheckIn", Zamba.Core.Sections.UserPreferences, "CheckIn") || Boolean.Parse(Zamba.Core.UserPreferences.getValue("CheckInColumnShortDateFormat", Zamba.Core.Sections.UserPreferences, true)))
                            {                            
                                string R = ((System.DateTime)r[i]).ToShortDateString();
                                row.Add(R);
                            }
                            else
                            {
                    ...
                    {
                        //(pablo) if the UserConfig option 'GridShortDateFormat' is False
                        if (Columns[i].ToString() == Zamba.Core.UserPreferences.getValue("ColumnNameCheckIn", Zamba.Core.Sections.UserPreferences, "CheckIn") && Boolean.Parse(Zamba.Core.UserPreferences.getValue("CheckInColumnShortDateFormat", Zamba.Core.Sections.UserPreferences, true)))
                        {
                            string R = ((System.DateTime)r[i]).ToShortDateString();
                            row.Add(R);
