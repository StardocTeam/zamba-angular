    ...
    public class OutlookGridRow : DataGridViewRow
    {
        ...
        protected override void Paint(System.Drawing.Graphics graphics, System.Drawing.Rectangle clipBounds, System.Drawing.Rectangle rowBounds, int rowIndex, DataGridViewElementStates rowState, bool isFirstDisplayedRow, bool isLastVisibleRow)
        {
            if (this.isGroupRow)
            {
                ...
                Font F = new Font(grid.Font.FontFamily,8,System.Drawing.FontStyle.Bold);
                graphics.DrawString(group.Text, F, Brushes.Black, rowHeadersWidth - grid.HorizontalScrollingOffset + 23, rowBounds.Bottom - 18);
                //graphics.DrawString(UserRowName + Value, grid.Font, Brushes.Black, rowHeadersWidth - grid.HorizontalScrollingOffset + 23, rowBounds.Bottom - 18);
                ...
                if (group.Collapsed)
                {
                    if (grid.ExpandIcon != null)
                        graphics.DrawImage(grid.ExpandIcon, rowBounds.Left + rowHeadersWidth - grid.HorizontalScrollingOffset + 8, rowBounds.Bottom - 18, 11, 11);
                }
                else
                {
                    if (grid.CollapseIcon != null)
                        graphics.DrawImage(grid.CollapseIcon, rowBounds.Left + rowHeadersWidth - grid.HorizontalScrollingOffset + 8, rowBounds.Bottom - 18, 11, 11);
                }
                brush.Dispose();
                brush2.Dispose();
            }
            //else if (this.isZamba == true)
            //{
            //    OutlookGrid grid = (OutlookGrid)this.DataGridView;
            //    int rowHeadersWidth = grid.RowHeadersVisible ? grid.RowHeadersWidth : 0;

            //    // this can be optimized
            //    Brush brush2 = new SolidBrush(Color.FromKnownColor(KnownColor.Blue));

            //    int gridwidth = grid.Columns.GetColumnsWidth(DataGridViewElementStates.Displayed);
            //    Rectangle rowBounds2 = grid.GetRowDisplayRectangle(this.Index,false);

            //    //draw bottom line
            //    graphics.DrawLine(new Pen(brush2, 4), rowBounds2.X, rowBounds2.Y, rowBounds2.X + rowBounds2.Width, rowBounds2.Y);
            //    graphics.DrawLine(new Pen(brush2, 4), rowBounds2.X, +rowBounds2.Height + rowBounds2.Y, rowBounds2.X + rowBounds2.Width, rowBounds2.Height + rowBounds2.Y);
