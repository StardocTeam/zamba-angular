...
public partial class WCSendMail : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            try
            {
                _res = SResult.GetResult(_docid, _doctype);
            }
            catch (Exception ex)
            {
    ...
    protected void btnSendMail_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ZOptB.GetValue("WebView_SendBySMTP") != null)
            {
            ...
            if ((to == string.Empty && cc == string.Empty && cco == string.Empty) || subject == string.Empty || body == string.Empty)
            {
    ...
    private void AttachOriginalDocument()
    {
        ...
        if (!chkAddWebLink.Checked)
        {
            ...
            {
                SResult sResult = new SResult();
                Result res = (Result)sResult.GetResult(_res.ID, _res.DocTypeId);
                ...
                if (res != null)
                {
                    ...
                    else
                    {
                        ...
                        if (!String.IsNullOrEmpty(sUseWebService) && bool.Parse(sUseWebService))
                            file = sResult.GetWebDocFileWS(res.DocTypeId, res.ID, MembershipHelper.CurrentUser.ID);
                        else
                            file = sResult.GetFileFromResultForWeb(res);
                    }
