...
public partial class Views_UC_Index_ucDocTypeIndexsForInsert : System.Web.UI.UserControl
{
    ...
    private List<IIndex> ParsePostBackFormToIndexs(List<IIndex> indexToParse)
    {
        ...
        for (int i = 0; i < indexCount; i++)
        {
            ...
            {
                if (postBackForm.AllKeys[j] != null && postBackForm.AllKeys[j].Contains(indexToReturn[i].ID.ToString()) &&
    ...
    public void ShowIndexs(List<IIndex> indexs, WebModuleMode mode)
    {
        ...
        Hashtable IRI = UserBusiness.Rights.GetIndexsRights(doctypeid, user.ID, true, true);
    ...
    /// <param name="IndexEnable"></param>
    /// <param name="IndexVisible"></param>
    /// <returns></returns>
            ...
            else
            {
                ...
                else
                {
                    if (!string.IsNullOrEmpty(Index.Data))
                    {
                        ...
                        string textDescription = (new SIndex()).getAutoSustitucionDescription(Index.Data, Index.ID, false, Index.Type);
            ...
            switch (Index.DropDown)
            {
                ...
                case IndexAdditionalType.AutoSustituciónJerarquico:
                    ...
                    if (Index.HierarchicalParentID > 0)
                    {
                        ...
                        foreach (IIndex index in CurrentIndexs)
                        {
                            if (Index.HierarchicalParentID == index.ID)
                            {
                        ...
                        resutlsTable = new SIndex().GetHierarchyTableByValue(Index.ID, Index.HierarchicalParentID, parentData);
                ...
                case IndexAdditionalType.DropDownJerarquico:
                    if (Index.HierarchicalParentID > 0)
                    {
                        ...
                        DataTable resutlsTableSearch = new SIndex().GetHierarchyTableByValue(Index.ID, Index.HierarchicalParentID, parentData);
                    ...
                    else
                    {
                        ...
                        ArrayList values = new SIndex().GetDropDownList((int)Index.ID);
