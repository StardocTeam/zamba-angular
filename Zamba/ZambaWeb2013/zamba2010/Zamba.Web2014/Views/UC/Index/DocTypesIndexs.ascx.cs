...
namespace Controls.Indexs
{
    ...
    public partial class DocTypesIndexs : UserControl
    {
        ...
        const Int16 CTR_INDEX_TB_WIDTH = 125;
        const Int16 CTR_INDEX_ILST_WIDTH = 110;
        const Int16 CTR_INDEX_DATE_WIDTH = 110;
        const Int16 CTR_INDEX_SLST_WIDTH = 80;
        ...
        {
            Int64 IndexId = Int64.Parse(hdIndexId.Value);
            if (sIndex == null)
            {
                sIndex = new SIndex();
            }
        ...
        private void UpdateHierachicalIndex(IIndex parentIndex, IIndex childIndex)
        {
            try
            {
                ...
                if (childIndex != null)
                {
                    dtOptions = sIndex.GetHierarchyTableByValue(childIndex.ID, parentIndex.ID, parentIndex.DataTemp);
        ...
        private void OpenPopUp(Int64 indexId)
        {
            ...
            else
            {
                if (currentIndex.HierarchicalParentID > 0)
                {
                    ...
                    string parentData = string.Empty;
                    foreach (IIndex indx in currentIndexs)
                    {
                        if (currentIndex.HierarchicalParentID == indx.ID)
                        {
                            parentData = indx.Data;
                            break;
                        }
                    }
                    dt = sIndex.GetHierarchyTableByValue(indexId, currentIndex.HierarchicalParentID, parentData);
            ...
            ((UpdatePanel)this.Parent.Parent).Update();
        ...
        private Visibility GetIndexVisibility(IIndex index, Hashtable IRI, IUser user)
        {
            ...
            if (Rights.GetUserRights(Zamba.Core.ObjectTypes.DocTypes, Zamba.Core.RightsType.ViewRightsByIndex, doctypeid))
            {
            ...
            for (int i = 0; i < indexCount; i++)
            {
                if (indexToReturn[i] != null)
                {
                    ...
                    for (int j = 0; j < keysCount; j++)
                    {
                        ...
                        if (currentKey != null)
                        {
                            if (currentKey.Contains("$" + indexId) &&
                            ...
                            {
                                if (currentKey.Contains("Value§" + indexId))
                                {
                                    //Se guarda el valor a buscar
                                    indexToReturn[i].Data = postBackForm[currentKey];
                                    indexToReturn[i].DataTemp = postBackForm[currentKey];
        ...
        public void ShowIndexs(List<IIndex> indexs, WebModuleMode mode)
        {
            ...
            long UserId = long.Parse(Session["UserId"].ToString());
            Zamba.Core.IUser user = (Zamba.Core.IUser)Session["User"];
            ...
            else
            {
                ...
                taskResult = service.GetTask(int.Parse( lbTaskId.Text.Trim()));
            }
            ...
            else
            {
                ...
                int lenind = 0;
                SRights Rights = new SRights();
                Hashtable IRI = Rights.GetIndexsRights((Int64)DtId, user.ID, true, false);
                ...
                foreach (Index currentIndex in (Page.IsPostBack ? indexs : CurrentIndexs))
                {
                    if (mode == WebModuleMode.Result)
                    {
                        ...
                        switch (GetIndexVisibility(currentIndex, IRI, (User)user))
                        {
                   ...
                   // tcNewIndex.Style.Add(HtmlTextWriterStyle.PaddingTop, "5px");
                    ...
                    if (currentIndex.Type == IndexDataType.Fecha || currentIndex.Type == IndexDataType.Fecha_Hora)
                    {
                        tbValue = new TextBox
                        {
                            Text = Server.HtmlEncode(currentIndex.Data),
                        ...
                        else
                        {
                            ...
                            tbValue = (TextBox)Helpers.Validators.GetControlWithValidations(currentIndex, tbValue, mode, taskResult, ZFieldType.IndexField);
                        }
                    ...
                    else
                    {
                        switch (currentIndex.DropDown)
                        {
                            ...
                            case IndexAdditionalType.AutoSustituciónJerarquico:
                                ...
                                {
                                    description = sIndex.getAutoSustitucionDescription(
                                        currentIndex.Data,
                            ...
                            case IndexAdditionalType.DropDown:
                                ...
                                {
                                    //Se agregan los atributos necesarios para generar las validaciones
                                    cbValue = (DropDownList)Helpers.Validators.GetControlWithValidations(currentIndex, cbValue, mode, taskResult, ZFieldType.IndexField);
                                }
                            ...
                            case IndexAdditionalType.DropDownJerarquico:
                                ...
                                if (currentIndex.HierarchicalParentID > 0)
                                {
                                    ...
                                    DataTable dt = sIndex.GetHierarchyTableByValue(currentIndex.ID,
