...
public partial class UC_Viewers_FormBrowser : System.Web.UI.UserControl
{
                        ...
                        foreach (Match item in matches)
                        {
                            if (SForms.buscarHtmlIframe(item))
                            {
                                ...
                                if (id == -1)
                                {
                                    ...
                                    {
                                        string Path = string.Format(GetDocFileUrl, newResult.DocTypeId, newResult.ID, Session["UserId"]);
                                ...
                                if (!useOriginal)
                                {
                                    ...
                                    docTypesAsocList = SDocAsociated.getAsociatedResultsFromResult(newResult, Int32.Parse(SUserPreferences.getValue("CantidadFilas", Sections.UserPreferences, 100)));
    ...
    private string CompleteFormIndexs(INewResult newRes, string strHtml)
    {
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["DocID"]) && !string.IsNullOrEmpty(Request.QueryString["DocTypeID"]) && long.TryParse(Request.QueryString["DocID"], out docID) && long.TryParse(Request.QueryString["DocTypeID"], out docTypeID))
        {
            ...
            IResult res1 = SResult.GetResult(docID, docTypeID);
        ...
        try
        {
            ...
            if (newResult.ID == 0 || newResult.GetType().ToString() == "NewResult")
            {
                ...
                if (insertresult == InsertResult.Insertado)
                {
                    ...
                    if (!IsModal)
                    {
                        Response.Redirect(string.Format("http://{0}{1}/Views/WF/TaskSelector.ashx?docid={2}&doctypeid={3}", Request.ServerVariables["HTTP_HOST"], Request.ApplicationPath, nr.ID, nr.DocTypeId));
                    }
                    else
                    {
