<%@ Control Language="C#" AutoEventWireup="false" CodeFile="UCHome.ascx.cs" Inherits="Views_UC_Home_UCHome" %>
...
<%  if (Page.Theme.ToLower() == "reintegros") {%>
...
<%  }%>
<%  if (Page.Theme.ToLower() == "mantenimiento") {%>
...
<%  } %>
<%  if (Page.Theme.ToLower() == "liberty") {%>
<input id="btnInsertar" type="button" onclick="SelectTabFromMasterPage('tabInsert')" />
<%  } %>
<%  if (Page.Theme.ToLower() == "aysagec") {%>
    <script language="javascript" type="text/javascript">
        function ShowTabGenRepGec() {
            openWindow("../Aysa/GEC/GeneralReportStandAlone2.aspx?userId=" + $("#ctl00_hdnUserId").val(), "", false);
        }

        function ZDispatcherRedirection_ShowGralReportGec() {
            parent.HomeCabPresentation();
            openWindow("Views/Aysa/GEC/GeneralReportStandAlone2.aspx?userId=" + $("#ctl00_hdnUserId").val(), "", false);
        }
    </script>
