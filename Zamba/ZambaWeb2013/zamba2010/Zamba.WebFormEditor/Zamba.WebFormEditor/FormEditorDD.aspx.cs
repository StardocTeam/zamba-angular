            ...
            if (!IsPostBack)
            {
                ...
                if (FormId > 0)
                {
                    try
                    {
                        ...
                        var ZForm = Zamba.Core.FormBusiness.GetForm(FormId);
...
//                if (RadTreeView1.Nodes.Count > 0) RadTreeView1.Nodes[0].Expanded = true;
                ...
                PopulateTreeViewScriptsFromDirectory(node2.Nodes, Server.MapPath("~/Content/scripts"));
        ...
        public String GetDocTypeName()
        {
            ...
            if (DocTypeId > 0)
            {
                ...
                return Zamba.Core.DocTypesBusiness.GetDocTypeName(DocTypeId, true);
            }
        ...
        private IDocType GetDocType(int DocTypeId)
        {
 return Zamba.Core.DocTypesBusiness.GetDocType(DocTypeId, true); 
        }
            ...
            int DocTypeId)
        {
            ...
            Indexs =  Zamba.Core.IndexsBusiness.GetIndexsSchemaAsListOf(DocTypeId, false);
            ...
            int DocTypeId)
        {
           ...
           var AsociatedDocTypes = Zamba.Core.DocTypes.DocAsociated.DocAsociatedBusiness.GetDocTypeAsociation(DocTypeId);

           List<Int64> docTypeAsoc = new List<Int64>();

           foreach (DataRow A in AsociatedDocTypes.Tables[0].Rows)
           {
               if (docTypeAsoc.Contains(Int64.Parse(A["DocTypeID2"].ToString()))==false)
               {
                   docTypeAsoc.Add(Int64.Parse(A["DocTypeID2"].ToString()));
                   RadTreeNode node = new RadTreeNode();
                   node.Text = GetDocType(int.Parse(A["DocTypeID2"].ToString())).Name;
                   node.ImageUrl = "~/Content/Images/toolbars/bullet_ball_blue.png";
                   ...
                   TagId = "zamba_associated_documents_" + A["DocTypeID2"].ToString();
                   ...
                   node.Value = A["DocTypeID2"].ToString();
                   node1.Nodes.Add(node);
               }
           }
            ...
            foreach (DataRow A in AsociatedDocTypes.Tables[0].Rows)
            {
                if (docTypeAsoc.Contains(Int64.Parse(A["DocTypeID2"].ToString())) == false)
                {
                    ...
                    foreach (IIndex I in Indexs)
                    {
                        ...
                        TagId = "zamba_asoc_" + A["DocTypeID2"].ToString() + "_index_" + I.ID;
...
//                    node.ImageUrl = ConvertAbsoluteToRelative(_file);// "~/Content/Images/" + Path.GetFileName(_file);
        ...
        protected void SaveForm()
        {
            try
            {
                ...
                if (FormId > 0)
                {
                    ZForm = Zamba.Core.FormBusiness.GetForm(FormId);
                    ...
                    //Se reemplazan las referencias arregladas por las que previamente tenia
                    string contentToSave = this.RadEditor1.Content;
                    //if (FixedReferences != null && FixedReferences.Count > 0)
                    //{
                    //    string newFixedValue;
                    //    string oldValue;
                    //    int listCount = FixedReferences.Count;
                    //    for (int i = 0; i < listCount; i++)
                    //    {
                    //        newFixedValue = FixedReferences[i].Split('|')[0];
                    //        oldValue = FixedReferences[i].Split('|')[1];
                    //        contentToSave = contentToSave.Replace(newFixedValue, oldValue);
                    //    }
                    //}
                 ...
                 contentToSave =   VerifyAndFixForm(contentToSave);
                    ...
                    FormBusiness.UpdateModifiedDate(FormId);
                }
                else
                { 
                    FormId = FormBusiness.GetFormNewId() ;
                    ...
                    CopyFiles(Request.PhysicalPath.Remove(Request.PhysicalPath.LastIndexOf("\\")) + "\\Content", path.Remove(path.LastIndexOf("\\")) + "\\Content");

                    ZForm = new ZwebForm(FormId, name, name, (FormTypes)FormTypeId, path, (Int32)DocTypeID, true);

                    FormBusiness.InsertForm(ZForm);
        ...
        protected void BtnTestForm_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (FormId > 0)
                {
                    String TestUrl = Zamba.Core.ZOptBusiness.GetValue("WFETestPage");
