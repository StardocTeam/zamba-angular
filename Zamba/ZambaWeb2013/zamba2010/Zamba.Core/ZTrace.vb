...
#Region "Métodos"
    ...
    Public Shared Sub AddListener(ByVal zModule As String)
        Try
            ...
            If Not _hsSingletonZCoreInstances.ContainsKey(zCoreKey) Then
                _hsSingletonZCoreInstances.Add(zCoreKey, New TextWriterTraceListener(Membership.MembershipHelper.AppTempPath & "\Exceptions" & "\Trace " & zModule & " - " & Now.ToString("dd-MM-yyyy HH-mm-ss") & ".txt", zModule))
            End If
    ...
    Private Shared Function ValidateTrace(ByVal level As TraceLevel) As Boolean
        ...
        If Int32.Parse(zTraceSw.Level) >= level Then
            ...
            Else
                ...
                ResetListeners()
                Trace.WriteLineIf(ZTrace.IsVerbose, GetTraceTimeandMemory(level) & "Iniciando Trace por un nuevo dia")
                Return True
            End If
        Else
    ...
    Public Shared Sub WriteLine(ByVal line As String)
        If Not IsNothing(GetInstance()) Then
            GetInstance().WriteLine(GetTraceTimeandMemory() & line)
            GetInstance().Flush()
        End If
    End Sub
