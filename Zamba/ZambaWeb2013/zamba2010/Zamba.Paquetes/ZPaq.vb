...
'''     [Tomas]     20/05/2011  Modified    Se agregan métodos abreviados para las llamadas a Server
''' </history>
''' -----------------------------------------------------------------------------
...
#Region "Métodos que verifican la existencia de objetos"
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ...
    Public Shared Function ExisteTabla(ByVal tabla As String) As Boolean
        ...
        If Server.isSQLServer Then
            str.Append("select count(1) from sysobjects where name = '")
            str.Append(tabla)
            str.Append("'")
            ...
            Return True
        Else
            str.Append("select count(1) from all_tables where TABLE_NAME = '")
            str.Append(tabla)
            str.Append("'")
            i = Convert.ToInt32(Servers.Server.Con.ExecuteScalar(CommandType.Text, str.ToString))
            If i = 0 Then Return False
    ...
    Public Shared Function ExisteColumna(ByVal columna As String, ByVal tabla As String) As Boolean
        ...
        tabla = tabla.ToUpper
        If Server.isSQLServer Then
            str.Append("select count(1) from syscolumns where id = (select id from sysobjects where name = '")
            str.Append(tabla)
            str.Append("') and name = '")
            str.Append(columna)
            str.Append("'")
            ...
            Return True
        Else
            str.Append("select count(1) from all_tab_columns where COLUMN_NAME = '")
            str.Append(columna)
    ...
    Public Shared Function IfExists(ByVal name As String, ByVal Objeto As Tipo, Optional ByVal Candrop As Boolean = False) As Boolean
        'TODO store: SPGetCountOfSysObjects
        Dim sql As String
        If Server.isSQLServer Then
            sql = "Select count(1) from sysobjects Where lower(name)='" & name.Trim.ToLower & "' and xType='" & GetobjectName(Objeto) & "'"
        Else
            'sql = "Select count(1) from all_objects Where object_type='" & Me.GetobjectName(Objeto) & "' and OBJECT_NAME='" & name.Trim & "'"
            sql = "Select count(1) from user_objects Where object_type='" & GetobjectName(Objeto) & "' and lower(OBJECT_NAME)='" & name.Trim.ToLower & "'"
        End If
        If Int32.Parse(Server.Con.ExecuteScalar(CommandType.Text, sql).ToString) > 0 Then
    ...
    Private Shared Function ObtenerNombreObjeto(ByVal Tipo As Tipo) As String
        ...
        End Select
    End Function
    Public Function Count(ByVal query As String) As Int32
        tmpObject = Server.Con.ExecuteScalar(CommandType.Text, query)

        If tmpObject Is Nothing Then
            Return 0
        Else
            Return Int32.Parse(tmpObject.ToString)
        End If
    End Function
#End Region

#Region "Métodos abreviados de Server"
    Public Sub ENonQuery(ByVal query As String)
        ZTrace.WriteLineIf(ZTrace.IsInfo, "SCRIPT: [" & query & "]")
        Server.Con.ExecuteNonQuery(CommandType.Text, query)
    End Sub
    Public Function EScalar(ByVal query As String) As Object
        ZTrace.WriteLineIf(ZTrace.IsInfo, "SCRIPT: [" & query & "]")
        Return Server.Con.ExecuteScalar(CommandType.Text, query)
    End Function
    Public Function EDataTable(ByVal query As String) As DataTable
        ZTrace.WriteLineIf(ZTrace.IsInfo, "SCRIPT: [" & query & "]")
