    ...
    public class DynamicWebservice
    {
        ...
        public object Consume(string wsdl, string methodName, ref object[] Parameters, Boolean _useCredentials)
        {
          ...
          //  object[] Parameters = new object[parameters.Count];
            ZTrace.WriteLineIf(ZTrace.IsVerbose,"Entrando a GetParameters");
            ParameterInfo[] p = method.GetParameters();
            
            ZTrace.WriteLineIf(ZTrace.IsVerbose,"Valor obtenido de GetParameters: " + p);
            ...
            for (int i = 0; i < p.Length; i++)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        ZTrace.WriteLineIf(ZTrace.IsVerbose, "Tipo de parametro: " + strTipo);
                        ...
                        ZTrace.WriteLineIf(ZTrace.IsVerbose,"Valor de  Parameters[i] " + Parameters[i]);
                    }
                    catch (Exception ex)
                    { 
        ...
        {
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Dentro de GetMethod");
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Valor de wsdl: " + wsdl);
            ZTrace.WriteLineIf(ZTrace.IsVerbose, "Valor de methodName: " + methodName);
            ...
            foreach (MethodInfo CurrentMethod in service.GetMethods())
            {
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Devuelve: " + CurrentMethod);
                ZTrace.WriteLineIf(ZTrace.IsVerbose, "Salgo de GetMethod");
            }
