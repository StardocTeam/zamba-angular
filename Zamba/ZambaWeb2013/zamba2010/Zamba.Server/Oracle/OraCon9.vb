...
Public Class OraCon9
    ...
    Public Function ConvertDatetime(ByVal data As String) As String Implements IConnection.ConvertDateTime
        Dim d As DateTime = data
        Return "TO_DATE('" & d.ToString("dd/MM/yyyy hh:mm") & "' ,'DD/MM/YYYY HH24:MI')"
    ...
    Public Overloads Function ExecuteNonQuery(ByVal spName As String, _
        ...
        SyncLock (CN)
            Try
                ...
                For i = 0 To parametersValues.Length - 1
                    cmd.Parameters.Add(New OleDbParameter(DirectCast(ParametersNames(i), String), parameterstypes(i))).Value = parametersValues(i)
                Next
...
#Region "ExecuteNonQuery"
   ...
   ByVal ParamArray commandParameters() As IDbDataParameter) As Integer Implements IConnection.ExecuteNonQuery
    ...
    Public Overloads Function ExecuteNoTimeOutNonQuery( _
   ByVal commandType As CommandType, _
   ByVal commandText As String, _
...
#Region "ExecuteDataset"
  ...
  ByVal ParametersNames() As Object, ByVal parameterstypes As Object, ByVal parametersValues() As Object) As DataSet Implements IConnection.ExecuteDataset
        SyncLock (CN)
            Try
                ...
                For i = 0 To parametersValues.Length - 2
                    cmd.Parameters.Add(New OleDbParameter(DirectCast(ParametersNames(i), String), parameterstypes(i))).Value = parametersValues(i)
                Next
                '        cmd.Parameters.Add(New OleDbParameter(CStr(ParametersNames(parametersValues.Length - 1)), OleDbType.Cursor)).Direction = ParameterDirection.Output
    ...
    Public Overloads Function ExecuteDataset( _
    ByVal commandType As CommandType, _
...
#End Region
    ...
    End Function
End Class
Public Class Ora9CreateTables
    Implements CreateTables


    Public Shadows Sub AddDocsTables(ByVal DocTypeId As Int64) Implements CreateTables.AddDocsTables
        Try
            Dim str As String = "CREATE TABLE DOC_T" & DocTypeId & " (DOC_ID int PRIMARY KEY IDENTITY,FOLDER_ID INT, DISK_GROUP_ID int NULL, PLATTER_ID int NULL, VOL_ID int NULL, DOC_FILE nchar (200) NULL, OFFSET int NULL, DOC_TYPE_ID int NULL, NAME char (255) NULL, ICON_ID NUMBER (2),SHARED NUMBER (2), Original_filename varchar2(255),ver_Parent_id number(10),version number(1),RootId number(10),NumeroVersion number(2)) ON PRIMARY;"
            Server.Con.ExecuteNonQuery(CommandType.Text, str)
            str = "CREATE TABLE DOC_I" & DocTypeId & " (DOC_ID int PRIMARY KEY IDENTITY) ON PRIMARY;"
            Server.Con.ExecuteNonQuery(CommandType.Text, str)
            'Dim strd As String = "CREATE TABLE DOC_D" & DocTypeId & " (INDEX_ID int NOT NULL, INDEX_NAME [char(30) PRIMARY KEY, INDEX_CREATED bit NOT NULL,INDEX_TYPE number(1) NOT NULL) ON PRIMARY;"
            'Server.Con.ExecuteNonQuery(CommandType.Text, str)
    ...
    Private Sub CreateView(ByVal docTypeId As Int64) 'Implements CreateTables.CreateView
        Dim Str As String
        Str = "CREATE OR REPLACE VIEW DOC" & docTypeId & " AS SELECT DOC_I" & docTypeId & ".*, DOC_T" & docTypeId & ".FOLDER_ID As FOLDER_ID, DOC_T" & docTypeId & ".DISK_GROUP_ID AS DISK_GROUP_ID, DOC_T" & docTypeId & ".PLATTER_ID AS PLATTER_ID, DOC_T" & docTypeId & ".VOL_ID AS VOL_ID, DOC_T" & docTypeId & ".DOC_FILE AS DOC_FILE, " _
 ...
 & " FROM DOC_I" & docTypeId & " INNER JOIN DOC_T" & docTypeId & " ON DOC_I" & docTypeId & ".DOC_ID = DOC_T" & docTypeId & ".DOC_ID"
    ...
    End Sub
    ''' <summary>
    ''' Crea la columna del indice en la tabla
    ''' </summary>
    ...
    Public Shadows Sub AddIndexColumn(ByVal DocTypeId As Int64, ByVal IndexIdArray As ArrayList, ByVal IndexTypeArray As ArrayList, ByVal IndexLenArray As ArrayList) Implements CreateTables.AddIndexColumn
        ...
        For i = 0 To IndexIdArray.Count - 1
            ...
            Select Case type
                ...
                Case 3
                    '[Sebastian 02-09-09] se cambio la precision del decimal
                    str1 = "I" & IndexIdArray(i) & " decimal(18,2) NULL"
                Case 4
                    str1 = "I" & IndexIdArray(i) & " DATE NULL"
                Case 5
                    str1 = "I" & IndexIdArray(i) & " DATE NULL"
                Case 6
                    'str1 = "I" & IndexIdArray(i) & " money NULL"
                    '[Sebastian 02-09-09] se cambio el tipo moneda a decimal
                    str1 = "I" & IndexIdArray(i) & " decimal(18,2) NULL"
                Case 7
                    str1 = "I" & IndexIdArray(i) & " char (" & IndexLenArray(i) & ") NULL"
                Case 8
                    str1 = "I" & IndexIdArray(i) & " char (" & IndexLenArray(i) & ") NULL"
            End Select
            'strd1 = "D" & IndexIdArray(i) & " boolean NULL"
            If i = 0 Then
                str = str & str1
                'strd = strd & strd1
            Else
    ...
    Public Shadows Sub AddIndexList(ByVal IndexId As Int64, ByVal IndexLen As Integer) Implements CreateTables.AddIndexList
        Dim str As String = "CREATE TABLE [ILST_I" & IndexId & "] ([ITEMID] [int] NULL, [ITEM] [char] (" & IndexLen & ") NULL) ON [PRIMARY];"
        ' Create the tables.
        Server.Con.ExecuteNonQuery(CommandType.Text, str)
    ...
    Public Shadows Sub DelIndexItems(ByVal IndexId As Int64, ByVal IndexList As ArrayList) Implements CreateTables.DelIndexItems
        ...
        Dim table As String = "ILST_I" & IndexId
        For i = 0 To IndexList.Count - 1
            Dim strInsert As String = "DELETE FROM " & table & " where(ITEM = '" & IndexList(i) & "')"
            Server.Con.ExecuteNonQuery(CommandType.Text, strInsert)
        Next
    End Sub
