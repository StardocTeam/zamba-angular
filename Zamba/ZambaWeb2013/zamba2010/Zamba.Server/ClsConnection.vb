...
Public Class Server
    ...
    Public Sub MakeConnection(ByVal ServerType As DBTYPES, ByVal DBServer As String, ByVal DB As String, ByVal User As String, ByVal Password As String)
        Try
            If Server.ConInitialized = False Then Throw New Exception("Coneccion no Inicializada correctamente")
...
#Region "Properties"
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Retorna True si el servidor al que se está conectado es del tipo Oracle
    ''' </summary>
    ''' -----------------------------------------------------------------------------
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Retorna True si el servidor al que se está conectado es del tipo SQL Server
    ''' </summary>
    ''' -----------------------------------------------------------------------------
    ...
    ''' -----------------------------------------------------------------------------
    ''' <summary>
    ''' Retorna True si el servidor al que se está conectado es del tipo ODBC
    ''' </summary>
    ''' -----------------------------------------------------------------------------
    ...
    Public Shared ReadOnly Property Con(Optional ByVal Forced As Boolean = False, Optional ByVal FlagClose As Boolean = True, Optional ByVal OutofPool As Boolean = True) As IConnection
        Get
    ...
    Public Shared Function currentfile(ByVal Optional IsService As Boolean = False) As String
        ...
        If IsService And IsNothing(File) Then
            ...
            If IsNothing(File) OrElse Not File.Exists Then
                ...
                End If
            End If
        ElseIf IsNothing(File) OrElse Not File.Exists Then
            Try
