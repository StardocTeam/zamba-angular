...
namespace Zamba.SetupLibrary
{
    ...
    public class HelperSecurity
    {
        ...
        private const string ARG_CASPOLZAMBA_SETSECURITYZAMBA = "-m -ag All_Code -url \"{0}\\*\" FullTrust -n \"ExportaOutlook\" -d \"Exporta mails desde outlook a zamba\" -pp off";
        ...
        private const string ARG_CASPOLZAMBA_REMOVESECURITYZAMBA = "-m -rg \"ExportaOutlook\" -pp off";
        private const string ARG_CASPOLOFFICE_REMOVESECURITYZAMBA = "-m -rg \"ExportaOutlookOffice\" -pp off";        
        private const string ARG_CASPOLADDIN_SETSECURITYADDIN = " -af \"{0}\" -pp off";
        private const string ARG_GACUTILADDIN_SETSECURITYADDIN = " -i \"{0}\" /nologo";
        private const string ARG_CASPOLADDIN_REMOVESECURITYADDIN = " -rf \"{0}\" -pp off";
        ...
        private void SetSecurityZamba(string pathOffice)
        {
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_CASPOLOFFICE_SETSECURITYZAMBA, pathOffice);
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
        ...
        private void RemoveSecurityZamba(string pathOffice)
        {
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = ARG_CASPOLOFFICE_REMOVESECURITYZAMBA;
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
        ...
        public void SetSecurityAddin()
        {
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_CASPOLADDIN_SETSECURITYADDIN, Path.Combine(sec.currentTargetPath, DDL_NAME_EXPORTAOUTLOOK));
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);

            HelperTrace.WriteAnswerFormat("add OK");
            sec = getSecurityAddin();
            sec.path = GetGacutilPath();
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_GACUTILADDIN_SETSECURITYADDIN, Path.Combine(sec.currentTargetPath, DDL_NAME_EXPORTAOUTLOOK)); ;
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
            HelperProcess hp = new HelperProcess();
        ...
        /// osanchez - 11/05/2009 version inicial
        /// </remarks>
        public void RemoveSecurityAddin()
        {
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments");
            sec.arg = string.Format(ARG_CASPOLADDIN_REMOVESECURITYADDIN,Path.Combine(sec.currentTargetPath, DDL_NAME_EXPORTAOUTLOOK));
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);
            HelperTrace.WriteAnswerFormat("add OK");
            ...
            HelperTrace.WriteAnswerFormat(sec.path);
            HelperTrace.WriteQuestionFormat("BuildArguments -->");
            sec.arg = string.Format(ARG_GACUTILADDIN_REMOVESECURITYADDIN, Path.Combine(sec.currentTargetPath, DDL_NAME_EXPORTAOUTLOOK));
            HelperTrace.WriteAnswerFormat(sec.arg);
            lista.Add(sec);
