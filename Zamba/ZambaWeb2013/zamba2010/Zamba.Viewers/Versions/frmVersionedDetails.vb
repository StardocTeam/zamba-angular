...
Public Class frmVersionedDetails
    ...
    Private Sub loadVersioned()
        Dim dt As DataTable = Zamba.Core.Search.ModDocuments.SearchVersions(DirectCast(_result.DocType, DocType), _result.RootDocumentId, _result.ID)
        For Each row As DataRow In dt.Rows
            If Cache.DocTypesAndIndexs.hsDocTypes.Contains(CInt(row("doc_type_id"))) = False Then
                Dim _doctype As New DocType(CInt(row("doc_type_id")), row("Tipo de Documento").ToString(), CInt(row("icon_Id")))
            ...
            Dim r As Result = New Result(CInt(row("doc_id")), Cache.DocTypesAndIndexs.hsDocTypes(doctypeid), row("Nombre del Documento").ToString(), 0, CInt(row("folder_id")))
        ...
        For Each result As Result In ResultsList
            If result.RootDocumentId = 0 Then
                ...
                Me.treeViewVersionedDetails.Nodes.Add(RootNode)
