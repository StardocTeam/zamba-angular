...
namespace Zamba.IndexManager.Analyzer
{
	/// <summary>
	...
	public class Indexer
	{
		...
		{
            Stream stream;
            if (File.Exists(Path.Combine(indexpath, filename)) == true)
            {
             stream = File.Open(Path.Combine(indexpath, filename), FileMode.Append);
            }
            else
            {
                stream = File.Open(Path.Combine(indexpath, filename), FileMode.OpenOrCreate);
            }
