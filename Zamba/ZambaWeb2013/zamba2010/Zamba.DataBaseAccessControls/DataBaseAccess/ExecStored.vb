...
#Region "Evento Publico"
    ...
    '    Dim sql As String = "select SelectTable from ZQColumns where ID=" & IDConsulta
    '    Try
    ...
    '        sql = "Select SelectColumns from ZQColumns where Id=" & IDConsulta
    '        Dim Dscolumns As DataSet = con1.ExecuteDataset(CommandType.Text, sql)
    '        sql = "Select Claves from ZQKeys where ID=" & IDConsulta
    '        Dim dsClaves As DataSet = con1.ExecuteDataset(CommandType.Text, sql)
    '        Dim i As Int32
    '        sql = "Select "
    '        Try
    '            For i = 0 To Dscolumns.Tables(0).Rows.Count - 1
    '                sql &= Dscolumns.Tables(0).Rows(i).Item(0)
    '                sql &= ", "
    '            Next
    ...
    '            sql &= dsClaves.Tables(0).Rows(i).Item(0)
    '            sql &= "="
    '            If IsNumeric(columnasclave(i)) = False Then
    '                sql &= "'" & columnasclave(i) & "' and "
    '            Else
    '                sql &= columnasclave(i) & " and "
    '            End If
    '            If sql.EndsWith(" and ") Then sql = sql.Substring(0, sql.Length - 5)
    '        Next
    '    Catch
    '    End Try
    '    Return sql
...
#Region "Metodos Privados"
    ...
    '        'Dim server As New server
    '        'server.MakeConnection()
    '        Me.Id = utilities.LastId(Zamba.Core.IdTypes.ZQUERY)
    ...
    '    '    sql = "Insert into ZQColumns values(" & Me.Id & ",'" & Tabla & "','" & ColumnasInsertadas(i) & "')"
    '    '    con1.ExecuteNonQuery(CommandType.Text, sql)
    '    'Next
    '    'For i = 0 To Claves.Count - 1
    '    '    sql = "Insert into ZQKeys values(" & Me.Id & ",'" & Claves(i) & "')"
    '    '    con1.ExecuteNonQuery(CommandType.Text, sql)
    '    'Next
    'End Sub
#End Region
    ...
    Private Sub CargarStored()
        ...
        Select Case Me.ServerType
            Case Zamba.Servers.Server.DBTYPES.MSSQLServer, Server.DBTYPES.MSSQLServer7Up
                Try
