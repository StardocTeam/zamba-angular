...
Public Class PlayDOCreateForm
    ...
    Public Function Play(ByVal results As System.Collections.Generic.List(Of Core.ITaskResult)) As System.Collections.Generic.List(Of Core.ITaskResult)
        ...
        Try
            For Each r As Core.ITaskResult In results
                ...
                Trace.WriteLineIf(ZTrace.IsInfo, "Creando formulario...")
                newresult = CreateForm(r, myRule.DocTypeIdVirtual)
                Trace.WriteLineIf(ZTrace.IsInfo, " OK")
            ...
            If myRule.AddToWf = True Then
                Trace.WriteLineIf(ZTrace.IsInfo, "Obteniendo el Workflow...")
                Dim wfid As Int64 = WFBusiness.GetWorkflowIdByStepId(myRule.WFStepId)
                Dim wf As IWorkFlow = WFBusiness.GetWFbyId(wfid)
                Trace.WriteLineIf(ZTrace.IsInfo, "Obteniendo la etapa...")
                Dim NewStep As WFStep = WFStepBusiness.GetStepById(myRule.WFStepId)
                Trace.WriteLineIf(ZTrace.IsInfo, "Agregando los formularios creados al Workflow " & wf.Name & "...")
                WFTaskBusiness.AddResultsToWorkFLow(arr, wf, NewStep)
                Trace.WriteLineIf(ZTrace.IsInfo, "Formularios agregados con éxito!")
            Else
                Trace.WriteLineIf(ZTrace.IsInfo, "Los formularios creados no serán agregados al Workflow.")
            End If
        Finally
    ...
    Private Function CreateForm(ByVal p_ResultOrigen As Core.Result, ByVal p_iDocTypeIdVirtual As Int32) As Core.Result
        ...
        nr.DocumentalId = p_ResultOrigen.DocumentalId
