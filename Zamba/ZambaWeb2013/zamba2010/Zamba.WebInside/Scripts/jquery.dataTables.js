	...
	_oExt.oPagination = {
		...
		"full_numbers": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				$(nFirst).text(oSettings.oLanguage.oPaginate.sFirst);
				$(nPrevious).text(oSettings.oLanguage.oPaginate.sPrevious);
				$(nNext).text(oSettings.oLanguage.oPaginate.sNext);
				$(nLast).text(oSettings.oLanguage.oPaginate.sLast);
	...
	$.fn.dataTable = function( oInit )
	{
		...
		this.fnUpdate = function( mData, mRow, iColumn, bRedraw, bAction )
		{
			...
			else
			{
				...
				if ( oSettings.aoData[iRow].nTr !== null )
				{
					...
					$(_fnGetTdNodes( oSettings, iRow )[iColumn]).text(sDisplay);
				}
			}
		...
		function _fnCreateTr ( oSettings, iRow )
		{
			...
			if ( oData.nTr === null )
			{
				...
				for ( var i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					{
						$(nTd).text(_fnGetCellData( oSettings, iRow, i, 'display' ));
					}
		...
		function _fnGatherData( oSettings )
		{
				...
				for ( i=0, iLen=nTrs.length ; i<iLen ; i++ )
				{
					if ( nTrs[i].nodeName.toUpperCase() == "TR" )
					{
						...
						for ( j=0, jLen=nTds.length ; j<jLen ; j++ )
						{
							...
							{
								_fnSetCellData( oSettings, iThisIndex, jInner, $(nTds[j]).text().trim() );
								jInner++;
							}
						}
					}
			...
			for ( iColumn=0, iColumns=oSettings.aoColumns.length ; iColumn<iColumns ; iColumn++ )
			{
				...
				{
					oSettings.aoColumns[iColumn].sTitle = $(oSettings.aoColumns[iColumn].nTh).text();
				}
		...
		function _fnBuildHead( oSettings )
		{
			...
			if ( iThs !== 0 )
			{
				...
				for ( i=0, iLen=oSettings.aoColumns.length ; i<iLen ; i++ )
				{
					...
					if ( oSettings.aoColumns[i].sTitle != $(nTh).text() )
					{
						$(nTh).text(oSettings.aoColumns[i].sTitle);
					}
				}
			}
			else
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			$(nFilter).html('<label>'+sSearchStr+'</label>');
		...
		function _fnFeatureHtmlLength ( oSettings )
		{
			...
			$(nLength).text('<label>'+oSettings.oLanguage.sLengthMenu.replace( '_MENU_', sStdMenu )+'</label>');
		...
		function _fnFeatureHtmlProcessing ( oSettings )
		{
			...
			$(nProcessing).text(oSettings.oLanguage.sProcessing);
				...
				for ( i=0 ; i<iColums ; i++ )
				{
					if ( oSettings.aoColumns[i].bVisible )
					{
						...
						if ( nTd !== null )
						{
							...
							{
								$(nTd).text($(nTd).text() + oSettings.aoColumns[i].sContentPadding);
							}
							nTr.appendChild( nTd );
						}
					}
		...
		function _fnGetWidestNode( oSettings, iCol )
		{
			...
			if ( oSettings.aoData[iMaxIndex].nTr === null )
			{
				...
				$(n).text(_fnGetCellData( oSettings, iMaxIndex, iCol, '' ));
				return n;
			}
