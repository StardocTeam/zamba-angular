...
public class UserPreferencesFactory
{
    ...
    public static DataTable getAllValuesDB(Int64 UserId)
    {
        try
        {
            ...
            if (Server.isOracle)
            {
                DataSet ds = Server.get_Con().ExecuteDataset(CommandType.Text, string.Format("SELECT c_userid userid, c_name name, c_section section, c_value value from ZUserConfig where c_userid={0} union SELECT c_userid, c_name, c_section, c_value from ZUserConfig d where not exists( select 1 from ZUserConfig l where d.c_name = l.c_name and d.c_section = l.c_section and l.c_userid <> 0 and l.c_userid = {0} )  and d.c_userid = 0 order by userid asc", UserId));
            ...
            {
                DataSet ds = Server.get_Con().ExecuteDataset(CommandType.Text, string.Format("SELECT userid, name, section, value from ZUserConfig where userid={0} union SELECT userid, name, section, value from ZUserConfig d where not exists( select 1 from ZUserConfig l where d.name = l.name and d.Section = l.section and l.UserId <> 0 and l.UserId = {0}) and d.userid = 0 order by userid asc", UserId));
                if (ds != null && ds.Tables.Count > 0)
                    return ds.Tables[0];
                else
                    return null;
    ...
    public static void setValueDBByMachine(string name, string value, UPSections section, string machineName)
    {
        if (Server.isOracle)
        {
            if (string.IsNullOrEmpty(value) == false)
            {
                ...
                ZTrace.WriteLineIf(ZTrace.IsVerbose, $"Buscando valor {name} para maquina {machineName}");
                ...
                {
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, $"Actualizo valor {name} : {value} para maquina {machineName}");
                ...
                {
                    ZTrace.WriteLineIf(ZTrace.IsVerbose, $"Insertando valor {name} : {value} para maquina {machineName}");
                    query = $"INSERT INTO ZMachineConfig(c_name, c_value, c_section, c_machinename) values('{name}', '{value.Replace("'", "''")}', {(int)section}, '{machineName}')";
                    Server.get_Con().ExecuteNonQuery(CommandType.Text, query);
                }
            }
        }
    ...
    public static string getValueDBByMachine(string name, UPSections Section, string machineName)
    {
        ...
        if (ReturnValue == System.DBNull.Value || ReturnValue == null)
        {
            ZTrace.WriteLineIf(ZTrace.IsVerbose, $"Valor {name} para maquina {machineName} NO encontrado");
            return null;
        }
        else
        {
            ZTrace.WriteLineIf(ZTrace.IsVerbose, $"Valor {name} : {ReturnValue.ToString()} para maquina {machineName}");
            return ReturnValue.ToString();
        }
    }
    ...
    public static DataTable getAllValuesDBByMachine(string machineName)
    {
        try
        {
            ...
            if (Server.isOracle)
            {
                DataSet ds = Server.get_Con().ExecuteDataset(CommandType.Text, string.Format("SELECT c_machinename machinename, c_name name, c_section section, c_value value from ZMachineConfig where c_machinename='{0}' union SELECT c_machinename, c_name, c_section, c_value from ZMachineConfig d where not exists(    select 1 from ZMachineConfig l where d.c_name = l.c_name and d.c_section = l.c_section and lower(l.c_machinename) <> 'default' and l.c_machinename = '{0}' )  and lower(d.c_machinename) = 'default'", machineName));
                if (ds != null && ds.Tables.Count > 0)
                    return ds.Tables[0];
                else
                    return null;
            ...
            {
                DataSet ds = Server.get_Con().ExecuteDataset(CommandType.Text, string.Format("SELECT machinename,name, section, value from ZMachineConfig where machinename = '{0}' union SELECT machinename, name, section, value from ZMachineConfig d where not exists( select 1 from ZMachineConfig l where d.name = l.name and d.Section = l.section and l.machinename <> 'default' and l.machinename = '{0}') and d.machinename = 'default'", machineName));
                if (ds != null && ds.Tables.Count > 0)
                    return ds.Tables[0];
                else
                    return null;
