...
namespace Zamba.FUP
{
	...
	public	string AppsPath = string.Empty;
		...
		public bool CheckNews()
		{
			...
			LastVersion = UserPreferences.getValue("LastVersion", UPSections.UserPreferences, "3.0.0.0");
			LastVersionPath = UserPreferences.getValue("LastVersionPath", UPSections.UserPreferences, "http://www.zambabpm.com.ar/zamba/release zamba cliente 2.9.4.0.zip");

			DestinationPath = UserPreferences.getValue("DestinationPath", UPSections.UserPreferences, "ApplicationData\\Zamba Software\\app");
		...
		public bool GetClientToExecute(string currentPath, string BtnAppToExecute, string Args)
		{
			try
			{
                ...
                FileInfo fi = new FileInfo(ClientToExecute);
				...
				else
				{
                    ...
                    FileInfo Afi = new FileInfo(AppToExecute);
					...
					{
                        FileInfo Lfi = new FileInfo(LocalClientToExecute);
						if (Lfi.Exists)
						{
							string commandline = string.Empty;
							if (!string.IsNullOrEmpty(Args)) commandline = Args;
		...
		private void Wp_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			try
			{
                ...
                if (((bool)e.Result) == true)
                {
                    ...
                    if (bool.Parse(UserPreferences.getValue("DeletOldVersionsFromApp", UPSections.UserPreferences,false).ToString()))                    
                    {
